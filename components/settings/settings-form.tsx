...
/**
 * THE SETTINGS FORM, SPLIT INTO CARDS (v4.4.0).
 *
 * The form used to be ONE component rendering eight cards over one "Save
 * settings" button. /settings now orders its cards by importance and lets the
 * user move them (components/layout/section-stack.tsx), so each card is its
 * own export that the page places as a `<Section>`, and the shared state lives
 * in `SettingsFormProvider` above the whole stack. Every card that holds form
 * fields carries its own "Save settings" button; each one saves EVERY pending
 * form edit, exactly as the single button did — the contract is unchanged,
 * only the button is no longer out of sight.
 *
 * Appearance is the exception (v4.4.0 ruling): it sits LAST on the page and
 * saves on its own through /api/settings/appearance. The form's save still has
 * to send theme / skin / density (/api/settings requires them), so it sends
 * the LAST SAVED look (`savedLook`), never an unsaved preview — a Preferences
 * save must neither persist a skin the user was only trying on nor revert one
 * they have just saved.
 */

type SavedLook = { theme: string; accentSkin: string; density: string };
...
function useSettingsForm(): SettingsFormState {
  ...
  if (!ctx) throw new Error("Settings cards must render inside <SettingsFormProvider>.");
  return ctx;
}

function useSettingsFormState(current: Settings) {
  ...
  // What /api/settings/appearance last stored — see the header. Updated by the
  // Appearance card's save handler, never by an effect.
  const [savedLook, setSavedLook] = useState<SavedLook>({
    theme: current.theme,
    accentSkin: asSkin(current.accentSkin),
    density: current.density ?? "compact",
  });
  ...
  async function save(over?: { openalgoEnabled?: boolean; openalgoAckVersion?: string | null }) {
    ...
    try {
      const res = await fetch("/api/settings", {
        ...
        body: JSON.stringify({
          type: "settings",
          goLiveDate, equityCapital, activeCapital, workspace, fyStartMonth,
          // The last SAVED look — required by the route, and never an unsaved
          // preview (Appearance has its own save).
          theme: savedLook.theme, accentSkin: savedLook.accentSkin, density: savedLook.density,
          ...
          // tintIntensity / panelStyle / wallpaperOpacity / customTheme are
          // OMITTED: the route keeps the stored value for an absent field, and
          // they belong to Appearance's own save.
        }),
      });
      ...
      // (rather than letting a server action do it) is what keeps the page's
  ...
  return {
    ...
    colorblind, applyColorblind,
    goLiveDate, setGoLive,
    equityCapital, setEquity,
    activeCapital, setActive,
    fyStartMonth, setFy,
    defaultBuyOrders, setBuyOrders,
    defaultSellOrders, setSellOrders,
    autoMtm, setAutoMtm,
    autoPull, setAutoPull,
    openalgoEnabled, openalgoAckVersion,
    openalgoDialogOpen, setOpenalgoDialogOpen,
    pending, setupPending,
    setSavedLook,
    save, toggleOpenalgo, rerunSetup, acceptOpenalgo,
  };
}

/**
 * Holds the settings form's state for every card below it. Wraps the page's
 * whole SectionStack, so the cards can sit anywhere — and move — and still
 * share one form. `current` refreshes with the server row on router.refresh();
 * the in-progress edits (useState) do not reset, as before.
 */
export function SettingsFormProvider({ current, children }: { current: Settings; children: React.ReactNode }) {
  const state = useSettingsFormState(current);
  return <SettingsFormContext.Provider value={state}>{children}</SettingsFormContext.Provider>;
}

/** Saves EVERY pending edit in the settings form (not Appearance, which saves on its own). */
function SaveSettingsButton() {
  const { save, pending } = useSettingsForm();
  return (
...
export function CapitalGoLiveCard() {
    ...
    <Card>
      ...
      <CardContent className="grid gap-4 sm:grid-cols-3">
        <Field label="Go-live date">
          <Input type="date" value={f.goLiveDate} onChange={(e) => f.setGoLive(e.target.value)} />
        </Field>
        <Field label="Equity bucket capital (₹)">
          <Input type="number" step="1" value={f.equityCapital} onChange={(e) => f.setEquity(e.target.value)} />
        </Field>
        <Field label="Trade F&O bucket capital (₹)">
          <Input type="number" step="1" value={f.activeCapital} onChange={(e) => f.setActive(e.target.value)} />
        </Field>
        <SaveSettingsButton />
      </CardContent>
    </Card>
...
export function WorkspaceCard() {
  const { workspace, setWorkspace } = useSettingsForm();
  return (
    <Card>
      <CardHeader>
    ...
    <Card>
      ...
      <CardContent className="grid gap-4 sm:grid-cols-2">
        <Field label="Financial year starts">
          <Select value={f.fyStartMonth} onChange={(e) => f.setFy(e.target.value)}>
            {MONTHS.map((m, i) => (
        ...
        <Field label="Default BUY orders / closed trade">
          <Input type="number" min={1} value={f.defaultBuyOrders} onChange={(e) => f.setBuyOrders(e.target.value)} />
        </Field>
        <Field label="Default SELL orders / closed trade">
          <Input type="number" min={1} value={f.defaultSellOrders} onChange={(e) => f.setSellOrders(e.target.value)} />
        </Field>
        <div className="flex items-center justify-between rounded-md border border-border bg-card-hover/40 px-3 py-2 sm:col-span-2">
          ...
          <Switch checked={f.colorblind} onCheckedChange={(v) => f.applyColorblind(Boolean(v))} />
        </div>
        <div className="flex items-center justify-between rounded-md border border-border bg-card-hover/40 px-3 py-2 sm:col-span-2">
          <div>
...
/** Mounted UNKEYED, as it always was — router.refresh() hands it the fresh row. */
export function SettingsLiveFeedCard() {
  const { current } = useSettingsForm();
  return <LiveFeedCard current={current} />;
}
...
export function IntegrationsCard() {
  ...
  const openalgoAckStale = f.openalgoEnabled && !isAckCurrent(f.openalgoAckVersion);
  const openalgoGateReason = openAlgoGate({
    enabled: f.openalgoEnabled,
    ackVersion: f.openalgoAckVersion,
  }).reason;
  ...
  return (
    <>
      <Card>
        ...
        <CardContent className="space-y-3" data-testid="integrations-section">
          <div className="flex items-center justify-between rounded-md border border-border bg-card-hover/40 px-3 py-2">
            ...
            <Switch
              checked={f.openalgoEnabled}
              onCheckedChange={(v) => f.toggleOpenalgo(Boolean(v))}
          ...
          {openalgoAckStale && (
            <div className="rounded-md border border-warning/40 bg-warning/5 px-3 py-2">
              ...
              <div className="mt-2">
                <Button type="button" size="sm" variant="outline" onClick={() => f.setOpenalgoDialogOpen(true)}>
                  Read it again
                </Button>
              </div>
            </div>
          ...
          <div className="flex items-center justify-between rounded-md border border-border bg-card-hover/40 px-3 py-2">
            ...
            <Switch checked={f.autoPull} onCheckedChange={(v) => f.setAutoPull(Boolean(v))} data-testid="auto-pull-switch" />
          </div>
      ...
      <OpenAlgoDialog
        open={f.openalgoDialogOpen}
        onOpenChange={f.setOpenalgoDialogOpen}
        onAccept={f.acceptOpenalgo}
      />
    </>
  );
}
...
export function FirstRunCard() {
  const { rerunSetup, setupPending } = useSettingsForm();
  return (
    <Card>
      <CardHeader>
...
/**
 * Appearance — previews live on <html>, persists through its OWN route
 * (/api/settings/appearance), and sits last on the page (owner, 2026-09-16).
 */
export function AppearanceCard() {
  const router = useRouter();
  const { current, setSavedLook } = useSettingsForm();
  ...
  // Apply theme to <html> instantly for live preview.
  ...
  /** Appearance's own save — route handler + fetch + router.refresh(), never a server action. */
  async function saveAppearance() {
    ...
    try {
      const res = await fetch("/api/settings/appearance", {
        ...
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          theme, accentSkin: skin, density,
          // customTheme goes only while the Custom skin is selected ("Save as
          // my theme" is explicit); otherwise it is omitted and the stored
          // theme is kept. wallpaperStoredName is NOT sent — the upload route
          // owns that column.
      ...
      if (json.ok) {
        toast.success(text);
        setSavedLook({ theme, accentSkin: skin, density });
        // The vars/classes on <html> are server-rendered by the root layout.
        router.refresh();
      } else toast.error(text);
    } catch (e) {
      toast.error((e as Error).message);
      ...
      <CardContent className="grid gap-4 sm:grid-cols-2" data-testid="appearance-section">
        <div className="sm:col-span-2 space-y-2">
          ...
          <p className="text-xs text-muted-foreground">
            ...
            colourblind-safe setting in Preferences.
          </p>
        </div>
        ...
        <p className="text-xs text-muted-foreground sm:col-span-2">
          ...
          <span className="text-foreground">Save appearance</span> below — it saves on its own, apart
          from the rest of Settings. Leave without saving and the app reverts to what was stored.
        </p>
        <div className="flex items-center gap-3 sm:col-span-2">
          <Button type="button" onClick={() => void saveAppearance()} disabled={pending} data-testid="save-appearance">
