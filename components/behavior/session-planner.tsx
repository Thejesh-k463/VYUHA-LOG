...
export function SessionPlanner({ playbooks }: { playbooks: { id: number; name: string }[] }) {
  ...
  async function submit(form: HTMLFormElement) {
    const fd = new FormData(form); setBusy(true); setMsg("");
    ...
    const data = await res.json(); setBusy(false); setMsg(data.ok ? "Plan saved." : data.message); if (data.ok) router.refresh();
  }
  return <form className="space-y-4" onSubmit={(e) => { e.preventDefault(); void submit(e.currentTarget); }}>
    ...
    <div className="flex items-center gap-3"><Button disabled={busy}>{busy ? "Saving…" : "Save session plan"}</Button>{msg && <span className="text-xs text-muted-foreground">{msg}</span>}</div>
  </form>;
}
