...
export default function MonthlyReportPage() {
  ...
  // v4.4.0 D4 — annualise on the NSE calendar of the series' last date.
  const basis = annualisationBasis(daily.reduce((m, d) => (d.date > m ? d.date : m), "") || today);
  // v4.4.0 D5 — the ONE dated risk-free setting, named beside the figures below.
  ...
  // v4.4.0 (D1): a loser with no recorded risk is measured against the cap its
  // OWN segment resolves to (`withSegmentCap`), never the global row alone; the
  // score's fallback cap is therefore null — no second, broader rule.
      ...
      {p.tradingDays === 0 ? (
        ...
        <>
          ...
          <p className="border-t border-border pt-3 text-[10px] text-muted-foreground">
            Generated by Vyuha — trade journal for Indian markets.{" "}
            {capitalKnown
              ? <>Time-weighted figures computed on running equity from configured capital; Sharpe/Sortino vs risk-free {riskFree.label}, annualised over {annualisationNote(basis)}.</>
