...
describe("summariseLedger", () => {
  ...
  it("derives available capital = opening + Σ flows per bucket", () => {
    ...
    expect(ac.availablePaise).toBe(toPaise(400000 - 1500 + 8000 - 500)); // 4,06,000
    expect(ac.chargesPaise).toBe(toPaise(-2000)); // charge + mtf interest
    expect(ac.realisedPnlPaise).toBe(toPaise(8000));
  });

  it("totals across buckets", () => {
    expect(s.totalOpeningPaise).toBe(toPaise(1700000));
    expect(s.totalFlowsPaise).toBe(toPaise(50000 - 20000 - 1500 + 8000 - 500));
  ...
  it("computes a chronological running balance per bucket", () => {
    ...
    expect(activeRows[activeRows.length - 1].balancePaise).toBe(toPaise(406000));
  });
