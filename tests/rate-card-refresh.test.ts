 ...
 * already autocommitted — the owner's DB ended with 50 overlapping open-ended
 ...
 * QS-EQ2012 the 2012 equity delivery STT one): 700 rows where bbdc4ec had 270 — 980 / 378 since
 * v4.6.0 W9 added four broker-plans (fyers default/prime, nuvama default/elite: every count
 * below that is per broker-plan grew by 14/10, re-measured by running this file). The owner's real
 ...
 * scripts/rate-card-refresh.mjs.
 *
 * Speed (CI 34586408007 timed the old sweeps out on the Windows runner): the
 * template is migrated and seeded ONCE, in memory, the seed inside one
 * transaction, and serialised; every user database is an in-memory copy of those
 * bytes. The refresh ATTACHes the template FILE from an in-memory main exactly as
 * it does from a file, so the unit under test is unchanged.
 *
...
type Result = { added?: number; refreshed?: number; removed?: number; skipped?: string };
...
/** The template's bytes, serialised once in beforeAll. */
...
/** A user database: an IN-MEMORY copy of the template (no file copy, no fsync). */
function userCopy(): Database.Database {
  const db = new Database(templateBytes);
  opened.push(db);
  return db;
}
 ...
 * three STT epochs (1970, 2024-10-01, 2026-04-01) and every other key its one
 * 1970 row, at bbdc4ec's flat exchange charges and 1970 F&O STT, each row re-closed at the next
 * surviving epoch (the newest left open). Checked once, row for row, against the
 * rows bbdc4ec's own buildChargeConfigSeed() emits. Returns the row count (270 since wave 3a:
 * each of the 40 ETF rate keys keeps its 1970 row, its later epochs are deleted like any other).
 */
function plantPreC8(db: Database.Database): number {
  db.prepare(
...
/**
 * Back to the pre-C-7 two-epoch key, from bbdc4ec's card: every 1970 epoch of a
 * multi-epoch key takes the STT of the epoch starting `sttFrom` and ends at `to`
 * (NULL = open), then the 2024-10-01 epoch is deleted. Returns the number of 1970 rows rewritten.
 */
function collapse(db: Database.Database, sttFrom: string, to: string | null): number {
  expect(plantPreC8(db)).toBe(378);
  ...
  const changed = db
    ...
    .run(to).changes;
  expect(db.prepare(`DELETE FROM charge_config WHERE effective_from = '${STT_EPOCH_2024}'`).run().changes).toBe(70);
  return changed;
}
/**
...
/** C-8's first and last NSE exchange-charge boundaries (FA56129, FA73061). */
const NSE_2023 = "2023-04-01";
const NSE_2026 = "2026-03-01";
/** The R1 options STT boundary (FATAX32385): the 1970 epoch of every NSE option key now closes here. */
const OPT_2016 = "2016-06-01";
 ...
 * ending at `to`, NULL = open) and k's six later epochs (every k is an NSE option key) are deleted.
 */
function ownThe1970Row(u: Database.Database, k: Key, to: string | null, extraSet = ""): void {
  expect(u.prepare(`DELETE FROM charge_config WHERE ${where(k)} AND effective_from > '1970-01-01'`).run().changes).toBe(6);
  const edited = u
...
beforeAll(() => {
  // Migrated and seeded ONCE, in memory, the seed in one transaction; the bytes
  // become the template FILE the refresh ATTACHes, and every user copy.
  ...
  correctedChanged = t
    ...
    .run().changes;
  fs.writeFileSync(CORRECTED, t.serialize());
  t.close();
...
/**
 * Runs `s`, then closes every (in-memory) user copy it opened — the mutant
 * table replays scenarios once per mutant. A scenario opens user copies only;
 * the templates are opened once, in beforeAll.
 */
function play(s: Scenario, refresh: Refresh): void {
  ...
  try {
    ...
    for (const db of opened.splice(mark)) if (db.open) db.close();
  }
}
...
const SCENARIOS: Scenario[] = [
  ...
  },
  {
    id: "(b)",
    title:
      "the owner's real state (4.2.0: two epochs, 50 corrupted 1970 rows, pre-C-8 exchange charges): every missing epoch is added, the 1970 rows corrected, overlaps gone, each era priced at its own STT and exchange charge",
    run: (refresh) => {
      const u = userCopy();
      expect(plantOwnerState(u)).toBe(70);
      expect(overlaps(u)).toBe(70); // the planted state really has the defect
      ...
      expect(refresh(u)).toEqual({ added: 672, refreshed: 252, removed: 0 });
      expect(snapshot(u)).toEqual(snapshot(tpl));
      expect(overlaps(u)).toBe(0);

      const tplRow = (from: string) => rowsOf(tpl, DHAN, from)[0] as { stt_pct: number; exchange_txn_pct: number };
      const oldStt = tplRow("2024-04-01").stt_pct; // R1: 30 Sep 2024 sits in the 2024-04-01 epoch
      const fy25Stt = tplRow(STT_EPOCH_2024).stt_pct;
      const newStt = tplRow(STT_EPOCH_2026).stt_pct;
      expect(new Set([oldStt, fy25Stt, newStt]).size).toBe(3);
      const fy24Txn = tplRow("2024-04-01").exchange_txn_pct;
      const mar26Txn = tplRow(NSE_2026).exchange_txn_pct;
      expect(new Set([PRE_C8.optNse, fy24Txn, mar26Txn]).size).toBe(3);

      const map = ratesMapOf(drizzle(u).select().from(chargeConfig).all() as unknown as ChargeRates[]);
      const on = (d: string) => findRates(map, "dhan", "index_option", "NSE", d);
      expect(on("2024-09-30").sttPct).toBe(oldStt);
      expect(on("2024-09-30").exchangeTxnPct).toBe(fy24Txn);
      expect(on("2026-03-15").sttPct).toBe(fy25Stt);
      expect(on("2026-03-15").exchangeTxnPct).toBe(mar26Txn);
      expect(on("2026-04-15").sttPct).toBe(newStt);
    },
  },
  {
    id: "(b2)",
    title: "the correct two-epoch state (1970 → 2026-04-01 at the FY25 STT): the missing epochs are added and the 1970 row closed at 2016-06-01 (R1)",
    run: (refresh) => {
      const u = userCopy();
      expect(plantFy25State(u)).toBe(70);
      expect(overlaps(u)).toBe(0);
      ...
      expect(rowsOf(u, DHAN, STT_EPOCH_2024)).toMatchObject([{ stt_pct: was[0].stt_pct, effective_to: NSE_2026 }]);
      expect(rowsOf(u, DHAN, "1970-01-01")).toMatchObject([{ effective_to: OPT_2016 }]);
      expect(rowsOf(u, DHAN, OPT_2016)).toMatchObject([{ effective_to: NSE_2023 }]);
    },
  },
  {
    id: "(b3)",
    title: "bbdc4ec's own state (C-7's three F&O epochs, pre-C-8 exchange charges): the C-8 epochs are added and every changed rate refreshed",
    run: (refresh) => {
      const u = userCopy();
      expect(plantPreC8(u)).toBe(378);
      expect(overlaps(u)).toBe(0);

      // Re-pinned for QS-EQ2012 (measured): 315 / 171 / 0 before, 351 / 171 / 0 after.
      // Wave 3a (measured): 390 / 190 -> 430 / 220. +40 added = 700 - 270 planted; +30 refreshed =
      // all 20 etf_equity 1970 rows (their effective_to must close at 2012-07-01) plus the 10 NSE
      // etf_other rows, whose ipft is planted at bbdc4ec's NSE figure; BSE etf_other already matches.
      expect(refresh(u)).toEqual({ added: 602, refreshed: 308, removed: 0 });
      expect(snapshot(u)).toEqual(snapshot(tpl));
      expect(overlaps(u)).toBe(0);
    },
  },
  {
    id: "(c)",
    title: "the pre-v3.2 shape (one open 1970 row per key at current rates) gains every later epoch, no overlaps",
    run: (refresh) => {
      const u = userCopy();
      expect(plantOwnerState(u)).toBe(70);
      ...
      // Wave 3a (measured): 490 -> 530 (the template grew by 80 rows and this state plants 40 of them).
      expect(count(u)).toBe(count(tpl) - 742);

      expect(refresh(u)).toEqual({ added: 742, refreshed: 196, removed: 0 });
      expect(snapshot(u)).toEqual(snapshot(tpl));
      expect(overlaps(u)).toBe(0);
    },
  },
  ...
  },
  {
    id: "(e2)",
    title:
      "a non-edited epoch inside a user-edited window is REMOVED (R54): the user row alone prices every date it covers, byte-identical, and every other key is the template",
    run: (refresh) => {
      const u = userCopy();
      // The pre-v3.2.0 shape R54 found: the user edited the open 1970 row (brokerage Rs 15)
      // and the six later seed epochs sit inside its window, the 2026 one stale.
      u.prepare(`UPDATE charge_config SET stt_pct = 0.0099 WHERE ${where(DHAN, STT_EPOCH_2026)}`).run();
      const edited = u
        .prepare(`UPDATE charge_config SET user_edited = 1, effective_to = NULL, brokerage_flat = 15 WHERE ${where(DHAN, "1970-01-01")}`)
        .run().changes;
      expect(edited).toBe(1);
      const mine = rowsOf(u, DHAN, "1970-01-01");
      expect(rowsOf(u, DHAN)).toHaveLength(7);

      const res = refresh(u);
      // What a trade dated after the stale epoch prices at: the rate the user verified.
      const map = ratesMapOf(drizzle(u).select().from(chargeConfig).all() as unknown as ChargeRates[]);
      expect(findRates(map, "dhan", "index_option", "NSE", "2026-06-15").brokerageFlat).toBe(15);
      expect(rowsOf(u, DHAN)).toEqual(mine);
      expect(res).toEqual({ added: 0, refreshed: 0, removed: 6 });
      const others = `NOT (${where(DHAN)})`;
      expect(snapshot(u, others)).toEqual(snapshot(tpl, others));
    },
  },
  {
    id: "(e3)",
    ...
    run: (refresh) => {
      ...
      expect(refresh(u)).toEqual({ added: 1, refreshed: 0, removed: 0 });
  ...
  {
    id: "(e5)",
    title: "a closed user-edited window that ENDED BEFORE the seed epochs does not own them: the two epochs after it are added",
    run: (refresh) => {
      const u = userCopy();
      ownThe1970Row(u, DHAN, "2026-01-01");
      const before = rowsOf(u, DHAN, "1970-01-01");

      expect(refresh(u)).toEqual({ added: 2, refreshed: 0, removed: 0 });
      expect(rowsOf(u, DHAN, "1970-01-01")).toEqual(before);
      expect(rowsOf(u, DHAN, STT_EPOCH_2024)).toHaveLength(0); // starts inside the user's window: theirs
      for (const from of [NSE_2026, STT_EPOCH_2026]) {
        const epoch = where(DHAN, from);
        expect(snapshot(u, epoch)).toHaveLength(1);
        expect(snapshot(u, epoch)).toEqual(snapshot(tpl, epoch));
      }
    },
  ...
  ...SIBLINGS.map(
    ({ column, edited, sibling }): Scenario => ({
      ...
      run: (refresh) => {
        ...
        // Every epoch on file in the template: an NSE option key has 7, a BSE index option key 8.
        for (const k of [edited, sibling]) expect(rowsOf(tpl, k), `${where(k)} in the template`).toHaveLength(k.exchange === "BSE" ? 8 : 7);
        ...
        expect(refresh(u)).toEqual({ added: 1, refreshed: 1, removed: 0 });
  ...
  ),
  {
    ...
    run: (refresh) => {
      ...
      expect(refresh(u, CORRECTED)).toEqual({ added: 0, refreshed: 1, removed: 0 });
  ...
  },
  {
    ...
    run: (refresh) => {
      ...
      expect(u.prepare(`DELETE FROM charge_config WHERE effective_from > '1970-01-01'`).run().changes).toBe(742);
      u.exec(
  ...
  },
  {
    ...
    run: (refresh) => {
      ...
      expect(snapshot(tpl, row)).toMatchObject([{ effective_to: OPT_2016 }]);
      expect(u.prepare(`UPDATE charge_config SET effective_to = NULL WHERE ${row}`).run().changes).toBe(1);

      expect(refresh(u)).toEqual({ added: 0, refreshed: 1, removed: 0 });
      expect(snapshot(u)).toEqual(snapshot(tpl));
    },
  },
  {
  ...
  },
  {
    ...
    run: (refresh) => {
      ...
      // Re-pinned for QS-EQ2012 (measured): 360 / 135 / 0 before, 396 / 135 / 0 after.
      expect(refresh(u)).toEqual({ added: 672, refreshed: 252, removed: 0 });
      // Every row that was there keeps its id; the 480 added epochs are the only new ones.
      ...
      expect(after.filter((r) => !had.has(k(r)))).toHaveLength(672);
      expect(snapshot(u)).toEqual(snapshot(tpl));
    },
  },
];
...
describe("the fixture is not vacuous", () => {
  it("the template holds 238 keys split at the union of their STT and exchange-charge boundaries (980 rows), and the user index is 5 columns", () => {
    const shapes = tpl
      .prepare(
    ...
    expect(shapes).toEqual([
      // Wave 3a: 20 MCX keys + the 20 etf_other keys, which are in no row of the s.98 table at all.
      { epochs: "1970-01-01", keys: 56 },
      // Wave 3a: etf_equity is delivery-like (1970, 2012-07-01) plus s.98 Sl. 2A from 2013-06-01.
      { epochs: "1970-01-01 2012-07-01 2013-06-01", keys: 28 },
      // QS-EQ2012 split the 27 BSE and 27 NSE cash keys: 18 delivery/MTF each gain 2012-07-01, 9 intraday do not.
      { epochs: "1970-01-01 2012-07-01 2022-12-01", keys: 28 }, // BSE delivery + MTF
      { epochs: "1970-01-01 2012-07-01 2023-04-01 2024-04-01 2024-10-01 2026-03-01", keys: 28 }, // NSE delivery + MTF
      { epochs: "1970-01-01 2013-06-01 2023-04-01 2024-04-01 2024-10-01 2026-03-01 2026-04-01", keys: 14 }, // NSE futures
      { epochs: "1970-01-01 2016-06-01 2022-05-02 2023-04-01 2023-11-01 2024-05-13 2024-10-01 2026-04-01", keys: 14 }, // BSE index options
      { epochs: "1970-01-01 2016-06-01 2022-05-02 2023-04-01 2024-10-01 2026-04-01", keys: 14 }, // BSE stock options
      { epochs: "1970-01-01 2016-06-01 2023-04-01 2024-04-01 2024-10-01 2026-03-01 2026-04-01", keys: 28 }, // NSE options
      { epochs: "1970-01-01 2022-12-01", keys: 14 }, // BSE intraday
      { epochs: "1970-01-01 2023-04-01 2024-04-01 2024-10-01 2026-03-01", keys: 14 }, // NSE intraday
    ]);
    // Re-pinned for QS-EQ2012 (measured): 522 before, 558 after. Wave 3a (v4.5.0, ruling R90, measured) added the two ETF STT RATE ROWS: 10 broker-plans x 2 exchanges x 2 rate segments = 40 keys, x (etf_equity 3 epochs + etf_other 1) = 80 rows, so the template is 170 keys / 700 rows.
    // v4.6.0 W9 (measured by running the seed): four broker-plans join (fyers default/prime, nuvama
    // default/elite), 70 rows and 17 keys each — 14 broker-plans, every shape above × 1.4, 980 rows.
    expect(count(tpl)).toBe(980);
...
const MUTANTS: (Mutant & { killedBy: string })[] = [
  ...
  { name: "stale-overlap DELETE removed", from: "removed: remove.run().changes,", to: "removed: 0,", killedBy: "(e2)" },
  { name: "delete: d.user_edited = 0 dropped", from: "d.user_edited = 0", to: "1 = 1", killedBy: "(e)" },
  {
...
const EQUIVALENT: (Mutant & { reason: string })[] = [
  ...
  {
    name: "the guard removed from the UPDATE",
    from: 'AND ${guard("t")}',
    to: "AND 1",
    reason:
      "the stale-overlap DELETE (R54) runs first in the same transaction and removes every non-edited row a user-edited " +
      "window covers, and the guarded INSERT adds none, so no row that reaches the UPDATE is covered",
  },
];
...
describe("the mutant table: every mutant of rate-card-refresh.mjs fails a named scenario", () => {
  ...
  /** One load per source, shared by that source's per-scenario rows below. */
  const loaded = new Map<string, Promise<Refresh>>();
  const loadOnce = (key: string, code: () => string): Promise<Refresh> => {
    if (!loaded.has(key)) loaded.set(key, load(code()));
    return loaded.get(key)!;
  };

  // The control and equivalence sweeps are one `it` per scenario: a whole-list
  // sweep inside one `it` timed out on the Windows runner (CI 34586408007).
  // Same meaning as before — every scenario must pass.
  describe("control: the unmutated source, loaded the same way, passes every scenario", () => {
    it.each(SCENARIOS)("$id", async (s) => play(s, await loadOnce("control", () => src)));
  });
  ...
  describe.each(EQUIVALENT)("equivalent: $name", (m) => {
    it.each(SCENARIOS)("$id", async (s) => {
      const refresh = await loadOnce(m.name, () => mutate(m));
      try {
        play(s, refresh);
      } catch (e) {
        throw new Error(`"${m.name}" was killed by ${s.id}, so it is not equivalent: ${m.reason}\n${String(e)}`);
      }
    });
  });
});
