...
describe("README importers line", () => {
  ...
  it("the hero line separates auto-detected formats, the column mapper and API pulls", () => {
    expect(readme).toMatch(
      /\*8 auto-detected brokers \(Zerodha, Dhan, Groww, Angel One, Upstox, Paytm Money, Fyers, Nuvama\) \+ any CSV via the column mapper \+ 4 broker-API pulls \(Kite, Dhan, Angel One SmartAPI, Upstox Analytics token\)/,
    );
  });
});
...
describe("README test counts agree with each other", () => {
  ...
  it("the unit-test FILE count matches tests/**/*.test.ts on disk", () => {
    ...
    // stale README the next time the suite grows. RECURSIVE, because vitest's
    // include is tests/**/*.test.ts — a non-recursive readdir pinned the
    // README to 133 while tests/sim/ made it 134 (audit, 2026-08-24).
    const files = readdirSync(path.join(root, "tests"), { recursive: true }).filter((f) => String(f).endsWith(".test.ts"));
