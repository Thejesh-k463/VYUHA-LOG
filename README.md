...
[![Tests](https://img.shields.io/badge/tests-6273%20passing-2ea44f)](tests)
...
> the journal, written once after the close or when you press *Save today's
> mark*, and never on a day the market did not trade. Your broker's API session
...
tests/       6273 unit + integration tests across 337 files (+ tests/load: 16 load cases, run separately)
...
| `npm test` | Vitest unit + integration suite (6273 tests) |
...
VYUHA-LOG/
  ...
  tests/          # 6273 Vitest unit + integration tests (+ tests/load)
```
