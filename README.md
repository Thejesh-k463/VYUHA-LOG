...
[![Tests](https://img.shields.io/badge/tests-6914%20passing-2ea44f)](tests)
...
> when you re-save the credentials — the one-time code is derived from the
> secret you enrolled rather than the secret being sent, so there is **nothing
> for you to click**, and a refused sign-in is tried at most three times before
> Vyuha stops until you re-save the credentials.
...
tests/       6914 unit + integration tests across 358 files (+ tests/load: 16 load cases, run separately)
...
| `npm test` | Vitest unit + integration suite (6914 tests) |
...
VYUHA-LOG/
  ...
  tests/          # 6914 Vitest unit + integration tests (+ tests/load)
```
