...
[![Tests](https://img.shields.io/badge/tests-11447%20passing-2ea44f)](tests)
[![E2E](https://img.shields.io/badge/e2e-135%20flows-2ea44f)](e2e)
...
> one — its row there stays greyed until you have saved your Upstox
> **Analytics token** under **Import → Connect broker**, and it prices nothing
> until you pick it — and it adds **no
> new credential and no daily sign-in**, because that token is issued
> read-only and lasts about a year. It is the **same token the trade import
> uses**, which is also the one way to break both at once: generating a fresh
> one at Upstox revokes the old one, and the import and the desk source stop
> together until you paste the new one back. While the desk is open Vyuha asks
> `api.upstox.com` for the instrument keys of the open positions of the
> selected account — **at most 500**, every **1 to 5 seconds** at the interval
> you set — and for **nothing else about them**: no quantity, no entry price,
> no P&L, no account. The prices stay on this machine: never uploaded, never
...
> **client code, PIN and TOTP secret** under **Import → Connect broker**. Vyuha
> signs in to `apiconnect.angelone.in` at most once a day while Vyuha stays
> open, again after a relaunch, after Angel One's 5 AM IST session flush, or
> when you re-save the credentials, and again when you switch the selected
> account, including to or from All accounts — the one-time code is derived
> from the secret you enrolled rather than the secret being sent, so there is
> **nothing for you to click**. If a sign-in is refused, Vyuha tries at most
> three times and then stops until you re-save the credentials or relaunch
> Vyuha. If Angel One
> reports an accepted session invalid, Vyuha signs in at most three times
> in a row without a priced answer in between, and then stops until you re-save
> the credentials or relaunch Vyuha.
...
> feed: each shows the position's recorded close, or a dash when no
...
> that day's date. `PRIVACY.md` item 3 states the poll and names the host; the
> list of network requests is still exactly four kinds, because this is the
> same kind — your own broker, only when you switched it on.
>
...
> the default), a mark you type, or that bridge. It is opt-in and stays behind a
...
> set on a 1–5 second slider, and the marks on screen refresh at that interval
> for as long as the screen is in front of you. The request carries the trading
> symbols and exchanges of your open positions and **nothing else about them**:
> no quantity, no entry price, no P&L, no account. **No new network host** is
> added, and **ticks are never written** — one mark per position per day reaches
> the journal, written by the app itself once after 15:30 IST while the desk is
> open — it reconnects its stream at 15:31 IST to do it — or the next time the
> desk opens that day, or when you press *Save today's mark*, and never at the
> **weekend**; exchange holidays are not modelled in this version. A price you
> type yourself is that day's mark: the automatic close-of-session mark does not overwrite it,
> and typing after the close replaces the automatic one; any bhavcopy applied for that day — the Auto-MTM job if you have switched it on, a file you drop or paste yourself, or the history backfill — replaces it with the exchange close. Already-marked
> is decided per symbol per IST day, so a second account's open positions get
> their own mark the same day. Your broker's API session
> expires daily and has to be signed in again at the bridge's own screen; that
> is the broker's rule. When the bridge is unreachable or has no key, the desk
> shows a once-a-day reminder to reconnect. Beside it: the Sizing Lab's
> volatility tab shows **both** of its variants — Turtle unit (N) and Varsity
> (% volatility) — side by side with each panel naming the variant that produced
> its number, and the seven tabs are unchanged; the Market Atlas prints the
> **sha256 of each bundled classification map's canonical JSON, as the app
> loaded it — not of the file on disk** — beside its as-of date; an instrument
> can carry the date its
...
> it, and **how old that price is** — end of day, entered by hand, or none
> stored yet. That much is **free on any copy**, licence or not, because it is
> the journal reading itself back. With Pro the same rows also carry the R
> multiple, the computed loss **if the stop is hit** (in rupees, and as a share
> of the capital you recorded) and the desk's portfolio heat — rows with no
> stop are counted and named as excluded rather than folded into the total, and
> with no capital on file heat says it has no denominator instead of inventing
> one. Expanding a row draws your entry, stop and target on the price itself,
> beside a stop computed four ways and a trailing rail of five, each printed
> with the parameters it used. Two more Pro screens ship with it: the **Sizing
> Lab**, which sizes one trade by seven methods side by side — fixed rupee,
> fixed fractional, Turtle unit, % volatility, Kelly, fixed ratio, equal weight
> — with every formula and input printed beside its number and charges taken
> from your own rate card; and the **Market Atlas**, which computes advance /
> decline with its denominators, % above the 20/50/200-day averages, 52-week
> highs and lows, volume expansion, the same window by sector and by NSE cap
> band, and your own open names against their sector cohort — all from
> end-of-day bars already on this machine, with no account, no upload and no
> new connection. Four migrations (0064–0067) run on first launch behind the
> automatic pre-migration copy.
>
...
> 2027-01-01; until then the launch offer sells at ₹7,999/yr and ₹29,999, and the savings
> badges are computed and floored — **38% off** and **16% off**, never rounded up. Lifetime is
...
> first time: **Pro — Annual ₹7,999/yr** and **Journal — Lifetime ₹29,999**, with the free tier
...
| **11447** | **45** | **0** |
| tests, 135 end-to-end flows | screens in the desktop app | bytes of *your data* uploaded without your say-so |
...
in-app with the date they were set, and the buy message embeds the price you saw.
...
| Backup & restore, full CSV/JSON export, audit log | The Costs hub — Charges & MTF Leak, Broker Costs (cross-broker MTF comparison) tabs |
...
- **Four live API pulls.** Zerodha (today's executions with fill times), Dhan (the only Dhan source that states **MTF** outright), **Upstox**, and **Angel One** — the one that needs no daily attention, because each pull mints its own login code from your TOTP secret.
...
- **Schedule-format export** — the same figures emitted in the return's own item codes: **Schedule CG** (A2 for STCG u/s 111A on ITR-2 — A3 on ITR-3, which numbers the schedule differently; the 112A box is AY-keyed — B4 for LTCG u/s 112A in AY 2025-26, B3 in AY 2026-27 — with the ₹1.25L deduction), **Schedule BP** for both business heads, and **Schedule CFL** with each loss vintage and the year it lapses. It also says which form your book implies — ITR-2, or ITR-3 the moment any intraday or F&O appears.
...
```bash
node scripts/license-issue.mjs buyer@email.com app --lifetime   # mint a Lifetime key (also records it)
node scripts/license-issue.mjs buyer@email.com app --years 1    # ...or a Pro Annual key
node scripts/license-issue.mjs buyer@email.com app --months 1   # ...or a Pro Monthly key (given on request)
...
tests/       11447 unit + integration tests across 482 files (+ tests/load: 16 load cases, run separately)
e2e/         135 Playwright flows through the real app, in 37 specs
docs/
...
| `npm test` | Vitest unit + integration suite (11447 tests) |
| `npm run test:e2e` | Playwright e2e — 135 flows incl. the Dhan transaction report, Lenses grouping and drill-down, delete-by-scope, unpriced-sale quarantine, status/outcome views, the backup export→restore round trip and account switching |
...
VYUHA-LOG/
  ...
  tests/          # 11447 Vitest unit + integration tests (+ tests/load)
```
