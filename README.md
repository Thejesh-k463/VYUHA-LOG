...
[![Tests](https://img.shields.io/badge/tests-11579%20passing-2ea44f)](tests)
...
> highs and lows, RSI-14 extremes, the advancing share of volume, volume
> expansion, relative strength against the market median, the same figures by
> sector, by industry and by AMFI cap band, your own open names against their
> industry cohort's median, and market breadth on the days you opened them —
> all from end-of-day bars already on this machine, with no account, no upload
> and no new connection. Four migrations (0064–0067) run on first launch behind the
...
tests/       11579 unit + integration tests across 489 files (+ tests/load: 16 load cases, run separately)
...
| `npm test` | Vitest unit + integration suite (11579 tests) |
...
VYUHA-LOG/
  ...
  tests/          # 11579 Vitest unit + integration tests (+ tests/load)
```
