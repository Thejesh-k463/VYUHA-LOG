...
- **Opt-in EOD auto-MTM**: once per trading day, fetch NSE's bhavcopy and mark open positions to close — and, on each app open, catch up to ten missed sessions of the last 252 (one file every 1.5 s) under the same consent. OFF by default; warns that it overwrites matched marks; skips silently offline; every run is audit-logged.
...
VYUHA-LOG/
  ...
  tests/          # 11879 Vitest unit + integration tests (+ tests/load)
```
