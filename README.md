...
[![Tests](https://img.shields.io/badge/tests-6392%20passing-2ea44f)](tests)
...
> and typing after the close replaces the automatic one; only the Auto-MTM bhavcopy job, if you keep it on, replaces it with the exchange close after 7 pm IST. Already-marked
...
tests/       6392 unit + integration tests across 341 files (+ tests/load: 16 load cases, run separately)
...
| `npm test` | Vitest unit + integration suite (6392 tests) |
...
VYUHA-LOG/
  ...
  tests/          # 6392 Vitest unit + integration tests (+ tests/load)
```
