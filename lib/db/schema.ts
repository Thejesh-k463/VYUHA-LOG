...
export const trades = sqliteTable(
  "trades",
  {
    ...
    /**
     * When this trade was REVIEWED on the Trade Review Desk (v3.7, migration
     * 0055). Written by the journal route on any review save and by the
     * queue's explicit "Mark reviewed"; nulled by "Reopen". Nothing clears it
     * automatically.
     *
     * Blank means UNREVIEWED — the same rule exitTrigger states above: the
     * queue is the set of closed trades where this is null, and no analytic
     * may bucket a blank as reviewed (invariant 6). Migration 0055 backfilled
     * it to COALESCE(updated_at, created_at) for rows that already carried
     * evidence of hand-journalling (notes / exit trigger / mistake tags), so
     * an upgrading book does not wake up with a queue of work already done.
     */
...
// ---------------------------------------------------------------------------
// weekly_reviews — the Sunday ritual, one row per (account, ISO week)
// (migration 0056, v3.7 WS1).
//
// `weekStart` is the ISO MONDAY 'YYYY-MM-DD' produced by the one week bucketer
// the product uses (lib/analytics/week.ts, shared with disciplineByWeek) — two
// bucketers that disagree would file a user's note against a week their score
// never covered.
//
// `scoreAtCompletion` is a HISTORICAL FACT: the Process Score the user was
// looking at when they completed the review, never a cache to read back as the
// current figure. The live score is always recomputed and labelled separately
// ("score then" vs "score now"), so a later import moves the live number
// without rewriting what the user actually saw. NULL is legitimate — under the
// floor the score refuses to exist, and storing 0 would fabricate one
// (invariant 6). `completedAt` NULL means the week is OPEN: a row may hold
// nothing but a note typed mid-week.
//
// UNIQUE(account, week): the week does not happen twice — a second write is an
// edit. The note is the USER'S OWN PROSE, which is why account deletion
// snapshots this table to trash and a merge APPENDS a colliding source note to
// the target's row instead of dropping a sentence the user wrote
...
export const weeklyReviews = sqliteTable(
  "weekly_reviews",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    accountId: integer("account_id").notNull(),
    /** ISO Monday of the week under review, "YYYY-MM-DD". */
    weekStart: text("week_start").notNull(),
    /** The user's own weekly note. Free text; never generated by the app. */
    note: text("note"),
    /** When the ritual was completed; NULL = the week is still open. */
    completedAt: text("completed_at"),
    /** The Process Score SHOWN at completion — history, not a cache. NULL when the week refused to score. */
    scoreAtCompletion: integer("score_at_completion"),
    createdAt: text("created_at").notNull().default(now),
    updatedAt: text("updated_at").notNull().default(now),
  },
  (t) => [uniqueIndex("weekly_reviews_account_week_uq").on(t.accountId, t.weekStart)],
);

// ---------------------------------------------------------------------------
// advance_tax_challans — dated advance-tax payments (migration 0058, v3.7 WS4).
// One row per challan the user really paid, transcribed from the receipt, so
// the engine can compute paid-AS-OF each s.234C instalment date instead of
// splitting one retyped scalar. Nothing is seeded or inferred from the journal.
//
// NO UNIQUE INDEX, deliberately: a challan serial is unique only per BSR code
// and BOTH are optional on a self-assessment receipt, so there is no natural
// key. A unique (fy, paid_on, amount) would refuse two genuinely separate
// payments of the same amount on the same day — real behaviour. The EDITOR
// warns on an exact duplicate; the schema allows it. The index below is a
// plain lookup index for the per-FY reads, not a constraint.
//
// Semantics = STATEMENTS OF FACT about money that left the user's bank, the
// same class as bfLossLots above (never aspirations like capitalGoals): on a
// merge they MOVE with the book, on a purge they are deleted with it, and like
// the lots they are NOT snapshotted to trash — the receipts exist.
//
// Money boundary (invariant 1): `amountPaise` is INTEGER paise at rest via
// `moneyPaise`, rupees at runtime. Converting again is the 100× bug.
// ---------------------------------------------------------------------------
export const advanceTaxChallans = sqliteTable(
  "advance_tax_challans",
  {
    id: integer("id").primaryKey({ autoIncrement: true }),
    accountId: integer("account_id").notNull(),
    /** FY the payment belongs to, "YYYY-YY" (e.g. "2026-27"). */
    fy: text("fy").notNull(),
    /** ISO date the challan was paid — what s.234C's per-rung "paid as of" reads. */
    paidOn: text("paid_on").notNull(),
    /** ₹ paid. Paise at rest, rupees at runtime. */
    amount: moneyPaise("amount_paise").notNull(),
    /** Bank branch BSR code from the receipt; optional (self-assessment receipts often omit it). */
    bsrCode: text("bsr_code"),
    /** Challan serial — unique only WITHIN a BSR code, hence no key here. */
    challanSerial: text("challan_serial"),
    note: text("note"),
    createdAt: text("created_at").notNull().default(now),
    updatedAt: text("updated_at").notNull().default(now),
  },
...
export const settings = sqliteTable("settings", {
  ...
  // First-run onboarding (v3.7, migration 0057). NULL = the wizard has never
  // been finished OR skipped; "Skip for now" sets it, so a skipped wizard does
  // not return every launch. MACHINE STATE like trialStartedAt above: absent
  // from BASELINE_SETTINGS_FIELDS and present in SETTINGS_MACHINE_COLUMNS, so
  // a restored backup neither re-shows the wizard to someone who finished it
