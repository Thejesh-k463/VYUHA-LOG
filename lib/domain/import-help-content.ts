...
export const IMPORT_HELP_CARDS: ImportHelpCard[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "As of Sep 2026, three more Dhan files are read and none of them books a trade. DP Charges (XLS) are the depository fees that appear in no other Dhan export; they land as charge entries on Cash & Ledger and as stated fees on Broker Truth, per scrip per day. The Demat Holding summary (XLSX) stores what the depository says you hold on one date. The signed contract note (PDF) adds the TIME of each fill to trades already imported — it never creates one, because the Global Transaction Report is the book and importing both would double-book the same executions — and its own brokerage, STT, GST and stamp lines are stored as stated charges, which Broker Truth compares against your book's charges for that trading day.",
  ...
  },
  {
    ...
    steps: [
      ...
      "As of Sep 2026 two more Angel files are read, and neither books a trade. The account statement (YourStatement) feeds the Cash & Ledger screen; its four charge tables are a breakdown of money the ledger has already posted, so they are shown, not posted again, and Broker Truth compares the DP and pledge lines against your Angel One trades' own charges — the CUSPA and interest lines have no column in the book and are shown with no comparison at all. The P&L statement (ProfitLoss_Statement) is stored as Angel's own stated figures — it states P&L, not the executions behind it, so your Angel book stays the tradebook.",
    ],
    api: [
  ...
  },
  {
    ...
    steps: [
      ...
      "Saving checks the host's format and the broker and makes no network call, so it works while OpenAlgo is stopped. The live check is Preview pull: press it once after saving, and a wrong key, a wrong port or an OpenAlgo older than 2.0.2.6 fails there with a message. A second instance appears as its own row with its own Preview / Pull & commit buttons.",
    ...
    notes: [
      ...
      "Vyuha's pull is read-only: it pulls trades only from /api/v1/tradebook, after reading OpenAlgo's version from /auth/app-info (a request that carries nothing, not even the key); saving the connection makes no network call, and the Live Desk's feed makes a read-only /api/v1/funds check each time it connects. It never places, modifies or cancels an order.",
      "Vyuha needs OpenAlgo 2.0.2.6 or later and refuses to pull from anything older, for every broker: before 2.0.2.4 Groww fills above ₹100 arrived at one hundredth of their price, before 2.0.2.6 Zerodha MCX sizes arrived in contracts, and older releases carry security holes since fixed. To upgrade: stop OpenAlgo, back up its db folder and .env, then git pull → uv sync → uv run upgrade/migrate_all.py (Windows: install\\update.bat) — and never copy .sample.env over an existing .env. A tradebook answered from OpenAlgo's Analyzer (sandbox) mode is refused whole, and rows on NCO (NSE commodities) or NCDEX are refused and named.",
    ],
    guide: OPENALGO_GUIDE,
  },
];
