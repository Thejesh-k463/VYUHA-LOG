...
export const IMPORT_HELP_CARDS: ImportHelpCard[] = [
  ...
  },
  {
    ...
    steps: [
      ...
      "As of Sep 2026, three more Dhan files are read and none of them books a trade. DP Charges (XLS) are the depository fees that appear in no other Dhan export; they land as charge entries on Cash & Ledger and as stated fees on Broker Truth, per scrip per day. The Demat Holding summary (XLSX) stores what the depository says you hold on one date. The signed contract note (PDF) adds the TIME of each fill to trades already imported — it never creates one, because the Global Transaction Report is the book and importing both would double-book the same executions.",
