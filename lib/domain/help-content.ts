...
export const HELP_ENTRIES: HelpEntry[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "The Upstox source needs no separate credential: it reuses the Analytics token you save under Import → Brokers, which you generate once in your own browser at account.upstox.com → Apps → Analytics. Upstox issues that token read-only and for about a year, so there is no daily sign-in, and it is stored encrypted at rest and sent nowhere except Upstox. Regenerating it there revokes the old one — which breaks the trade import and the Live Desk source at the same moment, so paste the new one back under Import → Brokers.",
