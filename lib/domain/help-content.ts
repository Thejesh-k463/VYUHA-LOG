...
export const HELP_ENTRIES: HelpEntry[] = [
  ...
  },
  {
    ...
    body: [
      ...
      "Marks are end-of-day by default. Which source prices the desk — the stored end-of-day bhavcopy, a mark you type, or the OpenAlgo bridge you run on your own machine, which is opt-in and offered only once you have accepted its disclosure — is chosen in Settings → Live feed, and the desk names the source and flags a mark older than the newest one on the screen.",
      "The OpenAlgo bridge is opt-in: it appears as a source only after you switch the integration on and accept the disclosure in Settings → Integrations, it asks your own bridge at 127.0.0.1 unless you entered another address, every 1 to 5 seconds while this screen is open, and its prices refresh on screen only — one mark per position per day is what reaches your journal, written after the close or when you press Save today's mark, whichever comes first: this screen reconnects its stream once at 15:31 IST while you have it open and the mark is written then, or the next time you open it that day. A price you type yourself is that day's mark: the automatic close-of-session mark does not overwrite it, and typing after the close replaces the automatic one; only the Auto-MTM bhavcopy job, if you keep it on, replaces it with the exchange close after 7 pm IST. Whether a mark already exists is decided per symbol per IST day, so a second account's open positions get their own mark on the same day. At the weekend nothing is written — exchange holidays are not modelled in this version. If the bridge cannot be reached, the desk offers to reconnect once a day rather than on every visit.",
  ...
  },
  {
    ...
    body: [
      ...
      "That same acceptance is what lets the Live Desk price your open positions from the third of its three sources — the stored end-of-day bhavcopy, a mark you type, or your own OpenAlgo bridge, which is opt-in, offered only behind that disclosure, and answers on 127.0.0.1 unless you enter another address — chosen in Settings → Live feed. It sends the trading symbols of your open positions and nothing else about them, and its prices refresh on screen only.",
