...
export const HELP_ENTRIES: HelpEntry[] = [
  ...
  },
  {
    ...
    body: [
      ...
      // discovered.
      "The Angel One source needs no separate credential either: it reuses the client code, PIN and TOTP secret you save under Import → Connect broker, and it is opt-in behind its own disclosure. Vyuha opens each API session by itself — at most once a day while Vyuha stays open, again after a relaunch, after Angel One's 5 AM IST session flush, or when you re-save the credentials — from a one-time code it derives from that TOTP secret, so there is nothing to click; if a sign-in is refused, Vyuha tries at most three times and then stops until you re-save the credentials. Those credentials are encrypted at rest and sent nowhere except Angel One's own API host.",
