...
export const HELP_ENTRIES: HelpEntry[] = [
  ...
  },
  {
    ...
    body: [
      ...
      // discovered.
      "The Angel One source needs no separate credential either: it reuses the client code, PIN and TOTP secret you save under Import → Connect broker, and it is opt-in behind its own disclosure. Angel One clears every API session at 5 AM IST, so Vyuha opens the next one by itself each morning from that TOTP secret — there is no daily prompt and nothing to click. Those credentials are encrypted at rest and sent nowhere except Angel One's own API host.",
      ...
      "If the feed you picked can no longer run — its disclosure changed since you accepted it, or a backup was restored on a machine that never accepted it, so your consent is asked for again — Settings → Live feed says the feed is blocked, the desk stays on end-of-day prices meanwhile, and Review and accept reopens that feed's sheet so you can read it and accept it again.",
