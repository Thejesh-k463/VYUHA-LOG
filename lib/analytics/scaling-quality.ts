...
export function scalingQuality(inputs: ScalingTradeInput[]): ScalingReport {
  const rows = inputs.map((t): ScalingRow => {
    ...
    const base = r2(gross - entryCharges - exitCharges); const impact = r2(pos.realisedNet - base); const threshold = Math.max(10, Math.abs(base) * 0.01);
    return { id: t.id, symbol: t.symbol, entries: pos.entryCount, exits: pos.exitCount, actualNet: pos.realisedNet, firstEntryOnlyNet: base, scalingImpact: impact, verdict: impact > threshold ? "improved" : impact < -threshold ? "harmed" : "neutral" };
  });
