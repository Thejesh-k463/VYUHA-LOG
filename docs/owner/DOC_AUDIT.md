...
| 9 | Landing page — version, prices, FAQ | `docs/sales/landing-page.html` | Hero pill `vX.Y.Z`; every price cell equals `lib/domain/pricing.ts` (₹9,999 / ₹13,000 · ₹29,999 / ₹35,999 today); the upgrade-credit sentence verbatim; refund FAQ (~lines 439–446) matches `REFUND_POLICY.md`; no macOS. Then **`npm run landing:build`** to regenerate `landing-page.standalone.html`. | Prices + upgrade sentence: `tests/pricing.test.ts` anti-drift block. Version pill: eyes only (grep `2\.99\.9`). Standalone freshness: `docs/sales/landing-page.standalone.html` is **gitignored** (.gitignore:32), so `git status` will never show it — check its mtime against `landing-page.html` instead, or just re-run `landing:build`, which is idempotent. |
