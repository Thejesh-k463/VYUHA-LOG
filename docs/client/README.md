...
|---|---|
| **Live Desk — every open position on one screen** | A new screen (**Live Desk**, at the top of Positions) lists what you still hold, the mark Vyuha has for each name, and the profit or loss that follows from it. Every row says its mark is a **stored** one and **how old that mark is** — or that none is stored yet — because a P&L computed on a stale price is a figure with a date attached, and hiding that date is how a screen lies quietly. Vyuha does not label where a stored mark came from: the end-of-day close and a price you typed sit in one table, and a label it cannot prove would be worse than none. Filter, sort and group as you do everywhere else; `j` / `k` move, Enter expands a row, `/` filters. **This screen is not gated.** Positions, marks and P&L are your own record looking at itself, and the core journal is never held back. |
