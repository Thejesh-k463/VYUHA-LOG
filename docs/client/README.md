...
| **Equities this release, and the rows say so** | The Upstox and Angel One sources price **equity** positions. Futures and options rows are not priced by this feed: each shows the position's recorded close, or its entry price when no close is recorded, and says so on the row — rather than showing a stale number as though it were fresh. |
| **When a feed is blocked, the desk says so — and one control puts it right** | A feed you picked can stop being allowed to run: its disclosure changed since you accepted it, or a backup was restored on a machine that never accepted it, so your consent is asked for again. **Settings → Live feed** then says the feed is blocked, the desk stays on end-of-day prices meanwhile, and **Review and accept** reopens that feed's sheet so you can read it and accept it again. |
