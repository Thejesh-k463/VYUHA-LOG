...
| **Angel One can price the Live Desk too, and signs itself in** | **Settings → Live feed** gains a fifth source: Angel One. It is opt-in like the others — its row here stays greyed until you have saved your Angel One **client code, PIN and TOTP secret** under **Import → Connect broker**, and it prices nothing until you pick it here — and it needs no new credential. Angel One clears every API session at **5 AM IST**, so Vyuha opens the next one itself each morning from the secret you enrolled: there is nothing for you to click. While the desk is open it asks `apiconnect.angelone.in` — already the host the Angel One trade pull uses, so no new host is added — for the open positions of the account you have selected, **at most 500 of them**, in batches of **50 symbols** at about one request a second, and for nothing else about them: no quantity, no entry price, no P&L, no account name. `PRIVACY.md` item 3 states the poll. |
| **Angel One's refresh is set by the size of your book, not by a slider** | Angel One allows about one request a second, so the interval is arithmetic rather than a preference: **3 seconds** up to 50 open positions, **5 seconds** from 51 to 200, **10 seconds** from 201 to 500. The 1–5 second slider is therefore hidden under this source and replaced by one line stating the interval and the number of calls each refresh takes. Every other source keeps the slider exactly as it was. |
| **Equities this release, and the rows say so** | The Upstox and Angel One sources price **equity** positions. A futures or options row keeps the mark already stored for it and states that on the row, rather than showing a stale number as though it were fresh. |
