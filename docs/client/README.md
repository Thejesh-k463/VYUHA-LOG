...
The release where the journal learns the market's real clock, two more brokers import themselves, every listed stock carries the exchange's own industry, and a covered short is stored as a short.

| Upgrade | What it gives you |
|---|---|
| **Fyers and Nuvama — eight brokers now auto-detect** | **Fyers:** the tradebook imports as trades (the mirror row Fyers prints beside every fill is skipped, never imported) and the Realised P&L file is kept as the broker's own figures on Broker Truth. A Fyers file is recognised **only when its filename contains "fyers"** — the file itself never names the broker, so a renamed file goes to the column mapper, which asks. **Nuvama:** the P&L report's realised and unrealised lines import as trades, with the brokerage, GST, STT, stamp and exchange charges **Nuvama billed stored as stated**. Rate cards ship for **Fyers Standard (up to ₹20 an order)** and **Prime (up to ₹15)** and for **Nuvama Lite Plus** and **Elite**, read from the brokers' own pricing pages; pick the plan per account in **Settings → Accounts**. Both brokers' **F&O and MCX rows are verified** against real exports; **equity rows are UNVERIFIED** — they import with a warning asking you to check them against your contract note. |
| **The market's real clock** | One bundled, effective-dated **market calendar** now answers every timing question: SEBI's **Closing Auction Session** from **3 August 2026**, the F&O session to **15:40**, the revised pre-open from **7 September 2026**, the NSE holidays, Budget Sunday and Muhurat. The day's mark waits for the **official close** — **15:36** for a closing-auction stock, **15:31** for any other stock, **15:45** for an F&O contract — so a pre-auction price is never saved as the close. A fill between 15:30 and 15:40 is no longer flagged "outside the session", and the sidebar's market dot is shut on a holiday. `/instruments` shows the calendar's as-of date, the date it covers through and its sha256. |
| **The exchange's own industry, and AMFI's cap band** | A bundled **stock universe** gives **5,450 of 5,482** listed equities (99.4%) the NSE/BSE **four-level industry classification**. **AMFI's half-yearly rank is the cap band** (large / mid / small); an NSE Emerge stock shows a blank band with **"SME — not ranked by AMFI"**, never a guess, and Nifty size-index membership (where "micro" lives) is its own separate lens. A sector you tagged yourself is never overwritten. A stock whose ISIN changed at a face-value split follows to its new ISIN. `/instruments` shows the universe's and the NSE index map's as-of dates and sha256. |
| **Market Atlas v3** | **Industry cohorts** (falling back to the sector when an industry is too thin, always stating "n of m priced"), **median** group returns with the mean behind a labelled toggle, **one rotation table** at sector or industry for 1w / 1m / 3m / YTD, **relative strength against the market median**, breadth per group and per AMFI band, the breadth **on the days you opened your positions**, an index-membership filter, and a list of what the Atlas does **not** compute. Below 30% coverage a tile shows its coverage instead of a number. The regime thresholds are **editable in Settings** and travel with your backup. With the end-of-day download switched on, **missed sessions are topped up** — at most ten files per app open, under the same consent, re-checked before every file. |
| **Three analytics hubs** | **Edge Clinic** (Setups · Discipline · Scaling & Replay), **Capital & Expiry** (Return on Margin · Expiry — the Expiry tab hidden for an equity-only workspace) and **Costs** (Charges & MTF Leak · Broker Costs). The Analytics menu goes from ten entries to six; every old address still opens the right tab, and a saved sidebar order carries over. |
| **Help, rewritten task-first** | `/help` opens on a search box over topics that fold by category; a topic opens in a **large-text dialog** with a few steps, a watch-out and related topics, and one Escape closes it. A **"?"** in every page header opens that page's topic, glossary terms explain themselves on hover, **`?`** opens a keyboard-shortcuts sheet, and the command palette finds help as you type. A **getting-started strip** on the dashboard — account made, trades imported, charges plan set, a stop recorded, a backup taken — disappears once all five are done or you dismiss it. |
| **A covered short is stored as a short** | A trade now records **which side opened it** wherever its fills, dates or notes say so. An overnight F&O sale covered by a later buy **in the same file** becomes **one closed short** (with a Data Quality line naming the other reading), filed in the **financial year it was covered** on every tax surface; a sale nothing covers stays an opening sell. A short pair imported before v4.6.0 — an opening sell plus an open buy — is listed in Data Quality and joined only when you confirm it. **Return on Margin** and the **Signal book** now read the side, so a short option is no longer priced or judged as if it were bought. |
| **A faster `/trades`** | The KPI strip and the side panels no longer read your whole book to draw — measured on a 25,001-row book, where that read was the page's largest cost. |
| **Tax follow-ups** | The **AIS reconcile** states a staged position's purchases **per entry fill**, each in its own financial year. **Harvest's** STT split counts **one trade per ladder**, with its fills listed beneath. The **31-Jan-2018 FMV editor** lists **one row per scrip** for the tax person, and one save sets every eligible lot of it — a partly-sold pre-2018 ladder can be edited at last. Upstox's **~₹1** difference from a contract note is explained where it shows: the contract note rounds STT once, Vyuha rounds it per trade row. |
| **Fixed: the ITR pack's grandfathered gain** | `/reports/itr` read the **per-share** 31-Jan-2018 FMV as a **total**, so a pre-February-2018 lot with an FMV entered showed too large a long-term gain on the ITR pack, Schedule CG/CFL and the export — in one worked case (100 shares bought in 2016 at ₹100, FMV ₹250, sold for ₹30,000) **₹20,000 instead of ₹5,000**. It now agrees with `/reports/tax`. The FMV editor also said "closing price"; the Act says the **highest price quoted on 31-Jan-2018** — **if you entered a close, check it.** |
| **Fixed: Paytm Money and Groww ladders sum to their position** | A fill split across two positions was handed to both, so a ladder could state more shares than its position. Each fill is now allocated once, and some positions that used to arrive as staged ladders now arrive as single rows. The positions' own quantities, values and P&L are unchanged. **A book imported before v4.6.0 keeps its old ladders:** Data Quality lists them as **"Staged positions whose fills do not add up to the position"** — delete the position, then re-import the same file, and this version writes a ladder that sums to it. Until then those positions' fill lists and per-fill tax rows are the old ones. |
| **Also fixed** | A basis write on a staged position is refused (it would leave the fills contradicting the position); the Atlas cap band resolves on a fresh install; the getting-started **"Charges plan set"** step can tick; the broker counts on screen are derived from the importer list, never typed. |
| **OpenAlgo: 2.0.2.6 or later** | Vyuha reads the running version from your own OpenAlgo and refuses a pull below **2.0.2.6** with the upgrade steps. A sandbox (Analyzer-mode) tradebook, an exchange Vyuha cannot price (NCO, NCDEX) and a zero-quantity MCX row are refused, never guessed. **Fyers** can be pulled through OpenAlgo; **Nuvama** cannot. If you use OpenAlgo you are asked once to re-acknowledge its disclosure (**version 4**). **OPENALGO_SETUP_GUIDE** in this package gains the version-and-upgrade section. |
| **Upgrades in place** | A v4.5.0 journal opens straight into v4.6.0. **Four database upgrades (0075–0078)** are applied on first launch: 0075 moves a Telegram digest time still at the old **15:35** default to **15:45**, after the F&O close (a time you typed is left alone); 0076 adds the Atlas regime thresholds; 0077 adds the side column, and a one-time data fix states each existing trade's side from its own dates and notes (blank where they cannot tell); 0078 adds the **16 Fyers and Nuvama margin rows**. The first start also adds the **280 Fyers and Nuvama rate rows**. A row you added or edited yourself is never touched, and no money figure already stored on a trade moves. Before any of it runs, the app writes a full copy of your database to `backups\pre-migrate-<timestamp>.sqlite` in your data folder; **take your own backup first, as always**. The installer runs the v4.5.0 uninstaller once first, and that one is the guarded uninstaller: ticking "Delete the application data" still erases the data folder, but only after your journal and licence key have been named and copied to `Documents\Vyuha-backup-<date>`, and Cancel leaves everything as it is. |

**Not in this release, said plainly.** There is **no macOS build** — Windows only. The installer is **not code-signed**, so
Windows SmartScreen warns once ("More info → Run anyway"); that is expected. **No network host** is added: the market calendar,
the stock universe and the index map are build-time snapshots, and the Atlas top-up reads the same NSE archive, under the same
consent, as the end-of-day download. **Kotak Neo and Sahi** still import through the column mapper only. **Fyers and Nuvama
equity rows** are unverified until a real equity export has been seen; Nuvama's per-lot option fees are estimated per order,
which under-estimates a multi-lot order's estimate (a bill Nuvama stated is kept as stated); Nuvama publishes no MTF interest
rate and none is invented. The **2027 NSE holiday list** is not yet published, so the calendar covers 2026 — Data Quality says
so from 60 days before its end. The **Clinic** tab of the Edge Clinic hub arrives in v4.7.0. **F&O contract notes** are still
not read. Charges already stored on trades are not rewritten by any of this.
...
metric shows its working.

| Upgrade | What it gives you |
|---|---|
| **Zerodha Console ledger** | The funds ledger exported from **Zerodha Console** imports. Every entry's Net Balance reconciles against the opening balance, and the closing balance matches the statement's own. |
| **Groww balance statement** | The statement from Groww's **Cash & Ledger** screen imports, across its **18** segment types. Its rows are sorted by settlement date but not by posting order within a day, so each row is placed by its own balance rather than by where it sits in the file — that is what makes a 447-row statement reconcile with no breaks. |
| **Groww and Upstox contract notes — equity** | The **equity** contract note of either broker imports its fills, its charges and its net obligation. A Groww note's BSE fills whose 19-digit order number wraps onto a second line are re-joined rather than dropped. **F&O contract notes are not read** in this release; only the equity ones. |
| **Upstox F&O lands as option contracts** | An Upstox trade report's F&O rows are read with Upstox's own grammar (European Call/Put, FON/FOB, BSX = SENSEX, dd-mm-yyyy expiry), so **NIFTY** and **SENSEX** option trades land as **options** and are charged as options instead of as equity. |
| **The sidebar resizes** | Drag the sidebar's edge between **180 and 420 px**, or focus the handle and use the arrow keys; **double-click resets** it to the default. The width is remembered **on this device**, and it is re-clamped on every read, so a width saved on a wide monitor still leaves room on a narrow one. |
| **Appearance has its own Save** | The **Appearance** block now sits at the **end** of Settings and saves on its own, so a theme change no longer depends on another section's save. |
| **A new default card order, and Rearrange** | Most pages open in a new default card order. **Settings** and the **Dashboard** — and only those two — gain a **Rearrange** button that **moves** sections into the order you want; nothing is hidden or added, and the order is kept **on this device**. |
| **The per-trade risk cap is per segment** | The cap that sizes a trade with no plan stop can now be set **per segment** on top of the global one. **Avg R moves on imported trades that carry no plan stop wherever a segment cap differs from the ₹9,500 default** — that is the point of the setting, and editing a cap re-prices those trades. A limit that was never set reads **"—"** rather than an invented number, and a breach check for a never-set segment follows the global cap as you have edited it. |
| **Avg R says where its R came from** | Beside Avg R, everywhere it is printed — inside the exported **share-card PNG** included — is the split between R derived from your **plan** stop, R you **typed**, and R sized by the **default cap**. |
| **A per-lot line on F&O rows** | Where the lot size is known and the quantity is a whole number of lots, an F&O row also states **"1R = ₹X per lot"**, with the lot size and the date it was read from. A row whose lot size cannot be established reads "—" rather than guess. |
| **Profit factor, after charges** | The headline profit factor is computed **after charges**, and a drill-down reproduces it from the same winners' and losers' net figures it divides. A **per-segment table** sits beside it, free on every copy. |
| **Annualised on the NSE calendar** | Ratios annualise over the trading days in the bundled NSE calendar — **245 sessions in 2026** — instead of a 252 convention. On the same trades, **Sharpe, Sortino and volatility read about 1.4% lower and alpha about 2.8% lower** than in 4.3.0, and the Monte Carlo horizon is 245. **CAGR, Calmar, total return and drawdown are unchanged**: they are calendar-day figures and never used the 252. |
| **One dated risk-free rate** | Settings holds a single risk-free rate with the date it applies from, **7%** by default, replacing three rates that were hard-coded in separate places. |
| **Empty figures read "—", and every explainer was rewritten** | A figure with no denominator prints a dash instead of 0. Every metric explainer now works the figure out from **your own arithmetic**, and any statement about other traders carries a **cited source**; the invented "healthy" bands are gone. |
| **Four fixes** | The **Outcome lens** states **"Share of trades"** — it is a count, and it says so. Every **trade-form number** goes through one rule: "1,23,456.50" is read as written, ".5" gets its leading zero, and an ambiguous "14,48" is **refused** rather than guessed at. Closing a **stale lot** refuses an unreadable stored date instead of joining it at zero days of interest. The **Signal book** has its help entry. |
| **The monthly plan** | Pro — Monthly is **₹999 for your first month** as a launch offer, then **₹1,499 a month**. **Pro — Annual (₹7,999/yr)** and **Journal — Lifetime (₹29,999)** are unchanged. It is given on request — message us and a one-month key is issued; nothing auto-renews and no card is stored. |
| **Upgrades in place** | A v4.3.0 journal opens straight into v4.4.0. **One database upgrade** (0073) is applied on first launch: it adds columns, and it moves **no money figure and no risk cap** — an install whose cap was never edited stays at ₹9,500 and a cap you edited is left exactly as you set it. Before it runs, the app writes a full copy of your database to `backups\pre-migrate-<timestamp>.sqlite` in your data folder. Upgrading from **v4.2.0** applies **0071, 0072 and 0073** in order. The installer runs the v4.3.0 uninstaller once first, and that one is the guarded uninstaller: ticking "Delete the application data" still erases the data folder, but only after your journal and licence key have been named and copied to `Documents\Vyuha-backup-<date>`, and Cancel leaves everything as it is. |

**Not in this release, said plainly.** An execution that closes a position your journal already holds is **not** matched
against it automatically — an imported or pulled exit still lands as its own row, and Data Quality is where you join the
two. **F&O contract notes** are not read; only the equity ones, and only from Groww and Upstox. Kotak Neo, Sahi and
Paytm F&O files still belong to the generic column mapper, where you say whose file it is. **Rearrange** is on Settings
and the Dashboard only, it moves sections and never hides or adds one, and the order is stored on this device rather
than in your journal — the same is true of the sidebar width. Tabs are not movable. No Greek changed, there is no
scanner, there is no post-tax view and no benchmark series. The Upstox and Angel One price sources still price equity
positions only. Telegram messages for a stop or target are not built and are not part of Pro. The rate card is still not
exact everywhere: a date before the earliest schedule verified from the exchanges' own circulars (NSE before 1 January
2021, BSE cash before 1 March 2021, BSE options before 20 August 2019) takes that earliest schedule; the card has one
BSE index-option rate, which follows Sensex and Bankex; MCX charges are unchanged; other BSE stock groups and BSE's own
investor-protection contribution are not modelled; and charges already stored on trades are not rewritten.

## New in v4.3.0

The release where the option legs you hold are named, and where the signal a trade was taken on is recorded beside it.

| Upgrade | What it gives you |
|---|---|
| **Option Strategies names your structure** | **Option Strategies** reads your open option legs per underlying symbol and names the structure they add up to, from a catalogue of **40 shapes**: verticals, straddles, strangles, condors, butterflies, ratios, backspreads, calendars, synthetics, the box and the jade lizard among them. A calendar or a diagonal stays one position. A symbol whose legs fit no single shape across two or more expiries is read per expiry. A stock or future you hold against the options is drawn into the structure, read-only. |
| **Every figure says how it was computed** | Each payoff is drawn at expiry from your entry premiums, intrinsic value only — no volatility and no time value. A group that spans expiries is drawn at its nearest expiry with the later legs valued at intrinsic only, and its maximum profit reads "Not computed" rather than a confident number. The figures are **before charges**, stated once under the cards beside one note on STT at exercise. A cap reached only if the underlying settled at zero is labelled **"Computed at underlying = 0"**. There are no Greeks on this screen. |
| **The Options Help Desk — free** | A highlighted Options section in the Help Desk writes up all **40** structures: the legs, how the payoff is computed, who uses it and what it risks. It is free on every copy. Each strategy card carries a **How this works** link to its entry, and the command palette finds an entry by its name. |
| **The shelf, and what is Pro** | The grouping, the payoff curve and the gross figures are free for every book, and the sixteen shapes Vyuha named before 4.3 keep their names on the free tier. The other twenty-four names are part of Vyuha Pro: on a free copy a group matching one of them reads **"Custom (n legs)"** with a Pro chip. Also Pro is the **shelf**: eight tiles by default above your named groups. **Browse all (40)** opens the picker, where you select or unselect any tile, with **Undo**, **Redo** and **Restore defaults**. Your selection is kept with your settings and travels in your backups. |
| **The Signal book** | An option trade can now carry the signal it was taken on. The Add and Edit trade forms gain a **Signal** section — model, spot, zone, the open-interest and volume figures, a score, and the **T1 / T2 / SL** ladder, which pre-fills at +30% / +60% / −25% of your entry and is editable. Option Strategies opens on a **Signal book** tab listing every recorded signal with its option data; with Pro it also shows rule adherence against each trade's **own** levels, edge by model, direction and exit status, and ladder hit rates from the day's recorded high and low. Vyuha records the signal — it does not produce one: there is no scanner, no zone engine, no open-interest feed and no alerting here, and this release adds no network host of any kind. |
| **Trades already in your journal join the Signal book** | A trade whose note carries the signal in full is read into the book on first launch. A note that cannot be read in full is left exactly as it is. A signal you clear stays cleared — a later backup restore does not bring it back. |
| **Data Quality joins a held lot to the sale that closed it** | A sale of something your journal already held lands as a row of its own, so the position still reads open and the sale reads as a second position. Data Quality lists those pairs side by side, and **Close with the recorded sale** (for a short, *the recorded purchase*) closes the lot at that row's own price and quantity and moves the sale row to Deleted items. Nothing is typed except the date, and only when the sale row states none — it is pre-filled with the day the row was pulled, because the close date sets the charge rates, the holding period and any MTF interest. A partial pair, a sale recorded in several fills, a staged lot, a sale carrying your own journal entries, and a pair another closed trade may already have taken are listed **without** the button, with a link to Trades. |
| **Data Quality finds copies held in two accounts** | Two more checks: the same broker client connected in more than one account, and the same broker record held in more than one account. A duplicated record gets a **Remove the copy in …** button, which asks you to confirm once, and Vyuha checks again before it deletes anything. A duplicated connection links to Import instead. |
| **One broker client, one account** | Saving a broker connection whose client is already connected in another account is refused before anything is written, and the message names that account — the same tradebook pulled into two accounts is two copies of every trade. |
| **Dhan catches up after a gap** | Dhan's positions call returns today's book only, so a connection last pulled days ago used to lose every day in between. When your last successful Dhan pull was on an earlier day, the next pull also reads Dhan's trade history from that day to today — at most **90 days** back and 50 pages. A fill read twice is de-duplicated when it is saved. While the connection is behind, the Dhan tab says **"Pulls missed since …"**. |
| **Dhan names what it could not read** | A gap longer than 90 days is read for its last 90 days only, and the pull says which dates it did not fetch and that a Dhan tradebook imported for those dates brings them in. A pull that stops at the 50-page limit says so, with the dates that may be missing. The notice stays on the Dhan tab — whether the pull was yours or the auto-pull's — until you press **Clear notice**, and both the notice and the clearing are kept in the Audit Log. |
| **Streaks and drawdown read the day forwards** | The streak and drawdown figures sorted closed trades on the exit date alone, so trades closed on the same day were read backwards and runs that crossed a day boundary were joined or cut. A 42-trade options book read "8 wins, best 11W" where its entry order says 7 and 10. Closed trades now tiebreak exit date, then exit time, then trade id — on the dashboard and on Lenses alike. |
| **MTF financing states only what you recorded** | An MTF position bills interest only on the funded amount you recorded, and a staged position splits it across its tranches by value. A row with none recorded states no financing figure anywhere, says why beside the dash, and accrues nothing until you record one; closed staged rows priced before 4.3.0 keep the figure they already stored. The Equity Tracker's MTF funded figure is the funding every MTF row states, the broker comparison includes no financing cost for a row with no recorded funded amount and says so, and closed MTF trades with no recorded funding are listed on Data Quality for information. |
| **The trade editor keeps the bill your broker stated** | A notes-only save no longer replaces broker-stated charges with Vyuha's own estimate, the charge preview prices a row exactly as the save stores it, an un-exited IPO allotment is never billed purchase STT, and a staged position's charges are written only by its own ladder. Where a stored date is not a real calendar day, the editor says so and waits for a day rather than clearing the date on Save. |
| **A figure with no basis is blank, not zero** | A dashboard whose closed trades have no cost basis no longer shows a 0% win rate or a ₹0 expectancy — the figure is blank until a priced trade exists — and the edge report's win rate, confidence interval and expectancy per setup and per segment count only priced trades, the same rule as the dashboard. |
| **The spot chip opens an editor** | On **Portfolio Risk**, the `spot?` chip now opens a field where you type the underlying's spot in place. With nothing typed, it shows the latest end-of-day close on this machine, labelled **EOD close**, and the ITM/OTM badges read from it. A price you type is saved as that day's mark for the underlying and outranks the close. |
| **Smaller things you will notice** | The splash carries the line *Every warrior had a charioteer. Yours keeps count.* The Help Desk's text and every dialog's body text are one size larger. A pasted broker token's expiry is stamped in IST ("05 Sep 2026, 23:24 IST"). A percentage printed beside a rupee figure takes that figure's sign, and zero carries no sign. |
| **Charge rates repaired on first launch** | The desktop rate-card refresh had failed on every launch since v3.2.0; 4.3.0 repairs the F&O STT rates for dates before 1 April 2026 on first launch, adds the STT rates in force before 1 October 2024, dates the NSE and BSE exchange transaction charges and NSE's investor-protection fee by the exchanges' own circulars, and corrects today's NSE transaction charges and the BSE stock-option charge; charges already stored on trades are not rewritten. |
| **A monthly plan, on request** | Alongside Pro — Annual (**₹7,999/yr**) and Journal — Lifetime (**₹29,999**) there is now **Pro — Monthly: ₹999 for your first month as a launch offer, then ₹1,499 a month**. It is **given on request** — message us and a one-month key is issued; each month renews with a fresh key. Nothing auto-renews and no card is stored, so there is nothing to cancel: if you do nothing, the key simply lapses and the core journal keeps working. Move to Annual or Lifetime whenever you like. |
| **Upgrades in place** | A v4.2.0 journal opens straight into v4.3.0. **Two database upgrades** (0071, the strategy-shelf preference; 0072, the optional signal column) are applied on first launch, they change no money figure, and before them the app writes a full copy of your database to `backups\pre-migrate-<timestamp>.sqlite` in your data folder. The installer runs the v4.2.0 uninstaller once first, and that one is the guarded uninstaller: ticking "Delete the application data" still erases the data folder, but only after your journal and licence key have been named and copied to `Documents\Vyuha-backup-<date>`, and Cancel leaves everything as it is. |

**Not in this release, said plainly.** An execution that closes a position your journal already holds is **not** matched
against it automatically — an imported or pulled exit still lands as its own row, and Data Quality is where you join the
two. The Upstox and Angel One price sources still price equity positions only: an option or future row under either says
so on the row, and valuing an option from its underlying's price is deferred rather than approximated. Telegram messages
for a stop or target are not built and are not part of Pro. The figures on Option Strategies are computed at expiry from
your entry premiums; they are not a forecast and not advice, and the Signal book reports what you recorded rather than
judging it. The day's high and low it reads are the whole session's range and not the range since your entry, and the
screen says so. The rate card is still not exact everywhere: a date before the earliest schedule verified from the
exchanges' own circulars (NSE before 1 January 2021, BSE cash before 1 March 2021, BSE options before 20 August 2019)
takes that earliest schedule; the card has one BSE index-option rate, which follows Sensex and Bankex, so Sensex 50
options — and, from 1 November 2023 to 13 May 2024, Bankex options and Sensex options other than the nearest expiry —
are charged at that rate; MCX charges are unchanged; other BSE stock groups and BSE's own investor-protection
contribution are not modelled; and charges already stored on trades are not rewritten.

## New in v4.2.0

The release where the Live Desk can be priced by credentials you already
saved — Upstox or Angel One — and where the close-of-session mark knows the
exchange calendar.

| Upgrade | What it gives you |
|---|---|
| **Upstox can price the Live Desk, on the token you already have** | **Settings → Live feed** gains a fourth source: Upstox. It is opt-in exactly like the others — its row here stays greyed until you have saved your Upstox **Analytics token** under **Import → Connect broker**, and it prices nothing until you pick it here — and it needs no new credential and no daily sign-in, because Upstox issues that token read-only and for about a year. While the desk is open Vyuha asks `api.upstox.com` for the instrument keys of the open positions of the account you have selected, **at most 500 of them**, every **1 to 5 seconds** at the interval you set, and for nothing else about them: no quantity, no entry price, no P&L, no account name. The prices stay on this machine — never uploaded, never resold — and nothing is written per tick: the one-mark-a-day rule below is unchanged. `PRIVACY.md` item 3 states the poll and names the host. |
| **One token, two jobs — and one way to break both** | The Live Desk source and the Upstox trade pull use the **same** Analytics token. You generate it once, in your own browser, at **account.upstox.com → Apps → Analytics**, and register your current IPv4 address under **Static IPs** as the import already asks you to. Generating a fresh one there **revokes the old one**, so a regeneration stops the import and the desk source at the same moment — paste the new token back under **Import → Connect broker** and both work again. Said here because one afternoon where both go quiet otherwise reads as two separate faults. |
| **Angel One can price the Live Desk too, and signs itself in** | **Settings → Live feed** gains a fifth source: Angel One. It is opt-in like the others — its row here stays greyed until you have saved your Angel One **client code, PIN and TOTP secret** under **Import → Connect broker**, and it prices nothing until you pick it here — and it needs no new credential. Vyuha signs in to `apiconnect.angelone.in` at most once a day while Vyuha stays open, again after a relaunch, after Angel One's 5 AM IST session flush, or when you re-save the credentials, and again when you switch the selected account, including to or from All accounts — it derives a one-time code from the secret you enrolled rather than sending the secret itself, so there is nothing for you to click. If a sign-in is refused, Vyuha tries at most three times and then stops until you re-save the credentials or relaunch Vyuha. If Angel One reports an accepted session invalid, Vyuha signs in at most three times in a row without a priced answer in between, and then stops until you re-save the credentials or relaunch Vyuha. While the desk is open it asks that same host — already the one the Angel One trade pull uses, so no new host is added — for the open positions of the account you have selected, **at most 500 of them**, in batches of **50 symbols** at about one request a second, and for nothing else about them: no quantity, no entry price, no P&L, no account name. `PRIVACY.md` item 3 states the poll. |
| **Angel One's refresh is set by the size of your book, not by a slider** | Angel One allows about one request a second, so the interval is arithmetic rather than a preference, and it is counted in **scrips** — the distinct symbols your positions come to, not the positions themselves, because two positions in one scrip are one price to ask for: **3 seconds** up to 50 scrips, **5 seconds** from 51 to 200, **10 seconds** from 201 to 500. The 1–5 second slider is therefore hidden under this source and replaced by one line stating the interval and the number of calls each refresh takes. Every other source keeps the slider exactly as it was. |
| **Equities this release, and the rows say so** | The Upstox and Angel One sources price **equity** positions. Futures and options rows are not priced by this feed: each shows the position's recorded close, or a dash when no close is recorded, and says so on the row — rather than showing a stale number as though it were fresh. |
| **When a feed is blocked, the desk says so — and one control puts it right** | A feed you picked can stop being allowed to run: its disclosure changed since you accepted it, or a backup was restored on a machine that never accepted it, so your consent is asked for again. **Settings → Live feed** then says the feed is blocked and the desk stays on end-of-day prices meanwhile. For the Upstox and Angel One feeds, **Review and accept** reopens that feed's sheet so you can read it and accept it again; for OpenAlgo, whose consent lives on the Integrations screen, the block is stated in words and you switch the integration back on under **Settings → Integrations**. |
| **The close-of-session mark knows the holidays** | The automatic mark already refused to write at the weekend. It now also refuses on an **exchange holiday**, from a holiday list bundled with this release — so a Diwali or an Independence Day no longer carries the previous session's price forward under that day's date. |
| **The OpenAlgo disclosure is asked once more** | If you had switched the OpenAlgo integration on, this release asks you to read and accept its disclosure again: one of its statements — that exchange holidays were not modelled — stopped being true with the row above, and a consent sentence that has become false about what is written to your journal is a new version rather than a copy fix. Nothing new is sent and nothing new is kept. |

## New in v4.1.0

The release where the Live Desk can be priced by a bridge you already run — and
where nothing about that is switched on for you.

| Upgrade | What it gives you |
|---|---|
| **Live prices from your own OpenAlgo bridge** | **Settings → Live feed** now offers three sources for the Live Desk: the end-of-day bhavcopy already on this machine (still the default), a mark you type, or the OpenAlgo instance you run yourself. The third one is opt-in and stays behind a disclosure you accept: it can be picked only once the OpenAlgo integration is switched on in **Settings → Integrations**, and picking it is a second, separate choice from the trade pull. While the Live Desk is open Vyuha asks *your* bridge — `http://127.0.0.1:5000`, your own machine, unless you enter another address — at the interval you set on a **1 to 5 second** slider, and the marks on screen change as the answers arrive, for as long as the desk is the screen in front of you. The connection opens when you arrive on it and closes when you leave it or switch to another tab. Vyuha adds no new internet host for prices and ships no market-data provider of its own. |
| **What each request carries — and what it does not** | One request holds your OpenAlgo API key and the **trading symbols and exchanges of the positions you have open**, at most 500 of them. Your quantities, entry prices, stops, P&L and account names are not in it: the bridge is told which scrips to price, never how much of them you hold or what you paid. Checking the connection calls OpenAlgo's `/funds` endpoint once, and the desk does the same each time it opens and each time its price stream reconnects — the cheapest call that proves the address and the key are both right. It is the same bridge and the same key, nothing further is sent, and Vyuha keeps nothing from the answer but that it replied, and how quickly. The polling starts when the desk opens and stops when it closes; nothing runs in the background. |
| **Prices refresh on screen — ticks are never written** | Your journal does not become a tick database. **One mark per position per day** is saved: the last price of the session, or the price when you press **Save today's mark**, whichever comes first — and every figure derived from that mark is dated to the day it belongs to. The close-of-session mark is written by the app itself: the desk reconnects its price stream once at 15:36 IST while it is open and each position's mark is written then (from 15:31 for most stocks, 15:36 for F&O stocks, whose close comes from the closing auction), or the next time you open the desk that day. The button is only the earlier of the two, and neither of them writes anything at the weekend or on an exchange holiday. A price you type yourself is that day's mark: the automatic close-of-session mark does not overwrite it, and typing after the close replaces the automatic one; any bhavcopy applied for that day — the Auto-MTM job if you have switched it on, a file you drop or paste yourself, or the history backfill — replaces it with the exchange close. Whether a mark already exists is decided per symbol per IST day, so a second account's open positions get their own mark on the same day. If the feed stops, the desk keeps the last mark it had, labelled with its own date, rather than blanking or guessing. Your broker's API session expires every day and has to be signed in again at OpenAlgo's own screen; that is the broker's rule, not Vyuha's. When the bridge cannot be reached or has no key, the desk shows a **once-a-day reminder to reconnect** — once that day, not on every visit. |
| **The disclosure is re-read on every install** | The OpenAlgo disclosure moves to version 2, with a new section — *What the live price feed does* — covering the cadence, what a request carries, the `/funds` probe and when it is sent, the address, and the fact that ticks are never written. Because the version changed, an existing acceptance is no longer current: nothing pulls and nothing polls until you read it and accept again. `PRIVACY.md` states the poll in item 3, beside the broker pull it belongs with — the list of network requests is still exactly four kinds, because this is the same kind: your own broker bridge, only when you switched it on. |
| **The results date you record, on the desk** | **Instruments** gains a **Results dates** card: type the date a company reports against its symbol, search to find any symbol, and clear it as easily. The Live Desk then shows **"Results in N days"** — or *Results today* / *Results tomorrow* — on the row and in the detail pane, **free on any copy**, because it is a date fact about the company, like the symbol itself. A date that has gone by stops counting rather than reading backwards, and the date stays on your record either way. **Nothing fetches this**: no calendar is bundled and no website is contacted — it is there because you typed it, and the desk states the distance and stops. |
| **Both volatility variants, side by side** | In the Sizing Lab's **Volatility · Turtle unit (N)** tab, a **Turtle unit (N) ⇄ Varsity (% volatility)** switch puts the two variants beside each other with their sizes and their formulas, each panel headed by the variant that produced its number — because the two divide different budgets by the same ATR, and a size printed without its variant is ambiguous by exactly that ratio. The seven tabs are unchanged, and so is every link into them. The tab's own description of the % volatility method has been corrected: it does not always return the larger quantity, and it now states the ratio that actually relates the two. |
| **The Market Atlas says which map it grouped by** | The Sectors tab now prints, beside each bundled classification map's as-of date, the **sha256 of that map's canonical JSON, as the app loaded it — not of the file on disk**, so two machines can be compared at a glance and a hand-edited map cannot hide behind the date it was edited from. The two differ because the file carries its own formatting; running `sha256sum` on the file will not reproduce the figure on screen, and that is expected rather than a sign of tampering. The full digest is in the tooltip, which says the same thing. |
| **The heat tile shows both sides of the strip** | Portfolio heat (Pro) adds **"Locked in at stop"** — computed from the rows whose stop already sits beyond entry, and what they would give back if every stop is hit. It is stated on its own line and never netted into heat, because a winner must not cancel another position's real risk. |
| **Upgrades in place** | A v4.0.0 journal opens straight into v4.1.0. **One database upgrade** is applied on first launch, and before it the app writes a full copy of your database to `backups\pre-migrate-<timestamp>.sqlite` in your data folder. The installer runs the v4.0.0 uninstaller once first, and that one is the guarded uninstaller: ticking "Delete the application data" still erases the data folder, but only after your journal and licence key have been named and copied to `Documents\Vyuha-backup-<date>`, and Cancel leaves everything as it is. |

**Not in this release, said plainly.** Telegram messages for a stop or target
are not built and are not part of Pro; they become their own release after
v4.2. Native Upstox and Angel One price feeds are v4.2 — until then the app
names them and refuses with the reason instead of pretending. Kelly still uses
the win rate and payoff you supply; nothing is inferred from your journal yet.

## New in v4.0.0

The Live Desk release. Three new screens, and the part of them that is your own
record — your open positions, their marks and their P&L — stays free.

| Upgrade | What it gives you |
|---|---|
| **Live Desk — every open position on one screen** | A new screen (**Live Desk**, at the top of Positions) lists what you still hold, the mark Vyuha has for each name, and the profit or loss that follows from it. Every row names the mark it was priced from — **End of day** with the session date for a bhavcopy close, **Stored mark** for a price you typed in (undated in v4.0.0; from v4.1.0 a typed mark is that day's mark, stored one row per symbol per IST day), a bare **End of day** when only the trade's own closing price is on file, or **No mark** — because a P&L computed on a stale price is a figure with a date attached, and hiding that date is how a screen lies quietly. In v4.0.0 a typed mark carried no date because its table had no reliable as-of column, so only dated marks could be flagged stale; v4.1.0 stores a typed mark one row per symbol per IST day, dated — the day you type it, or the as-of date you give a pasted list — see *New in v4.1.0* above. Filter, sort and group as you do everywhere else; `j` / `k` move, Enter expands a row, `/` filters. **This screen is not gated.** Positions, marks and P&L are your own record looking at itself, and the core journal is never held back. |
| **On the desk with Pro: R, risk at stop, portfolio heat** | The same rows also carry the R multiple and what the position loses **if the stop is hit** — in rupees, and as a share of the capital you recorded — and the desk adds that risk up into portfolio heat. A row with no stop is counted and named as excluded, never quietly folded into the total; with no capital on file, heat says it has no denominator instead of inventing one. Exposure by sector sits beside it, labelled as today's classification rather than a point-in-time one. |
| **Your own levels, drawn on the price** | Expanding a position opens its chart with your entry, your stop and your target drawn on it as bands, so the distance you are reading about is the distance you are looking at. Beside it: the stop computed four ways (the level you entered, structure, the ATR, a flat percentage) and a trailing rail — chandelier, a moving-average trail, an ATR ratchet, a percentage and the turtle exit — each printed with the parameters it used. Nothing here places an order or tells you to do anything; it states the arithmetic and its inputs. |
| **Sizing Lab — seven ways to size, each with its formula on screen** | A new screen (Risk → **Sizing Lab**) computes position size by fixed rupee amount, fixed fractional (% risk), the Turtle volatility unit, % volatility, Kelly and fractional Kelly, fixed ratio and equal weight — side by side, on the same trade, so the spread between the methods is visible instead of theoretical. The formula and every input are printed beside each number, brokerage and statutory charges come from your own rate card (a broker with no card on file is labelled "default schedule" so a fallback never reads as your own), and a chosen size can be stored as the Live Desk's risk settings — one set of risk inputs the desk computes with, held for the whole journal rather than on any one position — in one explicit step that shows old → new before it writes. |
| **Market Atlas — the market your book is sitting in** | A new screen (Positions → **Market Atlas**) computes breadth from the end-of-day bars **already on your machine**: advancing, declining and unchanged with their denominators; how much of the market is above its 20, 50 and 200-day averages, with a 90-session spark for each; new 52-week highs and lows; median volume expansion. Then the same window by sector and by cap band — bucketed by NSE's own index membership, not by a market cap Vyuha would have to guess at — and your own open equity names against the equal-weighted return of their sector cohort over a week and a month. A Coverage tab holds the ledger: what was excluded and why, the denominator behind every metric, and which symbols are stale. It adds no account, no upload and no new connection, and a copy without Pro sees the screen described rather than hidden — with em-dashes where the figures would be, because sample numbers on a screen about denominators would be the wrong way to sell it. |
| **The open-source notices travel with your copy** | `THIRD-PARTY-NOTICES.txt` now ships both inside the installed app and inside this ZIP: every open-source component Vyuha runs on, its licence and its copyright line, in one file you can keep. |
| **Upgrades in place** | A v3.9.1 journal opens straight into v4.0.0. **Four database upgrades** are applied on first launch, and before any of them the app writes a full copy of your database to `backups\pre-migrate-<timestamp>.sqlite` in your data folder. The installer runs the v3.9.1 uninstaller once first, and that one is the guarded uninstaller: ticking "Delete the application data" still erases the data folder, but only after your journal and licence key have been named and copied to `Documents\Vyuha-backup-<date>`, and Cancel leaves everything as it is. |
...
| Upgrade | What it gives you |
|---|---|
| **Broker Truth — your broker's numbers beside yours** | A new screen (Import → Broker Truth) shows what your broker's own statement says, next to what your journal says, per segment, per financial year and per scrip — with the difference, and what accounts for it. Nothing is averaged, corrected or merged: the two sides stay two sides. Where they differ, the screen names the cause as a counted fact from your own book — how many sales have no purchase and what they sold for, the charges a file states nothing about, quantity still open against a scrip the broker calls realised, a product filed under a different segment. A gap with no knowable cause is shown with no cause rather than an invented one. A demat holding summary adds a quantities-only comparison: what the broker holds, what your book has open, and the difference. Lines are joined on ISIN first and on the symbol only when the file states no ISIN — and the screen says which was used, because a symbol join can be wrong. On "All accounts" you get one book per account, never one summed book: two brokers' statements added together is a figure no statement states. A separate table, "Charges the broker states", puts the broker's own fees beside your book's: a DP fee per financial year, a contract note's charges for one trading day, a ledger's charge tables. Each of those lines is one broker's statement against that broker's own trades — two brokers' fees are never added together — and a fee your book has no column for, or a year or a day that broker traded nothing in, is shown with no Vyuha side and no difference at all. It fills in when you import a Dhan Realised P&L, a Paytm Money Realized P&L, an Angel One P&L statement, a Dhan holding summary, a Dhan DP charges report, a Dhan contract note or an Angel One account statement — seven files, kept as the broker's stated figures. Those imports now say what they stored ("47 broker figures stored") instead of reporting themselves as an import of nothing. |
| **Seven more of your broker's files read natively** | Paytm Money Realized P&L, Dhan DP Charges, Dhan Demat Holding summary, the Dhan contract note (PDF), the Upstox ledger, the Angel One account statement and the Angel One P&L statement. None of them imports a trade — your book stays the tradebook or the transaction report — and each says so out loud rather than looking like an import that did nothing. The DP-charge, Upstox and Angel statements also post to Cash & Ledger. A contract note is the one file that states the **time of every fill**: those times, and the instrument type, are added to trades you already have, matched on symbol, date, side and quantity. It never creates a trade, because the transaction report already carries every execution. Dhan's MTF Report offers no download in any format, so there is no MTF file to import; those figures come from the reports that do exist. |
| **The Trades list is faster, and its order is now defined** | The table is served in pages of 500 straight from your database, with the filters and views applied there rather than in the browser: on a 25,000-trade book the screen went from about 2.0 seconds to about 1.1. Every count beside a view is still computed over your **whole** filtered book, never over the page you were sent. And rows imported in one batch used to have no defined order among themselves — they now appear in the order they were imported, newest first. Your totals do not move: tax by financial year is identical to the paisa, and every harvest lot keeps its long/short status. Switching accounts no longer briefly leaves the previous account's rows on screen. |
| **Sold before you bought, on the same day — named** | A same-day sell-then-buy round trip is now labelled as the covered short it was. A sale an existing holding could have delivered is not called a short, and a sale left open past the day stays what it was: a sale with no purchase. If a holding was bought on NSE and sold on BSE, the position says so in a note — it stays one holding, because the exchange is where the fill happened, not a second position. |
| **Search reaches further, and can stay open** | Your **ledger** and your **audit trail** are now searchable alongside trades, symbols, playbooks, instruments, sessions, challans, help and screens. A floating search assistant (**Ctrl + Shift + K**) stays open while you move around the app, so you can open a result, read it, and try the next one without retyping. Drag it anywhere; it remembers where you put it, and comes back on-screen if you reopen it on a smaller display. Ctrl+K's palette works exactly as before. |
| **Upgrades in place** | A v3.8.x journal opens straight into v3.9.0. The v3.9.0 installer runs the v3.8.0 uninstaller once first, and that one is the **guarded** uninstaller: ticking its "Delete the application data" checkbox still erases the whole data folder, but only after it has named your journal and your licence key and copied them to `Documents\Vyuha-backup-<date>`, and Cancel leaves everything exactly as it is. The "leave the box unticked" warning in the v3.8.0 notes below is therefore only for people coming from v3.7.1 or earlier, whose uninstaller has no backup step. On first launch three database upgrades are applied, and before any of them the app writes a full copy of your database to `backups\pre-migrate-<timestamp>.sqlite` in your data folder. |
