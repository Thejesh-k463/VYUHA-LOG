...
|---|---|
| **Upstox can price the Live Desk, on the token you already have** | **Settings → Live feed** gains a fourth source: Upstox. It is opt-in exactly like the others — it appears only after you have saved your Upstox **Analytics token** under **Import → Brokers** and picked it here — and it needs no new credential and no daily sign-in, because Upstox issues that token read-only and for about a year. While the desk is open Vyuha asks `api.upstox.com` for the instrument keys of the open positions of the account you have selected, **at most 500 of them**, every **1 to 5 seconds** at the interval you set, and for nothing else about them: no quantity, no entry price, no P&L, no account name. The prices stay on this machine — never uploaded, never resold — and nothing is written per tick: the one-mark-a-day rule below is unchanged. `PRIVACY.md` item 3 states the poll and names the host. |
| **One token, two jobs — and one way to break both** | The Live Desk source and the Upstox trade pull use the **same** Analytics token. You generate it once, in your own browser, at **account.upstox.com → Apps → Analytics**, and register your current IPv4 address under **Static IPs** as the import already asks you to. Generating a fresh one there **revokes the old one**, so a regeneration stops the import and the desk source at the same moment — paste the new token back under **Import → Brokers** and both work again. Said here because one afternoon where both go quiet otherwise reads as two separate faults. |
| **Angel One can price the Live Desk too, and signs itself in** | **Settings → Live feed** gains a fifth source: Angel One. It is opt-in like the others — it appears only after you have saved your Angel One **client code, PIN and TOTP secret** under **Import → Brokers** and picked it here — and it needs no new credential. Angel One clears every API session at **5 AM IST**, so Vyuha opens the next one itself each morning from the secret you enrolled: there is nothing for you to click. While the desk is open it asks `apiconnect.angelone.in` — already the host the Angel One trade pull uses, so no new host is added — for the open positions of the account you have selected, **at most 500 of them**, in batches of **50 symbols** at about one request a second, and for nothing else about them: no quantity, no entry price, no P&L, no account name. `PRIVACY.md` item 3 states the poll. |
