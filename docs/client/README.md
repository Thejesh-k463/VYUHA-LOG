...
| **A monthly plan, on request** | Alongside Pro — Annual (**₹7,999/yr**) and Journal — Lifetime (**₹29,999**) there is now **Pro — Monthly: ₹599 for your first month as a launch offer, then ₹999 a month**. It is **given on request** — message us and a one-month key is issued; each month renews with a fresh key. Nothing auto-renews and no card is stored, so there is nothing to cancel: if you do nothing, the key simply lapses and the core journal keeps working. Move to Annual or Lifetime whenever you like. |
