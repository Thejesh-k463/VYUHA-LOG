...
| Upgrade | What it gives you |
|---|---|
| **Live Desk — every open position on one screen** | A new screen (**Live Desk**, at the top of Positions) lists what you still hold, the mark Vyuha has for each name, and the profit or loss that follows from it. Every row names the mark it was priced from — **End of day** with the session date for a bhavcopy close, **Stored mark** (undated) for a price you typed in, a bare **End of day** when only the trade's own closing price is on file, or **No mark** — because a P&L computed on a stale price is a figure with a date attached, and hiding that date is how a screen lies quietly. A typed mark carries no date because its table has no reliable as-of column, so only dated marks can be flagged stale; a date Vyuha cannot prove would be worse than none. Filter, sort and group as you do everywhere else; `j` / `k` move, Enter expands a row, `/` filters. **This screen is not gated.** Positions, marks and P&L are your own record looking at itself, and the core journal is never held back. |
