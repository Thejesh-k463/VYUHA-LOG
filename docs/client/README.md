...
|---|---|
| **Live prices from your own OpenAlgo bridge** | **Settings → Live feed** now offers three sources for the Live Desk: the end-of-day bhavcopy already on this machine (still the default), a mark you type, or the OpenAlgo instance you run yourself. The third one is opt-in and stays behind a disclosure you accept: it appears only once the OpenAlgo integration is switched on in **Settings → Integrations**, and picking it is a second, separate choice from the trade pull. While the Live Desk is open Vyuha asks *your* bridge — `http://127.0.0.1:5000`, your own machine, unless you enter another address — at the interval you set on a **1 to 5 second** slider. Vyuha adds no new internet host for prices and ships no market-data provider of its own. |
...
| **The Market Atlas says which map it grouped by** | The Sectors tab now prints the **sha256** of each bundled classification file beside its as-of date, so two machines can be compared at a glance and a hand-edited map cannot hide behind the date it was edited from. |
