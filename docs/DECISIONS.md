...
## 2026-09-22 — v4.5.0 BUMPED, not tagged: the sixteenth session's close-out (3b-ii, 3c, the fix-list wave, the bump + release copy)

The three remaining waves landed in one session with the wave discipline unchanged (product builder → test builder → `vyuha-verifier`;
commit + push + CI 6/6 between waves): **3b-ii** `59c4242`, **3c** `af115f5`, **the fix-list wave** `47088f7` — CI 6/6 first attempt on
each of the first two; `47088f7`: SUCCESS 6/6 first attempt (run 35748145274). The release-gate Playwright pass (V7) ran ONCE on the fix-list tree: **120 / 120 in 35 specs,
7.6 min**, `zz-console-audit` 0 messages. **The bump:** `npm run bump-version 4.5.0` (four files), `cargo update -p vyuha --offline`
(Cargo.lock one line), the two root fields of package-lock.json by hand (`git diff --numstat` 2/2); all six read back at 4.5.0 / v4.5.
**Release copy:** DONE in this commit — docs/client/* (README what's-new table, INSTALLATION_GUIDE 0074 + auto-close, deck chips, TERMS/PRIVACY/REFUND applies-to), docs/sales/* (landing pill + chip, brochure pill), README's v4.5.0 quote block, CHANGELOG.md v4.5.0; the client ZIP is NOT rebuilt (the tag session, after the build) — the 4.4.0 precedent (`80eb164`, "version bump and release copy", one commit before the tag) was followed.
`vyuha-release-steward` walked the 11 steps read-only: key id `4FF85F3BBE1DA21D` matches, no stale root key, Node 22.17.0 pinned,
`releases/latest` → v4.4.0 untouched; steps 4 / 5-deep / 7 / 9 are NOT OBSERVABLE before a build exists — they belong to the tag session.
**Process facts measured this session:** two product builders hit the 150-turn cap (both resumed from a written ledger — the ledger
file is the recovery, not the transcript); the gate was run seven times for three waves, three of them red on the whole-tree-scan
