...
**Measured / found:** `components/layout/sidebar.tsx:392` renders the literal string `Local · Offline · v2.99` — major and minor only, no patch, and not derived from `package.json`. A grep across `app/`, `components/`, `lib/`, `src-tauri/src/` and `scripts/desktop-server.mjs` for `npm_package_version`, `getVersion()` and `package_info` returns **nothing**: the running app never displays its own patch version anywhere. `scripts/bump-version.mjs` accordingly reports only three files (package.json, tauri.conf.json, Cargo.toml) — AGENTS.md's "and the sidebar footer" is stale, though harmlessly so, because the footer has no patch digit to sync.
