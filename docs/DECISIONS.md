...
## 2026-09-04 â v3.9 W2: `/trades` server pagination, and the order that was never total

**Context:** `/trades` was the one route over the 1,500 ms median budget â 1,968 ms on the
...
`created_at` is `datetime('now')` â SECOND resolution â and `lib/import/commit.ts` never sets
...
timestamps â so **only the real book shows what this actually was**, and the seed would never
...
came back ascending by id (814, 816, 817â¦) while the all-accounts view over the same rows came
back descending (822, 821, 820â¦). **So the order within an import batch was unspecified until
...
transaction with one `sell_date` and one `created_at` â
...
(`SEARCH trades USING INDEX trades_account_sell_created_idx`), 21.0 ms â 20.3 ms median.
...
- What moved, and only this: the ORDER of the harvest lot list (same multiset â every lot's
  ...
  moved and 12 ids differ in the SET â **every in-page move is a swap of tie-mates, and every
...
filtered set, never the page** â a count that quietly means "of what we fetched" is a
...
arm â `AssertionError: expected [ 12, 9, 8, 7, 6, 4 ] to deeply equal [ 12, 9, 4 ]`. The
"Delete byâ¦" scope keeps its WHOLE-book candidate list and a whole-filtered-set `viewIds`;
...
does not re-run when the prop changes â so after `router.refresh()` (the account switcher,
...
state DURING RENDER on a changed prop â not in an effect; the `loading` flag is derived
(`servedKey !== filterKey`) for the same reason.

**Perf, 25k seed, prod build, 43 routes Ã 3 rounds:** `/trades` **1,968 â 1,063 ms median**
(p95 2,280 â 1,115), a 46% cut and comfortably inside budget. A second sweep on the same
server read 1,093. **Caveat, stated rather than buried:** this machine was slower than the W4
baseline across the board â overall median 910 â 954 (sweep 1) and 1,067 (sweep 2), with
`/settings` +27%, `/` +26%, `/review` +16%, `/reports/edge` +14%, `/ipos` +14%,
`/reports/discipline` +13%, `/strategies` +12% â routes that share nothing with this change but
...
baseline by more than the Â±40 ms that run called noise; or `lib/import/commit.ts` starts
...
## 2026-08-31 â The render-windowing pass: 6 budget breaches â 1 (v3.4.0)
...
`/strategies` and `/options-journal` together as "~6 s â 8,058 option rows rendered". Measured
against the code:
...
| `/equity` | ~2,750 open | **Every row in the DOM, unvirtualised** â payload only ~1.2 MB. Not payload-bound. |
| `/risk` | ~3,460 Ã **2 arrays** | Two unvirtualised renders plus three more uncapped panels (~1,250 further rows). Not payload-bound. |
...
1. **The ordering change was NOT made, deliberately.** Both "Invalidated if" clauses (v3.0.0
   ...
   and it is safe â but it is **not output-neutral**: it moves `taxByFy` per-FY sums in the last
   ...
   **Worth knowing when it happens:** `created_at` defaults to `datetime('now')` (SECOND
   ...
   one `created_at`** â real-book tie groups are whole batches, not pairs.

2. **Everything here is a RENDER fix â no SQL predicate changed, no ordering changed.** That is
...
3. **`/equity` just needed `virtual`.** `DataTable` has supported row windowing since v3.0.0 and
   ...
   things, not prohibiting it â reworded so it cannot be read as a rule.
...
4. **Client windows say what they hold back (`components/ui/show-more.tsx`).** A silent
   ...
   lenses drill-down cap. **Only the DOM is windowed â every aggregate is still computed over
...
5. **Server components get a stated cap instead (`components/ui/capped-note.tsx`).** `/risk`'s
   ...
   its own module with **no `"use client"`** â importing it from `show-more.tsx` would drag three
...
6. **`LazyMount` fixed `/strategies` without touching the data.** Charts mount on approach via
   IntersectionObserver with a `minHeight` placeholder â required, not optional, because a
   ...
   `Promise.resolve().then` â the repo bans `react-hooks/set-state-in-effect` outright.
...
8. **`/trades` is the one route left over budget, and it is the one that needs the ordering
   ...
   remaining lever is server pagination â which needs `LIMIT/OFFSET`, which needs a total order.
   Left explicitly for the ordering pass rather than half-fixed.

**Measured** (`perf:seed` 25,001 trades â `perf:sweep`, 42 routes Ã 3 rounds, production build):

| Route | v3.3.0 | **v3.4.0** | |
|---|---|---|---|
| `/strategies` | 6026 ms | **1022 ms** | â83% |
| `/options-journal` | 5770 ms | **1082 ms** | â81% |
| `/equity` | 3208 ms | **931 ms** | â71% |
| `/risk` | 2503 ms | **1349 ms** | â46% |
| `/lenses` | 2114 ms | **1276 ms** | â40% |
| `/trades` | 2256 ms | 2040 ms | untouched |
| **Breaches (of 42)** | **6** | **1** | |
| Overall median | 985 ms | **939 ms** | |
| Slowest route | 6026 ms | **2040 ms** | |

Rendered `<tr>` counts on the perf tier, before â after: `/equity` ~2,810 â **62**,
`/options-journal` 8,058 â **157**, `/risk` 1,257 â windowed + capped. `/strategies` SSR now
carries **626 sized placeholders and zero recharts markup**.

**A measurement trap, recorded.** The first post-fix sweep showed `/lenses` at 2557 ms â WORSE
than its 2114 ms baseline, on a route not yet touched â while `/trades` improved without being
...
fixable by server pagination and this entry's Â§8 is discharged), or `perf:sweep` enters CI (it
is not there today, which is why `tests/render-windowing.test.ts` exists as a source guard â
none of these five routes has an e2e spec either).

## 2026-08-31 â v3.3.0 post-release checks: updater cryptography, and a perf re-sweep
...
key. It does not prove the signature VERIFIES over the bytes a user downloads â and that gap is
...
(`tauri.conf.json` â `plugins.updater.pubkey`): downloaded the published
`Vyuha_3.3.0_x64-setup.exe` (34,935,482 B), decoded the `windows-x86_64` signature out of the
published `latest.json`, and verified it. **â verifies.**
...
Windows only â macOS is not a surface this product sells, so 120 MB of dmg was not downloaded
to prove something that does not ship.

Also confirmed post-publish: `releases/latest` â **v3.3.0**; `revocations` still
`prerelease=true` and did NOT steal latest; manifest carries all six platform entries, every
signature on `4FF85F3BBE1DA21D`.

### Perf re-sweep at 25k trades â v3.3.0 added no regression

`npm run perf:seed` (25,001 trades, 3,460 open, 60k ledger rows, 100k audit rows) â
`npm run perf:sweep` (42 routes Ã 3 rounds, 126 visits) against the production build.

| | v3.0.0 baseline | **v3.3.0** |
|---|---|---|
| Overall route median | 987 ms | **985 ms** |
| Budget breaches (of 42) | 6 | **6 â the same six** |
| Console errors | 0 | **0** |
...
## 2026-08-31 â Empty-book sweep: what a new buyer's first launch actually renders

**Context:** the owner wiped app-data during an install, which briefly made a real
zero-trade database available â the exact state every new buyer starts in, and a state that
...
**Method (repeatable â `vyuha-dev-empty-db` in `.claude/launch.json`):**
`VYUHA_DB_PATH=data/empty-check.sqlite npm run db:migrate && npm run seed`, which yields the
representative first-run state â **162 charge_config rows, 1 account, seeded settings, 0
trades** â rather than a bare schema. Seeding matters: an unseeded DB is a HARSHER test than
...
**What the three new surfaces do on an empty book â all correct:**
...
- `/reports/harvest` keeps the **set-off rule card** (the statute is worth reading before you
  ...
  closed trades yet" instead of â¹0 with no explanation. "Holding clock" hides â no open lots.
- Harvest KPIs read â¹0, which is **honest rather than fabricated**: nothing was realised, and
...
("Pro trial â 7 days left"), and the sidebar footer reads `v3.3`.

**Kept:** `data/empty-check.sqlite` is gitignored, so the `vyuha-dev-empty-db` launch config
needs the two commands above re-run on a fresh clone. That is deliberate â a committed
fixture database would rot silently against future migrations.

## 2026-08-31 â The governing statute changed, and turnover had been wrong for three years (v3.3.0)
...
1. **The Income-tax Act, 1961 is repealed; the Income-tax Act, 2025 came into force on
   ...
   the 1st April, 2026."* Every section number this app displays â 111A, 112A, 43(5), 44AB,
   44AD, 234C, 87A, 115BAC â is repealed law for the current year, mapping to s.196, s.198,
   s.66(31)/(33), s.63, s.58, s.425, s.156, s.202. **The arithmetic survives; the citations do
   not.** The â¹10 Cr / â¹3 Cr limits in `itr.ts:61-62` and the 15/45/75/100 instalments are
   unchanged, and grandfathering survives verbatim as s.90. This is therefore an
   EFFECTIVE-DATING problem of exactly the shape WS1 solved for charge rates â deferred to
...
2. **F&O turnover omitted option premium, and that number decided the audit verdict.**
   `itr.ts:131` used `abs(grossPnl)` and fed `auditVerdict`. The CURRENT ICAI Guidance Note â
   **11th edition, 2026, para 5.11(b)(ii)** â says *"Premium received on sale of options is
   ...
   **REINSTATED in the 9th (2023)**, unchanged through the 10th and 11th. The widely repeated
   "premium never counts" is the 2022 position and has been wrong since 2023 â and a web search
   ...
   **Invalidated if:** ICAI issues guidance mapped to s.63. The 11th edition's Preface calls
   ...
   not authority â which is why `TURNOVER_BASIS` is shown to the user rather than buried.

3. **Three turnover formulas collapsed into one module.** `tax.ts` added premium,
   `itr.ts` did not, and `itr-schedule.ts:161` used `abs(NET P&L)` â net is after charges and
...
4. **s.425(2)'s safe harbour was never implemented, so the planner charged interest the statute
   does not.** Gazette text: no interest where advance tax paid by 15 June is **â¥12%**, or by
   15 September **â¥36%**, of tax due on the returned income. **First two instalments only â
   there is no tolerance for December or March.** The shortfall is still REPORTED (the payment
   obligation is real); only the interest is waived, and the row is badged "short Â· no interest"
   so it does not read as a bug.

5. **The s.425 interest RATE was already correct â do not "fix" it.** s.425's Table states flat
   3% / 3% / 3% / 1%. The old Â§234C reached the same figures as 1%/month Ã 3/3/3/1 months, which
...
6. **s.425(4) relief is opt-in, not inferred.** It waives interest on a shortfall from
   ...
   arising for the FIRST TIME"* â but only if the tax on it is paid in full by 31 March. The
...
8. **The s.175 dividend/bonus stripping check was scoped OUT, deliberately.** It is the one
   ...
   the user â which is exactly why it is safe to ship and why nobody else will. It needs a
...
9. **A competitor's floating-window workspace was assessed and declined.** Their 40 widgets are
   ...
   Note `settings.workspace` is already taken (`both|equity|fno`) â any such feature needs
   another name.
...
11. **Citations are resolved BY TAX YEAR, not relabelled wholesale** (`lib/analytics/statute.ts`).
    ...
    historical FY report cite law that never governed it â the same class of error effective
...
13. **`monthlyBreakdown` does not recompute retPct.** The geometric monthly return needs the
    ...
    two monthly return figures that could disagree â precisely the defect three turnover formulas
...
15. **The tax-lever module enforces (C) by ABSENCE, and a test enforces the absence.** There is
    ...
    journal can say here is "wait 30 days before buying back" â India has NO wash-sale rule, and
...
17. **`HARVEST_FIELDS` gained three columns, not a second query.** `netPnl`, `chargesTotal` and
    `sttCtt` were added to the existing projection. Columns only â no new WHERE â so the stated
    contract (identical row order feeding `allocate()`'s stable sort, identical float sums) holds
    by construction.

**Measured:** `npm run verify` EXIT 0 â **2,266 tests / 151 files** (was 2,184 / 146), lint **0
problems** (the three WS1 unused imports were removed here), production build clean. Verified
rendering against the repo dev DB: the STT split reported â¹28,478 forfeited across 42 delivery
...
`%APPDATA%/in.vyuha.tradejournal` is still PRE-0050 â `no such column: effective_from` and
...
## 2026-08-30 â Statistical inference, segment depth, and the columns nobody read (v3.2.0)
...
1. **BenjaminiâYEKUTIELI is the default multiplicity control, not BenjaminiâHochberg.** BH
   controls the false-discovery rate only under independence or positive regression dependence
   (PRDS). Slices of one trade book are neither â "morning trades" and "NIFTY trades" overlap,
...
2. **Wilson, not Wald, for every proportion.** The textbook Wald interval returns the single
   point 0 at k = 0 â absolute certainty from no evidence â and routinely runs outside [0,1].
...
5. **Segment depth covers FIVE segments, not all eight.** `SEGMENTS` has distinguished
   ...
   needed no data-model change â the single biggest de-risk in the release. Futures and
...
8. **Stop migration reports the expectancy GAP, never a counterfactual.** "You would have lost
   â¹4,200 less" needs the price path after the edit, which Vyuha does not have at intraday
...
9. **`effectiveStop` behaviour is deliberately UNCHANGED.** A trailing stop typed on the wrong
   ...
   and capital-at-risk â the position looks safer than it is on two screens at once. Silently
...
## 2026-08-30 â Charge rates become effective-dated (v3.2.0 WS1)

**Context:** `charge_config_uq` was `(broker, plan, segment, exchange)` with no time
dimension (`schema.ts:362`), so exactly one rate row existed per key and EVERY trade â
of every vintage â was priced at whatever that row holds today. A book spanning a
...
2. **The date argument is REQUIRED, not optional.** `findRates` gained `onDate` as a
   ...
   silently â which is the bug being fixed. The compiler found them; each was then given
...
5. **One pricing-date rule, not eleven call-site opinions.** `pricingDate()` prefers the
   ...
   priced wholly at its sell date's epoch** â a stated approximation, because
...
6. **Restatement of stored charges is deliberately NOT automatic â and the one path
   ...
   `chargesTotal`/`netPnl` back â so an open MTF position silently restated the moment
   ...
   period, so a broker with one open-ended epoch â every broker today â accrues
...
that matched no epoch, and `findRates` refused a perfectly valid trade â the whole Groww
...
## 2026-08-30 â The live-demo failure was three sentences, not three bugs
...
1. **The counts were right and the sentence was wrong.** `7,544 executions â 804 positions`
   was rendered as the second number alone â "804". To anyone who knows they placed 7,544
   ...
   "92 lines â 73 trades") and the lesson never reached the aggregate count at the top of
   ...
   Recent-imports row â three surfaces that had three different phrasings and one of them
...
2. **A false alarm was firing on BOTH demo files, on screen, saying "please report this
   ...
   2N half-paisa residues that no longer cancel â a ceiling of N Ã 0.01 that is REACHED,
   not approached. Measured: **â¹0.04 on â¹75.8 crore of Paytm turnover (804 positions) and
   â¹0.01 on Zerodha's (79)** â 1e-9 of turnover, with `qtyDelta` exactly 0 in both. The
   tolerance is now derived from the rounding that produces it (`positions Ã 0.01`, floor
   ...
   understanding (`relDrift < 1e-6`) â the parsers just never learned it.

3. **The blank P&L cells are the product working.** 72 of 804 Paytm positions and 11 of 79
   Zerodha positions are opening sells â shares sold from holdings the file never shows
   being bought. Invariant 6 says never fabricate a denominator, so they read "â". That is
...
**Measured on the demo files (for the next person who re-measures):** Paytm 7,544 â 804
positions (650 closed / 82 open / 72 opening sells), closed net â¹1,54,39,611 against
Paytm's own in-window realised â¹1,64,58,423 = **â6.19%**, the gap being pre-window cost
basis the tradebook cannot see. Zerodha 3,530 â 79 (64 / 4 / 11), gross â¹9,02,987 against
Console's stated realised â¹9,53,951; Zerodha's tradebook carries no charge columns at all,
...
for it and is **equity-only** â every row's Segment is `EQ` and zero symbols match the
...
## 2026-08-30 â Bundled ISINâsymbol snapshot: why the index map was never going to be enough
...
resolves **76**. The map covers index CONSTITUENTS â so it resolves large and mid caps and
...
4. **NSE's own lists are not sufficient for this book.** 69 of the 139 unresolved codes sit
   ...
   misses unresolved. The BSE list is required, not optional â and BSE's `segment=Equity`
   ...
   **Built and measured the same day: 5,671 ISINs (NSE main 2,559 + NSE Emerge 565 + BSE
   2,547 new), 142 KB, `asOf` 2026-08-30 â and ALL 215 demo scrip codes resolve, plus all
...
## 2026-08-29 â Account deletion: two coherent ends, capped capital carry, credentials never in trash

**Context:** The v3.1.0 headline â per-account Delete in Settings (`lib/queries/account-delete.ts`, trash envelope in `lib/trash-format.ts` / `lib/trash.ts`). Four decisions worth not re-litigating.
**Decisions:**

1. **The option set is exactly two: purge everything, or merge everything (with a separate
   broker-connections choice).** Half-merges â "merge the trades but delete the ledger",
   "keep the IPOs but drop the imports" â are deliberately NOT offered. The account-scoped
   tables reference each other (`ledger_entries.refTradeId`, `ipos.tradeId`,
   `import_batches` â trades), so any partial split either leaves dangling references or
...
2. **The capital-compounding marker carried into a merge target is
   ...
   "available to compound" went NEGATIVE â a click on Compound would have applied a
   ...
   `pnlRolledIn = original â carried` while subtracting `carried` back out of the target
   (floored at 0). The source's equity/active-capital figures are never added to the target â
   capital is the user's own statement, not something a merge may fabricate.
3. **Trash envelope v2 is ADDITIVE â optional `account` + `accountRows` fields â and
   ...
   dialog states the consequence â deleted connections are gone for good. Panel dismissals
   are regenerable and also not snapshotted.
4. **Restore refuses the WHOLE account restore on an id or name conflict â nothing partial.**
   If the snapshot's account id is taken by a different account (or its name now belongs to
   one), restoring rows into it would merge two books â the exact corruption account
   isolation (invariant 8) exists to prevent â so the restore refuses up front and changes
...
## 2026-08-29 â v3.0.0 perf pass â 42-route sweep at 25k trades

**Context:** The pre-launch performance pass for v3.0.0, measured with the new harness (`npm run perf:seed` â a deterministic 25,000-trade book â then `npm run perf:sweep` â all 42 routes timed on the production build, with a console-error gate). Every number below is a sweep median at the 25k tier.
**Measured / found:**
...
The /cash cause was NOT SQL: the page serialized a **113 MB SSR/RSC payload** (the full ledger, into the flight stream). Fix: sums and the running balance pushed into SQL, 200-row pages, export fetched on click (`tests/load` case `a7-cash-ledger` pins the class; the load suite is now 15 cases). A measured hazard shaped the payload work everywhere else: **adding WHERE clauses to the trades queries reorders `(sell_date, created_at)` ties**, so rows shift between runs and any row-for-row equivalence proof dies â which is why the trimming was done as **column projections, not filters** (projections were proven value-identical row for row against the untrimmed query). Remaining above the internal budget at this abusive tier, all payload-bound with no algorithmic defect found: options journal and strategies (~6 s â 8,058 option rows rendered), equity and risk (~3 s), trades and lenses (~2.2 s).
**Decision:** Budget = **median < 1,500 ms per route at the 25k-trade tier**, gated by `perf:sweep` (which also fails on any console error). The six breaching routes are deferred to v3.0.x as pagination work â they render everything they are handed; the fix is to hand them less, the same shape as the /cash fix, and it deserves its own tested pass, not a launch-night patch.
**Why not the obvious thing:** Filtering rows in SQL (WHERE) instead of projecting columns â the tie-reordering above means a filter cannot be proven output-identical, and a perf pass that changes what a page shows is no longer a perf pass.
...
## 2026-08-29 â Upstox reconciles across THREE contract notes; "STT-SQUP" makes settle-based STT industry practice
...
**Measured / found:** Trades 5/5 to the paisa AND the fill time â including EBGNG's blended two-fill MTF average (621.88) and all three option round trips (the F&O note lists SENSEX as "OPTSTK BSX" under a BSE-FO section; one note covers both exchanges). Charges: brokerage EXACT on the delivery note (â¹2.66 = the rate card); turnover within 4p; SEBI 1p; F&O STT **9.36 = exactly 0.15% of sell premium â the FOURTH broker document confirming the rate** (Dhan note, Angel note, and now Upstox Ã its own arithmetic), with a broker-side "STT-Round off" â0.36 showing Upstox rounds the note total where Vyuha rounds per row (â¹1 granularity, not a rate difference). Upstox's own line item is named **"STT-SQUP"** â square-up STT â on the same-day-closed DELIVERY and MTF equity trades (0.33 and 0.78 vs our product-rate â¹3 and â¹6): the second broker charging by what settled, which upgrades the 2026-08-28 settle-based-STT finding from one broker's behaviour to industry practice. GST deltas are explained, not mysterious: contract notes exclude DP charges (a depository bill item) that Vyuha includes on delivery sells, and the rest follows brokerage.
...
**Why not the obvious thing:** Patching STT before launch â unchanged from 2026-08-28: conservative direction, small magnitude, engine change deserves its own pass.
...
## 2026-08-28 â Angel contract note 0061896174: trades 6/6 to the fill time; STT follows what SETTLED, not the product code

**Context:** The gate on v2.99.103 â Angel One's contract note for the 2026-08-27 live pull, against the 6 committed rows.
**Measured / found:** Trades are a perfect 6/6: every quantity, WAP, per-contract net (â422 / â420 / +3.78 / â4.80 / Â±MOSCHIP) AND every entry/exit fill time matches the annexure. The annexure also shows the SENSEX contract as `BSXOPT SENSEX26AUG77600CE` under a summary line saying "Aug 27 2026" â the broker's own document carrying the symbol/expiry mismatch the adapter now codes around. Charges: exchange txn â¹2.52 vs â¹2.55, SEBI 1p, stamp 0=0. F&O STT re-confirms 0.15% of premium (SENSEX â¹2.65 = 0.15% Ã 1,768 exactly â Angel books paise where Vyuha rounds STT to the rupee, hence our â¹3; ICICIBANK â¹1.35 vs a computed â¹1.68 is unexplained, trade-wise levies available on request, not chased). **The real finding: equity STT is charged by what actually settled.** HFCL rode the DELIVERY product and WABAG the MARGIN (MTF) product, but both were squared off the same day â no delivery occurred, and Angel levied intraday STT (sell-side 0.025%, NSE-capital total â¹1.00) where Vyuha's segment-based engine charged delivery/MTF STT at 0.1% both sides (â¹3 + â¹4). Same-day-closed delivery/MTF trades therefore OVERSTATE STT.
**Decision:** Recorded, deliberately NOT fixed in v2.99.103 â the direction is conservative (overstates costs, never flatters), the magnitude is paise-to-rupees, and a settle-aware STT rule is a charges-engine change that deserves its own test pass, not a release-night patch.
**Why not the obvious thing:** Patching the engine immediately â charging by segment is load-bearing for every FILE import too (where a delivery product usually does deliver), and the same-day-square-off case needs its own rule with its own tests, per invariant 3's discipline.
...
## 2026-08-27 â Angel One's trade book STATES the derivative facts â and its symbol lied about the expiry on a real contract
...
**Measured / found:** The row shape (previously INFERRED from docs) is now VERIFIED: `instrumenttype` (OPTSTK/OPTIDX), `strikeprice` (â1 equity sentinel), `optiontype`, `expirydate` ("29SEP2026") are all stated. Two facts symbol-parsing would get wrong: (1) `SENSEX26AUG77600CE` carried a STATED expiry of **27AUG2026** â the symbol's own date token disagrees with the contract's expiry, so parsing the symbol books the wrong expiry day; (2) `producttype: "MARGIN"` arrived on a real **MTF equity trade** â the old mapping sent MARGINânull assuming it was the F&O carry product (F&O carry is CARRYFORWARD, confirmed in the same payload). Also: equity symbols carry NSE series suffixes ("HFCL-EQ") that no other source uses, and a real cross-exchange intraday pair (bought BSE, sold NSE) arrives as two one-sided positions â represented as such, not merged. Committed and verified end to end: 6 trades, options as index/stock_option with options-rate charges, WABAG as eq_mtf, gross P&L total â843.72 matching Angel's own UI to the paisa.
**Decision:** Canonical OPT/FUT names built from the stated fields (underlying recovered by stripping the stated strike+type suffix and a trailing date token); MARGINâmtf; series suffixes stripped; incomplete stated facts keep the raw name and say so. The "known defective for Angel One" line from 2026-08-26 is DISCHARGED. Zerodha/Kite remains the one API puller with no real F&O payload.
**Why not the obvious thing:** Parsing the compact symbol (as the OpenAlgo adapter must, having no stated fields) â Angel states everything, and its own symbols proved unreliable on the expiry.
...
## 2026-08-27 â Contract note 14721318 reconciles the live pulls to â0.081%, STT to the paisa â and the engine's 0.15% options STT was RIGHT
...
**Measured / found:** All 9 contracts match quantity and WAP to the 4th decimal (the note aggregates across products; our MIS+NRML splits sum to its figures exactly â e.g. 77400 PE 3,400 @ 55.8159 = our 2,840 @ 54.51 + 560 @ 62.44). Levies, ours vs note: STT **â¹1,222.00 vs â¹1,222.00 exact**; SEBI â¹1.91 vs â¹1.90; exchange txn â¹530.93 vs â¹531.39 (â0.087%); stamp â¹28 vs â¹29 (rupee-rounded per row vs per note); IPFT 0=0. Non-brokerage levies overall **â0.081%** â well inside the 0.69% claim. Brokerage â¹380 (flat config) vs â¹2,980 (Dhan charges per executed order â 206 fills) â excluded from the claim by design, and the GST gap is entirely brokerage-driven. **The note also settles yesterday's STILL OPEN question: F&O STT levied = 1,185.00 on 789,765.92 of pre-brokerage sell premium = exactly 0.1500%** â the engine's `charge_config` rate was correct and the "statutory 0.1%" recalled from training data was stale. Supersedes the STILL OPEN line in the 2026-08-26 entry below.
...
**Why not the obvious thing:** "Correcting" the STT rate to the remembered statutory figure two days ago would have broken an exact-to-the-paisa match â this is why invariant 3 forbids rates from memory.
...
## 2026-08-27 â `npm run verify` while `next dev` runs poisons `.next`: the browser silently serves stale production chunks
...
**Why not the obvious thing:** Trusting a reload (or even a hard reload) to fetch fresh code â the stale chunks are real files with valid names, so the browser gets 200s for all of them.
**Invalidated if:** dev and build output are separated (`next dev` gains a distinct dist dir, or verify builds into a temp dir).

## 2026-08-27 â A risky cross-source collision now BLOCKS a broker-API commit behind an explicit confirmation

**Context:** Pulling the same Dhan trading day twice â once natively (positions) and once through OpenAlgo (fills) â put the same SENSEX 78200 CE trade into the journal twice, silently, inflating the day's P&L headline by â¹5,402.
**Measured / found:** The two sources agreed to the rupee everywhere but hashed apart on ONE paisa of buy value (â¹4,36,567.00 from summing 89 fills vs â¹4,36,567.01 from qty Ã Dhan's 5-decimal average) â so `dedupHash` correctly saw different rows, and the existing cross-source detector (`lib/import/cross-source.ts`) flagged it exactly (same-quantity kind), but the broker-API route never consulted it. Verified live: preview and commit now surface the report, and a risky commit returns 409 + a dialog whose only "Commit anyway" button re-posts with `force:true`. Same-day-same-symbol overlap under a DIFFERENT broker is a new, separate check (`detectCrossBrokerEchoes`) that only informs â two brokers are two books. Also observed: the route's UTC-derived trade date rolls at 05:30 IST, before market open, so it always names the correct Indian trading day â accidental but correct; left as is.
...
**Why not the obvious thing:** Rounding or tolerancing the dedup hash â that trades a visible, confirmable near-miss for silent false-positive merges everywhere else, and same-source re-pulls already dedup exactly.
**Invalidated if:** `dedupHash` inputs change, or the broker route stops being the only API-pull commit path.

## 2026-08-27 â One OpenAlgo instance per broker: connections are `openalgo:<broker>` rows, and the form never represents saved state

**Context:** The owner runs TWO OpenAlgo instances on one machine (Upstox on :5000 for another project, Dhan on :5051 for Vyuha) â the single `openalgo` connection id could hold only one, and the connect form showed default host/broker over a saved 5051/Dhan config after reload.
**Measured / found:** The stale form was genuinely dangerous, not cosmetic: one innocent "Update connection" with the defaults would have silently repointed Dhan-stamped pulls at the Upstox instance. And host/underlying-broker are config, not credentials â there was no reason not to display them.
...
**Why not the obvious thing:** A schema migration for a new instances table â the (account, broker) unique key already provides identity and upsert; only the id vocabulary needed widening.
...
## 2026-08-26 â EVERY broker-API puller classified F&O as equity: the classifier reads only canonical names, and no API sends them

**Context:** First live API pull that ever returned F&O fills (Dhan positions, 11 rows, real money) â found on the day-one live test, in every shipped build since the API pullers existed.
**Measured / found:** `parseInstrumentName` reads only the file-canonical `OPT SYM DD Mon YYYY STRIKE CE` / `FUT â¦` shape. Dhan's API sends `SENSEX-Aug2026-78200-CE`, OpenAlgo sends `SENSEX27AUG2677400PE`, Kite/Angel send their own compact forms â all fell to the equity branch: options charged equity STT (â¹207.67 vs the correct â¹1,042.40 on one 7,040-qty SENSEX round trip â wrong in BOTH directions per row), segment `eq_delivery`, invisible to options analytics, open options nagging as unmarked equity "holdings". Fixes are grounded in captured real payloads: Dhan canonicalises from its STATED `drvExpiryDate/drvStrikePrice/drvOptionType` fields (sentinels `0001-01-01`/`NA`/`0` on equity rows); OpenAlgo gates on the stated exchange (NFO/BFO/MCX) and parses its documented compact form; a derivative row whose facts are incomplete keeps its raw name AND says so. Kite and Angel One remain DEFECTIVE for F&O â no real F&O payload exists to verify against, recorded here rather than guessed.
...
**Why not the obvious thing:** Teaching `parseInstrumentName` the compact shapes â shape-without-exchange is exactly the "a broker-named parser must see the broker's NAME" lesson again: `PONNIERODE` ends in CE too. The exchange is the stated fact; the shape only fills in details.
...
## 2026-08-26 â OpenAlgo live pull reconciles to the rupee, one corrupted row REFUSES itself, and Dhan states the mark for open positions
...
**Measured / found:** All 11 aggregates matched the native pull to the paisa (qty and value), the MIS/NRML product split survived, and totals matched Dhan's dashboard exactly (unrealised â2,532.50; Dhan's "realized" â7,008.52 = Vyuha's GROSS â7,009 â Dhan's figure is pre-charges). OpenAlgo's documented zero-quantity trap did NOT occur on the Dhan plugin (206/206 real quantities), but its `timestamp` is a full datetime against the docs' time-only sample. One genuine corruption: OpenAlgo relabelled a PIIND 2600 CE buy as `SILVERM23NOV26236750PE` on NFO â right numbers, wrong identity. The charges engine itself proved why such a row cannot be imported: commodity_option/NSE has no `charge_config` row and never will (it threw, initially surfacing as a bare 500). Open positions: Dhan's payload has no LTP but states `unrealizedProfit`, and entry Â± unrealised/qty reproduced Dhan's displayed LTPs exactly (1.30 / 2.90 / 38.25) â stored as `closingPrice`, so open positions import already valued.
**Decision:** An underlying/exchange conflict (commodity on NFO/BFO, index on MCX) REFUSES the row with a warning naming it; engine refusals in the broker route return 422 messages, never 500s; the mark derivation is stated-fact algebra, kept. STILL OPEN: the engine charges sell-side options STT at 0.15% of premium where the statutory rate is believed 0.1% (â¹648 vs â¹432 on a â¹4.32L sell) â deliberately NOT corrected from memory; the contract-note reconcile decides it.
**Why not the obvious thing:** Importing the corrupted row loudly-but-anyway (the first implementation) â there is no honest charge profile for a wrong identity, and the engine's refusal to invent one is the invariant working as designed.
...
## 2026-08-25 â A native HTML constraint can pre-empt the engine's designed refusal: Book-exit's `max=0` dead end

**Context:** A real user hit an unexplained dead end booking an exit on a fully closed staged position (SG Finserv, buy 600/sell 600 â verified in the live desktop DB): the 25/50/100% shortcuts appeared to do nothing and every typed quantity was refused with the browser-native bubble "Value must be 0."
**Measured / found:** The engine was right (0 open is the truth) and the panel's design rule â actions are never disabled by position state; the ladder refuses with a clear message â was already in place. What broke it was `<Input min="0" max={openQty}>`: at `openQty=0` the browser's constraint validation blocks form submission BEFORE the server action runs, so the ladder's message became unreachable, and `fractionOf(0, f)` made the shortcuts write nothing visible. The dialog also lacked the closed-state banner its Add-entry sibling already had.
**Decision:** Inside `BookExitDialog` only: a closed-position banner naming the two real paths (Add entry re-opens; delete the wrongly-booked exit leg and re-book reality), shortcuts hidden at 0 open, `max` dropped at 0 open so a determined submit reaches the ladder's own refusal. Submit stays enabled â the panel warns, it does not decide. Pinned in `e2e/staged-position.spec.ts`.
**Why not the obvious thing:** Disabling the submit button would repeat the exact UI-opinion block that the "never disables an action" test exists to forbid; and keeping `max=0` "for safety" is not safety â it replaces a designed, specific message with an unactionable browser bubble.
...
## 2026-08-24 â A frozen `--today` freezes only the caller: the sell-flow suite passed for exactly one day

**Context:** `npm run verify` failed on 2026-08-24; `tests/sell-flow.test.ts` (written 2026-08-23) had 2 of 11 failing.
**Measured / found:** `sell.mjs --today 2026-08-23` pins only sell.mjs's own outputs (receipt year, ledger note, backup-name prediction) â the SPAWNED `license-issue.mjs` and `license-backup.mjs` keep real time. So the minted expiry came out `2027-08-24` against a hardcoded `2027-08-23`, and sell.mjs verified/renamed a bundle named from the frozen date while the child wrote the real-date name (`vyuha-keys-2026-08-24.vkb`), making the post-backup existence check fail and the same-day-rename guard rename nothing.
**Decision:** sell.mjs now derives the bundle path from the same real-clock expression license-backup.mjs uses (`new Date().toISOString().slice(0,10)`, its line 94); the tests derive every expected date from the ledger's actual minted values â expiry asserted as shape + 360â370-day horizon, renewals probed at expiryâ100d / â22d / +9d, `.vkb` names matched by pattern.
**Why not the obvious thing:** plumbing `--today` down into `license-issue.mjs`/`license-backup.mjs` would put a clock-override into the two production scripts that mint and archive real keys â a test convenience is not worth a path that can backdate a licence.
...
Never edit an old entry to match new reality â append a new one that supersedes
it and say which. A changed mind is itself information.

Format:

```markdown
## <date> â <short claim, stated as the fact>
...
## 2026-08-20 â `winget-manifest.mjs` hashed the LOCAL installer while pointing InstallerUrl at the GitHub asset; `--sha` is now required
...
**Measured / found:** `scripts/winget-manifest.mjs` set `installerUrl` to the GitHub release download URL (line 53) but defaulted `sha` to `createHash("sha256")` over `src-tauri/target/release/bundle/nsis/` â the LOCAL build (line 66) â justified by an inline comment reading "identical, since the same build produced both". **That comment is false.** Downloaded and hashed both for v2.99.99: the published asset is **34,861,983 B / `46A3842ADD7B91A65F493330B8FAAEE0A1B06A2DA76A52DBFBA4CB6C74EB4343`**, the local build (and the copy inside the client ZIP) is **34,860,149 B / `27D8695E863D3426DE4016C86002C6A148E2F1A1E1457838A11835621BB23004`**. Same mismatch on v2.99.98 by size (34,857,616 vs 34,857,374). So `npm run winget:manifest` with no flags emitted a manifest whose `InstallerSha256` could not match its own `InstallerUrl`. The winget-pkgs validation pipeline downloads that URL and verifies the hash, so the PR fails; had one ever merged, `winget install` would fail hash verification for every user. **Nothing had been submitted yet, so no bad manifest ever reached Microsoft** â the defect was found before its first use.
**Decision:** Delete the local-file fallback outright. `--sha` is REQUIRED, and running without it exits 1 printing the exact `gh release download` + `sha256sum` recipe for the current version. Verified both paths: bare invocation refuses with that message; `--sha 46A3842Aâ¦` writes a manifest whose URL and hash agree. `WINGET_AND_SMARTSCREEN.md` gains a "Which file?" table at the top, and DOC_AUDIT rows 15/16 now name which binary each release-day action targets, with a new row 20 for the distinction itself.
**Why not the obvious thing:** Making the script download the asset and hash it automatically â more convenient, but it adds a ~35 MB fetch and a `gh` dependency to a script that has neither and currently runs offline. Keeping the fallback "as a default that is usually right" â it is never right, and a wrong hash is worse than a missing one because the manifest looks finished. Following the docs-only scope of the task that surfaced this â the docs cannot be made correct while the tool they document produces a broken artefact.
**Invalidated if:** the release workflow stops rebuilding and uploads the locally built artefact instead (then the two collapse into one file and the fallback would become safe â but re-measure before trusting it), or winget-pkgs stops verifying `InstallerSha256` against the downloaded URL.

## 2026-08-23 â Simulation testing: a known 10,028-fill book rendered as five broker formats is recovered exactly by every parser, and all five agree â five test failures along the way were all the TEST being wrong

**Context:** The owner asked for "simulation testing for broker files" and rigorous load testing before a release. Fixture tests already proved every parser survives a real layout; none proved a parser gets the NUMBERS back. `tests/sim/` is a deterministic book generator (`book.ts`) plus one writer per verified layout (`writers.ts`, built byte-for-byte from `docs/BROKER_FORMATS.md` and the redacted real-export fixtures), run through the REAL `detectParser` route so a layout drift fails as "wrong parser chosen" â the failure a user would see.
**Measured / found:** At 120 / 1,500 / 10,028 fills, every parser conserved buy and sell QUANTITY exactly and VALUE to the paisa: Zerodha Console (10,028 fills â 4,504 positions, fill times read), Dhan GTR (8,717 scrip-day bills â 4,504 positions, 179 mixed days split from stamp duty, stated charges carried), Paytm tradebook (numeric codes kept with ISIN, product derived from the day's STT/stamp signature), Groww orders (price derived as Value/Quantity), Angel One tax P&L (1,183 stated rows, none dropped; MTF read from Qty Breakup), and the generic mapper. **Cross-format agreement:** the same book yielded 4,504 positions with identical `symbol|buyQty|sellQty` signatures from Zerodha, Groww and the generic mapper, and zero per-symbol quantity mismatches against Dhan across 40 symbols; all four found the same 5 opening sells. **Five failures on the way, all in the test, none in the code:** (1) asserting `avgBuyPrice Ã buyQty == buyValue` â that is the recomputation AGENTS.md invariant 1 forbids; prices are rounded levels, value is exact, and it was off by 22 paise on a 3-tranche lot; (2) assuming Angel One's MTF Qty applies to holdings only â the parser correctly applies the per-ISIN breakup to every row it covers; (3) asserting Groww "invents no charges" â it ESTIMATES from the rate card and says so in a warning, which invariant 6 permits; (4â5) vitest's 5 s default timing out a 10k-row XLSX round-trip. Full load suite 34 passed / 1 expected fail (B7 pin) across 14 files; `npm run verify` EXIT 0, 1,956 tests / 133 files. Upstox is deliberately absent: its value behaviour is still INFERRED from zero-row exports, so a generator would test a guess against itself.
**Decision:** `tests/sim/` is part of `npm test` (24 cases, ~30 s at the large size, timeouts scaled per book). No production code changed â and that is the finding worth keeping: the import pipeline recovers a known book exactly at 10k rows across every verified format, proven by conservation rather than by "did not crash". No release is cut from this pass on its own because there is nothing to ship; the version question is decided by the owner with these numbers.
**Why not the obvious thing:** hand-written expected outputs per broker â they drift from the layout the first time a column moves, which is how the Groww-as-Zerodha misdetection happened (2026-08-12); generating from one book makes the expectation the book itself. Loosening the failed assertions until green â each failure was a real lesson about an invariant, and two of them (1, 3) are now documented in the test where the next person will meet them.
...
## 2026-08-22 â Defender flagged an INSTALLED v2.99.100 as `Trojan:Win32/Bearfoos.B!ml` (Severity 5); nothing reproduces on a static scan, and India cannot buy the cheap signing fix

**Context:** The owner updated v2.99.99 â v2.99.100 through the in-app updater. Windows Security reported a SEVERE risk and blocked/quarantined it. The same update had installed cleanly on another user's machine, more than once.
**Measured / found:**
...
- It named `%LOCALAPPDATA%\Vyuha\vyuha.exe` **plus** the Start Menu shortcut, the Desktop shortcut, the `Uninstall` registry key and the `startup:` entry â a **behavioural** verdict on the install action, not a file-content match.
- **The install SUCCEEDED first.** The installed `vyuha.exe` is present (14,169,088 B, FileVersion 2.99.100, written 17:09); the detection is timestamped 23:11 â hours later, from a scan or on execution, not at install time.
- **Nothing reproduces on a static scan** at definitions `1.457.274.0` / engine `1.1.26070.7`: the LOCAL installer, the LOCAL `src-tauri/target/release/vyuha.exe`, the **GitHub-built** v2.99.100 installer (downloaded fresh, SHA-256 `B84614E8â¦F299A`) and the GitHub-built v2.99.99 installer ALL scan "found no threats".
- A hypothesis that the CI-built binary trips the model while the local one does not was **tested and disproved** â both scan clean.
**Assessment (not certainty â Microsoft's model cannot be audited from here):** a machine-learning false positive on a brand-new unsigned binary with no reputation. Four independent supports: the `!ml` suffix, no static reproduction on current definitions, no detection on a second machine, and `Bearfoos` being a known generic ML family that fires on new unsigned installers. **The plausible trigger is the architecture itself:** the installer drops an app that bundles `node.exe` and spawns it as a sidecar, writes to AppData, and creates startup shortcuts and uninstall keys â an unsigned executable writing and launching another executable is close to a textbook dropper profile.
**Decision:** (1) **WDSI is now the right tool and yesterday's deferral is SUPERSEDED** â the 2026-08-21 entry deferred it because the required `Detection name` field had no honest value; it now has one. Submit BOTH binaries (client-ZIP installer and GitHub asset â different files, separate reputation). (2) **v2.99.100 stays published.** The owner's only two paying customers are close friends who are untroubled, there is no public download link (delivery is manual mail/WhatsApp), and the detection does not reproduce. Demoting to prerelease was considered and rejected as churn that protects nobody who currently exists. (3) **The real deadline is the winget merge** â once `winget install ThejeshK.Vyuha` is public, strangers install unattended and a Trojan warning reaches someone with no reason to extend goodwill. (4) `bundle.publisher` was unset and is now `"Thejesh K"` â free, and an unsigned binary with NO publisher name scores worse than one with consistent publisher metadata.
**Why not just buy a certificate â the numbers, researched 2026-08-22:**
- **Azure Trusted Signing (now Azure Artifact Signing), $9.99/mo Basic â NOT AVAILABLE.** Individual developers must be in the **US or Canada**; organisations are limited to US/Canada with three years of verifiable history. India is not supported and Microsoft has published no timeline. **This is the option `.github/workflows/release.yml` is already wired for** (`trusted-signing-cli`, dormant until `AZURE_*` secrets exist) â the prepared path is the one that cannot be used.
- **EV code signing** â **$279â297/yr** (Sectigo via resellers) to **$419â560/yr** (DigiCert). **Critically: EV lost its special status in 2020** â signed binaries no longer get instant SmartScreen reputation and now accrue it like any other Authenticode certificate. The historical reason to pay the EV premium no longer exists.
- **IV (Individual Validation)** is the correct product category here â built for independent developers, **no business documents**, personal ID validation, 3â5 days.
...
- **What signing actually buys:** reputation attaches to the **CERTIFICATE**, not to each file hash. With 61 tags there have been 61 cold hashes; signing collapses that to one accruing identity. It **reduces** ML false positives substantially but does **not** eliminate them for a new binary â the first signed release still starts with an unproven certificate.
**Invalidated if:** Azure Artifact Signing opens to India (then it is Â£-for-Â£ the obvious choice and the CI is already built for it), or Microsoft restores instant reputation for EV (then the EV premium becomes worth paying again), or a detection ever reproduces on a static scan of a released artifact â that would mean this entry's assessment is wrong and the build itself needs auditing.

## 2026-08-21 â The pairing engine was QUADRATIC on a single symbol: three O(lots) scans per sell, now a forward-only head pointer plus a per-date index

**Context:** The v2.99.98 rewrite of `lib/import/pair-legs.ts` (single pass â two) landed five days AFTER the load suite was written, and an import-graph scan of all thirteen existing cases found that **none of them import that module** â so the most algorithmically significant change in the import path had never been measured. It is the hot path for five sources: Zerodha, Paytm Money, Dhan GTR, Groww orders and the generic column mapper. Built `tests/load/c8-pairing-depth.load.ts` to close that gap.
**Measured / found:** the cost of a sell was proportional to the number of OPEN LOTS, not to the number of legs, because each sell ran **three separate O(lots) scans**: (1) `for (const lot of lots)` walking the whole queue to find same-day lots, (2) `lots.some(...)` **and** `lots.find(...)` re-scanning from the head on every iteration of the oldest-first `while`, and (3) a reverse `splice` compaction to drop exhausted lots. When buys outnumber sells the queue grows without bound, so the walk is O(nÂ²). Growth ratios at 4n (linear â 4, quadratic â 16), measured with `growthRatio`:
- one symbol, 65% buys â 8,000 â **79 ms**, 32,000 â **1,249 ms**, ratio **15.89Ã**; per-item 9.82 â 39.03 Âµs, i.e. per-item cost quadrupled when n quadrupled.
- one symbol, opening-sell heavy (25% buys, forces the pass-2 seed) â 14,000 â 70 ms, 56,000 â 937 ms, ratio **13.32Ã**.
- many symbols (500) â 24,000 â 26 ms, 96,000 â 109 ms, ratio **4.19Ã**, per-item flat at 1.10 â 1.14 Âµs. **The realistic book was never affected**: work partitions per symbol.
**Decision:** two indexes over the SAME `Lot` objects, so mutation is seen through both. `head` is a forward-only pointer to the oldest lot that may still hold quantity â lots are only ever emptied, never refilled, and the oldest-first pass always takes the first non-empty lot, so it never looks back; this replaces the `splice` compaction entirely. `byDate` maps each date to its lots in push order with its own head, so the same-day pass visits one day's lots instead of the queue. The seeded opening lot is deliberately NOT date-indexed: it pre-dates the file and must never satisfy a same-day match. **Selection is unchanged** â the head walk returns exactly what `lots.find((l) => l.qty > 0)` returned, and exhausting the queue ends the loop exactly where `lots.some(...)` did.
**Result:** ratios **15.89 â 3.70** and **13.32 â 4.10**; 50,000 legs on one symbol **775 ms â 63 ms** (15.49 â 1.25 Âµs/item) producing byte-identical output â 28,269 positions, 22,559 closed / 5,710 open / 0 opening-sell, quantity delta exactly 0, value drift â¹3.29 on â¹1,503,883,618 both before and after. Behaviour proven unchanged by 1,920 unit tests across 131 files AND both real-file reconciliations running (not skipping): Paytm's 414 executions against Paytm's own Realized P&L Detail, and the 1,554-fill Zerodha Console tradebook.
**Why not the obvious thing:** leaving it as an `it.fails` pin the way B7's double-decode is â defensible for a parser inefficiency, but this one silently degrades a *correctness-critical* engine as a user's history grows, which is exactly the "still works after months of data" property the product sells. Compacting the array less often instead of not at all â still O(lots) amortised, and the head pointer is simpler. Keeping `splice` to bound memory â the queue is bounded by leg count anyway, and trading a few thousand dead objects for removing an O(nÂ²) is obviously right.
**A second, smaller finding:** the first version of C8's conservation test asserted an ABSOLUTE â¹1 bar on value drift and failed at â¹3.29. That was the test being wrong, not the engine: a float64 sum over 50,000 legs totalling â¹1.5 billion drifts a few rupees by IEEE-754 alone. The assertion is now RELATIVE â value conserved to within one part in a million, against a measured 2.19 parts per BILLION. Quantity stays an exact `toBe(0)` because quantities are integers.
...
## 2026-08-20 â The installer buyers run and the installer on GitHub are DIFFERENT binaries; SmartScreen reputation is per hash, so they earn it separately

**Context:** Deciding which file to submit to Microsoft (WDSI) and which hash winget should carry, while cutting v2.99.99.
**Measured / found:** `.github/workflows/release.yml` triggers on a tag and rebuilds through `tauri-apps/tauri-action@v0` on its own runner matrix, producing a **draft** release. `npm run client:package` instead zips the installer built LOCALLY by `npm run desktop:build`. These are not the same file: for the already-published **v2.99.98**, the GitHub asset `Vyuha_2.99.98_x64-setup.exe` is **34,857,616 bytes** and the local build in `release-packages/Vyuha_2.99.98_Client_Package.zip` is **34,857,374 bytes** â a **242-byte** difference, so necessarily different SHA-256s. Both are validly signed: every `.sig` checked carries `4FF85F3BBE1DA21D`. The delivery model is manual mail/WhatsApp of the client ZIP, so **the binary a paying buyer actually runs is the LOCAL one**, which never touches GitHub.
**Decision:** Treat the two as separate reputation subjects. The **WDSI false-positive submission must use the installer from the client ZIP** â that is the file buyers double-click â and the winget manifest necessarily references the GitHub asset and its own hash. `CHECKSUMS.txt` inside the ZIP is generated from the local file and is correct for it; do not "fix" it against the release asset. Record both hashes in VYUHA-STATE per release so the two are never confused.
**Why not the obvious thing:** Assuming one release = one binary, and submitting whichever hash is easiest to copy â that spends the submission on a file no buyer runs, and every buyer still meets a cold SmartScreen warning. Also rejected: uploading the local installer onto the tag to make them identical â never re-upload assets onto a published tag (the updater compares version numbers, so an existing install is never re-offered the same version, and the ZIP's CHECKSUMS.txt is per file hash).
**Invalidated if:** the release workflow is changed to upload the locally built artifact instead of rebuilding, or delivery moves from mailed ZIP to a GitHub download link â either collapses the two files into one and this entry stops applying.

## 2026-08-20 â Signature key ids can be decoded from the local .sig BEFORE publishing; `release:verify` only works on a tag that already exists

**Context:** The release skill's step 5 says "verify by decoding the signature's key id, never by trusting â signed" â the stale v2.91.0 key produced signatures the build called valid while every installed copy rejected the update (v2.98.0). But `npm run release:verify` takes a TAG and reads assets off the GitHub release, so at the moment the local build finishes there is nothing to run it against, and `tauri-build` prints exactly the adjective the skill warns about: "â Vyuha_2.99.99_x64-setup.exe is signed (.sig present)".
**Measured / found:** A Tauri updater `.sig` is base64 of a minisign file. Base64-decode it once to get `untrusted comment: â¦
<base64 signature>`; base64-decode that second line to a 74-byte buffer laid out as **2-byte algorithm, 8-byte key id (little-endian), 64-byte signature**. Reversing bytes 2â10 and hex-upper-casing yields the key id. On the v2.99.99 artifacts both the NSIS `.exe.sig` and the MSI `.msi.sig` decode to **4FF85F3BBE1DA21D**. The public half in `.secrets/vyuha-updater.key.pub` decodes to the comment `minisign public key: 4FF85F3BBE1DA21D` and its base64 is **byte-identical** (`===`) to `plugins.updater.pubkey` in `tauri.conf.json`. The stale key is `8FFAF1B491EAD2F0`; no copy of `updater-private.key` exists in the repo root (deleted 2026-08-14, confirmed still absent).
**Decision:** Decode the key id off the LOCAL `.sig` files immediately after `desktop:build`, before committing â a ten-line Node snippet, no network, no tag. `release:verify <tag>` still runs after the Release workflow as the second, asset-level check. The two are complementary, not redundant: the local one gates the tag, the remote one gates publishing.
**Why not the obvious thing:** Trusting the build's "â signed" line â it asserts a `.sig` exists, not whose key made it, which is precisely the failure that shipped v2.98.0. Waiting for `release:verify` â by then the tag is pushed and the Release workflow has already built and uploaded assets, so a wrong key is discovered after the expensive half.
**Invalidated if:** the updater key is rotated (then both the expected id here and `tauri.conf.json` change together â the check is the comparison, not the literal), or Tauri changes the `.sig` envelope away from base64-wrapped minisign.

## 2026-08-20 â No runtime surface exposes Vyuha's patch version; the sidebar shows major-minor only, and the install guide claimed otherwise

**Context:** Bumping every buyer-facing version string to v2.99.99 for the release. `docs/client/INSTALLATION_GUIDE.md` Â§ Support told buyers to quote their version "shown at the bottom of the sidebar, e.g. `Local Â· Offline Â· v2.99.98`".
**Measured / found:** `components/layout/sidebar.tsx:392` renders the literal string `Local Â· Offline Â· v2.99` â major and minor only, no patch, and not derived from `package.json`. A grep across `app/`, `components/`, `lib/`, `src-tauri/src/` and `scripts/desktop-server.mjs` for `npm_package_version`, `getVersion()` and `package_info` returns **nothing**: the running app never displays its own patch version anywhere. **CORRECTION appended 2026-08-21:** this entry originally claimed `scripts/bump-version.mjs` does not sync the sidebar footer, and that AGENTS.md was stale on the point. **That was wrong.** It does sync it (`bump-version.mjs:76`), but the footer is MAJOR.MINOR only, so a patch bump is a no-op and the script prints three files instead of four â which is what was misread. The v3.0.0 bump printed `sidebar.tsx footer -> v3.0` and "Synced 4 file(s)". What remains true, and is the actual point of this entry, is that **no runtime surface shows the PATCH version**: the footer reads `v3.0`, never `v3.0.0`, so the install guide still must not send buyers there for a full version string.
**Decision:** The install guide now points at two places that genuinely carry the full version â the installer filename (`Vyuha_2.99.99_x64-setup.exe`) and Windows **Settings â Apps â Installed apps** â and describes the sidebar honestly as showing the release line `v2.99`. The sidebar itself is NOT changed: it is a deliberate one-line footer, and wiring a build-time version into a client component to satisfy a support instruction is a bigger change than the problem.
**Why not the obvious thing:** Bumping the example string to `v2.99.99` with the rest of the release â that is what the last several releases did, and it kept a sentence false in a document that ships inside the buyer's ZIP. A support instruction that sends a buyer to a screen which cannot answer the question costs a round-trip every time it is followed.
**Invalidated if:** a version string sourced from `package.json` is surfaced in the app (Settings â About, the Help Desk footer, or the sidebar gaining the patch digit) â then point the guide at that instead.

## 2026-08-20 â OpenAlgo ships OFF behind a versioned disclosure: two settings columns, a server-enforced gate, and consent in the Audit Log

**Context:** Adding OpenAlgo (third-party, self-hosted, AGPL-3.0, fronts 35 Indian brokers) as a fourth broker-API import source â the first Vyuha feature that asks the user to run a SECOND program and give IT their broker credentials. Owner's requirement: the user activates it themselves, after an in-app explanation of what it is, what it does and what the risks are.
**Measured / found:**
- Verified against OpenAlgo's own docs (2026-08-20): `POST /api/v1/tradebook` with an `apikey` body, `{status:"success", data:[â¦]}` envelope, **current trading day only**, and the documented sample really does return `quantity: 0.0` on a filled trade with `average_price: 1180.1` / `trade_value: 1180.1`. Their published broker list covers 7 of Vyuha's 8 (**no Sahi**), which is what the adapter's table already states.
- The app already had the right precedent for an opt-in network feature: `auto_mtm_enabled` â off by default, a Switch in Settings, the overwrite risk named in place. This is that pattern plus an explicit acceptance, because the risk is larger.
**Decision:**
...
- **All disclosure copy lives in that one pure module**, versioned, and is rendered by the dialog, the Settings card and the Import blurb. Copy written twice drifts â the dropzone hint did exactly that (2026-08-12) and advertised three brokers while the code read five.
...
- **Both columns are `SETTINGS_MACHINE_COLUMNS`** (redacted from every backup, re-applied from this machine on restore) and are **excluded from `BASELINE_SETTINGS_FIELDS`**: restoring a journal â or clicking "back to my defaults" â must never switch an integration on or assert a consent the user did not give.
...
**Invalidated if:** OpenAlgo's `/tradebook` starts returning a trustworthy `quantity` (the repair path and its warning then become noise), or the endpoint grows a date range (it stops being a daily pull and the "not a backfill" line must come off), or the risk statement changes materially â then bump `OPENALGO_DISCLOSURE_VERSION`, which re-prompts every install by design.

## 2026-08-20 â Angel One's live API pull has been broken since v2.99.80: `encryptSecret("")` is unreadable, and the pull refused on it

**Context:** Wiring OpenAlgo, whose credential shape copies Angel One's â key encrypted, `accessToken: encryptSecret("")`, extras in `auth_json` â made the OpenAlgo pull fail before it reached the network.
**Measured / found:** AES-GCM over an empty plaintext yields a **zero-length ciphertext**, so `formatVaultString` produces `venc:1:<iv>::<tag>`. `parseVaultString` (lib/vault-format.ts:76) rejects any empty segment, so `readSecret` on that value returns `{ok:false, reason:"malformed"}` â correctly, an empty secret is not a secret. The pull branch of `app/api/import/broker/route.ts` guarded with `if (!keyRead.ok || !tokenRead.ok)` **before** dispatching to a broker, so every `needsToken: false` broker was refused with "The saved credentials cannot be read: â¦ Re-enter the API key and access token." Angel One is the only shipped `needsToken: false` broker, so **its unattended pull â the feature README calls "the one that needs no daily attention" â has failed for every user since the vault landed in v2.99.80**, and `sweepPlaintextSecrets()` re-encrypting a legacy empty string produced the same broken shape. Verified by reading the shipped code at the v2.99.98 commit, not inferred from a stack trace.
**Decision:** the guard becomes `if (!keyRead.ok || (needsToken && !tokenRead.ok))`, with `needsToken` read from `API_BROKERS`, and the Kite/Dhan branches take `accessTokenPlain = tokenRead.ok ? tokenRead.value : ""`. The vault itself is NOT changed â refusing to read an empty ciphertext is right; the caller was wrong to demand a token it never collected. `tests/vault.test.ts` now pins the property ("an EMPTY secret encrypts to a string that cannot be read back") with the story attached, so the next caller meets this trap in a test rather than in production.
**Why not the obvious thing:** making `encryptSecret("")` round-trip (special-case the empty string, or store `null`) â that weakens a format whose whole job is to be unambiguous, to paper over one caller's bug; and `null` in the column would collide with the pre-vault plaintext path that `readSecret` deliberately still supports.
...
## 2026-08-20 â Paytm Money tradebook reconciled against Paytm's own lot statement: pairing now "same day first, then FIFO, opening inventory oldest"; 47 of 52 scrips agree within â¹25

**Context:** The owner supplied a real Paytm Money tradebook (414 executions, 3â18 Aug 2026) and the matching Equity P&L (`.xls`, 3 sheets, 124 realised lots) â the first live data for the parser built schema-only on 2026-08-12 (VYUHA-STATE Â§7 rule: reconcile before trusting it). No contract note was supplied; Paytm's **Realized P&L Detail** (one row per matched lot, with buy/sell dates and values) is the broker-stated reference used instead.
**Measured / found:**
- The old parser aggregated the whole file per `Script` and set `grossPnl = sellValue â buyValue`: **66 positions, â¹1,26,04,496.93 of "gross P&L"** â â¹2.17 Cr of it on rows where sells exceeded buys (a sell-only scrip booked as 100% profit). Paytm's own realised P&L for the period is **â¹20,23,631.97**.
...
- Paytm's lot values are **charge-inclusive**: 03-Aug buy fills averaged 52.4500 with 0.0632/share of charges, and the lot shows 52.5132; so its `P&L Value` is NET. Compare gross â apportioned charges, not gross.
- Paytm matches a sell against the **same day's buy first** (05-Aug sell 10,000 â 05-Aug buy 10,000 while 10,000 of a 03-Aug lot stayed open), then FIFO, and **pre-window holdings are the oldest lot** (a 48,000-share holding was consumed by the earliest delivery sells: 32,000 on 03-Aug, 4,800 on 04-Aug, 6,400 on 10-Aug, 4,800 on 11-Aug). Pure FIFO disagreed with the broker on 52 of 60 scrips; same-day-first alone on 36; same-day-first + seeding the inferred opening inventory at the head of the queue: **47 of 52 in-window scrips within â¹25, 4 within â¹400, 1 off by exactly 3,200 shares** of opening inventory the tradebook cannot see (the file proves a minimum of 44,800 sold from holdings; Paytm had 48,000). Totals: our closed net **â¹12,34,049.50 vs Paytm â¹12,51,954.19 (â1.4%)**; the â¹7,71,677.78 Paytm earns on lots bought before the window is what our 24 opening sells leave blank. Stated charges conserved: â¹1,98,914.88 apportioned vs â¹1,98,915.04 in the file.
- After the change: 414 fills â **142 positions (88 closed / 30 open / 24 opening-sell), 38 intraday / 104 delivery derived from the STT+stamp signature per scrip-day**, 0 unknown.
**Decision:** `lib/import/pair-legs.ts` now (1) consumes the same-day lot before older lots, (2) runs two passes â the first measures the orphan quantity, the second seeds it as an opening lot that FIFO retires first, so opening sells land on the EARLIEST delivery sells, where the broker puts them â and (3) dates a closed position by the oldest lot consumed. `paytm-tradebook.ts` aggregates fills per scrip-day-side, infers product from the day's charge signature, apportions the six stated charge components by each position's share, resolves the coded symbol from the ISIN at commit (instruments table â bundled NSE index map â keep the code with a note). `tests/private-reconciliation.test.ts` replays this against the real files where they exist (skipped elsewhere). The engine change applies to Dhan GTR, Groww orders and the generic mapper too â every existing pairing test still passes.
**Why not the obvious thing:** keeping pure FIFO "because the Income Tax Act says FIFO" â the exchange nets a scrip's same-day buy and sell before anything reaches delivery, and the broker's own statement (and its STT) follows that; FIFO across days is still what decides the holding period. Treating the 3,200-share gap as Paytm's error or padding the seed to close it â the tradebook has no evidence for a larger opening holding; the user supplies a basis, the importer never invents one. Using the Summary totals instead of the lot detail â totals hide exactly the per-scrip disagreement that revealed the matching rule.
**Supersedes:** the 2026-08-12 Paytm entry's *Invalidated if* â the invalidating event has now happened. Its value semantics are no longer INFERRED, and the reference used was the broker's own **Realized P&L Detail lot statement**, not a contract note (none was supplied). AGENTS.md's matching caveat was discharged the same day.
...
## 2026-08-20 â Zerodha tradebook: the Console export has an 8-row preamble, numeric date serials and a separate execution-time column; aggregating the whole file per symbol fabricated â¹31 L

**Context:** First real Zerodha Console tradebook (1,554 fills, AprâJun 2026, 23 symbols, mostly SME) and a column-A Console P&L (53 rows) â the 2026-08-12 samples were header-only.
**Measured / found:** the old tradebook branch produced 23 positions; **8 were sell-only** (holdings bought before the window) and 3 unbalanced, booked as `sellValue â buyValue` = **â¹31,04,586 of fabricated gross P&L** inside a â¹42,32,813 total; every position carried the first fill's date as both entry and exit (10 of 23 spanned several days); `Order Execution Time` (renders `2026-04-01 11:14:28`) was never read, so no position had a fill time. A first rewrite with one pairing leg per FILL gave 936 positions (one per sell fill on 2â11-share SME fills); per scrip-day legs give **28 positions (15 closed / 2 open / 11 opening-sell), Î£ closed gross â¹5,21,782, fill times on all 28, conservation check clean**. The Console P&L on disk covers a different period (JSLL 0 there, QUESTLAB absent), so it could not serve as the tradebook's reference. Under a neutral filename the Auction fingerprint alone scored 0.65 and the `- Z` heads 0.55 â below the 0.7 routing bar â so `tradebookFp` is now 0.5 and `consoleFp` 0.55 (0.75 / 0.70 measured).
**Decision:** `zerodha.ts` pairs per symbol + product via `pairLegs` on scrip-day-side legs, keeps every fill as an execution inside the position's window, reads the clock from `Order Execution Time`, derives product from the calendar when the export has no Product column (`productDerived`), refuses undated/unsided rows, and skips the three all-zero ISIN-in-Symbol rows of the Console P&L. `sourceRows` = fills, and the warning says "N fills â M positions (FIFO per symbol + day)".
**Why not the obvious thing:** one leg per fill â `pairSymbolLegs` emits one closed position per sell leg, so fill-level legs turn an SME tradebook into hundreds of "trades" nobody made; the bill/scrip-day is the unit Dhan GTR already uses. Keeping the whole-file aggregate for "simplicity" â invariant 6 forbids the 100%-gain rows it produces.
**Invalidated if:** Zerodha adds a Product column to the Console tradebook (then the derived-product branch should yield to it â it already does when the column exists), or `pairLegs` grows a fill-level mode.

## 2026-08-20 â Upstox reports fingerprint on the A1 legal name; layouts verified, values still INFERRED (zero data rows)

**Context:** Three real Upstox exports (trade report, realised P&L, ledger) from an account with no trades â the first Upstox files ever examined; `detectUpstox` looked for the broker's name in the header row only and scored 0 on all three.
**Measured / found:** A1 of every sheet is `UPSTOX SECURITIES PRIVATE LIMITED`; the TRADE header sits on row 11 (`Date | Company | Amount | Exchange | Segment | Scrip Code | Instrument Type | Strike Price | Expiry | Trade Num | Trade Time | Side | Quantity | Price`), the REALIZED_PNL header on row 22 (`â¦ Symbol | ISIN | â¦ Buy Date | Buy Rate | Buy Amt | Sell Date | Sell Rate | Sell Amt | Days | Total PL | Short Term | Long Term | Speculation | Turn Over`), the LEDGER_V3 sheet has no column header at all. Filenames name no broker. After the change: trade report 0 â **0.75**, realised P&L 0 â **0.95**, ledger 0 (nothing to read), Angel One's detector 0 on all three and unchanged elsewhere.
**Decision:** the Upstox fingerprint is any pre-header cell containing "upstox"; `Trade Time` feeds the clock; `Buy/Sell Date`, `Buy/Sell Amt`, `Total PL` are mapped; product is derived from `Speculation` (intraday) / `Short Term`+`Long Term` (delivery) only when no product column exists, flagged `productDerived`; F&O rows keep `Company` as the symbol with an import note â no F&O symbol grammar was invented. Value behaviour (signs, date/time formats, Side spellings, Instrument Type vocabulary) is INFERRED and written into the synthetic test rows as assumptions, not observations. The ledger stays unparsed.
**Why not the obvious thing:** adding "company" to the shared symbol-column list â it would change Angel One's scoring, which has no new evidence. Writing an F&O tradingsymbol from Company + strike + expiry â the first real row would show whether that guess was right, and a wrong symbol silently mis-prices a trade.
**Invalidated if:** a populated Upstox export arrives â re-verify every INFERRED item above and replace the synthetic rows in `tests/angelone-upstox.test.ts` with redacted real ones.

## 2026-08-20 â Redacted fixtures are SCHEMA-ONLY copies with three synthetic rows, produced by a scratch script and leak-scanned against every value in the private files
...
**Measured / found:** A generic transform (keep every label/header/footer row and the blank-row structure; scrub the value cells of identity rows; keep the first three data rows of each table with symbols â `SYNTHn`, ISINs â `INE0SYNnnnnn`, long numeric codes â `99900n`, money â small deterministic numbers, dates kept; re-apply date formats so numeric serials still render as `yyyy-mm-dd`) passed a scan that flags any cell matching an identity value, ISIN, scrip name token or long code from the private files â the only residual hits were Upstox's own registered-office address and Zerodha's charge-head words. The real files replay through the same assertions in a `describe.skipIf` block, proving redaction preserved exactly the cells detection reads.
...
**Why not the obvious thing:** committing the real files with the client id scrubbed â the trades themselves are the owner's book; hand-writing fixtures from the docs â a copy drifts from the quirks (formatted blank rows, preamble positions, BIFF `.xls`) that broke detection the first time.
**Invalidated if:** the private folder changes (re-run the scratch transform and the leak scan before committing a fixture).

## 2026-08-15 â The landing page is hosted on GitHub Pages from `main:/docs` via a redirect, not a copy

Public URL: https://thejesh-k463.github.io/VYUHA-LOG/ â `docs/index.html` (meta-refresh) â
...
Pages was enabled with `gh api -X POST repos/â¦/pages -f source[branch]=main -f source[path]=/docs`;
first build ~1 min; verified 200 on root, landing page and a screenshot. `docs/.nojekyll` added so
underscored paths are served. Everything under `docs/` (incl. `docs/owner/*`) is public â it already
...
## 2026-08-15 â Screenshots come from synthetic fixtures via one script; skin-royal retired from every surface
...
## 2026-08-15 â Reinstall over a running copy failed on `server\node\node.exe`: the sidecar was only killed on WindowEvent::Destroyed, and Tauri's NSIS template stops vyuha.exe alone

**Context:** buyers upgrading v2.99.96 â next saw the NSIS dialog "Error opening file for writing â¦ server\node\node.exe".
**Measured / found:** the Node sidecar was stopped only in the `WindowEvent::Destroyed` handler. An in-app update, a crash, or a Task-Manager kill of `vyuha.exe` never reaches that handler, so `node.exe` outlived the shell and held the file lock. Tauri's stock NSIS template kills the main executable only. Clicking *Ignore* was harmless â the locked file is byte-identical (same Node 22.17.0) â but nobody reads an installer error as harmless.
**Decision:** `stop_sidecar()` (kill + wait) runs on `WindowEvent::Destroyed`, on `RunEvent::ExitRequested` and `RunEvent::Exit`, and immediately before `update.download_and_install`. The NSIS `PREINSTALL` / `PREUNINSTALL` hooks additionally stop any `node.exe` / `vyuha.exe` whose `ExecutablePath` is under `$INSTDIR` â a `Get-CimInstance` filter, with `$INSTDIR` handed to PowerShell through an environment variable so a path with spaces or quotes never needs escaping.
**Why not the obvious thing:** `taskkill /IM node.exe` kills every Node process on the machine â a developer's dev server, another Electron app's helper â for a file we could otherwise leave alone. Killing by path is the only version that touches nothing but ours.
...
## 2026-08-15 â Buy CTAs open a contact dialog, not a link: the desktop webview blocks external `target=_blank` and we ship zero runtime Tauri plugins on purpose

**Context:** every "Get Pro / Get Lifetime" button was an `<a target="_blank">` to WhatsApp; in the desktop app it did nothing.
**Measured / found:** the Tauri webview refuses to open external URLs in a new window unless the opener/shell plugin is present. The shell deliberately carries no runtime Tauri plugins (updater aside) â that is what keeps the Rust side small and the permission surface flat â so the click was silently swallowed. The same buttons work in a browser tab, which is why nobody noticed in dev.
**Decision:** every "Get â¦" opens a dialog that states the number (+91 73936 73714), the pre-filled message, **Copy number** and **Copy message** buttons, an *Open WhatsApp* link (works in browsers, inert but harmless in the desktop shell) and the offline note. Settings â License pills open the plan card in a popup rather than a comparison table.
**Why not the obvious thing:** adding `tauri-plugin-opener` for one link â a runtime dependency, a capability entry and a review surface for a button whose whole job is to get a phone number in front of the buyer. A dialog with a copyable number works identically online, offline, in-browser and in-shell.
**Invalidated if:** the shell ever gains the opener plugin for another reason â then the link can be primary again, but keep the copyable number.

## 2026-08-15 â Appearance: tint curves cut from spec 0.20/0.22/0.14 to 0.13/0.11/0.11 so every skin holds â¥9:1 (dark) / â¥7:1 (light) at intensity 100; tokens go inline on `<html>`, not as classes

**Context:** v2.99.97 adds a tint-intensity slider (0â100, default 50; presets Subtle 25 / Balanced 50 / Vivid 75; â/+ steps of 10), panel styles (Flat / Soft / Luxe / Glow) and a wallpaper, all driven by the pure engine `lib/domain/appearance.ts`.
**Measured / found:** with the spec curves (card-top 0.20, card-hover 0.22, sidebar surface 0.14) Lime dropped card-top/hover body-text contrast to 8.5:1 and Luxe's surface to 11.9:1 at intensity 100 in dark. Reduced to 0.13 / 0.11 / 0.11, the worst skin measures: dark canvas/card/surface â¥12:1, card-top/hover â¥9:1; light â¥7:1 across all eight skins at intensity 100. Chart tokens must stay literal colours (lightweight-charts renders an invisible series on `color-mix()`/`var()`), and lightweight-charts re-reads its theme only on a class change â so charts re-theme via an `appearance-tick` class toggle on `<html>` while the colours themselves are injected inline by `app/layout.tsx` (no first-paint flash; an inline style beats any class-scoped token without a specificity fight).
**Decision:** curves as above; panel styles are `html.panel-*` classes with overrides in `@layer utilities` at (0,2,1) specificity; Terminal + Glow deliberately degrades to a flat shadow (a glow on a monochrome skin is a smear). Custom theme is a ninth skin `custom`: 7 fields Ã dark/light (accent, analytics, money, sidebar, cards, borders, canvas), derived shades computed in code, per-row WCAG badge that warns and never blocks, "Start from <skin>" seeds from the computed tokens; saved only with the form and only while Custom is selected. Wallpaper: PNG/JPEG/WebP â¤12 MB, magic-byte sniffed, stored in `<data>/wallpaper/` outside backups (said on the Backup screen), fixed cover behind `<main>` with a theme-aware scrim `rgb(ch / var(--wallpaper-scrim))`, opacity slider, removed in print. Migration 0048 adds `tint_intensity`, `panel_style`, `custom_theme`, `wallpaper_stored_name`, `wallpaper_opacity`. `ice`/`royal` still map to `sapphire`.
**Why not the obvious thing:** keeping the spec curves â Vivid on Lime would put body text on a card-top below the 9:1 floor this project has held since the first skins. `color-mix()` for the scrim â it is exactly the value class lightweight-charts and older WebView2 builds mishandle; `rgb(ch / var())` is plain CSS Color 4. Blocking a custom colour that fails WCAG â the user typed it on purpose; a badge tells them the number and lets them decide. Putting the wallpaper in backups â a 12 MB image in every nightly backup for a decoration.
...
## 2026-08-15 â Trades table shows Entry/Exit PRICE, not Buy/Sell VALUE; qty 0 â "â", never â¹0; MTF own-% is derived or absent

**Context:** v2.99.96 replaces the Buy value / Sell value columns on /trades with Qty, Invested, Entry price, Exit price.
**Measured / found:** Every import path fills `avgBuyPrice`/`avgSellPrice` â `NormalizedTrade` (`lib/engine/types.ts:47â64`) declares both non-optional, so the 6 broker parsers, the generic column mapper and the 3 API pulls (Kite, Dhan, Angel One) cannot produce a row without them; nothing on this surface can be undefined. An open trade has one side with qty 0 and price 0.
**Decision:** a side whose qty is 0 renders "â", never â¹0 (a zero price is not a fact about the trade). Invested on an MTF row shows the trader's own contribution % computed as `(buyValue â mtfFundedAmount) / buyValue`, and the funded amount beside it; when `mtfFundedAmount` is null the cell says "funding not yet resolved" â no default margin is substituted (invariant 6).
**Why not the obvious thing:** keeping "Buy value" â a value is qty Ã price and hides the level the trader actually acted on; showing â¹0 for the missing side of an open trade reads as a fill at zero.
**Invalidated if:** `NormalizedTrade` ever makes either price optional (the "â" rule then needs a null branch too), or a parser starts writing legs without the aggregate on the parent row (invariant 5).

## 2026-08-15 â Skins v2.99.96: the hue wheel is nearly full; Lime 83Â° / Rose 329Â° / Ember chosen, Ice and Royal retired, surface tints ~1.30:1
...
**Measured / found:** hues already taken: gold 42Â°, tape 45Â°, profit 157Â°, luxe 172Â°, cb-profit 217Â°, sapphire 224Â°, violet 255Â°, aurora 292Â°, loss 352Â°, cb-loss 38Â°. Rejected: coral (~16Â°, only 24Â° from loss), emerald (= profit), indigo (8Â° from Sapphire), sky (~198Â°, Ice reborn). Chosen: **Lime 83Â°** (13.29:1 dark / 5.41 light), **Rose 329Â°** (7.57 / 5.58), **Ember** (8.85 / 5.16 â 11Â° from the colour-blind-safe loss hue, accepted on Tape's precedent of 7Â° from gold). Ice â Sapphire â identical analytics hex, distinguishable only by the primary; Royal's primary was Luxe's analytics hex. Surface tints composite the border at ~1.30:1 against the card (was 1.24) â note the 1.48:1 floor in the 2026-08-09 entry is for `--color-rule` (row separators), not `--color-border`.
**Decision:** SKINS = luxe, mono, tape, sapphire, aurora, lime, rose, ember; `asSkin` maps a stored "ice"/"royal" â "sapphire". To let the hex-distinctness test hold, Tape/Aurora analytics retuned #2dd4bf â #5eead4 and Sapphire analytics #e879f9 â #f0abfc.
**Why not the obvious thing:** keeping ten skins â two of them were the same skin twice, and a "choose your accent" list where two entries look alike is a bug the user files, not a feature. Picking coral/sky by eye lands inside the P&L or an existing skin's hue band, which is how Tape's money had to move (2026-08-11).
**Invalidated if:** the canvases or `--color-profit`/`--color-loss` hues change, or a ninth skin is proposed â measure its hue against this wheel first.

## 2026-08-15 â The desktop app's phantom console: node.exe is console-subsystem; fixed with CREATE_NO_WINDOW + a log file, readiness never depended on stdout

**Context:** buyers saw a second, blank terminal window behind the desktop app.
**Measured / found:** the Tauri shell is a GUI-subsystem process; the Node sidecar (`node.exe`) is console-subsystem. A GUI parent spawning a console child with inherited stdio makes Windows allocate a fresh console for the child. Sidecar readiness was always the TCP poll on the loopback port â stdout was never read for it.
...
## 2026-08-15 â Load tests batch 2: five defects fixed with numbers, one prediction wrong, one non-defect proven

**Context:** `tests/load/` batch 2 (B1âB7, C2, C7) on synthetic books of 10kâ320k trades.
**Measured / found:**
- **B1 lens grouping** â growth ratio 14.3 (55 ms @10k â 794 ms @40k). After a per-array index: 5.1â5.3 (69 ms @80k â 347 ms @320k). Test asserts ratio < 8.
- **B2 entitlement** â 200 spaced licence reads did 200 UPDATEs (the "last seen" mark rewritten every read). Now the mark is written only when â¥ 24 h past the stored mark â 0 UPDATEs across the run; a pure read 65 â 56 Âµs.
- **B5 `dbCounts`** â read 125,195 rows in 421 ms to count them; now `COUNT(*)`: 29 rows / 1 ms.
- **B6 encrypted restore** â two scrypt derivations (498 + 527 ms) â one (495 + 30 ms) via a derived-key cache: 5-minute TTL, 4 entries, keyed by sha256(salt | params | password).
- **B7 import detect** â 15 full XLSX decodes / 1,331 ms â 8 / 802 ms by memoising `rankParsers` per `ParseContext`. Getting to â¤ 2 needs the parsers to share one parsed workbook â pinned as an `it.fails` follow-up, not claimed.
- **C2** â prediction WRONG: the skipped-row warning was already emitted. Adjacent defect found instead: executions-shaped generic imports left `sourceRows` unset (6,491 lines â 4,226 positions reported as if 4,226 rows); now `sourceRows = rows â skipped`.
- **C7** â no defect: over 250k trades a naive float fold drifts 5.0e-4 paise pre-rounding and 0 after, against `SUM(net_pnl_paise)`. `getTradeStats` over 250k = 3.1 s, reported not fixed.
...
**Why not the obvious thing:** "make it faster" without the ratio â B1's absolute time at 10k was fine; only the growth exponent showed the defect.
...
## 2026-08-15 â Annual â Lifetime upgrade: full credit within the year, not pro-rata

**Context:** an annual buyer wants Lifetime part-way through the year (tooling: `scripts/license-upgrade.mjs`).
**Measured / found:** due = lifetime launch price â annual amount paid (â¹29,999 â â¹9,999 = â¹20,000 at launch prices), for any upgrade inside the annual term.
**Decision:** full credit of the annual payment within its year â owner decision 2026-08-15. The same sentence appears on the pricing screen, landing page and brochure (`tests/pricing.test.ts` pins them verbatim).
**Why not the obvious thing:** pro-rata (credit Ã months remaining / 12) is "fairer" on paper and impossible to explain in one line on a pricing page; a buyer who cannot predict the number does not upgrade.
**Invalidated if:** the launch prices end (2027-01-01) â the credit then applies against the list price and the copy must say so.

## 2026-08-15 â Licence key archive: per-key plaintext to an owner-chosen folder, plus an AES-256-GCM bundle at scrypt N=2^15
...
**Decision:** N=2^15 rather than the backup-format's 2^17 because this runs interactively on the owner's machine once per session, not per customer restore; the ~4Ã cheaper derivation is fine for a passphrase the owner chose.
**Why not the obvious thing:** reusing `lib/backup.ts`'s format â it is built for a customer's database and pulls the app's schema in; the key archive needs zero app imports so it still runs when the app does not.
**Invalidated if:** the PEM moves to a hardware key or a KMS, in which case the bundle carries only the ledger.

## 2026-08-15 â "Preview pane" defect closed: it was the dev-tool browser pane on the /trades dev build, not the app

**Context:** an item on the open list said the trades preview pane rendered blank.
**Measured / found:** the report came from the IDE browser pane against `next dev` on /trades â the same "networkidle is not hydration" behaviour recorded 2026-08-10. Nothing reproduces in the desktop build or a real browser.
...
## 2026-08-15 â Launch-offer anchors are committed 2027 list prices, and the advertised percentages are derived, not the ones the owner asked for

**Context:** Owner requested strike-through launch pricing ("â¹13,000 â â¹9,999, 30% off"; "â¹35,999 â â¹29,999, 20% off, best value") plus a competitor table on /pricing.
**Measured / found:** 9,999/13,000 = 23.08% off and 29,999/35,999 = 16.67% off â the requested "30%/20%" labels do not survive division. A first cut used `Math.round`, and the review pass caught 16.67 â "17% off": rounded UP, overstating the discount. Anchors previously did not exist anywhere in the repo (grep for 13,000/35,999: zero hits); `wasInr` existed in the type, unused, and both `PricingTable` variants already rendered it.
**Decision:** Owner confirmed â¹13,000/yr and â¹35,999 as the REAL list prices effective **2027-01-01** (recorded here and in MONETIZATION_PLAN Â§2; deliberately NOT rendered in-app â owner's call). Percentages ship as the derived-and-FLOORED **23% / 16%** via `offerPct()` in `lib/domain/pricing.ts` â floor, not round, because a discount claim must never overstate; understating by <1% is fine. `featured` moved from annual to lifetime â the owner sells lifetime first. Lifetime's roadmap line (mutual funds, gold) is labelled "planned, not yet shipped".
**Why not the obvious thing:** Shipping the requested 30%/20% â a stated percentage that fails division is a false claim of exactly the kind v2.99.94 retired, and a fabricated anchor is a CCPA-2023 dark pattern. The alternative anchor (â¹14,299 to make 30% true) was offered; the owner kept â¹13,000.
...
## 2026-08-15 â Competitor comparison cells are sourced-or-"Not stated", with â  for third-party pricing
...
**Measured / found:** Official pricing pages read 2026-08-15: TradeZella $315/yr base, Edgewonk $197/yr, TradesViz â¹12,600/yr (their own â¹), OneTradeJournal â¹1,999â2,499/yr (two figures on their own page), TradeDiary â¹999/yr. TraderSync (HTTP 403) and Tradervue (404) pricing came from agreeing third-party 2026 reviews â flagged â  on every surface. No competitor page states an independent Indian statutory-charges engine; none advertises local-only storage.
**Decision:** Data lives in `lib/domain/pricing-comparison.ts` (pure, zero imports) with `COMPARISON_AS_OF = "2026-08-15"`; unverifiable cells say "Not stated", never a guess; "why Vyuha" rows are architecture/arithmetic only â no outcome claims (SEBI posture).
**Why not the obvious thing:** Omitting the â¹999â2,499 Indian competitors would make the table look better and be exactly the selective honesty the product positions against; a buyer who googles finds them anyway.
**Invalidated if:** Any competitor repricing (recheck the sources before a release that touches pricing copy), or TraderSync/Tradervue pages become fetchable â replace the â  cells with primary figures.

## 2026-08-11 â Royal/Sapphire/Aurora skin triples: measured before written, floors matched to the shipping four

**Context:** v2.99.70 adds three "more vibrant, luxurious" accent skins next to Luxe/Mono/Ice/Tape.
**Measured / found:** (WCAG contrast on the real canvases #05080f dark / #f4f6f9 light; circular hue separation; script â contrast + `colorsys` HLS hue.)
- royal `#a78bfa/#e5b13d/#22d3ee` dark 7.36/10.20/11.09, light `#6d28d9/#8f6207/#0e7490` 6.56/4.95/4.95; min role-sep 67Â°/70Â°; primary 89â97Â° from P&L.
- sapphire `#7196ff/#e5b13d/#e879f9` dark 7.16/10.20/8.14, light `#1d4ed8/#8f6207/#a21caf` 6.19/4.95/5.84; min role-sep 68Â°/70Â°; primary 67Â° from P&L.
- aurora `#e879f9/#e5b13d/#2dd4bf` dark 8.14/10.20/10.76, light `#a21caf/#8f6207/#0b7a70` 5.84/4.95/4.81; min role-sep 105â109Â° (widest of any skin shipped); primary 57â60Â° from P&L.
**Decision:** ship all three. Money stays GOLD in all three (no skin-tape-style money move needed â no primary lands near 41Â°). Worst case anywhere is aurora's light analytics at 4.81:1 â exactly equal to the recorded worst the first four skins already ship (Tape light), so the floor did not move. Sapphire's dark primary 7.16:1 is marginally under Royal's 7.36 benchmark and accepted: it is body-text-large/UI accent usage, > 7:1.
**Why not the obvious thing:** picking Tailwind palette hexes by eye â that's how skins drift under the 4.5:1 light-theme floor and land primaries 4Â° from gold (the measured reason Tape's money moved to violet).
**Invalidated if:** the canvas colours change, or `--color-profit`/`--color-loss` hues move (157Â°/352Â° today).

## 2026-08-11 â Metallic gold retune: #e5b13d â #f0b429 triple, gradient text from TOKENS

**Context:** owner asked for the money gold to look "metallic and vibrant" (v2.99.70).
**Measured / found:** #f0b429 base (10.75:1 on #05080f, hue 41.9Â° vs the old 41.4Â°, sat 76â87%), #ffd863 highlight (14.55:1), #cf8d12 shadow (7.13:1) â all above the old values' contrast, hue essentially unchanged so the 41Â° gold doctrine holds. Light theme untouched: #8f6207/#966808/#6f4b05 are already the lightest AA-clearing golds at this hue (documented at the light block).
**Decision:** retune the three dark gold tokens + `--color-warning`; resurrect the dead `text-grad-gold` utility as a vertical highlightâbaseâshadow gradient built from `var(--color-gold-*)` with a `drop-shadow` glint, applied to the 8 KPI-scale money values.
**Why not the obvious thing:** literal gradient stops (what the utility had) would paint GOLD money on the Tape skin, whose whole design is that money moved to violet â tokens make the metal follow each skin's money colour for free. And `box-shadow` can't glint clipped text; `drop-shadow` follows the glyph alpha.
...
compiler was on â but `next.config.ts` never enabled it. This was the
...
- `reactCompiler: true` (top-level Next 16 key) compiles and passes the full
  ...
  boundaries. Server rendered `" of realised P&L sitsâ¦"` (leading space),
  client rendered `"of realised P&L sitsâ¦"` â a one-character disagreement
  that throws "Hydration failed" and REGENERATES THE ENTIRE CLIENT TREE on
  every visit to an affected page.
- **Bisect:** same route, same DB â 3 hydration errors with the flag on,
...
- Fixing the first site (dashboard equity-curve note, moving the space into an
  ...
  (calendar-heatmap's "and cannot appear on any dayâ¦"). The pattern â
  an inline element followed by newline-indented text â is everywhere in this
...
- The three compiler-sensitive surfaces themselves (TanStack sorting under
  ...
  preview) all PASSED under the compiler â the codebase's effect discipline
...
dashboard-client string-expression form â inert now, correct later.
...
normalisation â retry by flipping the flag and running
`z-compiler-protocol.spec.ts` plus a hydration-error grep of a dashboard
visit's server log (the exact procedure above).

---

## 2026-08-10 â /trades at scale: slim projection + row virtualization, with numbers
...
that is ~16 MB per navigation plus ~500k DOM nodes â unusable.

**Measured / found:**
- Slim projection (`lib/domain/slim-trade.ts`, 43 of 74 columns â the union
  the client tree actually reads): **907 B/row, a 44.4% cut** â ~8.7 MB at
  10k rows (`scripts/measure-slim.mjs`, real data). The dialogs needed NO
  fetch-on-open â `notes`/`ruleViolations` stay in the projection.
...
- Composite index `(account_id, sell_date DESC, created_at DESC)`:
  ...
  trades_account_sell_created_idx` â no temp B-tree sort â for the query ~25
...
- The e2e contract change that follows from virtualization: row counts in
  specs must come from the "N of M" counter, never `tbody tr` counts â
...
was rejected â it breaks the counts-reconcile contract and moves the
derive-don't-sync filter architecture into SQL. Client virtualization alone
was rejected â it leaves the 16 MB flight payload untouched.

**Invalidated if:** a column starts reading a dropped field (tsc breaks â add
...
## 2026-08-10 â NSE surveillance files: two formats verified from real downloads; one file covers ASM+GSM+ESM
...
- **ASM/GSM/ESM:** the consolidated Surveillance Indicator file
  ...
  SIX-digit date, and `/content/cm/`, not `/content/equities/` â both probed,
  only cm answers). ~2,970 rows, one per listed security; columns include
  `Symbol`, `GSM`, `Long_Term_â¦ (Long Term ASM)`, `Short_Term_â¦ (Short Term
...
- BSE publishes its lists as notices/web tables â no machine-readable file
...
- One REG_IND upload replaces categories gsm+asm+esm; the ban file replaces
  fno_ban only â `replaceRestrictionCategories` deletes per-category, because
...
**Invalidated if:** NSE renames the REG_IND columns or moves the files â
...
## 2026-08-10 â Index derivative market lots, verified for the January 2026 series

**Context:** the Trade Calculator's new "Underlying index" picker bundles a
lot-size snapshot (`lib/domain/index-contracts.ts`).

**Measured / found:** every NSE index lot CHANGED for the January 2026 series â
NIFTY 75â**65**, BANKNIFTY 35â**30**, FINNIFTY 65â**60**, MIDCPNIFTY
140â**120** (NSE circular **FAOP70616**, effective with contracts expiring
January 2026 onward; the December 2025 monthly expiry was the last on old
lots). BSE: SENSEX **20** (raised 10â20 during 2025, unchanged in the January
revision), BANKEX **30** (15â30). Cross-checked 2026-08-10 against Zerodha's
support table and Sahi's published 2026 table â three sources agree on all six.
...
**Invalidated if:** a later exchange circular revises any lot â re-verify all
six against the circulars (not memory, not this entry) and update
`index-contracts.ts` + the pinned test + this log in one commit.

---

## 2026-08-10 â Plain `npm install` ALSO corrupts this lockfile; adding a dep needs a hand-merge
...
**Measured / found:** `npm install lightweight-charts` â plain, no flags, with a
fully installed tree to consult â reported *"added 2 packages, removed 27
...
So this breaks `npm ci` on **every** platform, Windows included â not only the
...
`npm ci` + `npm ls esbuild`. Splice into the existing key order â do NOT re-sort
...
## 2026-08-10 â `networkidle` is not hydration: a client-restored setting is not readable right after a reload
...
the table renders the DEFAULT column order. It is not broken â the order is
...
because it stands on its own â storage as the single source of truth, and it
...
shrinks to milliseconds â the poll stays correct either way, just faster.

---

## 2026-08-10 â Column reordering permutes the `columns` ARRAY, not TanStack's `columnOrder`
...
OLD arrangement â the frozen pair would take its `left` offsets from whichever
...
positionally â in a table that did not, it would be the right answer.
...
exactly one eligible column receives it, so the multiset of contributions â and
therefore the sum â is identical for every arrangement, pinned or not. It stops
...
## 2026-08-10 â A drag grip inside a `<th>` silently renames the column for screen readers

**Context:** The reorder grip is a real `<button aria-label="Reorder â¦ column">`
...
header is a plain string â for a rendered header the visible text is not known
...
## 2026-08-10 â Only the trade replay moves to lightweight-charts; every equity curve stays on recharts
...
- **lightweight-charts renders an INVISIBLE line, silently, if handed a colour
  ...
  missing DOM node â the series just is not drawn. The browser is no help
...
- The library builds **one chart out of 7 stacked `<canvas>` layers** (pane Ã2,
  right price scale Ã2, time scale Ã2, corner Ã1). Counting canvases is
  therefore not a way to count charts: measured 7 canvases / 1 chart instance
  across three unmountâremount cycles and a client-side navigation away and
...
- Adding the dependency with a plain `npm install` (never
  ...
  `vitest â vite` **optional peer** esbuild 0.28.1 and its 26 platform binaries.
  The `grep -c "darwin-\|linux-x64\|linux-arm64" package-lock.json` canary
  moved 202 â 190 for that reason alone. Every top-level platform variant
  (esbuild 0.25.12, `@next/swc-*`, `lightningcss-*`, `@rolldown/binding-*`)
  survived, and the suite is green without the nested copy â this is not the
...
never `color-mix()`. `attributionLogo` is set to `false` â this app is offline,
...
paper â the one output nobody re-checks after a chart refactor.
...
## 2026-08-09 â The desktop build ran every step twice
...
`npm run build && npm run desktop:bundle` â the same two steps. Cost per
...
break CI â tauri-action reads it from the config and silently ignores one
...
## 2026-08-09 â Light-theme gold is #8f6207, not the handoff's #9a6b08

**Context:** Applying the v3 design tokens to light mode.
**Measured / found:** The handoff proposed ~#9a6b08. Against this app's real
light canvas #f4f6f9 that measures **4.33:1 â under the 4.5:1 AA floor**. It
...
## 2026-08-09 â Table row separators need ~1.48:1, not the handoff's 1.12:1
...
structure â they do nothing to separate row 180 from row 181.
**Invalidated if:** The panel background lightens materially, or tables stop
rendering more than ~50 rows.

## 2026-08-12 â A lens group carries its own ids, not a predicate that "should" match
...
`dateRange 2026-08-01â2026-08-31` with basis `either` **also matches it**. The
group would say 1 and the delete would remove 2. `tests/lenses.test.ts` pins
this on the real case.
**Decision:** Month and hand-entered groups carry `{kind:"filter", ids}` â the
...
## 2026-08-12 â Delete writes a snapshot first, and aborts if it cannot
...
trade row **only** â not its legs, not its attachment rows, and the attachment
...
travels inside backups â so restoring a backup would resurrect its own trash.
...
grows.
**Invalidated if:** Attachment volumes make unbounded retention impractical â
at which point the answer is a size report and a prompt, not a silent sweeper.

## 2026-08-12 â Back navigation: an in-app route stack, not `history.length`
...
back-on-drill-downs.
**Measured / found:** The tree is **flat â 40 routes, zero dynamic segments**.
`reports/` and `targets/` have no index page, so a breadcrumb would render
"Reports âº Monthly" where "Reports" is not a page. The only nested route,
`/trades/report`, opens via `window.open(â¦, "_blank")`, where back means
...
performs the navigation. Breadcrumbs rejected. The Alt+â and mouse-button-4
handlers call `preventDefault()`.
**Why not the obvious thing:** `history.length` counts whatever preceded the app
in that tab, is browser-capped and never decreases â it cannot answer "is there
...
hierarchy â breadcrumbs become the better answer at that point.

## 2026-08-12 â Import detection: a broker detector must present evidence, and shape is not evidence

**Context:** A Groww stocks order-history export imported as broker "zerodha" â
...
Console P&L, meanwhile, won only by a filename clamp at 0.30 â its trade table
...
or a verified in-content fingerprint before shape adds anything; unqualified â
0 â the mapper asks. Fingerprints per format live in docs/BROKER_FORMATS.md,
...
distinctive content and no name â at which point that format belongs to the
generic mapper permanently, not to a weaker fingerprint.

## 2026-08-12 â Dhan GTR "73 rows, 0 trades": the import was innocent

**Context:** A GTR batch showed 73 rows / 73 added / 0 skipped while the trades
table showed none of them â rows in, nothing out, silently.
**Measured / found:** The same real GTR file replayed end-to-end (detect â
parse â commit) into a scratch DB: detected at 0.98, parsed to exactly 73
paired positions (92 bill lines pair down â `rowCount` counts positions, not
...
row left standing â the mirror of the "Import record removed" seam the Lenses
page surfaces.
...
shows "92 â 73" with the pairing explained on hover.
...
## 2026-08-12 â Paytm Money gets a parser: the unpublished-format rule, deliberately set aside

**Context:** AGENTS.md forbids inventing a parser for a format nobody has
published â written when Kotak Neo, Paytm Money and Sahi documented their
...
metadata rows 1â4 (`UCC`/`Name`/`PAN Number`/`Period`), header on row 5, one
row per execution WITH a full charge breakdown (Brokerage, ETT, GST, STT,
SEBI, Stamp Duty) â richer than Zerodha's tradebook, which carries no charges.
...
**Decision:** Build `paytm-tradebook.ts` â the rule's reason (unpublished â
...
semantics â reconcile the first live import against a contract note before
trusting the charge figures.

## 2026-08-12 â Broker API research: recorded so it is not re-derived, NOT built
...
**Measured / found (per-broker access instruments â CORRECTED 2026-08-12 in a
...
- **Upstox** â "Analytics Token": 1-year validity, READ-ONLY (cannot place
  orders). **BUT the Portfolio and Trade-P&L endpoints â exactly what a journal
  reads â require a whitelisted STATIC IP** (one primary + one secondary per
...
- **Dhan** â ~~validity configurable 8 hoursâ30 days; TTOP secret for 1-year
  read-only data~~ â access tokens are **24 hours** (renewable via
  ...
  before building â recollection and public docs disagreed once already.
- **Angel One** â SmartAPI is **free**; api_key + clientId + PIN + TOTP secret;
...
- **Groww** â API key + secret + TOTP; daily expiry; automatable;
  **â¹499+tax/month** â the only broker charging for basic access.
- **Zerodha** â ~~implicitly the costly one~~ â the **Personal tier is FREE**
  and covers orders/trades/holdings/portfolio; paid Connect (â¹500/mo) adds only
...
recording concluded Upstox's year-long token was "the correct instrument" â
...
**Invalidated if:** A broker changes its token model or pricing â re-verify
against the broker's own docs before building anything on this table.

## 2026-08-12 â Lenses is HYBRID-gated, and the gate is field omission, not CSS
...
client-side lock would have been decoration â the numbers were already in the
...
free (deleting a bad import is the recovery path from an import bug â gating it
...
"â" = cannot be computed (invariant 6); a Pro chip = computed, not yours yet.
**Why not the obvious thing:** Wrapping the page in ProGate â that gates the
free half and breaks invariant 7. Or blurring client-side â that ships the
numbers and pretends not to. Field omission is the only version that survives
devtools.
**Invalidated if:** `Kpis` gains a field â it lands on NEITHER side until a
human adds it to one allow-list, and `tests/lens-gating.test.ts` pins the split.

## 2026-08-12 â Per-account capital: the write path now lands where the read looks

**Context:** "Compounded +â¹X" while the number on screen never changed.
...
global â compounding in account A marked account B's realised P&L rolled in.
Pinned by a failing-first temp-DB test.
**Decision:** Migration 0044 moves `pnl_rolled_in` onto accounts, back-filling
the legacy global value into the DEFAULT account (single-account installs â
the overwhelming case â are exactly right; multi-account history is genuinely
...
(invariant 9).
**Invalidated if:** capital ever becomes bucket-per-account-per-bucket rows â
re-derive the rolled-in ownership then.

## 2026-08-12 â Backups: introspected coverage, and licence/trial state stays on the machine
...
guard test asserted a COUNT of 26 â structurally unable to notice a missing
...
left untouched rather than wiped â absent means "the backup never claimed to
know", empty means "known empty".
**Invalidated if:** a table is deliberately excluded from backups â it goes on
the test's EXCLUDED list with a written reason, which is the point.

## 2026-08-12 â Integrity sweep (v2.99.77): the account boundary is enforced where the table is touched
...
LIST OF TABLE NAMES â including "positions", a 28-column table nothing had
...
owner that never resolves the account â correctly, because D1's fix had moved
...
that delegates resolution (like the capital route) â declare the DELEGATE as
owner, not the route.

## 2026-08-12 â Exercise STT stays a named constant; futures STT moves to charge_config

**Context:** D18 â `lib/analytics/settlement.ts` hard-coded `exerciseSttPct`
...
for the `future` segment â same statute, same shape. `exerciseSttPct` is NOT:
...
**Decision:** Futures STT now reads from config (any broker's row â statutory
...
advisory â then it earns the column.

## 2026-08-12 â Secrets at rest (v2.99.80): envelope encryption, DPAPI-wrapped on Windows, no new dependency

**Context:** The broker-API roadmap ends with stored TOTP secrets â permanent
...
populating `$args` â the first DPAPI implementation silently fell back to the
...
in `vault.key` beside the DB â DPAPI (CurrentUser) on Windows via PowerShell,
...
plaintext (SQL migrations cannot run crypto â a migration file claiming to
...
restore.
**Why not the obvious thing:** An npm keychain/DPAPI module â this repo's
...
**Invalidated if:** The Tauri shell grows a secrets IPC â then DPAPI/KDF
becomes the fallback and the OS keychain the primary, changing only `wrapDek`.

## 2026-08-12 â Angel One sync (v2.99.90): the first unattended pull, and why it is safe to be

**Context:** The broker-API roadmap said Angel One first â free, TOTP-automatable,
...
trade-book ROW shape is INFERRED from doc examples â mapped defensively with
candidate field names and refuse-don't-coerce, flagged in the pull's own
warnings until a live pull is reconciled once. TOTP is RFC 6238 SHA-1/30s/6 â
...
convention: (1) all four credentials â API key, client code, PIN, TOTP secret â
...
save; (2) the module surface is READ-ONLY â login + trade book and nothing
...
**Why not the obvious thing:** Storing the day's jwt and refreshing it â a
...
contacting a broker is a posture change the user has not asked for â the pull
stays a button.
**Invalidated if:** A live pull shows field names outside the candidate set â
extend the row mapping and move the trade-book shape from INFERRED to VERIFIED
in this entry.

---

## 2026-08-12 â Remote revocation (v2.99.91): the list travels down, nothing travels up â and four "no kill switch" promises were retired
...
`src-tauri/src/lib.rs` runs a `tauri-plugin-updater` check at every launch â
...
only network activity is optional and explicitâ¦ Both can be left off"),
...
test: (1) **pull-only** â the request carries no key id, no machine id, no
account, nothing; the same public file is served to everyone, and which key it
names is decided before the download, not by it; (2) **warn, then lock** â an
...
discovers a withdrawal as a dead screen; (3) **anti-rollback** â the accepted
...
follows); (4) **reversible** â publishing a newer list without the id
un-revokes, without shipping a build; (5) **fails open** â no list, a corrupt
...
**Why not the obvious thing:** A malformed `effectiveFrom` fails CLOSED â the
...
optional would make the feature optional. The copy was corrected instead â all
...
release tagged `revocations` holding one asset â NOT `releases/latest/download/`
...
re-downloads it â the ratchet defends against an OLDER list displacing a newer
one, not against someone with write access to their own disk.
**Invalidated if:** The updater launch check is ever made optional or removed â
...
## 2026-08-12 â The revocation list's own release must be a PRERELEASE, or it steals `releases/latest` and kills auto-update
...
18:09 UTC. Querying `/repos/â¦/releases/latest` immediately after returned
`tag_name: "revocations"`, `assets: [revocations.json]`, **no latest.json** â
...
`https://â¦/releases/latest/download/latest.json`, so that URL began 404ing:
...
byte-identical content throughout â the prerelease flag does not touch it.
**Decision:** `--prerelease` is mandatory on that release and is now in the
publisher script's header and printed instructions, `LICENSE_OPERATIONS.md` Â§4,
...
`gh api repos/â¦/releases/latest --jq .tag_name` must print a `vâ¦` version.
...
`releases/latest/â¦` baked into its binary â the fix would reach nobody who
...
a user would ever see â it is indistinguishable from "no update available".
...
## 2026-08-29 â Perf quick wins: wallpaper compositing, router cache 120s, gzip off, chart mount animation off
...
- **Wallpaper moved from `background-attachment: fixed` on body to a
  ...
  + cover is Chromium's slow path â the background repaints on every scroll
...
- **`experimental.staleTimes.dynamic` 30 â 120** (`next.config.ts`). Safe
  ...
  the client router cache on write â a stale entry can only be one the user
  never wrote through.
...
- **Recharts mount animation off everywhere** (`isAnimationActive={false}`;
  ...
  the 700ms draw-in replayed every visit â main-thread work exactly when the
...
## 2026-08-29 â /reports/tax + /reports/harvest: column projection, on-click ITR export; an added WHERE reorders ties

Perf sweep at 25k trades (data/perf.sqlite, readonly). What was measured:

- **/reports/tax served 4.97 MB, of which 4.79 MB was RSC flight** â 21,540
  ...
  they cannot drift). `JSON.stringify` of those rows is only ~18 ms â the cost
  was React's flight serialisation plus shipping/parsing 4.8 MB per visit.
- **Whole-book `select *` (74 cols) is ~250â290 ms in SQLite alone at 25k
...
- **Pushing the pages' row filters into SQL is NOT safe here**: adding
  `WHERE is_open=â¦`/segment/sell_date changes the query plan and reorders rows
  that tie on (sell_date, created_at) â measured directly: the filtered
  ...
  projections with no new WHERE, and the pages keep their JS filters â
  identical output by construction.
- **The exception is `getDividendLedgerEntries`** (SQL-filtered): its ORDER BY
  ends on the unique `id`, a total order no plan can permute â filtered rows
  and per-company sums proved bit-identical against the JS filter.
- **/corporate-actions was never slow itself** (17â26 ms warm; it reads an
...
## 2026-09-01 â Zerodha's richest export fell to the column mapper because detection read only sheet 0; the new tradewise parser reconciles against Zerodha's own totals exactly
...
**Measured / found:** The trade table lives on sheet 0, the `- Z` charge-head fingerprint on sheet 1; `toMatrix` read `SheetNames[0]` only, so `detectZerodha` scored 0 and the file fell to the generic mapper â proven with the real registry, not reasoned from source. The preamble's "View Zerodha's guideâ¦" line is an in-content broker name. Parsed output vs the workbook's own F&O summary: realized profit Î=0 on both years; charges Î=â¹0.00 (FY25-26) and ââ¹187.31 (FY24-25 â the FILE's own gap, entry-side charges of positions open at FY end; pinned in tests so a change is investigated, not absorbed). Zerodha's per-trade `Turnover` â¡ |Profit| on all 693 rows. The compact NSE symbols (NIFTY2540323750CE) classified as EQUITY before the classifier learned the grammar â every row would have priced at equity rates.
**Decision:** Detection scans every sheet; the tradewise shape claims only once the workbook names the broker or shows the `- Z` heads; rows group per symbol + entry day + exit day (the tradebook's scrip-day unit, so counts match across import routes); the broker's stated charges ride `reportedCharges` as the stored truth; monthly compact symbols get `expiry: null` (the symbol states no day, and the exchange's expiry weekday rules moved twice in 2025 â a computed "last Thursday" is a guess).
**Why not the obvious thing:** one row = one trade (inflates counts ~3Ã: Zerodha splits one order into a row per execution â six 75-lot rows share one entry AND exit second); trusting the engine's computed charges over the file's (the broker cannot be out-argued about money it actually levied).
...
## 2026-09-01 â Turnover is shown on BOTH bases; on the same real book they differ 6.5â8.7Ã and can straddle the audit threshold (owner decision)

**Context:** Vyuha implements ICAI GN 11th-ed. 5.11(b) (differences + option premium); Zerodha's tax report prints differences only.
**Measured / found:** FY24-25: Zerodha tradewise â¹13.94L vs Vyuha ICAI â¹1.21Cr (8.7Ã); FY25-26: â¹1.64L vs â¹10.6L (6.5Ã). Zerodha's own two sheets also disagree with each other (26% / 8.8%). At those magnitudes the two bases can land on opposite sides of the â¹10 Cr audit line.
...
**Why not the obvious thing:** picking one basis makes either the broker's report or Vyuha look broken â the user's CA holds the broker's number, and the honest product puts both in their hand with the question to ask.
...
## 2026-09-01 â Scaling impact was charge-asymmetric: the actual side bore no entry charges while the counterfactual bore the first entry's, crediting every ladder ~one brokerage of phantom "improvement"
...
**Decision:** on a closed ladder every entry tranche is consumed, so the actual side now subtracts ALL entry-leg charges; each scenario bears its own entry + exit costs, and the extra tranches' brokerage counts against scaling â pinned by a fixture where identical economics net exactly the second tranche's contribution.
**Invalidated if:** `summarise()` starts folding entry charges into `realisedNet` (then the page-side subtraction double-counts â the test will catch the sign).

## 2026-09-01 â Nine files substituted invented capital (â¹17L/â¹13L/â¹4L) when settings were empty; every fresh install computed returns, risk heat, utilisation and limit checks on fiction

**Context:** audit A1 generalised â the performance page's `|| 1700000` had eight siblings (trackers, risk cockpit, monthly report, targets, cash ledger openings, pre-trade limits).
**Decision:** capital-unknown is a stated state, chosen per surface: %-metrics render "â" with ONE Settings nudge; risk heat says "unrated" (a â¹1 fallback graded every book fake-dangerous); the pre-trade limits engine gained a fourth status `"skipped"` (reported as "not evaluated", rank 0 â silently dropping the check read as a pass); cash shows flows-only balances with "opening â". A comment-stripping source guard (`tests/capital-fallback-guard.test.ts`) fails the suite if a `?? 1300000`-style fallback returns to any of the nine files.
**Why not the obvious thing:** defaulting to 0 everywhere â it prints "â¹0" (an invented figure), makes `available` negative, and floods limits with false breaches.
...
## 2026-09-01 â Vyuha Intelligence thresholds adopted with the v3.5.0 build (owner-approved plan)
...
**Decision:** fast re-entry after a loss = <15 min (timed trades only, coverage stated); sizing escalation after a loss = >25% jump in median buyValue; rule sample floors â cockpit rules 15 (matching `MIN_SAMPLE`), pair-wise comparisons 10, lens-group rules 10, SL/TSL/win-loss verdicts 20 (matching `behavior.ts`); stop-vs-exit tolerance 0.5% of stop price; near-breakeven margin 0.05 win-rate points; group insights capped at 3 per lens group. Every rule refuses below floor and passes the shared PRESCRIPTIVE_LANGUAGE scan (`tests/intelligence-contract.test.ts`).
**Invalidated if:** real user books show the lines mislabelling behaviour (tune with data, then update this entry â a changed mind is information).

## 2026-09-01 â The pre-publish adversarial audit of the v3.5.0 diff confirmed 10 new bugs (0 of 15 candidate findings refuted); the v3.5.0 tag was superseded by v3.5.1 before anything reached a user

**Context:** v3.5.0 was tagged and its release workflow drafting when a 15-agent audit (5 diff-dimension finders â per-finding adversarial refuters, several running live probes on migrated temp DBs) reported.
**Measured / found:** the three worst â (1) harvest/advance-tax fed PER-SHARE `fmv31Jan2018` into `classifyGain`, which takes TOTAL units (tax-itr.ts scales ÃbuyQty for exactly this reason): a 100-share pre-2018 lot overstated realised LTCG by â¹15,000 in the worked scenario, driving every downstream harvest figure; (2) `seedDatabase` inserted the new 2026-04-01 F&O STT epoch beside a USER-EDITED 1970âopen row (the unique index includes effective_from, so onConflictDoNothing never fires) and `findRates` picks the newest covering epoch â reproduced live: a user's verified â¹0 brokerage silently became the seed's â¹20/order for every trade â¥ the epoch; (3) the taxpnl in-content name check scored ANY /zerodha/i cell in the first 10 rows, so a user's multi-broker CSV with "Zerodha" in a Broker column auto-imported as Zerodha (the 2026-08-12 misclaim class, verified by running the registry). Plus: fully-closed shorts inverting stop-migration widen/tighten, sessions PATCH 404ing outside the lowest-id account, tradewise executions silently assuming buy-first, an n=1 expectancy accusation, cap-unit R wearing the "plan-derived" label, winner exclusions counted in the losers card, and a literal "â%" cell.
**Decision:** all 10 fixed with tests that fail on the reverted code (fault-injection proven for the seeder and sessions fixes); the v3.5.0 DRAFT is never published â v3.5.1 is the release. In-content broker naming is now constrained to preamble-shaped rows (â¤2 non-empty cells) and NEVER qualifies a file without a format fingerprint. Plan-derived R now means the R denominator ties to the recorded stop within 2%. The widened-stop finding needs a â¥5-trade widened wing.
**Why not the obvious thing:** publishing v3.5.0 and patching later â two of the bugs silently rewrite money (the product's one unforgivable class), and the tag was cheap to supersede while nothing had shipped. Trusting the green suite â the suite WAS green; two of its own source-guard tests pinned a buggy line as correct, which is the strongest argument this audit pass stays in the release ritual.
**Invalidated if:** never â but the audit's cost (~4.3M subagent tokens for find+verify+fix across the day) should be re-weighed if a future diff is a tenth this size.

## 2026-09-01 â The blanket non-goals list is RESCINDED: the owner's standing directive is "evolve every day"

**Context:** VYUHA-STATE Â§8.6 froze the product surface ("no more brokers, no more report screens, no cloud AI, no backtesting, no new subsystems") after the 2026-08 consolidation era; the owner, planning v3.6, reversed it explicitly on 2026-09-01.
**Decision:** growth is allowed everywhere â new brokers, screens, capabilities, subsystems â but each expansion is gated, not free: proposed with its cost, owner-approved BEFORE building, invariants respected, landed with a test that reddens on the reverted code, and released only through `npm run verify` gates plus the adversarial diff-audit. The bundled correctness rules survive independently (no parser without a verified real export; SEBI-safe taglines; local-first stays the differentiator â cloud-touching features are per-feature, opt-in, owner-decided). Â§8.6 and docs/SESSION_PROMPT.md rule 3 rewritten the same day.
**Why not the obvious thing:** deleting Â§8.6 outright â the old list was doing double duty as a growth ban AND a correctness fence; only the ban is rescinded, and losing the fence would reintroduce the 2026-08-12 wrong-parser class.
**Invalidated if:** the owner reinstates a freeze, or unreviewed expansion starts landing broken features â then the gate needs teeth, not the wall back.

## 2026-09-02 â v3.6âv3.9 roadmap approved: eleven owner decisions set the "Navigate & Connect / Review & Discipline / Money correctness / Live Desk" slicing
...
**Decision:** (1) 4-bump slicing approved as proposed. (2) Dhan connects via PIN+TOTP stored in the vault (consent screen â Vyuha becomes a second factor), historical backfill lands v3.8 on captured real payloads. (3) Zerodha gets the official request_token+api_secret session exchange; enctoken REJECTED (ToS-fragile, against the honesty positioning). (4) Goals: absolute â¹ AND %-profit targets, per bucket (equity/active/total), optional target date, baseline frozen at creation. (5) Sidebar regroup as proposed; new groups Import, Tax, and **Back Office** (Surveillance, Cash & Ledger, Corporate Actions, Symbol Aliases, Instruments); Arjun's Eye moves to Analytics. (6) Telegram: per-user own bot only, at-user's-own-risk consent, setup popup card, test alert; shared bot rejected (token would ship in the binary â extractable, impersonation risk). (7) Discipline/Review keep the expectancy-gap framing â no literal "profit if avoided" counterfactuals (unobservable without intraday paths, and "avoid" is lint-banned). (8) Trade Review Desk is Pro/lifetime-gated once trial ends. (9) Live Desk approved with DOUBLE consent incl. links to broker data-fee pages (Angel/Upstox free; Dhan â¹499/mo, Zerodha â¹500/mo are broker-side); openalgo-charts engine adoption deferred until it stabilises (repo is days old; its Apache-2.0 licence and OpenAlgo's WS feed are the durable facts). (10) MTF interest recompute ships opt-in with before/after proof, never a silent migration. (11) Groww native adapter deferred (owner delegated; OpenAlgo covers it and the â¹499/mo is Groww's fee either way).
**Why not the obvious thing:** shipping everything as one v3.6 â the money-adjacent changes (direction migration, MTF recompute, challan ledger) need their own audit rigour and before/after proofs; mixing them with UI/connectivity work is how silent money rewrites slip through (the v3.5.0 lesson).
...
## 2026-09-02 â Goals are per-account statements, not mergeable assets: merge DROPS the source's goals, trash keeps no snapshot, and BACKUP_VERSION stays 3

**Context:** v3.6.0 WS2 added `capital_goals` (migration 0052; per-account Ã per-bucket expected-capital targets, â¹ or %-with-frozen-baseline).
**Decision:** three shapes chosen at build time, each documented at its code site: (1) an account merge DROPS the source account's goals rather than summing them â capital (and a target on it) is the user's own statement, and summing two statements fabricates one nobody made (the same rule the capital merge itself follows, DECISIONS 2026-08); the merge preview and result message both say so. (2) Deleted goals are NOT snapshotted to the trash envelope â one trivially-restated row per bucket, same class as panel dismissals. (3) BACKUP_VERSION stays 3: since v3 restore is per-key (absent table = preserve, present = truth), so pre-goals backups restore cleanly into v3.6 and vice versa â bumping to 4 would only make older builds refuse newer files whole; pinned by two roundtrip tests (paise-unscaled roundtrip, absent-key preserve).
**Why not the obvious thing:** summing goals on merge "loses nothing" â it invents a target the user never set, which is invariant-6 fabrication wearing a different hat.
...
## 2026-09-02 â B/f loss lots MOVE on account merge, and a shared vintage keeps the LARGER amount with a note â never the sum, never a silent drop
...
**Decision:** lots are STATEMENTS OF FACT about a demat account's filed ITR history â the opposite of goals (aspirations), so they follow the trades: on merge they MOVE to the target wherever no (incurred_fy, head) row exists there. Where BOTH accounts recorded the same vintage, the two rows are two transcriptions of possibly the SAME filed loss: summing could double-count one return; dropping could lose a genuinely larger remainder. The LARGER amount survives (and the larger non-null originalAmount), the collision is appended to the surviving row's note and audit-logged, and the merge preview names every colliding vintage so the user verifies against the actual return. On purge the lots delete un-snapshotted (goals' rationale: trivially restated from filed ITRs). Pinned by `tests/bf-losses.test.ts` (merge keeps 90k over 40k+90k=130k; move intact; purge gone).
**Why not the obvious thing:** drop-with-warning is simpler but destroys a fact the Act still honours for up to 8 years â a user merging two journal accounts for the same PAN would silently lose set-off they are entitled to; and summing is invariant-6 fabrication (two transcriptions â  two losses).
**Invalidated if:** the app ever models multiple PANs per journal (then a merge across PANs must NOT move lots â set-off does not cross assessees), or lots gain absorption history that a keep-larger overwrite would corrupt.

## 2026-09-02 â The v3.6 perf sweep flagged /lenses over budget; a worktree build of v3.5.1 proved the breach SHIPPED WITH v3.5.0 and v3.6 actually improves it
...
**Measured:** a clean `git worktree` at tag v3.5.1 (npm ci + next build, same perf DB, same harness) sweeps /lenses **1817 ms**, /trades **2383 ms**, and the dashboard `/` **1559 ms** â all three over budget in the SHIPPED release. v3.6 measures 1675 / 2031 and `/` back under budget: faster than v3.5.1 on every flagged route.
**Decision:** no perf work blocks v3.6.0 â it is a net improvement over the shipped state. The /lenses breach is v3.5.0-vintage (its LensEdge/per-group-charge work was never swept; perf:sweep is not in CI, which is exactly how the v3.0.0-era six-route pileup happened too). A /lenses windowing pass is v3.7 backlog, not a v3.6 gate.
**Why not the obvious thing:** trusting the v3.4.0 baselines as "current" â two releases of untracked drift sat between them and today; the sweep-vs-shipped comparison is the honest one.
**Invalidated if:** perf:sweep joins the release ritual/CI (then baselines stay current and this class of archaeology ends â recommended).

## 2026-09-02 â The v3.6.0 pre-release adversarial audit: 6 finders, 22 findings, 0 refuted, all fixed the same day â including a probe-proven tax misstatement a green suite had blessed
...
**Measured / found â the worst five:** (1) b/f-loss seeding double-counted a vintage present in BOTH the editor and the journal (probe: FY 24-25 tax understated â¹10,000) and accepted future-FY typos ("2035-36" absorbed current gains â pruning only looks backward); (2) All-accounts aggregate goal progress overstated (probe: 50% shown where 16.7% honest â pre-baseline profit counted twice, goal-less accounts inflated the numerator); (3) the Dhan PIN+TOTP consent â the release's most dangerous credential â had NO server-side gate while OpenAlgo/Telegram both 403; (4) `shiftDays` mixed local midnight with UTC slicing, making the "30-day" run-rate window 31 days on IST (the machine it ships on) â green in every timezone the fixture tested; (5) "Update connection" silently wiped Dhan/Zerodha auth_json enrollment on a token-only re-save. Plus: a cross-tab Telegram double-send race, chat-id leaking into backups via audit afterJson, an undeletable-token UI state, a Kite exchange that never checked whose session it minted, an empty-settings forged-envelope hole, and a family of privacy-copy sentences v3.6 made false (PRIVACY.md's "there is no fourth thing", README's "nothing ever uploaded by any path").
**Decision:** all 22 fixed in one wave (4 fix agents, disjoint files), each with a test proven red on the reverted code; final gates `npm run verify` EXIT 0 (2,896 tests / 188 files) and e2e 54/54 EXIT 0. Also settled by worktree measurement: the /lenses perf-budget breach predates v3.6 â see the same-day perf entry.
**Why not the obvious thing:** trusting the three green verify gates that preceded the audit â the test-integrity finder confirmed the new suite is honest (no v3.5-class pinned bugs), and STILL two probe-proven money-display bugs and a live timezone bug sat under full green. The audit pass stays in the ritual.
**Invalidated if:** never â but note the one open release gate it could not discharge: Dhan's `generateAccessToken` API-key-mode question is owner-verified live before the connect-once copy ships.

## 2026-09-02 â Dhan PIN+TOTP live-verified end-to-end; two undocumented behaviors found on the owner's account forced the reuse-first token cache
...
**Measured / found:** (1) first attempt failed with `{"message":"Invalid TOTP","status":"error"}` â **in HTTP 200**: Dhan reports auth failures in an error-in-200 envelope the docs never mention (root cause was a bad first TOTP enrollment; Dhan's setup shows the secret as text only behind the "enter the code shown" alternative to the QR â easy to miss, re-enrollment fixed it); (2) after a successful mint + preview ("5 normalized trades"), commit failed with `"Token can be generated once every 2 minutes."` â **Dhan rate-limits generateAccessToken to one mint per 2 minutes**, so the stateless mint-per-pull design broke on the very first preview â commit sequence.
**Decision:** `resolveDhanAccessToken` is now REUSE-FIRST: a stored token whose own `exp` is alive is used without minting; a mint returns `minted:true` and the route persists it encrypted into `broker_connections.access_token` (same vault path as a pasted token; a vault refusal only costs the cache). Paste-only mode still returns the stored token untouched â even unreadable â so Dhan's own 401 hint stays the judge there. The error-in-200 envelope is parsed with per-cause hints (TOTP/PIN/rate-limit). Re-verified live: preview minted + cached, commit reused the cache, **5 trades committed, 0 duplicates** â the flow that failed now passes. Tests pin the new ordering red-on-revert (mint-first calls auth.dhan.co where the test requires silence). The API-key-mode question is settled for practical purposes: verified WITH the toggle active.
**Why not the obvious thing:** stamping `last minted at` and sleeping out the 2-minute window â it leaves preview â commit racing a clock; reusing the 24h token is what the limit is telling clients to do.
...
## 2026-09-02 â v3.7.0 approved: the Review Desk's "researched design" was never written to disk, so the plan re-derives it â and the carry-over lists were wrong
...
**Measured / found:** (1) **The Trade Review Desk research report does not exist anywhere.** "Process Score", "review queue" and "weekly ritual" occur in ZERO files; "Review Desk" occurs in exactly three (`VYUHA-STATE.md`, `DECISIONS.md`, `V360_BUILD_PLAN.md`) plus two Turbopack dev-cache `.sst` binaries that merely echo them. No research folder exists under the workspace and no indexed session transcript carries it. The owner-approved spec was, in full, one roadmap table row plus decisions #7 and #8. (2) **The "7 sibling global-capital reads" list does not exist either** â and the code holds **EIGHT**: `app/equity/page.tsx:75`, `app/active/page.tsx:31`, `app/risk/page.tsx:99-100`, `app/reports/monthly/page.tsx:41`, `app/targets/equity/page.tsx:34`, `lib/queries/ledger.ts:114-115,127-128`, `lib/queries/limits.ts:47-52`, and the one the count missed, the dashboard capital tile `app/page.tsx:28` â which sits three lines from an account-scoped goal badge, so the dashboard renders a per-account goal beside a global capital figure. (3) **Zero OS-notification capability exists**: no `tauri-plugin-notification`, no `@tauri-apps/api` at all, no `notification:*` permission, no `Notification(` call site. (4) `disciplineByWeek` has **no sample floor, no coverage and no refusal** â a 1-trade week scores like a 40-trade week â and falls back to a hard-coded `cap = perTradeCap || 9500`; a second, differently-shaped adherence number already ships (`session-review.ts:53`).
**Decision:** the design is re-derived from surfaces that exist and written down as `docs/V370_BUILD_PLAN.md`, which IS the spec â approved by the owner 2026-09-02 with all nine open questions answered yes: (Q1) the re-derived Review Desk stands as spec; (Q2) the Process Score is five equal-weight components with a floor of 10 closed trades per week and REPLACES the three-component weekly discipline score, so numbers on `/reports/discipline` and the monthly PDF change on upgrade â one weekly number, because two that disagree is the failure mode this log keeps recording; (Q3) `reviewed_at` is backfilled where notes / mistake tags / exit trigger already exist, so an existing book does not wake to a 500-deep queue; (Q4) onboarding keeps capital OPTIONAL, so invariant-6 "â" paths stay live; (Q5) challans follow b/f-lot semantics (MOVE on merge, delete on purge, no trash snapshot); (Q6) the Telegram fallback ships as a durable failure note plus a WebView `Notification` probe, with the Tauri plugin deferred to v3.8 unless the probe fails â no new npm dependency this release; (Q7) the /lenses on-demand member fetch is pre-approved if render-only windowing does not clear 1500 ms; (Q8) `REFUND_POLICY.md` bumps to v3.7.0 and joins the `client-docs-version` guard; (Q9) the perf double sweep becomes a written step of the `release` skill.
**Why not the obvious thing:** building from the roadmap row and the backlog prose as though they were a spec. Every one of the four findings above contradicts something a reasonable reader would have assumed from the docs â the count of capital reads, the existence of a design, the existence of a notification path, and whether a weekly score refuses on thin data. The recon reports are maps; the code was the territory, and the map was wrong in four places.
**Invalidated if:** the missing research report resurfaces and materially disagrees with Â§1 of the v3.7.0 plan (then reconcile before building further), or the owner re-cuts the roadmap.

## 2026-09-02 â The weekly discipline score averaged un-scoreable weeks as ZERO on a printed report: 80 became 32, and the fix is a five-component Process Score that refuses

**Context:** v3.7.0 replaces `disciplineByWeek`'s three plainly-averaged sub-scores with the transparent Process Score (owner Q2). The old module had **no sample floor, no coverage and no refusal** â a 1-trade week scored like a 40-trade week â and fell back to a hard-coded `cap = perTradeCap || 9500` when the user had configured no per-trade limit, i.e. it scored discipline against a number the user never set.
**Measured:** on a five-ISO-week fixture (two weeks of 10 closed trades scoring 100 and 60; three weeks of 4/3/2 trades that cannot honestly score), the honest average is **80 across 2 scoring weeks of 5**. The shipped v3.6 arithmetic returned **32** â because every refused week entered the mean as a literal 0. That figure appears on `/reports/discipline` AND in the print-optimised monthly report a user shares. The legacy arithmetic is now asserted live inside `tests/discipline-page-guard.test.ts` so the defect is pinned, not merely described.
**Decision:** five equal-weight components (planned Â· risk-cap Â· daily-stop Â· rules-followed Â· reviewed), each carrying `{numerator, denominator, pct, coverage}` so the arithmetic is on screen beside the number; `pct` is `null` â never 0, never invented â when a component has no denominator or when the limit it needs was never configured; the score is the mean of the non-null components; under **10 closed trades in the window** the week returns no score and states why ("4 closed trades this week; the score needs 10"). Refused weeks are EXCLUDED from every average and colour scale, and each average prints its coverage ("across 7 scoring weeks of 12"); with no scoring week the figure is "â", never 0. The â¹9,500 / â¹25,000 fallbacks are deleted and the two pages now pass `null`, so the components refuse instead. One weekly number in the product: `disciplineByWeek` delegates to `processScoreByWeek`, so the monthly report moved with no change of its own.
**Why not the obvious thing:** keeping the three-part score and merely adding a floor. The floor alone does not fix the invented cap, and it does not fix the averaging â a refused week still has to be *excluded*, not scored as zero, and that exclusion has to be visible or the average silently changes meaning. Note the shape of the near-miss: `WeekScore.score` had to stay a bare `number` through Wave 2 because two pages did arithmetic on it, so the wrong figure was deliberately left in place for one wave and handed forward in writing. A deferral that is not written down is how this class of defect ships.
...
## 2026-09-02 â Eight surfaces read GLOBAL capital, and COMPOUNDING made that wrong on single-account books too â not just multi-account ones

**Context:** v3.6 fixed the performance page to resolve capital account-first via `getBucketCapital()`; the backlog said seven siblings remained. Wave 2d verified against code: there are **eight** â the missed one is the dashboard "Total â¹XL" tile (`app/page.tsx`), which sat three lines from an already-account-scoped goal badge, so the dashboard rendered a per-account goal beside a global capital base.
**Measured / found:** the blast radius is wider than "multi-account". `compoundRealised` writes **only the account row**, so the settings row goes stale the moment a single-account user compounds realised P&L â and until now /equity, /active, /risk, /cash, the monthly report, the target tracker, the pre-trade concentration limit and the dashboard tile all showed the **pre-compound** figure while Settings, /reports/performance and the goal badge already showed the compounded one. Red-on-revert on four of the eight was proven by reverting each in turn: two produced wrong-number failures (`expected 111111 to be 500000` on the pre-trade limit base; `expected {equity: 11111100â¦} to deeply equal {equity: 50000000â¦}` on /cash openings), not merely source-guard failures.
**Decision:** all eight resolve through `getBucketCapital()`, which returns rupees and preserves `0 = NOT CONFIGURED`, so every "â", nudge and skip branch is untouched. The helper is extracted to `lib/queries/bucket-capital.ts` importing only settings + accounts, so the pre-trade limit path does not inherit `capital.ts`'s trades/ipos graph. `tests/capital-account-first.test.ts` now proves all eight, positively and negatively, against a migrated DB. `app/page.tsx` and the new helper join `GUARDED_FILES` â the `?? 0` chain now lives in one file, which is the single place a fabricated literal would poison every denominator at once.
**Why not the obvious thing:** trusting the backlog's count of seven. The list had never been written down (the "goals build report" it cited does not exist in the repo), and re-deriving it from `GUARDED_FILES` found the eighth. Also: "All accounts" (id 0) still falls back to the settings row â unchanged, and correct, because the aggregate has no single account's capital.
**Invalidated if:** capital becomes required at onboarding (v3.7 keeps it OPTIONAL â owner Q4 â precisely so the unknown-state paths stay live), or accounts gain a third capital bucket.

## 2026-09-02 â Two latent traps found in passing: the week bucketer emitted SUNDAY east of UTC, and `getWriteAccountId()` does not refuse the aggregate view

**Context:** both were found by v3.7 agents working on something else, and neither was in any backlog.
**Measured / found:** (1) `isoWeek()` (the repo's only ISO-week bucketer, private inside `lib/analytics/discipline.ts`) built the Monday from **local midnight** and then serialised it with `toISOString()` (**UTC**) â so in IST every emitted `weekStart` was the **Sunday**. Bucketing itself was never wrong, only the emitted date, which is exactly why nothing caught it; it becomes load-bearing the moment a date is stored, and `weekly_reviews.week_start` now stores it. This is the same family as the v3.6 audit's `shiftDays` defect that made a 30-day window 31 days on IST. (2) `getWriteAccountId()` resolves a no-selection state by falling back to the **lowest account id** (`lib/queries/accounts.ts`) â the silent guess invariant 9 exists to forbid. **Two agents hit it independently** while wiring unrelated write paths, which is the signal that it is a repo-wide shape, not a local mistake.
**Decision:** the bucketer is extracted to `lib/analytics/week.ts` as the single source, with the Monday emitted in local terms; nine tests redden on the revert. Both new v3.7 write paths (`review.ts`, `challans.ts`) refuse the aggregate view **before** calling the resolver, so the fallback is never reached. **Every pre-existing caller of `getWriteAccountId()` is an open audit lead for the v3.7 adversarial pass** â the resolver is not being changed mid-release, because callers that legitimately pass an explicit id rely on it.
...
**Invalidated if:** the audit finds a caller that reaches the lowest-id fallback in a real flow â then the resolver throws and the callers are fixed in the same change.

## 2026-09-02 â Three source guards passed while guarding nothing; only running the revert exposed them

**Context:** the standing rule is that every fix lands with a test that reddens on the reverted code. v3.7 agents were additionally told to PROVE it by actually reverting, observing the failure, and restoring â rather than asserting the property.
**Measured / found:** three separate guards passed against reverted code. (1) A challan-editor guard matched `router.refresh()` inside the component's **doc comment**, so deleting the real call left it green. (2) A challan ordering test passed with `orderBy` removed, because the `(account_id, fy)` lookup index happened to return rows in ascending order â the assertion was true by accident of the query plan. (3) A route-union test could not fail, because the query layer beneath refused the same bad input, so the union under test was never the thing being exercised.
**Decision:** all three were repaired and re-proven â comment-stripping before matching (the `capital-fallback-guard.test.ts` stripper is the house tool for this), a fixture whose insertion order contradicts the desired order, and a case pinning what the union ALONE prevents. The wider rule, now demonstrated three times in one release: **a green guard is evidence of nothing until the revert has been run against it.** Prefer fixtures whose natural order contradicts the assertion, and strip comments in every source guard.
**Why not the obvious thing:** trusting "the test exists and passes". Two of these three would have shipped as decorative tests that a future reader would reasonably believe was coverage â which is worse than no test, because it stops the next person looking.
**Invalidated if:** nothing; this is a permanent lesson about source guards in this repo.

## 2026-09-02 â /lenses was never a rendering problem: 99.5% of its cost was a 9.3 MB props payload, and the plan's Stage A was worth ~0 ms
...
**Measured (before):** the `trades` prop serialised to **9,343 KB â 99.5% of the payload** (`rows` was 47 KB); the server loop cost **381 ms**, of which `runRules` was **214 ms**, because the page computed KPIs, charge heads and Pro insights for **all six lenses on every request** rather than the visible one; the whole book crossed the RSC wire because grouping was recomputed client-side. Meanwhile the things Stage A targeted were nothing on a real book: the worst lens renders **24 groups** against a `WINDOW_STEP` of 150, `getImportBatches()` returned **0 rows**, and client re-grouping cost **8 ms**.
**Decision:** Stage A shipped (the group-list window is correct for a long book even though it is worth nothing on this one) but the fix is **Stage B**, pre-approved by the owner as Q7: the page ships the server's own grouped output, and drill-down members are fetched on click from `app/api/lenses/members` using the SAME projection and the SAME order. `getLensChargeRows()`'s second whole-book scan left the page entirely. **After: payload 9,390 â 565 KB (â94%), server loop 381 â 186 ms, and the swept median 1718/1557 â 920/901 ms against a 1500 ms budget. Breaches 2 â 1.** The residual 565 KB is deliberate â the filter-scope id arrays keep the delete scope fixed at page render, which `delete-scope.ts` requires.
**Why not the obvious thing:** doing what the plan said. Stage A was derived from a recon report, and the recon report was a map â on the actual book its three targets summed to roughly zero while the real cost sat in a prop nobody had weighed. **Measure the specific page before applying a pattern that worked on a different one.** Output-neutrality was proved group-by-group against a real migrated DB: for every group of all six lenses the route's members equal the page path's ids *in order*, with identical charge heads and the same 19-column shape.
**Invalidated if:** a book has enough groups that the group-list window starts mattering (then Stage A's value appears and this entry's "worth ~0 ms" becomes book-specific, as stated), or `/trades` gets its server pagination â which is NOT output-neutral and stays its own change with its own proof.

## 2026-09-02 â The Telegram OS-notification fallback ships as a PROBE labelled INFERRED, and the failure note moved to storage rather than a column

**Context:** v3.6 deliberately did not build the "local OS notification" half of the Telegram degrade path, and no shipped copy may claim it. v3.7 Â§5.3 carried it forward. Recon confirmed the app has **zero** notification capability: no `tauri-plugin-notification`, no `@tauri-apps/api` dependency at all, no `notification:*` permission, no `Notification(` call site anywhere.
**Measured / found:** a second, larger gap sat underneath it â **nothing durable recorded that a digest send had failed.** The job deliberately reverts its once-a-day claim on failure so the next launch retries, and the note lived in ephemeral React state on a component mounted **only on the dashboard**, so a refresh cleared it and a user who landed anywhere else never learned.
**Decision:** (1) the failure record is now a versioned `{v:1,â¦}` storage envelope written by the runner and rendered from the root layout on every route, cleared on the next confirmed send. **This is a deviation from the plan, which preferred a settings machine-state column** â migrations are serialized through one agent and that wave was closed, and reopening it for a UI nudge was not worth the schema risk. The honest cost is documented in the module header: it is per-device, not per-database. On the desktop product the app *is* the device, so per-device â per-install here; promoting it to a column is a v3.8 candidate. (2) The OS notification ships as a **browser `Notification` probe only** â feature-detected, opt-in per device on the existing breach-notify pattern, firing only on a digest failure â with **no npm or Cargo dependency added** (a test asserts that). It is labelled **INFERRED, not VERIFIED**, because proving it needs a built installer; the copy says Vyuha *asks* the system and that the on-screen strip remains the record, and a test greps that copy for over-claiming phrases.
**Why not the obvious thing:** adding `tauri-plugin-notification`. It is two new npm packages plus Cargo.toml, Cargo.lock and a capability file â and on this dependency graph any npm-driven lock rewrite deterministically prunes vitest's nested esbuild and breaks `npm ci` on every platform. Paying that in a release whose audit budget is already spent, for a nudge, is the wrong trade. Note the guard gap it exposes: `tests/egress-guard.test.ts` scans network constructs only and does not read `src-tauri/capabilities/*.json`, `Cargo.toml` or `package.json` â **nothing in the suite would review a new Tauri plugin's permission surface.**
...
## 2026-09-02 â Four v3.7 schema decisions that cannot be retrofitted, recorded before they are load-bearing
...
**Decision:** (1) **`trades.reviewed_at` is backfilled** to `COALESCE(updated_at, created_at)` for rows already carrying review evidence â non-blank notes, non-blank exit trigger, or a non-empty mistake-tag array â so an existing journalled book does not wake to a 500-deep review queue. Blank still means UNREVIEWED and no analytic may bucket blanks as reviewed. (2) **`settings.onboarding_completed_at` is MACHINE STATE**: excluded from `BASELINE_SETTINGS_FIELDS`, listed in `SETTINGS_MACHINE_COLUMNS`, backfilled to now wherever any trade exists, stamped by the dev/e2e seed and left NULL under `VYUHA_SEED_CLEAN=1`. A restored backup must neither re-show nor re-hide the wizard â the same reasoning `trialStartedAt` gets. The e2e seed stamp is load-bearing: the shared e2e database has zero trades, so an unstamped flag would put a blocking modal in front of specs that dismiss nothing. (3) **`weekly_reviews`** is unique on `(account_id, week_start)`; on merge it MOVES where the target has no row for that week, and where the target does have one the target's row wins and the source's note is APPENDED â a user's own prose is never silently dropped â and it IS snapshotted to trash, because it is prose the user wrote. (4) **`advance_tax_challans` has NO unique key on purpose**: a challan serial is unique only per BSR code and both are optional on a self-assessment receipt, so two genuine payments of the same amount on the same day are legal; the editor warns on an exact `(fy, paid_on, amount)` match and still allows it. Challans are STATEMENTS OF FACT about a real bank payment (the b/f-loss-lot class), so they MOVE on merge and delete on purge, and are NOT snapshotted.
**Why not the obvious thing:** giving challans a unique key "for safety". It would refuse a legal duplicate, and there is no natural key to fall back on â which is also why the editor's row actions are labelled by POSITION rather than identity.
**Invalidated if:** a real user proves the duplicate warning is noise, or onboarding is made mandatory (which would kill the capital-unknown paths â see the 2026-09-01 invented-capital entry's own invalidation clause).

## 2026-09-02 â The v3.7.0 adversarial audit: 27 findings under a fully green suite, and FOUR of them were classes rather than one-offs

**Context:** the release ritual's diff-audit, run on the complete v3.7.0 working tree (102 paths, 63 files, +2527/â338) **after** five consecutive `npm run verify` gates had passed at exit 0 with e2e 70/70. Six finders â money, schema/migrations, security/gating/consent, UI/regressions, test-integrity, docs/claims â each required to execute probes on throwaway migrated databases rather than reason, and to **refute its own findings before reporting**. Precedent: 22 findings in v3.6, 10 in v3.5 (which superseded the v3.5.0 tag).
**Measured:** **27 distinct findings** (one â the audit-log asymmetry â found independently by two finders from opposite directions, which is the strongest signal this pass produces). One finding was **downgraded by the finder that raised it** after it proved the exposure unreachable, and that is a result worth as much as a confirmed bug. Every finding was fixed the same day by five fix agents on disjoint file sets, each fix landed with a test **proven red by actually reverting the code and observing the failure**. Closing gates: verify EXIT 0 at **210 files / 3,417 tests** (from 2,899 at the v3.6.0 cut), e2e **70/70**, final double sweep with **one** budget breach (`/trades`, unchanged and deliberately out of scope).

**The four CLASSES, which matter more than the individual bugs:**

1. **`recordAudit`'s API makes absence indistinguishable from null â four call sites in one feature, wrong three different ways.** It takes `before`/`after` as two independent objects and diffs their **union**, normalising a missing key to `null`. So the log rendered `reviewedAt: <timestamp> â null` for a clearing the code explicitly prevents; rendered *nothing* for the stamp that actually landed; rendered a phantom `null â []` where the row holds NULL; and rendered an audit row with **zero diff rows** for `markReviewed`, i.e. a mutation invisible in the viewer. These falsified three shipped claims â README's "immutable audit log", `docs/client/README.md`'s "keeps the full record" (which ships in the buyer ZIP), and the in-app help's "append-only history of **every mutation**". Four sites getting it wrong in three distinct ways is an API defect, not four careless authors. **Fixed by computing each written value ONCE and using the same binding for both the write and the snapshot.** v3.8: make `recordAudit` flag or refuse a key-set disagreement (throw in dev/test, log in production â `lib/audit.ts` swallows errors by design so logging never breaks the mutation it describes, and that must not change). Typing the pair alone does NOT close it: it misses the `[]`-vs-`null` value half.

2. **`getWriteAccountId()` does not refuse the aggregate view, and SEVEN pre-existing callers reached it.** With no account selected it falls back to the **lowest account id** â the silent guess invariant 9 exists to forbid. Probed live: saving Settings from "All accounts" rewrote account #1's capital snapshot; a â¹50,000 ledger deposit and an IPO filed against account #1 **with a success toast**; importing a broker cash ledger filed the ENTIRE statement on account #1 in one transaction; a session POST did the same; and â found by a fix agent's own probe, not by the audit â a session **PATCH** accepted an `accountId` naming no real account and reviewed a different book's session (the pre-existing test covered only the case where the fallback account does *not* own the row, which 404s, and that is exactly what hid it). All ship in v3.6 today. Fixed by refusing the aggregate view **before** the resolver runs, the house shape from `bf-losses`/`challans`. **The resolver itself was deliberately NOT changed** â that alters behaviour for every caller in a release not auditing them all, and the importer and add-trade form legitimately pass explicit ids. v3.8 candidate.

3. **UTC-versus-local, three times in one release.** The week bucketer emitted the **Sunday** east of UTC; the dashboard review card and the desk it links to named **different weeks** for 5Â½ hours every Monday; and the advance-tax page dated payments on UTC while the ledger accepted them on local, so the card could state "45% paid" and "â¹4,50,000 short now" simultaneously when the truth was â¹0. There are now four hand-rolled copies of the IST literal plus **two exported functions named `todayIso` on different clocks** (`lib/queries/challans.ts` IST, `lib/engine/rates.ts` UTC, seven consumers). The IST one was renamed `todayIstIso` to make the collision unwritable â and the proof is the sharpest artefact of the audit: an agent wrote the mistake exactly as autocomplete would produce it, and **`tsc --noEmit` reported "No errors found"**. The type system cannot see this; only the source guard can. v3.8: one `todayInIst()` in `lib/analytics/week.ts` and migrate the copies. **Deliberately NOT done now** â the three defects are fixed and proven; what remains is duplication, and refactoring five files after the audit has cleared them trades certain regression risk for a hypothetical future bug.

4. **SIX guards passed while guarding nothing â three during the build, three more found by the audit.** Build: one matched `router.refresh()` inside the component's own **doc comment**; one asserted an order supplied by accident by a lookup index; one exercised a route union a lower layer refused first. Audit: **every** week-ordering assertion in the diff was satisfied by insertion order alone (proven by deleting the `.sort()` and watching all three files still pass â while the sort is load-bearing live, because all three production callers feed a **newest-first** query, so removing it renders the Review Desk history and the weekly discipline table backwards); a paywall assertion `insights?.length ?? 0 <= 3` was true even with the key absent, and on a seeded DB the trial branch made the vacuous path the one that executed; and a backfill assertion `r === (u ?? c)` could not fail because the schema's `datetime('now')` has **second** granularity and the fixture was created in the same second, so the reverted code passed too. **Rules, now demonstrated six times: strip comments before matching; build fixtures whose natural order contradicts the assertion; back-date timestamps to a literal; and a green guard is evidence of nothing until the revert has been run against it.**

**Individually severe, beyond the classes:** the review stamp landed on **open** trades (backfill and route both), so a journalled open position was permanently invisible to the desk once closed and counted as reviewed in the score â every fixture in the diff was closed, which is why nothing caught it; and the Sunday-ritual note survived an account switch (unkeyed component + a soft `router.refresh()`), so one book's private prose could be saved against another. The latter is fixed by **deriving** with an owner-scoped draft rather than a `key` or a state-sync effect (which `AGENTS.md` forbids â it silently broke the Trades filter under the React Compiler), and stranded prose now surfaces in a strip naming the book and week it was typed for rather than being discarded.

**Why not the obvious thing:** trusting five green verify runs and 70 green e2e flows. Not one of the 27 findings was visible to them. The audit's own cost discipline also held: the finders' *clean* results carried real weight â an 840-case field-by-field sweep proving the new dated advance-tax path leaves the v3.6 scalar path **byte-identical (47,400 fields, 0 differences)**, a group-by-group proof that the /lenses payload move changed no figure, and live merge/purge runs proving no user prose is lost.
**Invalidated if:** any class-2 or class-4 item recurs after its v3.8 fix lands â that would mean the guard, not the bug, was mis-specified.

## 2026-09-02 â v3.8 candidates banked by the v3.7 audit, with the evidence that earns each one
...
**Decision â the list, in priority order:**
1. **`recordAudit` key-set enforcement** + the single-binding convention (class 1 above). Four instances in one feature.
2. **`getWriteAccountId()` tightening** â make it refuse 0 and fix the explicit-id callers (class 2). Seven reachable callers found; all now guarded at the call site, none at the resolver.
...
10. **`perf:sweep` is still not in CI** â the reason baselines rot (owner Q9 put the double sweep in the release ritual, which is the cheaper half of the fix).
...
## 2026-09-03 â A Paytm tradebook misimported because Paytm SWITCHES its Script column from ticker to code mid-window, and the parser keys on the raw cell instead of ISIN

**Context:** the owner's live book showed 905 trades / 175 open and "83 sales with no purchase on record â â¹4,83,88,311 of proceeds". He asked why intraday trades were being treated as unpriced sales. The tradebook was assumed to be Dhan (his API connections are Dhan) and its `Trade Time` assumed zero everywhere â both premises were wrong and were dropped by the investigating agent.
**Measured:** `rankParsers` on the real bytes: `paytm-tradebook` 0.75, everything else 0 â no parser is missing. `Trade Time` is a real clock on 5,688 of 7,544 rows; empty only on the 1,856 rows whose `Script` is numeric. Order numbers are never read by pairing. **The mechanism:** by month, numeric-vs-ticker `Script` is Apr 270/1328, May 145/3467, Jun 558/373, **Jul 727/0, Aug 676/0**. `paytm-tradebook.ts` groups fills by the raw `Script` (`groupKey`, lines 167/257/272) and carries ISIN without keying on it â **35 ISINs split into two positions, 17 with buys under the ticker and sells under the code** â phantom open buy + phantom unpriced sale (e.g. `INE0JR301013`: open buy 4,400 @ â¹11.4 L labelled MADHAVBAUG *and* three unpriced sales of 4,400 labelled `208578`; a real ~â¹2.7 L loss reported as neither). Temp-DB reproduction from the file: 804 positions, 72 unpriced (â¹4.46 Cr proceeds). **Re-keyed on ISIN, identical pairing: 38 unpriced, â¹2.75 Cr** â the owner confirms the 38 are genuine SME IPO allotments. ISIN+Exchange keying is worse (101). The numeric labels themselves: 162 of 215 are **BSE scrip codes** (exact match to the cached BSE `SCRIP_CD`; Paytm emits BSE codes on NSE rows too), 53 are NSE exchange tokens; all 215 already resolve via ISIN â only the name is missing.
**Invariant 6 held:** an unpriced sale carries `netPnl = âcharges`; the 72 rows contributed ââ¹49,942, none of the on-screen â¹1.6 Cr is costless proceeds. But `acquisition-panel.tsx:151` says the sale "is already counted in your Net P&L" â false; only the charges are.
...
## 2026-09-03 â Same-day equity round trips are NOT automatically intraday: the broker's charge signature decides, and on the real file it split 34 / 49

**Context:** the same parser collapses `mixed` â delivery (`toHint`, line 174) where `dhan-gtr` already uses `splitMixedRow`, so 83 fully-closed same-day round trips (â¹16.23 Cr turnover) were booked as delivery â STCG instead of speculative income. The owner leaned "intraday" but asked for a way to CONFIRM.
**Measured** (rates from the `charge_config` seeds â eq_delivery STT 0.1% both sides / stamp 0.015%; eq_intraday STT 0.025% sell-only / stamp 0.003%): of the 83, **34 carry a mixed signature** (stamp between the two rates; `splitMixedRow`'s algebra recovers the squared-off value within ~0.5% of the FIFO-matched value on 34 of 34 â two independent methods agree) and **49 carry a pure delivery signature** (stamp 0.015% on the buy AND STT = 0.1000% of buy+sell). Zero ambiguous. Paytm's own `Realized P&L Detail` sheet has no product column and lists 287 same-day rows with no speculative flag â the broker's P&L is silent; only the tradebook's charge columns carry the answer. Sub-bug: `corroborate()` divides STT by `buyValue` alone, so 23 of the 49 read "uncorroborated"; the delivery denominator is buy+sell.
**Decision (owner):** **follow the scrip-day charge signature, never the calendar.** The statute's test (`section(fy, "speculative")`) is settlement otherwise than by delivery â a CNC buy and CNC sell the same day took delivery obligations and paid delivery STT on both legs, so it is STCG. Blanket reclassification would have moved 49 correct trades into the wrong head.
...
## 2026-09-03 â The Dhan PIN+TOTP fix was never on the desktop database, and the form that would have enrolled it has an EMPTY Client ID box that looks filled
...
**Found:** the 09-02 verification ran on the **dev server against the repo dev DB** (credentials purged after); the desktop app has its own DB, both rows' `last_pull_at` predate 09-02, and that stamp is written only on a successful commit â so no enrolled pull ever landed there. Unenrolled â stale pasted token â Dhan 401 â the exact string, reproduced with the mint endpoint never called. **Why he could not enrol:** the save gate is `!apiKey || â¦` and `apiKey` is reset to `""` at mount, on broker switch and after every save; `1112â¦â¢â¢â¢â¢` is a masked PLACEHOLDER â the box is empty while looking filled. (An earlier agent claimed the gate required a token; a second agent replicated the gate line by line and refuted that â the token is already optional in TOTP mode.) The v3.6 wipe fix is intact. Nothing renders this component in any test (node-only vitest, no broker-connect e2e); the server-side test passes because the server does accept PIN+TOTP without a token. Also: no retry-on-401, a bare `catch {}` swallows an unparseable `auth_json`, `jwtLooksUnexpired` treats an ms-epoch `exp` as alive, and in All-accounts a save creates a SECOND Dhan row on the picker's default account. The only visible sign of enrolment is a ghost "Remove PIN + TOTP enrollment" button; the checkbox is `useState(false)`.
...
**Invalidated if:** a Dhan API change alters the 401 envelope (the resolver parses it with per-cause hints â re-verify against a real 401).

## 2026-09-03 â The symbol universe needs no scraper: the build script already downloads every listed equity and throws away the name and the BSE code
...
**Measured:** `build-isin-symbols.mjs --fetch` already pulls NSE `EQUITY_L.csv`, NSE Emerge `SME_EQUITY_L.csv` and BSE `ListofScripData` (2,559 + 565 + 4,979 rows cached) and keeps ISINâticker only. Keeping name + BSE code + board + series: **5,672 ISINs, 389 KB raw / 129 KB gzip (+247 KB)**, zero new network. **Two bugs make SME names unreachable today:** `instruments-file.ts:154` matches `NAME OF COMPANY` but the SME file ships `NAME_OF_COMPANY`, and `:158` drops every `ST` series row â TECHNOCRAT (`MARC`, NSE Emerge) cannot be imported at all. Cadence: 2026 YTD â 1.6 listings per trading day of 5,672; a 12 h refresh gains ~1 row over daily, and a journal records the PAST â a security cannot appear in a tradebook before it has listed and traded. An in-app fetch would add `api.bseindia.com` to the egress allow-list and falsify PRIVACY.md's "exactly four kindsâ¦ no fifth thing", which already ships in two client packages; TRADE-SENTINAL's own code shows what NSE's cookie handshake, UA rotation and BSE's silent-zero-without-Referer cost. **TRADE-SENTINAL, read-only, nothing changed:** its `classification-reconciliation-multisource.csv` (2,305 rows with sector/industry) is the only artefact anywhere extending sector beyond the 1,155 index members; its universe is NSE-only, has no rename/delisting handling, and parts are under non-redistributable licences.
...
**Why not the obvious thing:** 12 h is a scanner's cadence, not a journal's; and the BSE-code index must be keyed on the CODE, never the ticker â FOCUS, HSIL and KALYANI are different companies on the two boards.
**Invalidated if:** the product grows a live component (v4.0 Live Desk) â then the cadence question returns with a consent gate.

## 2026-09-03 â Pre-open fills already pair correctly; they are only MISNAMED as off-hours

**Measured:** pairing carries `date` only â no time field â so a 09:07 buy sold at 11:00 nets and a carried one stays open; verified on 133 real pre-open fills (5 same-day closed, 10 carried, 1 open, nothing mis-grouped). But `SESSIONS[0]` starts 09:15, so `sessionOf("09:07") = null` and `timeEdge`/`exitClock` route pre-open to `offHours` â 15 real trades that read like a parse error. 13 of 21 pre-open scrip-days are a scrip's first day (the IPO listing call). `session-review.ts:49` treats a null `entryTime` as "not after cutoff" instead of reporting coverage. No broker file flags pre-open; no fixture carries a 09:00â09:08 stamp. `docs/BROKER_FORMATS.md` still says Paytm's Trade Time is empty on every row â true of the old 414-row export, false of the 7,544-row one.
**Decision (owner, v3.8 WS5):** a sixth `SESSIONS` band `preopen` 09:00â09:15 naming the FILL (a pre-open order that fills at 09:15:00 is indistinguishable from a regular fill â say so); fix the session-review leak; fix the doc.
**Invalidated if:** NSE changes the pre-open window.

## 2026-09-03 â Import/Trades are engine-tested, not shape-tested; the harness that would have caught this week's bug is a table of real files with exact closed/open/opening-sell counts
...
## 2026-09-03 â Search v1 design decisions, and the four traps a naive build would fall into
...
**Decision (owner):** FTS5 external-content table over trades synced by trigger, **returning ids only** so the page keeps its own ORDER BY (no tie-order change â a source guard asserts search never adds an ORDER BY to a trades query); in-memory search over the enriched symbol snapshot and the small tables; per-source account scoping with its own guard; **gated destinations shown with a lock and one line on what unlocks them, never hidden; a user's own trade rows never locked**; both backs â a search-session stack AND "back to where I was", neither via `router.back()`; the deep-link contract repaired first and the query string kept; rapid-click rigour proven as a load test (N ranked queries against 25k trades with `growthRatio()`) plus a Playwright stress spec of a few hundred cycles, not thousands in the browser. Floating draggable assistant â v3.9 on Radix Popover with a 2-D drag hook, no new dependency.
...
## 2026-09-03 â v3.8/v3.9 shape approved; Live Desk â v4.0; twelve decisions taken via pop-up; and how the owner wants to be asked

**Decision:** v3.8 "Trust the import" (Paytm ISIN/signature/dedup + banner copy; golden-book harness + KPI link + CI load; Dhan connect full set + Ledger registered + Realised P&L parser; enriched symbol snapshot + SME importer + Sentinel sector CSV; pre-open band; installer pre-uninstall warning + raw DB copy to Documents; deep-link repair + Search v1; all three banked audit items). v3.9 "Trust the numbers" (broker-truth reconciliation screen + Paytm Realized P&L sheet-2 parser; floating assistant; `/trades` server pagination with its own before/after proof; Dhan MTF Report + Contract Note; short-sell + cross-exchange modelling). Live Desk â v4.0. macOS stays unsold (real `.dmg`s exist on every release, unnotarised, never run); the owner tests one when a Mac is available. Next session builds v3.8 from `docs/V380_BUILD_PLAN.md` directly, re-verifying each claim first.
**Process:** the owner asked to be asked in **batches via the pop-up, options offered, my recommendation FIRST and marked** â recorded as a standing preference. All twelve v3.8 decisions were taken that way; all twelve chose the recommended option.
**Licence recovery:** the owner's lifetime key vanished when the installer's "delete app data" box was ticked mid-upgrade; every issued key is stored IN FULL in `license-ledger.jsonl` (`scripts/license-list.mjs <keyId> --full`), so it was copied back, not re-minted, and nothing was revoked. `VYUHA_KEY_ARCHIVE_DIR` was unset when the keys were minted â set it. The backup envelope would not have saved it (it blanks `licenseKey`). The installer option renders at uninstall AND mid-upgrade, names neither the journal nor the key, and three docs claim uninstall never deletes data â v3.8 WS6.
**Invalidated if:** the owner re-cuts the slicing.

<!-- First entry goes here. -->

## 2026-09-04 â v3.8 recon corrections and eight owner rulings before the first line of code

**Context:** seven read-only agents verified `docs/V380_BUILD_PLAN.md` Â§0/Â§1 claim by claim against
the v3.7.1 tree before the build. Most held; the ones that did not are listed in the plan's new Â§4.

**Measured:** the Paytm `dedupHash` migration as specified is near-void â commit already resolves
codesâtickers via ISIN before hashing (`lib/import/commit.ts:316+`), so an unsplit position's hash
...
`classification-reconciliation-multisource.csv`: 2,305 rows â source 1,481 SCREENER_MAPPING+NSE_TAXONOMY,
...
with fixed tokens, â¥3 real rows per distinct case; (2) 0059 re-keys Paytm hashes ISIN-first AND WS1
...
before/after charge comparison on the owner's book (my recommendation was IST-copies-only â overruled);
...
post-migrate runner â the first "data fix" mechanism in the repo; a broker-scoped remove was scoped
...
## 2026-09-04 â Wave 0 baseline, the owner's 29 broker files inventoried, four more rulings
...
Perf sweep #1 on the 25k seed: 43 routes Ã 3 rounds, 129 visits, overall median 949 ms, 0 console
errors; `/trades` 2041/2391/2391 (median/p95/max) is the only breach (pre-existing, v3.7.1 read
2054/2173); `/lenses` 984, `/review` 1135. Detection: Paytm tradebook 0.95, Zerodha 1.00 Ã4, Groww
0.95 Ã2, Upstox 0.95/0.75, Angel Tax P&L 0.95, PDFs 0.90. **Two misclaims of the AGENTS.md class:**
...
marker in the file. Uncovered: Dhan Realised P&L `.xls` (segment summary r7 with 13 charge columns Ã
...
(01-Aprâ28-Aug-2026) in two report types; the Zerodha pair is FY24-25 (632 exits) + FY25-26 (59
...
**Decision (owner, pop-up):** (9) sector work in v3.8 = DATA LAYER ONLY â bundle the 4-level
taxonomy keyed by ISIN with confidence + provenance (2,229 classified rows), sector fallback chain
(user instruments â taxonomy â index map), normalise the casing forks, expose the confidence tier
where sector concentration is shown; analytics â v4.0 Live Desk. (10) parsers in v3.8 beyond Dhan
...
holdings, Upstox/Angel ledgers, Angel P&L statement â v3.9. (11) Dhan golden book: GTR is the book,
...
## 2026-09-04 â v3.8 Wave 1/2a: what the build measured that the plan did not know
...
- **Paytm, owner's 7,544-row book, after ISIN pairing + signature split:** positions 793 (was 804),
  closed 693, open 62, opening sells **38** (= the SME allotments; was 72), proceeds â¹2,75,35,637.20,
  closed net â¹1,61,44,747.01, intraday positions 185, relabelled securities 35, Î£ charges 1,249,096.80
  vs the file's 1,249,096.81 (its own 4-dp component rounding). `inferProduct`'s Â±35% tolerance
  means "mixed" covers only 9â56% delivery â a 60/40 day still reads as plain delivery (left as is).
...
- **0060 FTS:** `content='trades'` would index pre-existing rows as raw JSON on `'rebuild'` while the
  triggers deleted the flattened text â stale tokens. The content table is therefore a VIEW
  ...
  statement and a `BEGINâ¦END` body is one.
...
- **Snapshot:** 5,691 ISINs (NSE 2,568 + SME 568 + BSE-only 2,555; 4,994 BSE codes), tuple rows
  ...
  parallel maps 745 KB); before 142,470 / 49,853. JSON.parse 1.24 â 2.19 ms. `sector-map.json`
  ...
  eight. **MARC â  TECHNOCRAT**: MARC = Marc Technocrats (NSE Emerge, INE0TD401015), TECHNOCRAT =
  Technocrats Plasma Systems (BSE 544877, INE19QK01022) â the plan's "TECHNOCRAT = MARC" was wrong.
  Today's SME list has 118 `ST` rows, not 125. The 5 size indices' absence from `nse-index-map.json`
  has NO recorded reason and no exclusion code â the 08-06 map was built from a 54-file folder.
- **Dhan:** the Realised P&L file never says "Dhan"; the content marker is the legal name "Raise
  Securities Private Limited". Its own summary does not tie to the paisa (Gross â Total â  Net by
  3â14 p). The GTR files (both accounts) detect at 0.98. Angel `Trades_History` names no broker
  anywhere â its fingerprint is the `TradesAndCharges` sheet + `Scrip/Contract` + `IPFT Charges` +
...
## 2026-09-04 â v3.8 Wave 2b: the golden book found the Dhan book empty; the money gates held

**Context:** four agents â cross-cutting WS8 (audit key-set guard, account-0 refusal, one IST today,
...
- **Dhan GTR (the BOOK by owner ruling) parsed both real 2026 exports to ZERO rows** while detecting
  ...
  "confidently empty" import â the class the plan's shape harness exists to catch, and it caught it
...
- Charge leaks the harness pinned: Angel `Trades_History` Î£ per-trade 157.76 vs stated 157.79
  (â¹0.03 in six qty-0 per-order lines); Zerodha Console `reported.charges` null vs stated 3,269.4101;
  Zerodha tax P&L Î£ 3,269.50 vs columns 3,269.41 (FY24-25 â¹0.02). Fixed in the same wave.
- `b7-import-parse-count` load bound (â¤ 8 XLSX decodes per import) broke at 11 once the new
  detectors landed â decode is now memoised once per `ParseContext`.
- **Money gate for the IST migration:** owner's 7,544-execution book â 793 positions, 793/793 rows
  ...
  (can shrink, cannot grow) â Wave 3 work, not done.
...
- Search v1 on the 25k book: FTS p95 15.11 ms / query (max 20.67), in-memory fan-out p95 3.37 ms,
  `growthRatio` 5kâ20k = 3.57. Result href for a trade uses only `symbol|from|to`.
...
- Golden shapes frozen (sourceRows â closed/open/openingSells): Paytm 7,544 â 693/62/38; Zerodha
  tradebook 3,530 â 64/4/11; Zerodha tax P&L FY24-25 632 â 206/0/0, FY25-26 59 â 26/0/0; Groww
  orders 952 â 466/1/16; Dhan P&L a1 â 1011/2/0. Dhan Realised P&L gross ties to its four segment
  rows to â¹0.05; Dhan's two own files disagree by â¹0.93/â¹177.99 and its segment rows do not foot
  by â¹0.14. "691 exits" holds as Î£ sourceRows; the parser books 232 positions (per symbol + entry
  day + exit day) â a design fact, not a defect. Redacted fixtures: 27 files, ~4.5 MB, `.xls` kept
...
**Invalidated if:** a broker changes a date grammar again â the GTR empty-result warning is the
tripwire; a golden row that goes to 0/0/0 is a stop-ship, never a re-pin.

## 2026-09-04 â v3.8 Wave 3 (UI): what the six surfaces settled
...
- **NSIS template facts** (extracted verbatim from `@tauri-apps/cli` 2.11.3, NSIS 3.11): the hooks
  ...
  to uninstall" â hence Cancel = `SetErrorLevel 1` + `Quit`, never `Abort`. `NSIS_HOOK_PREINSTALL`
  runs AFTER the old uninstaller has already executed, so a copy there protects nothing â not
  implemented. **The v3.7.1 â v3.8.0 upgrade still runs the unguarded v3.7.1 uninstaller once;
  the release note must say "leave the box unticked".** Nothing in the suite executes NSIS â the
...
- **Deep links**: `view` accepts every value the trades select offers (8), not only open/closed/all,
  because a URL that cannot mirror "Loss â closed" is a URL that lies. `SlimTrade` carries no
...
- **Palette**: keyword coverage 27/43 â 43/43 by deriving from `HELP_ENTRIES` (drift guard in
...
- **Broker-connect**: Wave 2's `connectionModeLabel` reads three live shapes as "not connected"
  (Upstox analytics token, OpenAlgo API key, Zerodha api-secret-only) â the component wraps it
  (`modeLabelOf`) rather than widening the gate; in All-accounts mode there is NO default pick
  (the second-Dhan-row bug's root), the picker offers an explicit "Pick an accountâ¦" row.
- **Egress guard**: two new same-origin fetches tripped the static-host rule â the remove panel now
  ...
  PRIVACY's "exactly four kinds" is unchanged â both are same-origin.
...
## 2026-09-04 â v3.8 double perf sweep: search and the enriched snapshot moved no route

**Context:** owner rule â Search v1 must not move any swept route; `/trades` stays out of scope.
Sweeps on the 25k seed, prod build, idle machine, 43 routes Ã 3 rounds, 1500 ms median budget.

**Measured:** W0 baseline (v3.7.1 + docs, BUILD `r2dBohbj59BbphovX5n1x`) vs W4 final (`9007c2d`,
BUILD after the hydration fix): overall median 949 â 910 ms; zero console/page errors in the final
sweep (the W3 sweep had React #418 on `/import` Ã3 â a `<p>` wrapping a Badge `<div>`, fixed);
worst deltas `/equity` +39, `/risk` +8 (the sector resolution now runs there; a W3 sweep read +96,
noise), `/data-quality` +6; best `/options-journal` â225, `/arjuns-eye` â168, `/reports/charges`
â147. `/trades` 2041 â 1968 ms median, still the only breach (exit 1 is that breach alone).
...
**Invalidated if:** a later sweep on the same seed moves any route by more than the Â±40 ms this
run showed as noise.

## 2026-09-04 â v3.8 audit fix wave (docs/installer half)
...
`CopyFiles /SILENT`, which reports failure ONLY through the NSIS error flag â a full disk, a
...
`backups\pre-migrate-*.sqlite` snapshots are copied too â the install guide cites them, and
...
it erases the data folder, and either promise the `Documents\Vyuha-backup` copy or â if it is
about the v3.7.1 upgrade â say that the old uninstaller has no backup step. Five evasions that
...
(`lib/backup-format.ts:83`) â the file restores the journal and not the thing the checkbox
...
app, records the entry and warns â a mutation must not lose its trail over a logging defect.
(2) "restorable from Trash" named no screen; every site now says **Backup & Restore â Deleted
...
POPULATED exports (realised P&L against Upstox's own gross â1.05 / net â4.28 / charges 3.23, met
to the paisa; trade report 11 executions â 4 positions, net â271.90 as our own arithmetic since
...
## 2026-09-04 â v3.8 second audit (over the fix wave): what it found, and the sector-map ruling
...
- **The Angel One parser replayed the GTR defects** the first wave had fixed in the GTR parser
  alone â same defect class, second file.
...
- **DH-902 is a permissions error, not an authentication one** â the Dhan mapping sent the user
...
- **The uninstall guard blocked uninstall on a journal-less folder**: the outer gate
  ...
  `/S` included â the app could never be uninstalled. Now gated on `vyuha.sqlite` itself (nothing
  to protect â skip silently) and the `backups\` copy on `backups\*.sqlite`.
- **Three hollow claim/installer pins**: (1) `tests/installer-hooks.test.ts` pinned the SHAPE of
  the failure guard â deleting `${If} $9 != ""` (always stop) or inverting it to `== ""` passed
  ...
  spelling is named.
- **PII in `tests/fixtures/dhan-pnl.csv`** since the initial commit â a fixture that was never
  redacted.
...
**OWNER RULING (pop-up, 2026-09-04) â sector map.** `lib/data/sector-map.json` ships ALL 2,229
rows, each with provenance. The labels are NSE's own taxonomy (industry/sector names as NSE
publishes them); Screener was used ONLY as the identity bridge (name/ISIN â symbol) and
contributes no label of its own. The owner accepts the licence question this leaves open and
ships the full map. **Revisit if a redistribution basis is ever required** â the fallback is to
...
## 2026-09-04 â v3.8 CI lessons: `describe.skipIf` still runs its callback; ripgrep is not on the runners

**Context:** CI on `15d3c4b` (the first fix wave) went red in four jobs while every local gate was
green. **Measured:** (1) `tests/today-clock.test.ts` shelled out to `rg` â absent on ubuntu/windows
runners â the guard failed with ENOENT, not a UTC finding; replaced by an in-process, comment-aware
...
line read the owner-only 7,544-row book â ENOENT on the runner. Guard: `if (!BOOK) return;` as the
...
parser works â assert against the re-import's own "Imported N" figure. (4) An undetectable case,
by construction: a day-first Angel/Dhan numeric-date file whose days are ALL â¤ 12 imports with
every date swapped; only a day > 12 or a month > 12 cell can reveal the grammar â the parsers refuse
when evidence conflicts and say so, and cannot do better. (5) Redacting a fixture in the tree does
not scrub git history; `tests/fixtures/dhan-pnl.csv`'s originals remain in commits â¤ `15d3c4b` of
the PRIVATE repo.
...
## 2026-09-04 â the `load` CI job's first red was the 25 ms floor, not the pairing engine

**Context:** `load` became a required CI job in v3.8. Its first run on `910aa60` failed
`c8-pairing-depth` "many symbols" (bar 6 on a 4Ã growth) while `lib/import/pair-legs.ts` had not
changed since v3.7.1. **Measured:** locally the same tree read 3.08 / 3.51 / 3.82Ã; at n = 24,000
the baseline finished in 24.0 ms â ON `growthRatio`'s 25 ms floor â so the ratio was timer noise on
...
inflates a growth ratio). Doubling the ONE-symbol cases to 192kâ240k legs overflowed the call stack
at `pair-legs.ts:302` (`out.push(...pairSymbolLegs(arr))`) â a spread over one symbol's whole
position list; unreachable for any real book (the perf seed is 25k trades across many symbols) but
a v3.9 cleanup (`forâ¦of push`). Those cases stay at 24k/30k, where their baselines already clear the
floor. After the change: 2.30â3.80Ã (many), 2.92â4.07Ã (one), 3.54â4.23Ã (opening-sell heavy).

**Decision:** load thresholds are measured against the floor before they become CI gates; a red
`load` job is investigated for the floor first, then for the engine â never re-pinned to pass.

**Invalidated if:** the many-symbols baseline drops under 25 ms again on a faster runner (raise n).

## 2026-09-04 â v3.8.0 "Trust the import" cut and tagged: the evidence

**Context:** release skill Â§11 â evidence, not adjectives. Tag `v3.8.0` on `74e8d49`.

**Measured (gates on the tagged tree):** `npm run verify` EXIT 0 â 254 files / 4,515 tests
...
moved; `/trades` 2041 â 1968 ms, still the only breach. Three audit passes: 6 finders over the
diff (12 must-fix), 3 over the fix wave (10 findings), 1 over the second pass (4 should-fix) â
every fix proven red by revert.

**Still owed at tag time:** `npm run release:verify v3.8.0 -- --deep` over the published draft;
install on a non-build machine (owner); publish the draft (owner â `gh release` is denied to the
...
**Invalidated if:** the deep verify fails (then delete the draft and re-run the workflow â never
re-upload by hand).

## 2026-09-04 â v3.8.0 deep signature verify: safe to publish

**Measured:** `npm run release:verify v3.8.0 -- --deep` EXIT 0 â `tauri.conf.json` pubkey id
...
## 2026-09-04 â v3.8.0 PUBLISHED (owner-confirmed)

**Measured:** draft published; installed and launched clean on a non-build machine; WDSI
submission filed with the client-ZIP installer SHA-256 `47A2B542â¦4865`. `releases/latest` is
...
## 2026-09-04 â v3.9 recon corrections, owner rulings, and what Wave 1 measured
...
- Broker `reported` figures were never persisted â `PreviewResult.reconciliation` was the only
...
- Paytm's `Realized P&L Detail` is ONE lot table with a `Total` row (918 lots on the owner's file);
  ...
  the plan predicted â headers are found by text). The detail Î£ equals its Total to the paisa
...
- `trades-client.tsx:92` was already IST (`todayIstIso`, :104); VYUHA-STATE Â§2 and Â§8.0 were stale.
- NSE-buy/BSE-sell split: measured and rejected in v3.8 (38 â 101 opening sells on the 7,544-row
...
- `dp-charges.xls` and Angel `ProfitLoss_Statement` name no broker anywhere. **Ruling:** format
  fingerprint (sheet + header + title) = 0.9, broker in filename +0.1 â the named exception
...
**Covered short â ruling: same-day only.** `chronological()` already sorts buys before sells within
a date, so the arithmetic of a same-day sell-then-buy was already right; what was missing was the
meaning. `shortCoverQtys()` reads FILE ORDER before the sort (the only sequence signal legs carry â
...
venues gets "Bought on NSE, sold on BSE â one holding, the exchange is where the fill happened."
Position `exchange` unchanged.

**`pair-legs.ts:302` spread â `forâ¦of` push.** Proven red first: `RangeError: Maximum call stack size
exceeded at pairLegs (pair-legs.ts:302:39)` at ~123k positions on one symbol (190,000-leg C8 case,
`time(...)` not `growthRatio` â the 25 ms floor and the 4n run would mask the RangeError).
...
- Dhan DP charges: 173 rows / Total 2,492.50 (a2: 94 / 1,325.00), both conserve to the paisa. 352
  merges; the only merge touching the Charges column is the title row â no data cell is lost.
...
- Dhan contract note (PDF, `pdf-parse` in-process): 0.95 named / 1.00 vs `detectPdf`'s flat 0.90;
  requires BOTH "CONTRACT NOTE" and a Dhan marker in the raw bytes. Emits `enrich` rows only â
...
- Upstox `LEDGER_V3`: 4 rows, dd-mm-yyyy proved by day > 12; Î£debit 437.29 / Î£credit 2,500.00 /
  closing 2,062.71 exact. Angel `YourStatement`: running balance chains 0 â 1,417.56 with no break;
  the Charges tables are `chargeRows` + reference, NOT ledger rows â the DP charge is already posted
  in the Broking Ledger on a different date and folding it would debit twice.
- Angel P&L statement: no ISIN and no date column â keyed by symbol, `asOf` null, FY from `To Date`;
  delivery 3.95 + intraday â2.21 = summary 1.74, F&O 149.25, both reconcile.
...
- `redact-broker-export.mjs`'s parity check compared only trades/rows/gross/charges/reported â vacuous
...
**SUPERSEDED 2026-09-05:** credit limits reset, no governor exists â model choice per
...
The W1 gate's load run failed `a7-cash-ledger` at 7.3Ã (ceiling 6Ã). Suspect was 0061's trigram
index inflating the page-cache footprint. Discriminator: three runs at HEAD `30332a3` with every W1
change stashed â 2 of 3 failed (8.4Ã, 8.2Ã); three runs with W1 restored â 1 of 3 failed (7.2Ã).
...
**BIFF8 declares the whole sheet as its used range â every detector paid for 65,536 rows.**
The W1 gate's golden row for `dhan-dp-charges-â¦.xls` hit vitest's 10 s hook timeout under load.
Measured idle: `XLSX.read` 19 ms, but `rankParsers` 3,717 ms â each of ~13 xls-reading detectors
spent 270â440 ms in `sheet_to_json({ defval: "" })` because the sheet's `!ref` is `A1:Q65536` for
1,400 real cells. `workbookOf` now trims every sheet's `!ref` to its populated bounding box once on
the memoised workbook (`trimSheetRanges`, `lib/import/types.ts`): 440 ms â 2 ms per flatten.
...
any Dhan `.xls` (Realised P&L is BIFF too) and any detector added later â the memo is the fix, not
per-parser row caps.

## 2026-09-04 â v3.9 W3 (UI): what the two screens and the e2e run settled

- **Broker Truth (`/reports/reconcile`)** â Pro-gated, nav group Import (no Reports group exists),
  per-FY / per-scrip / per-segment / holdings tables; ISIN-first join, symbol fallback, the match
  key printed per row (the Paytm tickerâcode relabel makes a silent symbol join unsafe). Five
...
- **The Dhan Realised P&L was double-counting the book.** It still emitted per-scrip trades (v3.8,
  ...
  account the F&O segment read exactly 2Ã the broker and dedup skipped 0 (hashes differ by
  ...
  manual trades â neither `trades` nor `import_batches` carries a source id. Golden leg 5 (one
  account per fixture) untouched. **Proof the screen works:** with the double count gone, the F&O
  segment reconciles to â¹0.01 on the a2 pair; equity's â¹665.98 gap carries "1 sale worth â¹21,904 has
...
- `reconcileFrom` had two defects before that: reasons were computed at scrip scope only (segment
  rows â the only rows the Dhan file states â showed an empty Why column), and `FAMILY_OF` used an
  invented segment vocabulary so every F&O trade fell to `null` (Vyuha â¹0). Now `Record<Segment,â¦>`
  so tsc enforces exhaustiveness; an out-of-tolerance line with zero facts prints the facts checked.
- **Floating search assistant** â the first e2e run showed the panel at (0,0) for everyone, not just
...
- Ledger + audit FTS: p95 15.5 ms at 25k rows each (200 queries Ã 2 scopes); trades unchanged.
- e2e first full run: 82 passed, the two new specs failed on real defects above (not on the harness).
- README counts drifted twice in one wave (a fix agent adds a test file after the sync) â the sync
  is the LAST thing before verify, never before fix agents run.

**W3 double perf sweep (25k seed, prod build, idle machine, 44 routes Ã 3, 1,500 ms median
budget): no breach.** Sweep 1: overall median 978 ms, slowest 1,472, `/trades` 1,010 (p95 1,025).
Sweep 2: overall 929, slowest 1,394, `/trades` 954 (p95 1,009). The W2 sweep's +12â27% moves on
`/`, `/settings`, `/review` were machine load, as suspected: on an idle machine they sit inside the
budget. `/trades` is no longer the one route over budget (1,968 â ~1,000).

## 2026-09-04 â v3.9 first audit (6 finders over 30332a3..c04e1ad): what they found
...
- **Covered-short label changed STT on authentic Dhan input.** `shortCoverQtys` started `held = 0`
  and ignored the engine's own pre-file seed lot, so `sell â buy` on one day with an older holding
  was labelled an intraday short and `unknown â intraday` reached `productHint` â sell-side STT
...
with no label; panel close dropped focus to `<body>` (no PopoverTrigger â null triggerRef); help
...
names; zero PII in fixtures and outputs; upgrade 0060â0063 populates FTS for pre-existing rows and
removes the temp B-tree from the plan; keyset paging correct across the NULL sell_date boundary; all
filter values bound, LIKE escaped; SQL/JS parity on 23 filter shapes Ã 4 account scopes on the 25k
...
wave).** Three sweeps read every route ~37% slower than the W3 pair (overall median 929 â 1,278 ms;
...
## 2026-09-04 â v3.9 second audit (3 finders over the fix wave c04e1ad..4d9e9bd)
...
- **CRITICAL: a stated ISIN never disqualified an enrichment match.** `identityMatches` treated the
  ...
  â¥4-char name-prefix rule, which put the Tata Motors DVR's fill time on the ordinary share
  (`TATAMOTORSDVR` â `TATAMOTORS`; `HDFC` â `HDFCBANK`), and `free.find(qty)` took the first by id.
...
- `scripKeysOf` summed two ISINs under a shared ticker for symbol-keyed Angel rows (a â200 delta
...
- `scope: "charge"` rows were written by three parsers and READ BY NOTHING while three shipped
  ...
  FY vs Î£ `trades.dp_charges`; contract-note charges per note date; Angel ledger charge types shown
  as stated, no fabricated Î).
- **The RIVALS veto scanned every cell**, so a Dhan DP-charges sheet holding ANGELONE or PAYTM as a
  SECURITY scored 0 â a real holding would have cost the owner his own import. Veto now reads the
...
- Figures-only trash snapshots reported "0 trades" everywhere (counts, restore title, purge
  ...
  destroying the only copy â both now name the figures. Merge restore asymmetry (discarded duplicates
...
no broker key and handed the whole book's totals to every broker's line: â¹60 Dhan + â¹40 Angel DP
charges stated, the book holding exactly those, printed Î â40 and Î â60 â two fabricated deltas on
...
keyed by broker; a broker with no trades in that FY/date reads "Not compared", never â¹0. Also: the
feed list is seven files now that three sources emit `charge` rows (the docs said five); a
contract-note line on a date with no trades is "Not compared", not "Broker higher" against â¹0.
Checked clean by the same finder: enrichment identity both ways, units (paise â rupees on read),
FY assignment by levy date, veto region documented, empty states, versions still 3.8.0.

## 2026-09-05 â v3.9.0 perf gate: the "uniform regression" was the measuring session itself

Five sweeps of HEAD read ~35% slower than the W3 pair on every route. A same-session control of
the W3 commit (`c04e1ad`, rebuilt in place) read 967 ms overall â so it looked like a real shared-
path regression in `4d9e9bd..fcddb22`. The discriminators said otherwise: server TTFB Ã5 on `/`,
...
overall, slowest `/risk` 1,384 / `/` 1,380, `/trades` 998 / 950 â all 44 routes inside 1,500 ms,
...
## 2026-09-05 â v3.9.0 "Trust the numbers" cut, tagged, deep-verified: the evidence

- Release commit `e5ea549`; CI run 33911278690 **6/6 green** (check, windows-gate, e2e ubuntu,
  e2e macOS, desktop-bundle-macos, load) BEFORE the tag. Tag `v3.9.0` â release run 33911990805,
  3/3 jobs; draft release with latest.json, Windows setup.exe + .sig, both macOS dmg/app.tar.gz + .sig.
- `npm run release:verify v3.9.0 -- --deep` â **3/3 signatures verify over the published bytes**,
  every key id `4FF85F3BBE1DA21D` = the pubkey shipped in the app. Local artefacts: same key id.
- Verify 276 files / 5,208 tests (was 254 / 4,515 at v3.8.0 â up, nothing deleted); load 16/16;
...
- Client ZIP `release-packages/Vyuha_3.9.0_Client_Package.zip` (36.1 MB), installer SHA-256
  `094949764548D46010AB9E51BEB08F9F164758F21E1A3EF1C2FD38F2327443E8` (the local build â the GitHub
...
## 2026-09-05 â v3.9.0 PUBLISHED (owner-confirmed)
...
## 2026-09-05 â The overlay entrance keyframe must not set a transform end state

`@keyframes dialog-in` ended at `transform: translate(-50%, -50%) scale(1)` â the centring idiom,
...
â the standalone `translate` property. Individual `translate` **composes with** `transform`
...
- `ui/popover.tsx` and `ui/tooltip.tsx` â no translate utilities, Radix anchors them. Offset by a
...
- `system/command-palette.tsx` â `-translate-x-1/2` with `top-[12vh]`. Offset a further -50% in X
  and -48% in Y.
- `ui/dialog.tsx` â centred in both axes, so it was displaced to (-100%, -98%) and snapped to
...
receiving **189** â inside the computed 183â190px window. `macos-14` passed both times and the next
...
**Fix, both halves â one alone is not enough.** Fixing only the spec hides a user-visible bug;
fixing only the CSS leaves the next timing-sensitive assertion exposed.

1. `dialog-in` now runs `transform: translateY(6px) scale(0.965)` â `translateY(0) scale(1)`. The
   ...
   **A first attempt used the standalone `scale:` property** to sidestep the composition entirely.
   ...
   back as `0%{opacity:0;transform:translateY(6px)}` â the pop simply gone, no warning, no error,
...
## 2026-09-05 â "100% local & offline" is retired as positioning: Vyuha is sold as "Desktop or Web"

Owner directive: Vyuha is sold as **"Desktop or Web: the trader chooses"** â one platform for a
...
single trade", "offline-first", "local-first", "LOCAL Â· OFFLINE", "offline by design") is removed
or rewritten across README, `docs/sales`, `docs/client`, `docs/index.html`, `src-tauri`, `app/`,
`components/`, `lib/domain` and `scripts/` â 27 files, 64 replacements as built (the proposal
...
**Rejected: option B, deleting the four-kinds list** â it would strip the one verifiable privacy
...
Buyer surfaces may say the web platform is **"in development"** â that phrasing only, never a date
and never "coming soon", so nothing published is false (owner, Q10).
**Rejected: silence until the web platform ships** â it would force the desktop copy to keep
...
`docs/owner/CREATOR_KIT.md:80`'s "â Syncs to cloud / mobile app â local-first is the point; there
is neither" contradicted the roadmap and is rewritten. **Rejected: rewriting all thirteen owner
assets in this release** â none of them is shipped to a buyer by the build, and the tagline is
still open (Q11: the owner rejected both proposed taglines and wants one line, not a set).

`docs/DECISIONS.md:2138-2141` (the 2026-09-01 rescind entry) and `VYUHA-STATE.md` Â§8.6 are left as
history; Â§8.6's closing paragraph is rewritten and marked "Positioning superseded 2026-09-05".
...
## 2026-09-05 â `xlsx` moves to the SheetJS CDN build 0.20.3, vendored as a tarball in the repo
...
**Rejected: `exceljs`** â a different API, so it rewrites all 13 parsers with no fixtures for the
swap; that is a rewrite of the import engine wearing a security patch's clothes.
**Rejected: depending on the CDN URL directly** (`"xlsx": "https://cdn.sheetjs.com/â¦"`) â it puts
...
result **only if the diff is xlsx-only**, otherwise hand-edit per `AGENTS.md` Â§ "Adding a
dependency"; prove it with `npm ci` in a throwaway worktree. This is a deliberate, owner-approved
narrowing of "never let npm rewrite `package-lock.json`" â the rule exists because a plain
...
## 2026-09-05 â The import route caps at 32 MB before it reads, and the parse guard checks magic bytes
...
not an exposure â one trusted user, their own files â but it is a live remote DoS on any hosted
...
- `const MAX_IMPORT_BYTES = 32 * 1024 * 1024;` (`:18`), checked from `file.size` AFTER the
  ...
  `{ error: "â¦" }` with **status 413**. The route's real error shape is `{ error: string }` â the
...
- Magic-byte pre-check in `guardReadable(filename, bytes)` before `XLSX.read`:
  ...
  copy constants. Signatures are written as numeric arrays, never as string escapes â escapes
  written through a shell heredoc land as control characters and a dead check then sits under a
  green gate.
- **`guardReadable` still short-circuits `.csv` and `.pdf` at `:75` â by design, and kept.** CSV
...
- The module's stated design rule survives: junk bytes SheetJS can open at all still pass through
  to the generic column-mapper. **Rejected: turning `guardReadable` into a format whitelist** â it
  would refuse real broker exports whose extension lies, which is the case the generic mapper
  exists to serve.

## 2026-09-05 â Licence activation now checks the machine binding; documenting it was not enough
...
the read path does refuse the key) â an activation that succeeds and then silently yields a
...
locked to a different computer. Send your Machine ID (â¦) to support to have it re-issued." Two
messages for one condition drift apart.

**Unbound keys must keep activating forever** â `lib/license.ts:130` guards with
...
`tests/license.test.ts:10-23` and covers three cases: bound key + foreign machine â refused; bound
key + own machine â activates; unbound key + any machine â activates. Evidence from W3: 103 licence
tests green, red-on-revert **"expected 200 to be 400"**.

## 2026-09-05 â Owner rulings that set the v3.9.1 â v4.2 shape (recorded from 06-ANSWERS)

- **Deadline: Monday 2026-09-07 for everything up to v4.2** â "push till Monday; use your full
...
- **Order: v3.9.1 is tagged first, then v4.0** (Q2), and the rest ships as a ladder of back-to-back
  tags â **4.0 core â 4.1 OpenAlgo â 4.2 broker feeds** (Q4). **Rejected: one large v4.0** carrying
  every feed, which would make the whole release hostage to the broker APIs.
- **Cut order if not green by Monday: Atlas â broker feeds â Lab extras â attribution** (Q3), with
  ...
  justifies it. **Rejected: cutting by whatever happens to be unfinished at the deadline** â it
...
- **Feeds are phased, not simultaneous: EOD-only marks in v4.0, OpenAlgo opt-in in v4.1, broker
  APIs in v4.2** (Q20), all attempted by Monday. **Rejected: live broker quotes in v4.0** â it
...
- **The one-pop-up-per-task cap is OVERRULED** (Q69a): *ask ALL questions via pop-ups BEFORE
  building; groups of â¤10; be dynamic.* The cap in `~/.claude/CLAUDE.md` was written to stop
  mid-build interruptions; the owner's correction is that the questions are not the problem, the
  *timing* is. **Rejected: keeping the single-pop-up cap** â it forced the model to guess at
  product choices it had no basis to decide, then log the guess.

## 2026-09-05 â The credit governor is gone: model choice is an accuracy rule, not a spend rule

Credit limits reset 2026-09-05. The "Governor" paragraph in the 2026-09-04 v3.9 entry above
(weekly all-models 76% / Fable 85%) and the matching line in `VYUHA-STATE.md` Â§2 are marked
**SUPERSEDED 2026-09-05** in place â the originals stay, because they explain why v3.9's wave plan
looks the way it does. The live governor text in `docs/prompts/NEXT_SESSION_V390_V400.md`
Â§ "Budget governor (non-negotiable)" is **deleted**, because unlike the other two it is an
instruction a future session would obey.

What replaces it: model choice per `~/.claude/CLAUDE.md` â Opus builds, Fable orchestrates â is an
...
**Rejected: leaving the governor in place as a conservative default** â a rule that no longer
...
## 2026-09-06 â Price surfaces guard: prose is a price surface too

`â¹9,999` survived on **six** surfaces after the 2026-08-31 reprice (annual â¹9,999 â â¹7,999,
anchor â¹13,000 unchanged, so `offerPct()` derives 38%): `lib/domain/pricing-comparison.ts`'s
...
green throughout, because it pins the app's amounts to the landing page's price CARD â nothing
...
(`lib/domain/pricing.ts`, itself pure and browser-safe â the "zero imports" header is amended,
not broken). In prose the figures are literal and `tests/price-surfaces.test.ts` scans them:
every â¹ figure in a price context on README.md, the landing page (source and generated
...
`COMPETITORS`, and ranges (`â¹47,000â95,000`) and approximate conversions (`â â¹27,700/yr`) are
excluded structurally â neither shape can ever be one of our two exact prices.
...
"no wrong price" is trivially satisfied by a page with no price at all â the exact funnel leak
`lib/domain/pricing.ts` was written to fix.

## 2026-09-06 â All eight NSE size indices ship in the bundled map, and membership is effective-dated from now on (Q46/Q47/Q50)

The 2026-08-06 `lib/data/nse-index-map.json` carried **no size data at all** â every one of its 54
...
build-time script** â the app itself never contacts that host (Q59), and v4.0 adds no network host
of any kind (Q58).
...
400 is ignored outright â it is Midcap 150 + Smallcap 250 restated and defines no band of its own.
...
rounding of ours. The bands ARE the index memberships â SEBI's rank-based definition (top 100
large, 101-250 mid, 251+ small) as NSE publishes it (Q47) â and a symbol that appears in two bands
...
lists carry `captured_at` 2026-09-06 and their own `effective_at` â 2026-09-06 for the four fetched
that day, 2026-08-14 for the four read from the owner's existing copies â while the sectoral
...
**This OVERRIDES `08-BUILD-PROMPTS/V400-LIVE-DESK-BUILD-PROMPT.md` Â§0.2 ruling Q-2** ("do NOT
bundle the 5 size-index lists in v4.0; the market-cap column ships `null` with its reason stated"),
by owner ruling Q46 â and it is eight lists, not five. The prompt's stated reason was that bundling
...
**Rejected: fetching the lists from inside the app**, at first run or on a schedule â it would add
a host PRIVACY does not disclose and make a user's classification depend on the day they installed.
**Also rejected: deriving cap bands from our own share counts** (TRADE-SENTINAL's work) â a second,
...
## 2026-09-06 â The owner's proprietary Atlas widgets ship as a signed results file, never as code (Q42b)

Q42 ruled that every Atlas widget is the owner's own formula: the stock lists MAY ship, the filters
and formulas must stay concealed. **Concealment inside the installer is not achievable** â the
...
published by the owner from Sentinel/Chartink to the **existing GitHub release channel** â the same
host PRIVACY item #1 already discloses for the update check and the revocation list, so no new
egress appears â and **fetched opt-in** by the desktop app, verified exactly the way the revocation
...
**Rejected: computing the concealed formulas inside the installer** â see above; it would publish
...
## 2026-09-06 â Web platform: server-side broker credentials and a server-side feed are Phase 2, not a permanent no (Q61/Q62)

`VYUHA-WEB-PLATFORM-RESEARCH` converged on **"web = file-import only"** â no stored broker
credential, no server-side quote feed â as a standing rule. **The owner reverses it (Q62), and the
...
instead â the irreversibles the pack settled (region, tenancy, per-tenant SQLite, auth) are
untouched.

## 2026-09-06 â The owed macOS DMG test is DONE: two users run the 3.9.0 build (Q65b)

`VYUHA-STATE.md` Â§7.5 has carried an open item since 2026-08-15: the macOS artefacts are built and
...
report it works** (owner, Q65). That is the test â a real install from the published bytes on a
machine that did not build them.

**Standing position (Q65b):** macOS is **supported by request** and those two users are treated as
beta; macOS is **not marketed** â no macOS line on README, the landing page, the brochure, the
client pack or any listing â **until the app is notarised**, because an un-notarised DMG makes the
...
**Rejected: announcing macOS support now on the strength of two working installs** â two installs
prove the artefact runs, not that a stranger who has never spoken to the owner can get it running.
**Also rejected: keeping the DMG test open** â the evidence exists, and an owed item nobody can
close makes the whole checklist read as decorative.

## 2026-09-06 â The daily broker re-authentication sentence carries a VERIFY-CIRCULAR marker until the circular is in hand (Q24)
...
names no broker.
**Rejected: shipping the sentence unmarked** â it would age into an uncitable regulatory claim on a
screen a buyer reads. **Also rejected: dropping the sentence** â then the daily login looks like our
defect, which is exactly the misreading the owner asked to prevent.

## 2026-09-06 â Prices are decided later, with investors; â¹7,999 / â¹29,999 stand and are guarded (Q54)

The owner will set Vyuha's prices **later, together with investors, once the product is ready**
(Q54). Until then the two figures in `lib/domain/pricing.ts` â **â¹7,999 annual** and **â¹29,999
lifetime**, against the unchanged â¹13,000 anchor from which `offerPct()` derives 38% â are the
...
**Rejected: repricing now, or building a price-change mechanism ahead of the decision** â both
...
## 2026-09-06 â The v4.0 build-prompt rulings (Â§0.2 Q-1â¦Q-12), and where the build deviated from them

`08-BUILD-PROMPTS/V400-LIVE-DESK-BUILD-PROMPT.md` Â§0.2 pre-ruled twelve design questions so no wave
...
- **Q-1 `/sizing-lab` is its OWN route in the Risk nav group** (`components/layout/nav-config.ts:76`),
  not a tab inside `/calculator`. **Rejected: a tab in `/calculator`** â that page is already at its
...
- **Q-2 SUPERSEDED by owner ruling Q46** â the size indices ARE bundled; see the entry above.
- **Q-3 one default column order**, identity â mark and P&L â time â the Pro risk columns
...
- **Q-4 no `openalgo-charts` pilot in v4.0** â and none was added; the range diff touches neither
  `package.json` nor `package-lock.json`, so the "v4.0 adds zero dependencies" ruling held exactly.
  **Rejected: piloting it behind the facade now** â 49 stars, one maintainer, a 2.0.2 published one
  day before the research read it. Re-evaluated only against a sustained 2.x line (Q30 puts the
  pilot at v4.1 at the earliest).
- **Q-5 the research pack's Â§6 list is the IP exclusion list**; anything not on it still prints a
  plain public formula on screen. `/atlas` computes nothing else (see the Q42b entry above).
- **Q-6 Turtle is the volatility tab** (`Volatility Â· Turtle unit (N)` in `components/sizing/lab-config.ts`).
  **Deviation:** the ruled "Varsity as a labelled switch inside it" is NOT built â the tab ships with
  the Turtle unit and the % volatility method beside it. v4.1.
- **Q-7 Kelly inputs are manual in v4.0** â the win rate and payoff are the user's own entries, and
  the method's description says so. Journal-derived Kelly (at â¥ 30 closed trades) stays v4.1.
- **Q-8 the risk inputs EXTEND `risk_config`** (migration 0064: `risk_pct_ppm`, `stop_method`,
  ...
  `heat_ceiling_ppm`). **Rejected: a new `live_settings` table** â it would split risk config across
...
- **Q-10 the bhavcopy backfill was ruled v4.1** because it needed a PRIVACY amendment.
  **DEVIATION, deliberate:** owner ruling Q43 brought it into v4.0 WITH the amendment â PRIVACY
...
- **Q-11 Telegram live alerts are v4.1** â not built. `lib/live` contains no alert builder; the word
...
- **Q-12 no published or hosted feed exists in v4.0** â held; the signed proprietary feed is the
  separate `atlas-ip-feed` wave (Q42b entry above).

## 2026-09-06 â The 4.0.0 version bump was applied BEFORE the verify gate, because the gate was coord-denied
...
â that is the property the order was protecting, and it is unchanged. `VYUHA-STATE.md` Â§2 carries a
...
of idling buys nothing â while a docs wave that cannot start makes the release later for a reason
...
## 2026-09-06 â The release ladder after 4.0: 4.1 is the OpenAlgo feed already in the tree, 4.2 is Upstox + Angel One â never Dhan

> **SUPERSEDED IN ITS 4.1 HALF (2026-09-06, later the same day).** The gate described below â
> "`settings.live_feed_provider` plus the existing disclosure consent" â is NOT what gates 4.1.
...
> **"2026-09-06 â v4.0.0 pre-tag audit"** below (`docs/DECISIONS.md:3576`). The 4.2 half
...
the existing OpenAlgo disclosure consent~~ â a setting is a preference, not a consent, and the v1
...
Settings and in the consent sheet, and **marketing may say the bridge IMPORTS trades â it may not
...
**Rejected: shipping the live provider ON in 4.0.0** â a feed switched on by a release rather than
...
**Rejected: Dhan**, which the earlier build-order note had in the set â the owner ruled Upstox and
Angel One, in that order, and Dhan is not in v4.2. **Also rejected: NSE `quote-equity` and Yahoo at
any version** (Q22) â no ToS grant, and either would force a fifth PRIVACY item.



## 2026-09-06 â v4.0.0 pre-tag audit: 33 findings confirmed, 32 fixed in one wave, the rest ruled
...
re-tested every survivor: 33 in â 32 CONFIRMED, 1 UNVERIFIABLE (the SEBI re-authentication
...
- **Derivatives get NO quote from the cash-bar EOD provider.** `isCashKey()` in
  ...
  and `persist-mark.ts` share it. An option was being marked at the underlying's close (a â¹45 premium
  showed â¹14 lakh unrealised, badge "eod"). *Rejected:* an exchange-only test â an option mis-tagged
...
- **Panel Open R and the R ladder use R FROZEN at first entry (`riskAmountP / qty`)**, null when
  ...
  *Rejected:* the current stop's distance â the row and the panel printed two R definitions for one
...
- **Portfolio heat adds `max(riskAtStopP, 0)` per row**; locked-in profit is published separately
  as `lockedInProfitP` (computed, not yet rendered). *Rejected:* netting â a well-trailed winner
  cancelled another row's real risk and the ceiling could never be crossed.
- **The desk multiplies the REAL average once (`investedP = round(qty Ã avgPrice)`)** and builds
  unrealised, risk-at-stop and the panel's figures from it; `avgEntryP` is display only. *Rejected:*
  rounding the level first â journal â¹6,544 vs desk â¹6,540 on a three-tranche average (invariant 1).
...
- **Pro is enforced server-side.** `/api/atlas` (GET/POST/backfill/import-files) and
  ...
  *Rejected:* `{pro = true}` â a default that leaks by omission; a nullable `pro` sub-object â a
...
- **OpenAlgo quote polling is withheld from 4.0.0 behind ONE constant, `OPENALGO_FEED_ENABLED`
  ...
  authorise a 1â5 s poll carrying the API key and every held symbol. v4.1 re-enables it WITH
...
- **The daily re-authentication sentence is a broker-attributable fact and the `VERIFY-CIRCULAR`
  ...
  2026-09-06 entry "The daily broker re-authentication sentence carries a VERIFY-CIRCULAR markerâ¦"
...
- **"alerts" leaves every Pro label** (`lib/license.ts` /live label, `desk-copy.ts` proColumns,
  ...
  `lib/live` or `components/live`. *Rejected:* keeping the word â a paid capability advertised in the
...
- **Atlas preview renders as a SIBLING of `<ProGate>`**, not a child: `<ProGate>{null}</ProGate>`
  ...
  children, so the preview nested inside was unreachable â the inverse of Q57. *Rejected:* dropping
...
- **A stale Atlas snapshot is refused on read, never deleted or recomputed by a GET**
  (`getVerifiedSnapshot()` â `{snapshot:null, stale:true}`); `refreshAtlasSnapshot()` deletes rows
...
- **The two 0066 settings columns are declared in `schema.ts` and, with `lastLiveMarkDate`, join
  ...
  stay OUT â preferences, and selecting a provider is not consent. `lastAutoMtmDate` has the same
...
- **Sizing Lab prefill is all-or-nothing** (`seedFromParams`: symbol + entry + stop, else the
  ...
  partial prefill â one real level beside one unrelated level prints a computed quantity from a
  fiction.
- **Copy guards scan the whole comment-stripped source** (`live-tracker-copy`, `live-feed-copy`);
  ...
  "You should â¦ {expr}" passed green. Proven red by planting one.
- **Other copy made true:** THIRD-PARTY-NOTICES Â§1 no longer says "runs offline and sends nothing
  ...
  color-primary)]` (`--primary` never existed); `/calculator` â `/sizing-lab` cross-linked; `j`/`k`
...
## 2026-09-06 â v4.0.0 fix-wave re-audit (`16b1ec1..5938aa6`): 17 items survived the skeptic, 0 refuted
...
(EXIT 0, 328 files / 6,025 passed / 35 skipped on `5938aa6`) did not predict the two CI reds â the
runner's timezone and the e2e job are facts the local gate never evaluates. Each item with the
alternative rejected:

- **S-1 â `row.stop` is an OBJECT (`StopComputed`) carrying `riskAtStopP`, `riskBudgetP`, `qty`
  and `deployedP`; it was pushed verbatim past the Pro gate after the four scalar row fields were
  nulled.** Fix: `stop: entitlement.pro ? stop : gateStop(stop)` â `{kind:"gated", source}`.
  *Rejected: hiding the columns client-side* â hiding is not gating; the numbers were still on the wire.
- **S-X (cross-auditor) â the desk and the SSE stream resolve the stored provider exactly as the
  ...
  screen while `OPENALGO_FEED_ENABLED` is false. *Rejected: rewording the radio* â copy cannot fix
  two resolvers disagreeing.
- **M-1 â the Sizing Lab hand-off passes `side` EXPLICITLY; `seedFromParams` REQUIRES it and never
  ...
  stop-side inference as a fallback* â the trailed-long trap survives for every deep link, which is
  exactly the path that carries no side.
- **U-1 â `scrollPaddingStart` equals the sticky `thead`'s height**, so `j`/`k` no longer scroll a
  row underneath the header it is meant to sit below. *Rejected: moving the `thead` out of the
  scroll element* â it breaks column alignment, which is a worse defect than the one being fixed.
...
- **U-2 â the screen-reader-only R cell carries the same lock as the visible one.** A free copy's
  assistive-tech reading announced a Pro number the screen did not show. One gate, both renderings.
- **D-7 â the refresh slider and the re-authentication box are gated on `OPENALGO_FEED_ENABLED`.**
  Controls for a provider that cannot be selected are an offer the build does not honour.
- **D-2 â the help copy says what actually runs** (stored marks, no broker feed in 4.0.0).
- **T-3 â the four consent-gate route tests are RESTORED and run under `vi.mock` with the flag
  forced `true`.** Withholding the feed had made them unreachable, which quietly deleted the
  coverage of the gate itself. *Rejected: `it.todo`* â a todo is a note, not a test; the consent
  gate would have shipped 4.1 unproven.
- **T-1 â the trail block is retitled to say what it pins: trail ARITHMETIC.** Its title claimed
  M3's guarantee, which lives in `position-chart-copy`; two files appeared to cover the same thing
  and neither fully did.
- **T-2 â the M7 pin reads BOTH headers for "9.0 crore".** It matched one and passed while the
  other drifted.
- **CI-1 â the `sizing-lab-page` test expects `todayIstIso()`.** The page computes its date in IST;
...
- **CI-2 â the `z-sidebar-fold` e2e spec matches the shipped fold.** Positions is **6** screens;
...
- **D-1 â the 4.0.0 CHANGELOG entry says stored marks and no broker feed**, and "OpenAlgo live feed
  ...
  *Rejected: leaving it and correcting it at 4.1* â a buyer reads the CHANGELOG shipped with 4.0.0.
- **D-3 â `docs/client/README.md` describes the mark WITHOUT claiming provenance.** `mtm_prices`
  has no source column, so "end of day, entered by hand, or none stored yet" was a label the code
  cannot produce. *Rejected: adding a source column to make the copy true* â a schema change to
  rescue a sentence, in a release already frozen.
- **D-4 â the write-back is "stored as the Live Desk's risk settings", not "written back to the
  ...
  `tests/no-indicators-in-client-docs.test.ts` â the feed guard is conditional on
  `OPENALGO_FEED_ENABLED` (it skips itself when 4.1 flips the constant), the write-back guard is
  unconditional. *Rejected: fixing the copy without a guard* â this exact sentence had already
  survived one docs wave.
- **D-5 â `VYUHA-STATE.md`'s four verify claims agree**: EXIT 0, 328 files / 6,025 passed / 35
  ...
  the local gate alone. *Rejected: recording only the green gate* â that is how a red CI becomes a
  surprise at tag time.
- **D-6 â the release-ladder entry is marked SUPERSEDED in its 4.1 half** (pointer to the
  ...
  pairing â marketing may say the bridge imports, and may not say it prices. *Rejected: deleting
  the entry* â this file is append-only, and a decision that was made and then overtaken is worth
...
### 2026-09-06 â load suite: c8 one-symbol baselines doubled (24k â 48k, 30k â 60k)
...
timer-noise guard (`tests/load/helpers/measure.ts:24`), not a hot-path regression â the wave touched
...
baselines 133 ms / 147 ms, ratios 4.6Ã / 4.2Ã against the 6Ã bar. *Rejected:* lowering
`MIN_MEASURABLE_MS` â the floor is what makes a ratio meaningful; the fix is more work per sample,
not a smaller ruler.

### 2026-09-06 â v4.0.0 second-wave re-audit (`5938aa6..aba43f5`): 5 confirmed, all cosmetic, fixed in wave 3
...
- **U-1b â the virtualised path needed `scrollMargin: theadHeight` as well as `scrollPaddingStart`,
  ...
  (h = 40, row 22 of 44 px, clientHeight 600): before, `start` â scrollTop 928, row top at 80 (one
  header below the header) and `end` clipped the row's bottom 40 px; after, `start` â 968 (row top
  at 40, flush) and `end` â 452 (row bottom at 600). `getTotalSize()` is `end â scrollMargin`, so
  the bottom spacer must subtract the margin from the last item's absolute `end` or scrollHeight
  is `h` short. *Rejected: `scrollMargin` alone* â the padding is what keeps a row within `h` px of
  the header from being scrolled needlessly.
- **TI â skip-family constructs went 26 â 30: four `it.skipIf(OPENALGO_FEED_ENABLED)` guards** that
  skip nothing today and relax themselves at the 4.1 flip. Recorded so the count is not read as
  four dark tests.
- **D2-F1 â the mark chip copy.** CHANGELOG, the client README and STATE said every row shows a
  dated "stored mark". The desk prints "End of day Â· <session close>" for a bhavcopy-priced row and
...
- **D2-F2 â the S-1 bullet above described a string/blank stop; the defect was the `StopComputed`
  object.** Rewritten in place.
- **D2-F3 â STATE Â§2 claimed the verify sha "is the one tagged `v4.0.0`" before any tag existed.**
