...
4. `AGENTS.md` before any code change (on conflict AGENTS.md wins over this file; the code wins over both). Owner rulings: the spec's §0 table; `06-ANSWERS.md` for older ones; `docs/DECISIONS.md` newest entries at the END (grep `^## 2026-09-24`, never read it whole — the newest is the v4.6.0/v4.7.0 scope entry) — binding, never re-ask one.
...
**The one-line state (re-verified 2026-09-24 by the twentieth session: HEAD `ea3b369` on `16ea079` on `06d02a9`; CI run 35903568848 SUCCESS 6/6 on `ea3b369`; tag v4.5.0 = `b6b8863`; `releases/latest` v4.5.0; `npm run drift` 10 PASS / 0 FAIL / 1 SKIP; `npm run verify` NOT re-run — no code changed since the tagged tree):** published = **v4.5.0**. **Gate on the tagged tree (unchanged since `47088f7`): 473 files / 10,894 passed / 40 skipped**; Playwright 120 / 120 in 35 specs. The tag session's release evidence (release run 35758523118, deep verify 3/3 key id `4FF85F3BBE1DA21D`, client ZIP sha `86A73A21…0B9865`, winget HELD) is in the archived blocks and DECISIONS 2026-09-22 — v4.5.0 is closed; nothing there is re-checked.

- **What the twentieth session did:** five research reports (R0 inventory; R1 prescriptive analytics; R2 help desk; R3 stock universe; R4 market calendar — subagents wrote them to disk, ≤ 450-word summaries came back); three owner question groups (12 questions, all answered); the spec; `docs/LEDGER.md` (5 [RESEARCH], 3 [LESSON], 2 [DECISION]); DECISIONS 2026-09-24 scope entry. No code, no migration, no dependency.
...
- **Owed outside the build:** the owner's eight calendar source files (spec §6) into one folder; two hook proposals in `~/.claude/coord/learnings/HOOK-PROPOSALS.md` (SENTINEL README repoint; `coord/config.json` SENTINEL root → `SENTINEL/bot`), both `[PENDING]`; next `/fleet-tune` ~2026-09-30.

### §0.1 Pickup table — one row per open item

| # | Item | Status | Resume from EXACTLY here | Closed when |
|---|---|---|---|---|
| 18 | **v4.6.0** (W1 calendar → W2 universe + row 10 → W3 hubs → W4 Help + strip → W5 Q51/Atlas v3 → W6 row 9 → W7 row 17) | **OPENED + SPECIFIED 2026-09-24; not built** | spec §2 W1; W1's JSON needs the owner's download folder (§6); W5 needs the 20 06a AQ answers; W6 `side` and W7 need `vyuha-design-reviewer` first | owner publishes v4.6.0 |
