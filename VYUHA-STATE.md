...
## §0 START HERE — reconciled 2026-09-26 IST by the twenty-ninth session (v4.6.0 wave W7 BUILT — row 17 tax follow-ups: the AIS purchase side per entry leg, the harvest STT split one count per ladder with fills beneath, the FMV grandfathering editor per scrip, the Upstox ~₹1 residual ACCEPTED; NO migration — 0077 is still the newest): **v4.5.0 is the published release and is CLOSED** (tag `v4.5.0` = `b6b8863`; NO REWORK on any closed item). **v4.6.0 is FULLY BUILT — W1, W8, W2, W9, W3, W4, W5, W6 and W7 are DONE and committed; what remains is the RELEASE PATH: seam pass → ONE rigorous audit → at most one fix wave → bump → help:shots → the release skill → `release:verify --deep` → STOP, the owner publishes.** Only v4.6.0 and v4.7.0 carry everything left (rulings B1–B6). Fixed in the tree, not yet released: everything W1–W6 listed before (archive), plus: an AIS statement now shows a staged ladder's purchases in the years its legs were bought, never all in the first (W7); the harvest STT split counts a ladder once and shows its fills (W7); one FMV per scrip writes every pre-2018 lot of that scrip for the tax person, and a pre-2018 partly-sold ladder is editable at last (W7); broker-compare, the tax footnotes and Help say why a day's charges can sit up to ₹0.50 × N per head from the contract note (W7). Three hook proposals still `[PENDING]`.
...
4. `AGENTS.md` before any code change (on conflict AGENTS.md wins over this file; the code wins over both). Owner rulings: the spec's §0 table; `docs/DECISIONS.md` newest entries at the END (grep `^## 2026-09-2`, never read it whole — the newest is the 2026-09-26 **W7 built** entry, with the W6 built, W5 built, W5 opened, W4, W3, W9, W2, W8, W1 and brokers entries before it) — binding, never re-ask one. The release procedure is the `release` skill; `docs/LEDGER.md` D-14 lists W7's session decisions and the ONE recorded follow-up (`setAcquisitionAction` has no ladder guard).
5. §3 of this file for where any other answer lives. Every earlier §0 block is in `VYUHA-STATE-ARCHIVE.md`, byte-for-byte (the twenty-eighth session's block archived by this session).

**The one-line state (re-verified 2026-09-26 by the twenty-ninth session at its close: the W7 build commit `5548d46` sits on `33fc459` (the W6 close-out) and this close-out docs commit sits on it — the CI run on `5548d46` is 36181295940 SUCCESS 6/6 (first pass, no re-run); newest migration **0077** (`trades-side`; W7 added none, so the release path's first migration, if any, is 0078); tag v4.5.0 = `b6b8863`; `releases/latest` v4.5.0):** published = **v4.5.0**. **Gate on the W7 tree: `npm run verify` (vyuha-verifier, alone, second run) EXIT 0 — 499 files / 11791 passed / 35 skipped**, lint 0 errors (6 pre-existing warnings), `next build` "Compiled successfully in 12.8s"; 35 skipped = the owner-file cases with `VYUHA_LICENSE_PEM` set (unset: 5 fewer passed, 5 more skipped). The FIRST gate run was FAIL-J (three timeouts in files W7 does not touch while another session ran 14–16 python processes (10 at the second run's start; `tasklist` counts them, `ps -W` under-counts here); the three pass alone in 16 s; 47 skipped that run = a timed-out `beforeAll` marking its suite skipped). Playwright: **141 flows in 38 specs** by `--list` (unchanged: W7 adds no screen, tab or route). `package-lock.json` and `package.json` UNCHANGED (no new dependency).

- **What the twenty-ninth session did:** asked W7's two owner questions FIRST after sizing the Upstox residual (engine Plus 227.56 vs the ledger's 226.57; ₹1.00 of the ₹0.99 gap is STT rounded per row against once per contract note) — both answered with the recommended option (one count per ladder with fills beneath; the residual ACCEPTED + explained, no engine change); a scout mapped the four surfaces; a DECIDED contract went to `vyuha-design-reviewer` (Opus, 86 calls) which returned **REVISE, R-1…R-9** — the sharpest: `setAcquisitionAction` rewrites a staged row's buyQty/buyValue/buyDate and leaves the legs, so the naive purchase split would have stated a purchase in the SALE's FY; ONE Opus builder built D1–D4 (17 files, 5 new); the skeptic (Opus, 76 calls, 14/14 confirmed) found 4 more (the legs-state-the-parent guard passed BY COINCIDENCE at break-even → a date condition; an unreadable leg date dropped its rupees; a short cover's fill line; a blank Save over a MIXED FMV group would have WIPED every lot's value); a fresh Opus builder landed the 8-item fix wave. LEDGER U-14…U-16, L-36…L-41, F-29…F-34, D-14. One incident: the first builder ran `taskkill //F //IM python.exe` machine-wide and killed another session's pytest — the other session was told; the rule (kill only a PID you own) is in shared memory and every builder prompt.
- **What WRONG looks like from here:** an AIS purchase total that does not sum to Σ `buyValue` across years, or a purchase filed in a SALE's FY (the basis-write shape — `tests/purchase-rows.test.ts` red); the oracle purchase pins (49000 / 52000 / 51000) moving; a harvest STT rupee amount changing (only the COUNT changed); a fill line at a tranche's date with the whole cover quantity; an FMV reaching a second scrip, a post-2018 lot or another person's account (`tests/fmv-route.test.ts` red); a blank Save clearing lots that had a value; a byte comparison against `GRANDFATHER_DATE` anywhere but inside `isGrandfatherEligible`; the ≤ ₹2 Upstox pin loosened or an engine/`charge_config` change "for the residual"; a `NOT NULL`/migration appearing for W7; `package-lock.json` with any change; a wave tag; `OPENALGO_DISCLOSURE_VERSION` bumped before v4.6.0 ships; plus every W6 signature (a closed short read as a long, COFORGE off `openingSells 1`, a leg write without `side`).
- **Owed outside the build:** the RELEASE PATH (row 18); the release-day **perf sweep** re-measures `/trades` through `next start` (the W6 lever was proven at the query level only); the OWNER reviews the Help copy (H4); **re-run `npm run help:shots` AFTER `npm run bump-version 4.6.0`** (L-19); **measure `/atlas` on the owner's real DB before the release** (L-25); a LIVE OpenAlgo 2.0.2.6 preview pull (owner); a Fyers tradebook and a Nuvama report WITH equity rows whenever the owner has them; Q24's citation for the release claims audit; the 2027 NSE holiday list + the Muhurat 2026 circular; three hook proposals in `~/.claude/coord/learnings/HOOK-PROPOSALS.md`, all `[PENDING]` (the third, 2026-09-25: deny any Agent call on Haiku, owner ruling D101); next `/fleet-tune` ~2026-09-30.

### §0.1 Pickup table — one row per open item

| # | Item | Status | Resume from EXACTLY here | Closed when |
|---|---|---|---|---|
| 18 | **v4.6.0** (W1 ✓ → W8 ✓ → W2 ✓ → W9 ✓ → W3 ✓ → W4 ✓ → W5 ✓ → W6 ✓ → W7 ✓) | **ALL NINE WAVES DONE 2026-09-26; the release path is next** | `vyuha-seam-tester` over the W1–W9 boundaries (values crossing builder file sets — the market calendar into the marks, `side` into the tax/AIS/harvest readers, the universe into the Atlas, `purchaseRows` into the AIS route) → ONE rigorous `vyuha-audit` (all six lenses, one run; the audit's fix list may take the recorded follow-up: `setAcquisitionAction` gains a `hasLadder` guard like the IPO push) → one fix wave max → `npm run bump-version 4.6.0` → `npm run help:shots` → the `release` skill (the perf sweep with `/trades` and `/atlas` on the real DB inside it) → `release:verify --deep` → **STOP; the owner publishes** | owner publishes v4.6.0 |
| 19 | **v4.7.0** (C1–C2 Edge Clinic → the Clinic TAB of `/reports/edge-clinic` becomes the default, C3 journal Kelly, C4 row 6 Positions tab, C5 Q18 Telegram, **C6 native pulls Fyers / Kotak Neo / Nuvama**, **C7 OpenAlgo WebSocket feed**) | SPECIFIED; opens after 4.6.0 is published | spec §3; C1 adds a `clinic` tab to `HUBS` and flips `defaultTab`; a new tab needs a `HELP_ENTRIES` entry AND a `HELP_TASKS` record; C2 asks its two questions at ITS start; C6 needs `vyuha-design-reviewer` first; retire `sector-map.json` there if its fallback-only count (8 on 2026-09-25) is still about zero; a Dhan API overnight short across two daily pulls (LEDGER D-8) belongs to C6/C7 if anywhere | owner publishes v4.7.0 |
| 6 / 8 / 9 / 10 / 17 | Rows folded into 18/19 | row 6 → v4.7.0 C4; row 8 → DONE in W5 (Q51/Atlas), Kelly (C3), Q18 (C5); row 9 → DONE (strip W4; `side`, speed lever, `--ref` W6); row 10 → DONE in W2; **row 17 → DONE in W7 (all four items; the Upstox residual is ACCEPTED, not fixed — by the owner's answer)** | the spec | with their release |
...
### §0.2 Decisions taken this session under the decision policy (recorded in DECISIONS 2026-09-26 W7 built entry and LEDGER D-14; the owner may overrule)
**Twenty-ninth session (2026-09-25/26):** purchase consideration per leg = qty × the leg's own price, no charges, no average cost, settled to the parent's `buyValue`; the split is refused (parent row whole) unless the legs still state the parent by quantity, value AND the aggregate's own date and every leg date is readable; the IPO-link dedup stays keyed on the parent id; open ladders state purchases too; harvest counts distinct parents, lists only ladders, a fill line is (fillLegId, sellDate), a native `<details>`; one FMV per scrip per TAX PERSON through the one `grandfatherKey`, the lots are the parents behind the REALISED rows (`!isOpen` not required), the route refuses whole on a second scrip / ineligible lot / foreign account / blank over a mixed group, one transaction, one audit row per lot, "Trade not found" now 400; the Upstox residual: no engine change, no Data Quality warning, the bound in three copies, the new `it` derives the delta from the seed; no migration; no e2e. `setAcquisitionAction`'s missing ladder guard is RECORDED for the audit fix wave, not built. The earlier sessions' decisions stand.

### §0.3 Verification — PASS and FAIL signatures (V3–V6 of the v4.4.0 era are in the archive)

**V1 — CI on the newest CODE sha.** `gh run list --limit 5`; judge only the run whose `workflowName` is `CI` with 6 jobs. **PASS:** `success`, 6/6. **FAIL-B** — a job red with a timeout / network / cold-runner signature: re-run that job; never re-tag. **FAIL-G** — macOS Playwright red with ONE spec dead on `ECONNREFUSED :3100` and the dev server's memory-threshold restart line: the heap watchdog. **FAIL-H** — BOTH Playwright jobs red on ONE spec with `strict mode violation`: pin both, never `.first()`. **FAIL-K** — a Playwright spec asserting a market-open/closed label or a mark minute: re-pin to the calendar's answer. **FAIL-R** — `z-hubs.spec.ts` red on `config-level redirect: expected 307`: fix the config, never the spec. **FAIL-U** — `zz-console-audit` red on `/settings` or `/atlas` with a React key or render-phase warning: a PRODUCT regression, never a flake. **FAIL-V** — `z-atlas.spec.ts` red on "cache did not grow" or a cohort/dark-state assertion: an earlier spec now visits `/atlas` or seeds bars — read the shared-DB order. **FAIL-A / C / D / E / F** as before (archive).

**V2 — the gate.** `rtk proxy npm run verify` (heavy; coord serialises it; a builder NEVER runs it — `vyuha-verifier` does, and NOT while another session is loading the machine — **FAIL-J: three or more `timed out` in files the wave does not touch, each green alone → count the machine's python/node processes (`tasklist`, not `ps -W`, which under-counts here), wait, re-run ONCE**). **PASS:** exit 0 and the RAW vitest line `Test Files ≥ 499 passed`, `Tests ≥ 11791 passed | 35 skipped` with `VYUHA_LICENSE_PEM` set, no `expected fail`; a skipped count ABOVE 35 with a red `beforeAll` is that suite's tests, not the PEM. **FAIL-AC (new)** — `tests/purchase-rows.test.ts` red on a guard case or `tests/ais-purchase-legs-db.test.ts` red on "in the year it was bought": the purchase split trusts legs again or bumps the parent — fix the guard, never the fixture. **FAIL-AD (new)** — `tests/fmv-route.test.ts` red on a 400 case or `tests/grandfather-groups.test.ts` red on "15-06-2019": a partial write or a byte compare came back — a product defect. **FAIL-Y / Z / AA / AB / W / X / T / S / Q / P / M / N / O / L / E / I / A / B / C / D** as before (archive and the W6 block).

**V7 — the release-gate Playwright pass:** 120 / 120 in 35 specs on the v4.5.0 tagged tree; 141 in 38 by `--list` on the W7 tree (unchanged since W5; nothing ran locally this session — W7 touched no screen, tab or route). **PASS signature:** `141 passed`, `PROBE-END console-audit (0 message(s))`.

**V8 — the release itself.** `npm run release:verify <tag> -- --deep` must print N signatures ALL `4FF85F3BBE1DA21D` and ALL "verifies over"; `gh release list --limit 2` must show the new tag as `Latest` and `gh release view revocations --json isPrerelease` must stay `true`. A background Bash `until` loop on `gh run view --json status` is the reliable wait. `$CLAUDE_SCRATCHPAD` is NOT set inside the Bash tool's shell here — write the absolute scratchpad path. The OWNER publishes (stop after `release:verify --deep`). **Before the release build: `npm run help:shots` after the bump (L-19), the `/atlas` timing on the real DB (L-25), and the perf sweep's `/trades` median (W6's lever, proven at the query level only).**
...
1. ~~Before W7~~ — ANSWERED 2026-09-25 (twenty-ninth session): one count per ladder with fills beneath; the Upstox residual ACCEPTED + explained. 2. Before C2 (v4.7.0): the setup-grade labels, and whether intra-trade high/low is typed or later auto-filled. 3. Before C6: the first real Fyers `/trade-history` pull pins its fields; Kotak Neo stays "documented, not verified" (B2). 4. Still open from before: whether the A4 / A7 decisions and the sixteenth session's §0.2 stand; row 5 / the derivative-marks schema release (archive §0.4 item 3). 5. Noted, not asked: BSE SME companies (486) DO get an AMFI band (small) — U3 names NSE Emerge only. 6. Noted, not asked: a Fyers tradebook or a Nuvama report WITH equity rows, whenever one exists. 7. **Owner review owed by ruling H4:** the Help copy in `lib/domain/help-topics.ts` (50 cards) and `lib/domain/glossary.ts` (35 terms). 8. Noted, not asked: the "backup taken" strip step counts a backup on THIS machine only. 9. **Noted, not asked (W5):** `COVERAGE_FLOOR_PPM` 30%, the cohort floor 5 priced / 60% coverage and the group floors 3 / 8 are PROPOSALS. 10. **Noted, not asked (W6):** the legacy F&O pair is joined through the confirm dialog, and an uncovered derivative sale ALWAYS stays an opening sell — say so if either should change. 11. **Noted, not asked (W7):** the FMV route now refuses an FMV on a lot no reader would apply it to (a post-2018 or non-delivery lot) and answers 400 where it answered 404 — say so if the old permissiveness was wanted; and `setAcquisitionAction` still accepts a basis write on a STAGED row (the legs then contradict the parent) — the audit fix wave proposes a `hasLadder` refusal like the IPO push's; say so if a rebuild is preferred to a refusal. 12. **Before C4:** whether the round-3 Positions canvas is built AS SHOWN or with changes (spec §4).
---
