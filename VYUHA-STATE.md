...
**The one-line state (verified 2026-09-11):** published release = **v4.2.0** (tag `9da7bc8` → `b488dde`). v4.3.0 wave 1 (`265d684`) audited twice, fix waves `beaea48` / `786d288` / `6198c8b` CI 6/6. **v4.3.0 wave 2 = `a00ddd0`** — CI run 34472737988 SUCCESS 6/6, seam-tested, **audited (round 3, 2026-09-11: 142 → 14 → skeptic 14) → fix wave 3 = `f167e6c` (24 repo paths + README, pushed; **CI 34518836233 SUCCESS 6/6**, Pages 34518835928 3/3); round 4 = that fix wave's own audit DONE (117 → 8 → skeptic 5, all five fixed) → **fix wave 4 = `34db076`** (9 code/test paths + README + this file + DECISIONS; **CI 34523410516 SUCCESS 6/6**); round 5 (scoped) DONE: money 10 → 0, ui 15 → 2, test 19 → 1 → skeptic 3 → 3 → **fix wave 5 = `5494d9f`** (`strategies-client.tsx` + `tests/strategies-page.test.ts` + README + this file + DECISIONS; **CI 34526537168 SUCCESS 6/6**; the docs-only commit on top of it carries this line) → **round 6 = a SCOPED audit of fix wave 5 (ui-regressions + test-integrity on Fable + skeptic) NOT yet run — the hand-off point; NOT bumped, NOT tagged**. Gate on the fix-wave-5 tree: the raw vitest line is in §2's header (7,634 expected = 7,632 + 2). Estate paths moved under `T:/Thejesh/CLAUDE-CODE/VYUHA/` on 2026-09-10 (sweep record: `docs/owner/HANDOFF-HYGIENE-2026-09-10.md`).

### §0.1 Pickup table — one row per open item

| # | Item | Status | Resume from EXACTLY here | Closed when |
|---|---|---|---|---|
| 1 | Audit ladder on wave 2: round 3 (14 → 14) → fix wave 3 `f167e6c` (CI 6/6) → round 4 (117 → 8 → 5) → fix wave 4 `34db076` (CI 34523410516 6/6) → round 5 scoped (44 → 3 → 3) → **fix wave 5 = `5494d9f`** (CI 34526537168 SUCCESS 6/6) → **round 6 = SCOPED audit of fix wave 5, NOT yet run** | fix wave 5 pushed, CI 6/6; gate EXIT 0, 378 / 7,634 / 35 | `vyuha-audit` skill, scoped: `vyuha-auditor` ui-regressions + test-integrity on Fable over `34db076..5494d9f`, then the Fable `skeptic`; known inputs = DECISIONS "2026-09-11 — v4.3.0 audit round 5 (scoped)" — its "Recorded, not defects (round 5)" bullet and the "Fix wave 5" bullet (the behavioural undo test DUPLICATES the component's revert expression by necessity; `sameSelection` is local to the island by decision; the tick-C residual is recorded, not fixed; no jsdom harness) | round 6 reaches 0 open findings (or every survivor is ruled recorded); CI 6/6 on the last fix commit; §0.3 V1–V5 all PASS → then row 2 |
...
## 2. Current state — v4.3.0 WAVE 2 AUDITED THREE TIMES, FIX WAVE 5 BUILT 2026-09-11 IST (`npm run verify` EXIT 0 on the fix-wave-5 tree — **378 files / 7,634 passed / 35 skipped** (verify this line against the raw vitest output of the fix-wave-5 gate: it was written before the run finished and corrected in the same commit if it disagreed); fix wave 4 = `34db076`, CI 34523410516 SUCCESS 6/6 (R4-M-1 de-dup guard `be >= cHi − 0.005`, R4-U-1 `committed` ref revert, R4-U-2 heading scroll margin + `OPTIONS_SECTION_ANCHOR`, R4-T-1 `BANNED` union, R4-T-2 positive "from the shelf" control; DECISIONS "2026-09-11 — v4.3.0 audit round 4"); round 5 scoped: money 10 → 0, ui 15 → 2, test 19 → 1, skeptic 3 → 3 → **fix wave 5 = `5494d9f`, CI 34526537168 SUCCESS 6/6** (R5-U-1 the refusal revert keeps the undo stack, R5-U-2 a stale acceptance re-syncs + refreshes, R5-T-1 the advance pinned; DECISIONS "2026-09-11 — v4.3.0 audit round 5 (scoped)"); **round 6 (scoped, ui + test-integrity + skeptic) NOT yet run**; wave 1 `265d684` → audit round 1 (12 → 12) → fix wave `beaea48` + CRLF cap `786d288` (CI 34465570290 6/6) → audit round 2 (6 → 6) → fix wave `6198c8b` (CI 34467572963 6/6) → **wave 2 = `a00ddd0`, CI 34472737988 SUCCESS 6/6** → audit round 3 (142 candidates → 14 → skeptic 14; DECISIONS "2026-09-11 — v4.3.0 audit round 3") → **fix wave 3 = `f167e6c`, CI 34518836233 SUCCESS 6/6** (U-1 `router.refresh()` after the shelf fold; U-3 rollback on refusal; U-2 anchor; G-2 "sixteen"; G-1 comments; T-1 gate; M-1 analytic breakeven; S-1 shelf comparator; D-1/D-2 help copy; U-4 hash-targeted card + scroll) — **its own audit (round 4) launched 2026-09-11, result in §0 row 1 / the ledger "Session 2026-09-11"; NOT bumped, NOT tagged**). **v4.2.0 remains the PUBLISHED release** (section below, unchanged).
