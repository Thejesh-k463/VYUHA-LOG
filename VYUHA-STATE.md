...
§0 below is a LATER reconciliation record (2026-09-26) and outranks §2.
...
## §0 START HERE — reconciled 2026-09-26 IST by the thirtieth session (v4.6.0 RELEASE-LEVEL AUDIT DONE — ONE run, every lens, 52 candidates → 51 confirmed — and its ONE FIX WAVE BUILT: commit `1c8b393`; migration **0078** `margin-config-fyers-nuvama` is now the newest): **v4.5.0 is the published release and is CLOSED** (tag `v4.5.0` = `b6b8863`; NO REWORK on any closed item). **v4.6.0 is FULLY BUILT AND AUDITED — W1, W8, W2, W9, W3, W4, W5, W6, W7 and the audit fix wave are committed with CI green; what remains is the RELEASE ITSELF: `npm run bump-version 4.6.0` → `npm run help:shots` → the `release` skill (perf sweep `/trades` via `next start` and `/atlas` on the real DB) → `release:verify --deep` → STOP, the owner publishes.** Only v4.6.0 and v4.7.0 carry everything left (rulings B1–B6). Fixed in the tree, not yet released, beyond W1–W7 (archive): the ITR pack's grandfathering read the per-share FMV as a total (LTCG 4×, HIGH — pre-existing); an overnight F&O short is filed in the FY it was COVERED at every tax surface; ROM and the signal book read `side`; a flat no-signal row is stored with NO side by every writer; a basis write on a staged row is refused; upgraded/restored/baseline databases receive the Fyers/Nuvama margin rows (0078); the Atlas cap band resolves on a fresh install; the FMV route refuses a blank over a mixed group and says "highest price quoted on 31-Jan-2018"; Nuvama Elite MTF = its delivery rate; Paytm and Groww ladders allocate executions per fill (Σ legs = parent; a `ladder_mismatch` Data Quality line names old books); the help dialog opens on its title; the getting-started plan step can tick; the catch-up re-reads consent inside its wait; broker/feed counts are derived. Three hook proposals still `[PENDING]`.
...
2. `VYUHA/LIVE-DESK-RESEARCH/22-V460-BUILD/00-SPEC-PLAN.md` — the v4.6.0/v4.7.0 spec and wave plan (rulings table §0, waves §2–§3 — every §2 wave carries a BUILT line, questions owed §4); its `research/R0…R4` and `VYUHA/LIVE-DESK-RESEARCH/23-BROKERS-OPENALGO-2026-09-25/R5…R9` are the evidence (never re-run them). `docs/LEDGER.md` is the tagged record ([LESSON] [FIX] [UPGRADE] [RESEARCH] [DECISION]) — grep it before building anything; R-12 and L-42…L-50 / F-35…F-50 / U-17 / D-15 are the audit's. `22-V460-BUILD/SESSION-LOG.md` is the per-session ledger and carries the ONE paste block. Research is OUTSIDE the repo — cite it as `VYUHA/LIVE-DESK-RESEARCH/…`, never as a repo-relative path (LEDGER L-4).
3. Re-derive before asserting: `git log --oneline -3` · `git tag --sort=-creatordate | head -1` (**v4.5.0**) · `package.json` `"version"` (**4.5.0** until the bump) · `gh run list --limit 3` (a plain list, never `--commit`) · `gh release list --limit 2` (v4.5.0 Latest) · the RAW vitest line (§0.3 V2) · `npm run drift:close-out` (0 FAIL before `/clear`; a FAIL is fixed in the DOC, never in the check).
4. `AGENTS.md` before any code change (on conflict AGENTS.md wins over this file; the code wins over both). Owner rulings: the spec's §0 table; `docs/DECISIONS.md` newest entries at the END (grep `^## 2026-09-2`, never read it whole — the newest is the 2026-09-26 **v4.6.0 release-level audit** entry, then the W7 built entry) — binding, never re-ask one. The release procedure is the `release` skill; the release-copy DRAFTS (CHANGELOG section, README quote, client README "New in", install-guide lines, landing chip — 50 claims with file:line) were written by the audit and must be re-derived by the release session from LEDGER F-35…F-50 + U-1…U-17 (the scratchpad copy died with the audit session).
5. §3 of this file for where any other answer lives. Every earlier §0 block is in `VYUHA-STATE-ARCHIVE.md`, byte-for-byte (the twenty-ninth session's block archived by this session).

**The one-line state (re-verified 2026-09-26 by the thirtieth session at its close: the audit fix-wave commit `1c8b393` sits on `3657793` (the W7 close-out), the test follow-up `3cdde8d` (the copy-pin test reads docs CRLF-normalised — FAIL-AI below) sits on it, and this close-out docs commit sits on that — the CI run on `1c8b393` is 36219088249 = 5/6 (the Windows unit job red on ONE test, the new copy pin, CRLF — not the product), the CI run on `3cdde8d` is 36219858147 SUCCESS 6/6 (first pass); newest migration **0078** (`margin-config-fyers-nuvama`, 16 INSERT OR IGNORE rows; proven +1 then +0 on a temp DB AND on the smoke copy of the owner's live DB, `data/smoke-v460/`, 78 → 79 then 79, 16 rows, integrity ok); tag v4.5.0 = `b6b8863`; `releases/latest` v4.5.0):** published = **v4.5.0**. **Gate on the fix-wave tree: `npm run verify` (vyuha-verifier, alone, second run) EXIT 0 — 505 files / 11879 passed / 35 skipped**, lint 0 errors (6 pre-existing warnings), `next build` "Compiled successfully in 11.9s"; the FIRST run was EXIT 1 on ONE test — `tests/readme-claims.test.ts` "expected 499 to be 505", README's own count (fixed in the doc). 35 skipped = the owner-file cases with `VYUHA_LICENSE_PEM` set. Playwright: **142 flows in 38 specs** by `--list` (the fix wave added ONE flow to `e2e/z-help.spec.ts`: a glossary-term topic opens on its title, one Esc closes, focus returns to the card). `package-lock.json` and `package.json` UNCHANGED (no new dependency).

- **What the thirtieth session did:** ran the `vyuha-audit` skill's §1a release-level form ONCE over `v4.5.0..HEAD`, every lens on Opus: the upgrade lens on a copy of the live DB (PASS), the release-claims draft (50 claims / 45 file:line), the seam pass (`tests/seams-v46-release.test.ts`: 8 boundaries, 3 seam defects), six dimensions × two lenses, ruling conformance (64 rulings, 0 diverge, 3 unpinned), 14 user journeys, mutation on money paths (33 + 18 mutants), the completeness critic, then the skeptic over the union — **52 → 51 confirmed** (22 PRODUCT incl. 1 HIGH, 14 TEST-ONLY, 15 DOCS). The plan and its two scope calls went to the owner (approved; **MO-4 Paytm/Groww ladders fixed NOW, not 4.7.0; MO-1 Nuvama Elite MTF = the recorded rule**); `vyuha-design-reviewer` returned REVISE on six designs (all folded in) and REVISE on MO-4; TWO Opus builders on disjoint sets (A identity/tax/money end to end, B UI/copy/docs; A resumed twice at its turn limit, never re-spawned); the scoped skeptic re-check graded every id FIXED with 20 red-on-revert runs; ONE gate. LEDGER R-12, L-42…L-50, F-35…F-50, U-17, D-15; DECISIONS 2026-09-26 audit entry.
- **What WRONG looks like from here:** `/reports/itr` and `/reports/tax` disagreeing on LTCG for a pre-2018 lot (`tests/itr-page-fmv.test.ts` red — the per-share FMV read raw again); a short sold 30 Mar / covered 1 Apr appearing in FY 2025-26 on ANY tax surface (seam 2d); a `fyDate` defaulted anywhere; ROM capital for a same-day covered short option at its premium (`tests/rom.test.ts` CC-1); a fresh import storing `side='long'` on a flat no-signal row (`tests/side-column.test.ts`); `setAcquisitionAction` returning `ok:true` on a staged row (seam 3b); an upgraded DB with 0 Fyers/Nuvama `margin_config` rows or a baseline restore wiping them (`tests/margin-config-refresh.test.ts`); RELIANCE with no cap band on a fresh install (seam 4d); the FMV route answering 200 to a blank over a MIXED group or "closing price" reappearing in the editor; Elite `eq_mtf` at ₹20 on ₹5,00,000; a Paytm/Groww position whose Σ executions ≠ parent, or a PARENT digest moving (`tests/golden-books.test.ts`) — the parents must NOT change, only the legs; a help topic opening with a tooltip; the strip's plan step stuck "not done" for a one-plan broker; a hand-written broker count anywhere; a `NOT NULL` on 0078; `package-lock.json` with any change; a wave tag; `OPENALGO_DISCLOSURE_VERSION` bumped before v4.6.0 ships; plus every W6/W7 signature (archive).
- **Owed outside the build (the RELEASE session):** `npm run bump-version 4.6.0` (four files; the footer → `v4.6`), then the bump's doc inputs — the CHANGELOG v4.6.0 section, README quote, client README "New in v4.6.0", install-guide upgrade lines (0075 15:35 → 15:45, 0076, 0077 + `trades-side-v1`, 0078; 280 Fyers/Nuvama rate rows + 16 margin rows on first start), landing chip — re-derived from LEDGER; the six-broker counts (landing-page.html:290,537,550,613,623; brochure.html:202; INSTALLATION_GUIDE.md:79-82,219), PRIVACY.md:3; "No dependency changes" would be FALSE (`@radix-ui/react-accordion`); **`npm run help:shots` AFTER the bump** (two webp still show "v4.5"; L-19); the client package rebuild (the OpenAlgo docx changed); the perf sweep `/trades` through `next start` and `/atlas` on the real DB (L-25 — the smoke copy `data/smoke-v460/` has a FRESH `vault.key`, so its Pro tabs refuse: set `VYUHA_LICENSE_PEM` or read the live data dir read-only); `desktop-dist/vyuha.seed.sqlite` is the 4.5.0 seed — `desktop:build` regenerates it, verify Fyers/Nuvama rows + 0078 in the built seed; the OWNER reviews the Help copy (H4); a LIVE OpenAlgo 2.0.2.6 pull (owner); Fyers/Nuvama files WITH equity rows (owner); the 2027 NSE holiday list + Muhurat 2026 circular; three hook proposals `[PENDING]`; next `/fleet-tune` ~2026-09-30.

### §0.1 Pickup table — one row per open item

| # | Item | Status | Resume from EXACTLY here | Closed when |
|---|---|---|---|---|
| 18 | **v4.6.0** (W1–W9 ✓ → seam pass ✓ → release-level audit ✓ → fix wave ✓ `1c8b393` + `3cdde8d`) | **BUILT + AUDITED 2026-09-26; the RELEASE is next** | `npm run bump-version 4.6.0` → the bump's doc inputs (above) → `npm run help:shots` → the `release` skill step by step (gate, lockfile rule, build + installer freshness grep for a 0078/`ladder_mismatch` marker, signing key id `4FF85F3BBE1DA21D`, CI 6/6 on the tag, install off the build machine, revocations PRERELEASE, SmartScreen, claims audit) with the perf sweep inside it → `npm run release:verify v4.6.0 -- --deep` → **STOP; the owner publishes** | owner publishes v4.6.0 |
| 19 | **v4.7.0** (C1–C2 Edge Clinic → the Clinic TAB of `/reports/edge-clinic` becomes the default, C3 journal Kelly, C4 row 6 Positions tab, C5 Q18 Telegram, C6 native pulls Fyers / Kotak Neo / Nuvama, C7 OpenAlgo WebSocket feed) | SPECIFIED; opens after 4.6.0 is published | spec §3; C1 adds a `clinic` tab to `HUBS` and flips `defaultTab`; a new tab needs a `HELP_ENTRIES` entry AND a `HELP_TASKS` record; C2 asks its two questions at ITS start; C6 needs `vyuha-design-reviewer` first; retire `sector-map.json` there if its fallback-only count (8) is still about zero; the audit's RECORDED LOWs (D-15: `bf-losses.ts:97`, Atlas `bandView`, `ais.ts:81` local-time FY, cockpit date sort, generic-map quadratic, TI-2/3/4/5, RC-3, DA-6) are 4.7.0 candidates | owner publishes v4.7.0 |
| 6 / 8 / 9 / 10 / 17 | Rows folded into 18/19 | all DONE in W1–W7 (archive) | the spec | with their release |
| 11 / 13 | Hook proposals ×3; `fleet-tune` | operator-owed ×3 / DONE 2026-09-23 | `~/.claude/coord/learnings/HOOK-PROPOSALS.md` (all three `[PENDING]`); next tune ~2026-09-30 | markers flipped; weekly |

### §0.2 Decisions taken this session under the decision policy (recorded in DECISIONS 2026-09-26 audit entry and LEDGER D-15; the owner may overrule)
**Thirtieth session (2026-09-26):** `fyDate` REQUIRED and never defaulted on every tax row type; the AIS route stays on `sellDate` (delivery only); `sideAfterEdit` returns `Side | null`; the ladder Data Quality predicate checks QUANTITIES only (values legitimately differ: fill prices vs pairLegs' average); the delete → re-import proof is seam 9c, not a book-op; an explicit default plan is stored as `"default"`; Atlas `bandView` and `/reports/itr` leaving out exited IPOs are RECORDED, not changed; the client README v4.1.0 row stays current (ruling S6b); CL-1 refuted (Fyers/Paytm/Groww OpenAlgo parity is deliberate); nine LOW test findings recorded, not fixed. The earlier sessions' decisions stand.

### §0.3 Verification — PASS and FAIL signatures (V3–V6 of the v4.4.0 era are in the archive)

**V1 — CI on the newest CODE sha.** `gh run list --limit 5`; judge only the run whose `workflowName` is `CI` with 6 jobs. **PASS:** `success`, 6/6. **FAIL-B** — a job red with a timeout / network / cold-runner signature: re-run that job; never re-tag. **FAIL-G** — macOS Playwright red with ONE spec dead on `ECONNREFUSED :3100` and the dev server's memory-threshold restart line: the heap watchdog. **FAIL-H** — BOTH Playwright jobs red on ONE spec with `strict mode violation`: pin both, never `.first()`. **FAIL-K** — a Playwright spec asserting a market-open/closed label or a mark minute: re-pin to the calendar's answer. **FAIL-R** — `z-hubs.spec.ts` red on `config-level redirect: expected 307`: fix the config, never the spec. **FAIL-U** — `zz-console-audit` red on `/settings` or `/atlas` with a React key or render-phase warning: a PRODUCT regression, never a flake. **FAIL-V** — `z-atlas.spec.ts` red on "cache did not grow" or a cohort/dark-state assertion: read the shared-DB order. **FAIL-AH (new)** — `z-help.spec.ts` red on `toBeFocused` for the topic title or the card: the dialog's `onOpenAutoFocus`/close-focus handler regressed — a PRODUCT regression. **FAIL-A / C / D / E / F** as before (archive).

**V2 — the gate.** `rtk proxy npm run verify` (heavy; coord serialises it; a builder NEVER runs it — `vyuha-verifier` does, alone; **FAIL-J**: three or more `timed out` in files the wave does not touch, each green alone → count the machine's processes with `tasklist`, wait, re-run ONCE). **PASS:** exit 0 and the RAW vitest line `Test Files ≥ 505 passed`, `Tests ≥ 11879 passed | 35 skipped` with `VYUHA_LICENSE_PEM` set, no `expected fail`. **FAIL-AI (new)** — a doc/copy pin red on the WINDOWS CI job only, green locally, with `not to match` or a block-terminator miss: the runner checks out CRLF — normalise `
` in the test's read helper (never the doc, never the pin); `tests/copy-pins-v46.test.ts` is the pattern. **FAIL-AE (new)** — `tests/readme-claims.test.ts` "README states a tests/ file count that is not on disk": the DOC's count — fix README in six places (badge, the table, `tests/` line, "N tests", `npm test` row, the tree), never the test. **FAIL-AF (new)** — `tests/golden-books.test.ts` red on a PARENT digest for Paytm or Groww: the fix must not touch parents — a defect in the allocation; red on "positions whose ladder is not their parent": the date-window filter is back. **FAIL-AG (new)** — `tests/itr-page-fmv.test.ts` red or `readers-follow-writers` red on `fmv-per-share`: a consumer reads the per-share FMV raw into a total-unit function. **FAIL-AC / AD** (W7) and **FAIL-Y … D** as before (archive and the W6/W7 blocks).

**V7 — the release-gate Playwright pass:** 120 / 120 in 35 specs on the v4.5.0 tagged tree; 142 flows in 38 specs by `--list` on the fix-wave tree (Builder B ran `e2e/z-help.spec.ts` alone: 8 passed). **PASS signature:** all listed flows passed, `PROBE-END console-audit (0 message(s))`.

**V8 — the release itself.** `npm run release:verify <tag> -- --deep` must print N signatures ALL `4FF85F3BBE1DA21D` and ALL "verifies over"; `gh release list --limit 2` must show the new tag as `Latest` and `gh release view revocations --json isPrerelease` must stay `true`. A background Bash `until` loop on `gh run list --json` is the reliable wait. `$CLAUDE_SCRATCHPAD` is NOT set inside the Bash tool's shell here — write the absolute scratchpad path. The OWNER publishes (stop after `release:verify --deep`). **Before the release build: `npm run help:shots` after the bump (L-19), the `/atlas` timing on the real DB (L-25), the perf sweep's `/trades` median (W6's lever, proven at the query level only), and grep the built bundle for a fix-wave marker (`ladder_mismatch` or `fmvIsMixed`) — v1.12–v1.20 shipped installers frozen at v1.11.**
...
1. ~~Before the audit fix wave~~ — ANSWERED 2026-09-26 (thirtieth session): plan approved; MO-4 fixed now; MO-1 = the recorded rule. 2. Before C2 (v4.7.0): the setup-grade labels, and whether intra-trade high/low is typed or later auto-filled. 3. Before C6: the first real Fyers `/trade-history` pull pins its fields; Kotak Neo stays "documented, not verified" (B2). 4. Still open from before: whether the A4 / A7 decisions and the sixteenth session's §0.2 stand; row 5 / the derivative-marks schema release (archive §0.4 item 3). 5. Noted, not asked: BSE SME companies (486) DO get an AMFI band (small) — U3 names NSE Emerge only. 6. Noted, not asked: a Fyers tradebook or a Nuvama report WITH equity rows, whenever one exists. 7. **Owner review owed by ruling H4:** the Help copy in `lib/domain/help-topics.ts` (the task-first cards) and `lib/domain/glossary.ts` (35 terms; 8 terms are used by no card — DA-6). 8. Noted, not asked: the "backup taken" strip step counts a backup on THIS machine only. 9. **Noted, not asked (W5):** `COVERAGE_FLOOR_PPM` 30%, the cohort floor 5 priced / 60% coverage and the group floors 3 / 8 are PROPOSALS. 10. **Noted, not asked (W6):** the legacy F&O pair is joined through the confirm dialog, and an uncovered derivative sale ALWAYS stays an opening sell. 11. **Noted, not asked (audit):** existing Paytm/Groww books keep their over-counted ladders until the user deletes and re-imports (dedup is parent-only); the `ladder_mismatch` Data Quality line names them — say so if an in-place rebuild is wanted instead; and `/reports/itr` leaves out exited IPOs while `/reports/tax` folds them in (pre-existing) — say which is right. 12. **Before C4:** whether the round-3 Positions canvas is built AS SHOWN or with changes (spec §4).
---
...
(Windows; no macOS build — macOS is never advertised) with a Next.js standalone server running as a Node sidecar. Commercial
...
## 2-published. v4.2.0 PUBLISHED 2026-09-09 01:48 IST (tag `v4.2.0` = `9da7bc8` → `b488dde`; CI 34267807794 6/6; release run 34268619222 3/3; deep verify 3/3; `releases/latest` → v4.2.0; installed off the build machine, both feeds smoke-tested; WDSI submitted; winget HELD by #421585) — HISTORY: it was the live release until v4.3.0; §0 names the current one
