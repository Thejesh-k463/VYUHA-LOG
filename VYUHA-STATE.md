...
§0 below is a LATER reconciliation record (2026-09-25) and outranks §2.
...
## §0 START HERE — reconciled 2026-09-25 IST by the twenty-seventh session (v4.6.0 wave W5 BUILT — Q51 cohort analytics + Atlas panel v3: `atlas-core/2.0.0`, industry cohorts, relative strength vs the market median, the freshness catch-up, regime thresholds in Settings; the twenty 06a AQ questions ANSWERED first): **v4.5.0 is the published release and is CLOSED** (tag `v4.5.0` = `b6b8863`; NO REWORK on any closed item). **v4.6.0 is IN BUILD: W1 (market calendar), W8 (OpenAlgo safety), W2 (stock universe), W9 (Fyers + Nuvama), W3 (analytics hubs), W4 (Help + strip) and W5 (Q51 + Atlas v3) are DONE and committed; W6 (row 9: `trades.side`, the `/trades` speed lever, `--ref`) is NEXT and needs `vyuha-design-reviewer` FIRST, then W7.** Only v4.6.0 and v4.7.0 carry everything left (rulings B1–B6). Fixed in the tree, not yet released: the day's mark waits for the official close (W1); an OpenAlgo pull refuses sandbox, pre-2.0.2.6, `NCO`/`NCDEX` and zero-quantity MCX rows (W8); sectors come from the exchanges' own classification and the cap band is AMFI's (W2); Fyers and Nuvama are brokers with rate cards and native file parsers (W9); seven analytics screens are tabs of three hubs and their old URLs answer a real 307 (W3); every help entry carries task-first steps, `/help` is search-first, every page header has a "?" (W4); the Atlas computes the MEDIAN group return (mean behind a labelled toggle), industry cohorts with "n of m priced", RS vs the market median, rank Δ that prints its snapshot shortfall, the honesty list, regime thresholds a user can edit, and a polite catch-up of missing sessions under the existing auto-MTM consent (W5); a pre-existing duplicate React key on every `/settings` render is gone (W5 fix wave). Two hook proposals still `[PENDING]`.
...
4. `AGENTS.md` before any code change (on conflict AGENTS.md wins over this file; the code wins over both). Owner rulings: the spec's §0 table; `docs/DECISIONS.md` newest entries at the END (grep `^## 2026-09-2`, never read it whole — the newest are the 2026-09-25 **W5 built** entry and the **W5 opened** entry (the twenty AQ answers), with the W4, W3, W9, W2, W8, W1 and brokers entries before them) — binding, never re-ask one.
5. §3 of this file for where any other answer lives. Every earlier §0 block is in `VYUHA-STATE-ARCHIVE.md`, byte-for-byte (the twenty-sixth session's block archived by this session).

**The one-line state (re-verified 2026-09-25 by the twenty-seventh session at its close: the W5 build commit `the W5 build commit (sha named by the close-out commit; see the session log)` sits on `762974c` (the W4 close-out) and this close-out docs commit sits on it — the CI run on `the W5 build commit (sha named by the close-out commit; see the session log)` is recorded by the close-out commit, both shas are in the session log; newest migration **0076** (`atlas_regime_thresholds`; W6's `side` is 0077); tag v4.5.0 = `b6b8863`; `releases/latest` v4.5.0):** published = **v4.5.0**. **Gate on the W5 tree: `npm run verify` (vyuha-verifier, alone) EXIT 0 — 489 files / 11,579 passed / 35 skipped**, lint 0 errors (6 pre-existing warnings), `next build` "Compiled successfully"; 35 skipped = the owner-file cases with `VYUHA_LICENSE_PEM` set (unset: 5 fewer passed, 5 more skipped). Playwright: **141 flows in 38 specs** by `--list` (z-atlas adds 6); run locally this session: `z-atlas` 6/6 and `zz-console-audit` 7 pages / 0 messages (the audit now visits `/atlas` and `/settings`). `package-lock.json` and `package.json` UNCHANGED (no new dependency).

- **What the twenty-seventh session did:** FIRST asked the twenty unanswered 06a AQ questions in two groups of ten (the owner took the recommended option on all twenty — DECISIONS "W5 opened"; Q51's nine decided by the session), then W5 from a written contract (scratchpad `W5-CONTRACT.md`) after a scout mapped the surface and `vyuha-design-reviewer` returned REVISE with five confirmed defects, all folded into the contract before the builder (rank Δ needs `atlas_metric JOIN atlas_daily` for the spec version; one gap map over the ALIGNED universe; the regime re-derived at read time; the index filter never cached; the catch-up envelope needs a dead-run reset). ONE builder in two phases (Fable this session by owner ruling): phase 1 maths/persistence/jobs/settings (six new pure modules, `computeGroupTable`, `getRankDeltas`, `getEntryDayBreadth`, `GET /api/atlas/view?index=`, `lib/jobs/bhavcopy-catchup.ts`, migration 0076, the `type:"atlas-regime"` settings branch), phase 2 UI/help/docs/e2e (`rotation-table`, `cohort-table`, `not-computed-list`, `atlas-regime-editor`, `MyNameRow` deleted, `AtlasView.windowSpans`, `e2e/z-atlas.spec.ts`). Skeptic: 15 claims, 13 CONFIRMED / 2 REFUTED (the dead-run threshold ≤ one fetch timeout; a cleared editor field posted as 0) → a five-item fix wave that also found and fixed the pre-existing duplicate React key in `app/layout.tsx`. LEDGER D-11, D-12, L-21…L-27, F-19…F-24, U-9, U-10.
- **What WRONG looks like from here:** `/atlas` still showing "equal-weighted mean" as the shipped statistic or an unlabelled mean/median; a cohort row without "n of m priced" or without the level it used; a rank Δ that prints a number (or 0) with fewer than 21 daily snapshots under `atlas-core/2.0.0`, or one that mixes a 1.0.0 row; a symbol excluded on the Rotation tab for a corporate-action gap that still appears in a cohort or RS row; a filtered (`?index=`) view leaving a row in `atlas_daily` / `atlas_metric`; a tile printing a percentage over < 30% coverage instead of the coverage sentence, or a regime label voting on an input the tile beneath it blanks; `GET /api/atlas` and the page disagreeing on the regime after a threshold edit; a `running` backfill envelope that survives 36 s of silence and blocks the button; two bhavcopy walkers at once; `lib/jobs/bhavcopy-catchup.ts` containing a URL; the word "money" or a rating verb anywhere under `components/atlas/`; a React key warning on `/settings` or `/atlas` (`zz-console-audit` is now the guard — a red there is a product regression, never a flake); `package-lock.json` with any change; a wave tag in git; `OPENALGO_DISCLOSURE_VERSION` bumped before v4.6.0 ships.
- **Owed outside the build:** the OWNER reviews the Help copy (ruling H4) — now including the rewritten `/atlas` card; **re-run `npm run help:shots` AFTER `npm run bump-version 4.6.0`** and before the release build (L-19; the Atlas shot was re-taken this session on the v3 panel — 942 KB, 14 files); **measure `/atlas` on the owner's real DB before the release** (L-25: ~3 s steady-state on 1,900 × 300 stored bars because the cache checksum re-reads every bar; the lever is not built); a LIVE OpenAlgo 2.0.2.6 preview pull (owner); a Fyers tradebook and a Nuvama report WITH equity rows whenever the owner has them; Q24's citation for the release claims audit; the 2027 NSE holiday list + the Muhurat 2026 circular when NSE publishes them; two hook proposals in `~/.claude/coord/learnings/HOOK-PROPOSALS.md`, both `[PENDING]`; next `/fleet-tune` ~2026-09-30.

### §0.1 Pickup table — one row per open item

| # | Item | Status | Resume from EXACTLY here | Closed when |
|---|---|---|---|---|
| 18 | **v4.6.0** (W1 calendar ✓ → W8 OpenAlgo safety ✓ → W2 universe + row 10 ✓ → W9 Fyers + Nuvama ✓ → W3 hubs ✓ → W4 Help + strip ✓ → W5 Q51/Atlas v3 ✓ → **W6 row 9** → W7 row 17) | **W1 + W8 + W2 + W9 + W3 + W4 + W5 DONE 2026-09-25; W6 next** | spec §2 W6 — `vyuha-design-reviewer` FIRST over the `trades.side` design (migration **0077**; backfill from the existing derivation, then every `sellQty > buyQty` re-derivation reads the column; the four invariant guards + `harness-book-sequences` gate it; W6 also owns overnight F&O shorts — `pairLegs` files them `opening-sell` + `open`, LEDGER D-8); the `/trades` speed lever is MEASURED first (`app/trades/page.tsx:50`) and built only if the number justifies it; referral `--ref` in `scripts/license-issue.mjs` + `license-mint.mjs` `ref` + `license-list.mjs --by-ref` + the share-card footer. W6 owes no question group (spec §4). W7 asks its two questions at ITS start | owner publishes v4.6.0 |
| 19 | **v4.7.0** (C1–C2 Edge Clinic → the Clinic TAB of `/reports/edge-clinic` becomes the default, C3 journal Kelly, C4 row 6 Positions tab, C5 Q18 Telegram, **C6 native pulls Fyers / Kotak Neo / Nuvama**, **C7 OpenAlgo WebSocket feed**) | SPECIFIED; opens after 4.6.0 is published | spec §3; C1 adds a `clinic` tab to `HUBS` and flips `defaultTab`; a new tab needs a `HELP_ENTRIES` entry AND a `HELP_TASKS` record; C6 needs `vyuha-design-reviewer` first; retire `sector-map.json` there if its fallback-only count (8 on 2026-09-25) is still about zero | owner publishes v4.7.0 |
| 6 / 8 / 9 / 10 / 17 | Rows folded into 18/19 | row 6 → v4.7.0 C4; row 8 → Q51 + Atlas v3 DONE in W5, Kelly (C3), Q18 (C5); row 9 → strip DONE in W4, `side` + speed lever + `--ref` in W6; row 10 → DONE in W2; row 17 → W7 | the spec | with their release |
...
### §0.2 Decisions taken this session under the decision policy (recorded in DECISIONS 2026-09-25 W5 built entry; the owner may overrule)
**Twenty-seventh session (2026-09-25):** both statistics persisted per group per window so the mean toggle is a READ; rank Δ's denominator is DAILY SNAPSHOTS read through `atlas_metric JOIN atlas_daily … spec_version` (the detail table has no version column); ONE gap map (`buildGapMap`) over the aligned universe for rotation, cohort, RS and the trades join; the cohort level is chosen ONCE per symbol on the 1m priced count and `groupByLevel` never falls up (fall-up is the cohort's rule); a user tag is sector-only and REPLACES the taxonomy row; the dead `tier` field is deleted; the regime label is re-derived at read time in `getVerifiedSnapshot()` with the current thresholds and `classifyRegime` refuses ANY input below the 30% coverage floor; `atlas_regime_thresholds` travels WITH the backup and joins `BASELINE_SETTINGS_FIELDS`; the catch-up is gated on `autoMtmEnabled` ONLY (the backfill ack is consent to a button), excludes the top-up's own date, re-reads consent + abort per file, and shares one process lock and a DERIVED dead-run threshold (2 × fetch timeout + 4 × rate = 36 s) with the backfill, which now heartbeats before every fetch; the index filter computes in memory and never writes the cache; `2m` is gone; `COVERAGE_FLOOR_PPM`, the 5 / 60% cohort floor and the 3 / 8 group floors are PROPOSALS; Q51's nine questions decided per the pack (no new consent line — PRIVACY item 2 restates the cadence); `zz-console-audit` visits `/atlas` and `/settings`; three one-line edits outside the builders' file sets were accepted (`default-settings-card.tsx` label, `atlas-page.test.ts` key list, `app/settings/page.tsx` constants) and `app/layout.tsx`'s duplicate keys were fixed in the same wave. The earlier sessions' decisions (DECISIONS W1, brokers, W8, W2, W9, W3, W4 entries) stand.

### §0.3 Verification — PASS and FAIL signatures (V3–V6 of the v4.4.0 era are in the archive)

**V1 — CI on the newest CODE sha.** `gh run list --limit 5`; judge only the run whose `workflowName` is `CI` with 6 jobs. **PASS:** `success`, 6/6. **FAIL-B** — a job red with a timeout / network / cold-runner signature: re-run that job; never re-tag. **FAIL-G** — macOS Playwright red with ONE spec dead on `ECONNREFUSED :3100` and the dev server's memory-threshold restart line: the heap watchdog. **FAIL-H** — BOTH Playwright jobs red on ONE spec with `strict mode violation`: pin both, never `.first()`. **FAIL-K** — a Playwright spec asserting a market-open/closed label or a mark minute: re-pin to the calendar's answer. **FAIL-R** — `z-hubs.spec.ts` red on `config-level redirect: expected 307`: fix the config, never the spec. **FAIL-U (new)** — `zz-console-audit` red on `/settings` or `/atlas` with a React key or render-phase warning: a PRODUCT regression (find the keyed map; React 19 does not name the component in the text — use a CDP owner-stack capture), never a flake. **FAIL-V (new)** — `z-atlas.spec.ts` red on "cache did not grow" or on a cohort/dark-state assertion: an earlier spec now visits `/atlas` or seeds bars — read the shared-DB order before touching the spec. **FAIL-A / C / D / E / F** as before (archive).

**V2 — the gate.** `rtk proxy npm run verify` (heavy; coord serialises it; a builder NEVER runs it — `vyuha-verifier` does, and NOT while another agent is loading the machine — FAIL-J). **PASS:** exit 0 and the RAW vitest line `Test Files ≥ 489 passed`, `Tests ≥ 11,579 passed | 35 skipped` with `VYUHA_LICENSE_PEM` set, no `expected fail`. **FAIL-W (new)** — `tests/atlas-daily.test.ts` / `atlas-query.test.ts` red on `spec_version` or "2m": a formula-set change without the `SPEC_VERSION` bump, or a stored-row read that skips the `atlas_daily` JOIN — bump the version / add the join, never loosen the pin. **FAIL-X (new)** — `tests/atlas-copy.test.ts` red on the forbidden-words scan or the median wording: product copy drifted into advice or the shipped statistic was renamed — fix the copy. **FAIL-T** — `tests/help-content.test.ts` red on "its webp exists": run `npm run help:shots`, never delete the pin. **FAIL-S** — `tests/hubs.test.ts` / `pro-gating.test.ts` red after a screen is added, moved or re-keyed: derive from `HUBS` / `navGroupHrefs`, never a second literal list. **FAIL-Q** — a broker/plan set pin red after a broker or plan is added: re-derive by RUNNING the seed. **FAIL-P** (`stock-universe.test.ts` after a refresh: rebuild both snapshots, never loosen a floor), **FAIL-M** (OpenAlgo request-list pins), **FAIL-N** (a dotted version in PRIVACY item 3), **FAIL-O** (a CRLF-blind doc-copy regex), **FAIL-J** (a 5 s / 30 s timeout under load: check free memory and concurrent agents, re-run the file alone; `search-route` "answers no-store" carries a 15 s timeout since W4), **FAIL-L** (a timing literal outside the calendar), **FAIL-E** (README's counts — fix the prose; a redirect-only `page.tsx` is not a screen), **FAIL-I / A / B / C / D** as before.

**V7 — the release-gate Playwright pass:** 120 / 120 in 35 specs on the v4.5.0 tagged tree; 141 in 38 by `--list` on the W5 tree; `z-atlas` (6) and `zz-console-audit` (7 pages) ran locally this session. **PASS signature:** `141 passed`, `PROBE-END console-audit (0 message(s))`. A red `zz-console-audit` is a product regression, never a flake.

**V8 — the release itself.** `npm run release:verify <tag> -- --deep` must print N signatures ALL `4FF85F3BBE1DA21D` and ALL "verifies over"; `gh release list --limit 2` must show the new tag as `Latest` and `gh release view revocations --json isPrerelease` must stay `true`. A background Bash `until` loop on `gh run view --json status` is the reliable wait. `$CLAUDE_SCRATCHPAD` is NOT set inside the Bash tool's shell here — write the absolute scratchpad path. The OWNER publishes (stop after `release:verify --deep`). **Before the release build: `npm run help:shots` after the bump (L-19), and the `/atlas` timing on the real DB (L-25).**

**V9 — a dev-server probe.** Before blaming a wave for a 404, probe an UNCHANGED sibling route; if it 404s too with nothing in the log, the Turbopack cache is stale: stop the server, delete `.next/dev`, restart (L-18). Prove a redirect with `curl -s -o /dev/null -w '%{http_code} %{redirect_url}'`, never by reading `redirect()`.

### §0.4 Questions the owner still has to answer
1. **Answered 2026-09-25 (twenty-seventh session, W5):** the twenty 06a AQ questions — every one the recommended option (DECISIONS "W5 opened"). 2. **Before W7 (asked at W7's start):** harvest `sttSplit` counting (recommended: one per ladder, fills listed beneath) and what "accepted" means for the ~₹1 Upstox residual. 3. Before C2 (v4.7.0): the setup-grade labels, and whether intra-trade high/low is typed or later auto-filled. 4. Before C6: the first real Fyers `/trade-history` pull pins its fields; Kotak Neo stays "documented, not verified" (B2). 5. Still open from before: whether the A4 / A7 decisions and the sixteenth session's §0.2 stand (the `stated-bill` note marker rather than a column); row 5 / the derivative-marks schema release (archive §0.4 item 3). 6. Noted, not asked: BSE SME companies (486) DO get an AMFI band (small) — U3 names NSE Emerge only. 7. Noted, not asked: a Fyers tradebook or a Nuvama report WITH equity rows, whenever one exists. 8. **Owner review owed by ruling H4:** the Help copy in `lib/domain/help-topics.ts` (50 cards, the `/atlas` card rewritten this session) and `lib/domain/glossary.ts` (35 terms). 9. Noted, not asked: the "backup taken" strip step counts a backup on THIS machine only. 10. **Noted, not asked (W5):** `COVERAGE_FLOOR_PPM` 30%, the cohort floor 5 priced / 60% coverage and the group floors 3 / 8 are PROPOSALS — say so if a different floor is wanted; the W5 sessions changed no owner-visible default except the group statistic (median), which the owner chose (AQ18). 11. **Noted, not asked (W5):** W6 is the next design-reviewed identity wave (`trades.side`); W6 and W7 touch disjoint files and COULD run as two one-builder waves side by side after their reviews — the cost is a seam pass and consecutive migrations (0077, 0078); say so if that split is wanted.
---
## 1. What Vyuha is
...
whether OpenAlgo's "Kotak Securities" is Neo or legacy (ANSWERED 2026-09-25: Neo — research R8 §2 #46), and the dialog's rendered appearance are all
...
| How do I open a new session here? | **§0 START HERE** (top of this file), then its read order: the current build folder's `00-SPEC-PLAN.md` and the paste block at the bottom of its `SESSION-LOG.md` (today `VYUHA/LIVE-DESK-RESEARCH/22-V460-BUILD/`). `VYUHA/LIVE-DESK-RESEARCH/NEXT-SESSION-CONTINUATION.md` is HISTORY (superseded 2026-09-24, banner at its top). `docs/SESSION_PROMPT.md` keeps the standing RULES (its ready-to-paste block is superseded, banner at its top) |
...
| What v3.9.1 / v4.0 "Live Desk" was planned from | `T:/Thejesh/CLAUDE-CODE/VYUHA/LIVE-DESK-RESEARCH/00-INDEX.md` — research pack (its "next step" lines are history); `06-ANSWERS.md` holds every owner ruling up to v4.5.0, newest table LAST — binding, never re-ask; the v4.6.0/v4.7.0 rulings are the spec's §0 table (06-ANSWERS carries a pointer section) |
| What is already built vs still left (so nothing is redone) | From v4.6.0: **`docs/LEDGER.md`** (in the repo; tagged [LESSON]/[FIX]/[UPGRADE]/[RESEARCH]/[DECISION] rows; owner ruling R1, 2026-09-24) + §0's pickup table. `VYUHA/LIVE-DESK-RESEARCH/09-BUILD-LEDGER.md` (owner ruling Q3) is the ledger up to v4.3.0 — history, banner at its top |
| What v4.6.0 / v4.7.0 build, and the owner's rulings for them | **`VYUHA/LIVE-DESK-RESEARCH/22-V460-BUILD/00-SPEC-PLAN.md`** §0 (binding) and §2–§3 (waves); `research/R0–R4` beside it are the evidence — where research disagrees with the spec's §0, §0 wins |
...
### 8.00 Open Positions card redesign for `/live` (owner-driven, 2026-09-06; NOT built — SCHEDULED 2026-09-24 as v4.7.0 wave C4 with rulings P1 (NSE 4-level Industry cohort) and P2 (inline sizing calculator); status lives in §0's pickup table, the plan in `VYUHA/LIVE-DESK-RESEARCH/17-POSITIONS-TAB-PLAN.md`)
...
(page "Blend (round 2)"). Plan, constraints and open questions: `VYUHA/LIVE-DESK-RESEARCH/17-POSITIONS-TAB-PLAN.md`
(moved from the continuation file's §3 on 2026-09-11). Ships in v4.7.0 (wave C4); the free/Pro map, SEBI copy rules and the
