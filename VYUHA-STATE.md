...
## §0 START HERE — reconciled 2026-09-26 IST by the thirty-first session (v4.6.0 TAGGED and DEEP-VERIFIED — tag `v4.6.0` = `37408e3`, the DRAFT release is built and signed; **the OWNER publishes**): **v4.5.0 is still the published release (`releases/latest`) until the owner flips the v4.6.0 draft to Latest**; **v4.6.0 is BUILT, AUDITED, TAGGED and VERIFIED — nothing in the tree is owed before publish.** Only v4.7.0 carries everything left (rulings B1–B6). What the release session measured beyond the build: the upgrade path on a copy of the OWNER's real journal (75 → 79 migrations, 161 sides re-keyed, 16 margin rows by 0078, 280 rate rows by the launcher's refresh, integrity ok); `/atlas` on that copy 84–230 ms (no cached bars — L-25 does not arise on this book); the perf sweep's three over-budget routes (`/`, `/strategies`, `/settings`) are PRE-EXISTING — v4.5.0 built from its tag and swept on the same machine minutes apart breaches `/` and `/settings` too (LEDGER L-51, R-13, D-16) — while `/trades` improved 1,028 → 772 ms (W6's lever at the page level). Three hook proposals still `[PENDING]`.
...
2. `VYUHA/LIVE-DESK-RESEARCH/22-V460-BUILD/00-SPEC-PLAN.md` — the v4.6.0/v4.7.0 spec and wave plan (rulings table §0, waves §2–§3 — every §2 wave carries a BUILT line, questions owed §4); its `research/R0…R4` and `VYUHA/LIVE-DESK-RESEARCH/23-BROKERS-OPENALGO-2026-09-25/R5…R9` are the evidence (never re-run them). `docs/LEDGER.md` is the tagged record ([LESSON] [FIX] [UPGRADE] [RESEARCH] [DECISION]) — grep it before building anything; L-51, L-52, R-13 and D-16 are the release session's. `22-V460-BUILD/SESSION-LOG.md` is the per-session ledger and carries the ONE paste block. Research is OUTSIDE the repo — cite it as `VYUHA/LIVE-DESK-RESEARCH/…`, never as a repo-relative path (LEDGER L-4).
3. Re-derive before asserting: `git log --oneline -3` · `git tag --sort=-creatordate | head -1` (**v4.6.0**) · `package.json` `"version"` (**4.6.0**) · `gh run list --limit 3` (a plain list, never `--commit`) · `gh release list --limit 2` (**v4.5.0 Latest until the owner publishes; v4.6.0 shows as Draft**) · the RAW vitest line (§0.3 V2) · `npm run drift:close-out` (0 FAIL before `/clear`; a FAIL is fixed in the DOC, never in the check).
4. `AGENTS.md` before any code change (on conflict AGENTS.md wins over this file; the code wins over both). Owner rulings: the spec's §0 table; `docs/DECISIONS.md` newest entries at the END (grep `^## 2026-09-2`, never read it whole — the newest is the 2026-09-26 **v4.6.0 TAGGED** entry, then the release-level audit entry) — binding, never re-ask one. The release procedure is the `release` skill; from v4.6.0 the session STOPS after `release:verify --deep` and the owner publishes (DECISIONS 2026-09-23).
5. §3 of this file for where any other answer lives. Every earlier §0 block is in `VYUHA-STATE-ARCHIVE.md`, byte-for-byte (the thirtieth session's block archived by this session).

**The one-line state (re-verified 2026-09-26 by the thirty-first session at its close: the bump + release-copy commit `37408e3` sits on `1dfaa24` (the audit close-out); CI on `37408e3` = 36222263293 SUCCESS 6/6 (first pass); annotated tag `v4.6.0` = `37408e3`; release run 36223372445 SUCCESS 3/3 (Windows x64 14 min, macOS Apple silicon 11 min, macOS Intel 26 min); the DRAFT holds 8 assets + latest.json; `npm run release:verify v4.6.0 -- --deep`: EXIT 0 — 3 signatures, key id `4FF85F3BBE1DA21D`, each "verifies over" the published bytes (35.1 / 62.6 / 65.0 MB, prehashed), "Safe to publish"; `gh release list` = v4.5.0 Latest + v4.6.0 Draft; `revocations` `isPrerelease=true`; newest migration **0078**):** published = **v4.5.0** (until the owner clicks). **Gate on the bumped tree: `npm run verify` (vyuha-verifier, alone, ONE run) EXIT 0 — 505 files / 11879 passed / 35 skipped**, lint 0 errors (6 pre-existing warnings), `next build` "Compiled successfully in 10.7s", 137.39 s. 35 skipped = the owner-file cases with `VYUHA_LICENSE_PEM` set. Playwright: **142 flows in 38 specs** (unchanged; not re-run — the tree's code is the audited tree plus the footer string). `package-lock.json`: the two root version fields only (`git diff --numstat HEAD~1` 2/2; `npm ls esbuild` resolves). The local desktop build: `desktop-dist/.next/BUILD_ID` 11:12:23 IST, `ladder_mismatch` in 4 server chunks, `.sig` key id `4FF85F3BBE1DA21D`, the built seed 79 migrations / 280 Fyers+Nuvama rate rows / 16 margin rows; client ZIP `release-packages/Vyuha_4.6.0_Client_Package.zip`, installer SHA-256 `08F481E17E55796D8000EB2D2027B1A59AAE1DFADCBC801A6177BA667928DD8F` (certutil = CHECKSUMS.txt = the local NSIS exe).

- **What the thirty-first session did:** the gate → the bump (four files + Cargo.lock + the lock's two root fields by hand) → ONE Opus builder wrote the release copy from LEDGER U-1…U-17 / F-1…F-50 (ten doc files; broker count 8 derived from `lib/import/registry-meta.ts`; one `privacy-feed-disclosure` doc fix) → `npm run help:shots` after the bump (14 webp, footer v4.6 read back) → `desktop:build` → `client:docx` + `client:package` → `vyuha-verifier` → commit `37408e3`, CI 6/6 → the perf sweep (denied once by coord while HUB was active, then run twice) and the v4.5.0 A/B in a throwaway worktree → tag → the release run → `release:verify --deep` → STOP. `vyuha-release-steward` walked the 11 steps read-only (0 stop-the-line; claims audit 6 surfaces / 0 contradictions; WDSI form in DECISIONS). LEDGER L-51, L-52, R-13, D-16; DECISIONS 2026-09-26 v4.6.0 TAGGED entry.
- **What WRONG looks like from here:** `gh release list` showing v4.6.0 as Latest before the owner said "published" (a session clicked — forbidden since 2026-09-23); `revocations` `isPrerelease=false` (it stole `releases/latest`); `latest.json` serving a version other than 4.6.0 for any of the six platforms after publish; a `.sig` whose key id is not `4FF85F3BBE1DA21D`; `release:verify --deep` red while key ids pass (an asset re-written after signing — delete the draft and re-run the workflow, never re-upload); a `desktop-dist/vyuha.seed.sqlite` with 0 Fyers/Nuvama rows; a v4.6.0 install whose footer reads v4.5 (a frozen bundle); the dashboard, `/strategies` or `/settings` sweep medians moving by MORE than the v4.5.0 A/B spread (L-51) — that would be new; plus every audit fix-wave signature (archive).
- **Owed AFTER the owner publishes (the next session's first act):** the post-publish proofs — `gh release list --limit 2` (v4.6.0 Latest), `latest.json` for six platforms, `revocations` still prerelease, the live landing page (fetch `sales/landing-page.html` itself — the root is a meta-refresh shell) — then `npm run winget:manifest -- --sha <GitHub asset digest>` into `release-packages/winget/4.6.0` (HELD, never submitted by a session), the OWNER's non-build-machine install + smoke, WDSI (the form is in DECISIONS), the OWNER's Help review (H4), a LIVE OpenAlgo 2.0.2.6 pull (owner), Fyers/Nuvama files WITH equity rows (owner), the 2027 NSE holiday list + Muhurat 2026 circular, three hook proposals `[PENDING]`, next `/fleet-tune` ~2026-09-30; then v4.7.0 opens (§0.1 row 19). The Pages deployment on `37408e3` already shows v4.6.0 on the public landing page while `releases/latest` is v4.5.0 — the same transient as the v4.5.0 release, closed by publishing.

### §0.1 Pickup table — one row per open item

| # | Item | Status | Resume from EXACTLY here | Closed when |
|---|---|---|---|---|
| 18 | **v4.6.0** (W1–W9 ✓ → seam pass ✓ → release-level audit ✓ → fix wave ✓ → bump + copy `37408e3` ✓ → tag ✓ → draft built + deep-verified ✓) | **TAGGED + VERIFIED 2026-09-26; the OWNER publishes** | the owner flips the draft to Latest → the next session runs the post-publish proofs (above) and the winget manifest | owner reports "published" and the proofs pass |
| 19 | **v4.7.0** (C1–C2 Edge Clinic → the Clinic TAB of `/reports/edge-clinic` becomes the default, C3 journal Kelly, C4 row 6 Positions tab, C5 Q18 Telegram, C6 native pulls Fyers / Kotak Neo / Nuvama, C7 OpenAlgo WebSocket feed) | SPECIFIED; opens after 4.6.0 is published | spec §3; C1 adds a `clinic` tab to `HUBS` and flips `defaultTab`; a new tab needs a `HELP_ENTRIES` entry AND a `HELP_TASKS` record; C2 asks its two questions at ITS start; C6 needs `vyuha-design-reviewer` first; retire `sector-map.json` there if its fallback-only count (8) is still about zero; the audit's RECORDED LOWs (D-15: `bf-losses.ts:97`, Atlas `bandView`, `ais.ts:81` local-time FY, cockpit date sort, generic-map quadratic, TI-2/3/4/5, RC-3, DA-6) and the release session's perf candidates (D-16: the dashboard's 13.1 MB `DashTrade` payload at 25k rows, `/strategies`' 6 MB leg arrays and its 1,323 → 1,522–1,769 ms move) are 4.7.0 candidates | owner publishes v4.7.0 |
...
### §0.2 Decisions taken this session under the decision policy (recorded in DECISIONS 2026-09-26 v4.6.0 TAGGED entry and LEDGER D-16; the owner may overrule)
**Thirty-first session (2026-09-26):** the three over-budget sweep routes are NOT a stop-ship — `/` and `/settings` breach at v4.5.0 on the same machine and `/strategies`' move (1,323 → 1,522–1,769, ONE v4.5.0 sample) is recorded as a 4.7.0 candidate; the dashboard payload is a v4.7.0 candidate, not a release-session fix; the tag went on the bump + copy commit itself (CI 6/6 on it; the v4.5.0 precedent tagged the docs commit for the same reason — the sha with the green run); `fmvIsMixed` is not a bundle marker (minified away) — `ladder_mismatch` is; the landing page's early v4.6.0 (Pages deploys on push) is accepted as the v4.5.0 precedent's transient. The earlier sessions' decisions stand.

### §0.3 Verification — PASS and FAIL signatures (V3–V6 of the v4.4.0 era are in the archive)

**V1 — CI on the newest CODE sha.** `gh run list --limit 5`; judge only the run whose `workflowName` is `CI` with 6 jobs. **PASS:** `success`, 6/6. **FAIL-B** — a job red with a timeout / network / cold-runner signature: re-run that job; never re-tag. **FAIL-G** — macOS Playwright red with ONE spec dead on `ECONNREFUSED :3100` and the dev server's memory-threshold restart line: the heap watchdog. **FAIL-H** — BOTH Playwright jobs red on ONE spec with `strict mode violation`: pin both, never `.first()`. **FAIL-K** — a Playwright spec asserting a market-open/closed label or a mark minute: re-pin to the calendar's answer. **FAIL-R** — `z-hubs.spec.ts` red on `config-level redirect: expected 307`: fix the config, never the spec. **FAIL-U** — `zz-console-audit` red on `/settings` or `/atlas` with a React key or render-phase warning: a PRODUCT regression, never a flake. **FAIL-V** — `z-atlas.spec.ts` red on "cache did not grow" or a cohort/dark-state assertion: read the shared-DB order. **FAIL-AH** — `z-help.spec.ts` red on `toBeFocused` for the topic title or the card: the dialog's `onOpenAutoFocus`/close-focus handler regressed — a PRODUCT regression. **FAIL-A / C / D / E / F** as before (archive).

**V2 — the gate.** `rtk proxy npm run verify` (heavy; coord serialises it; a builder NEVER runs it — `vyuha-verifier` does, alone; **FAIL-J**: three or more `timed out` in files the wave does not touch, each green alone → count the machine's processes with `tasklist`, wait, re-run ONCE). **PASS:** exit 0 and the RAW vitest line `Test Files ≥ 505 passed`, `Tests ≥ 11879 passed | 35 skipped` with `VYUHA_LICENSE_PEM` set, no `expected fail`. **FAIL-AI** — a doc/copy pin red on the WINDOWS CI job only, green locally, with `not to match` or a block-terminator miss: the runner checks out CRLF — normalise `\r\n` in the test's read helper (never the doc, never the pin); `tests/copy-pins-v46.test.ts` is the pattern. **FAIL-AE** — `tests/readme-claims.test.ts` "README states a tests/ file count that is not on disk": the DOC's count — fix README in six places (badge, the table, `tests/` line, "N tests", `npm test` row, the tree), never the test. **FAIL-AF** — `tests/golden-books.test.ts` red on a PARENT digest for Paytm or Groww: the fix must not touch parents — a defect in the allocation; red on "positions whose ladder is not their parent": the date-window filter is back. **FAIL-AG** — `tests/itr-page-fmv.test.ts` red or `readers-follow-writers` red on `fmv-per-share`: a consumer reads the per-share FMV raw into a total-unit function. **FAIL-AC / AD** (W7) and **FAIL-Y … D** as before (archive and the W6/W7 blocks).

**V7 — the release-gate Playwright pass:** 120 / 120 in 35 specs on the v4.5.0 tagged tree; 142 flows in 38 specs by `--list` on the fix-wave tree (Builder B ran `e2e/z-help.spec.ts` alone: 8 passed); CI's two Playwright jobs green on `37408e3`. **PASS signature:** all listed flows passed, `PROBE-END console-audit (0 message(s))`.

**V8 — the release itself.** `npm run release:verify <tag> -- --deep` must print N signatures ALL `4FF85F3BBE1DA21D` and ALL "verifies over"; after the owner publishes, `gh release list --limit 2` must show the new tag as `Latest` and `gh release view revocations --json isPrerelease` must stay `true`. A background Bash `until`/`for` loop on `gh run list --json` is the reliable wait (a 10-minute Bash timeout ends it early — restart it, it is idempotent). `$CLAUDE_SCRATCHPAD` is NOT set inside the Bash tool's shell here — write the absolute scratchpad path. The OWNER publishes (stop after `release:verify --deep`). **Before the release build, every release:** `npm run help:shots` after the bump (L-19), grep the built bundle for a fix-wave marker that survives minification (a string literal such as `ladder_mismatch`, never an identifier), the built seed's broker rows, the perf sweep with an A/B against the previous tag when a route breaches (L-51), and a migrated copy of the real journal under `next start` (L-52).
