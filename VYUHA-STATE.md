...
- **What v4.5.0 contains so far** (owner rulings 2026-09-22; every wave TWO builders — product, then tests — plus `vyuha-verifier` for the gate; each red-on-revert):
  - **Upstox is plan-dependent** — `accounts.broker_plan` + `broker_plan_from` (0074), `resolvePlan` keyed on (account, trade broker), ONE pricing door `ratesForTrade` at 13 sites; Upstox **Plus** seeded (₹30, MTF 14.60 %, no subscription); Basic delivery min(2.5 %, ₹20); DP ₹20. Owner's 2026-08-28 day: 156.89 default / 227.69 plus vs ledger 226.57 (the ₹1 residual is a separate open finding).
...
- **What is NOT built (the next session's first three waves, in this order):** **3b-ii = P1** (a partly-sold staged position in no tax year → per-fill rows from `summarise().fills`, apportion the per-row add-backs by fill quantity, replace all EIGHT `!isOpen` filters incl. `app/api/ais/route.ts` — `3b-notes.md` "Hand-off to 3b-ii", design-review revision 17) → **3c** (`vyuha-seam-tester` writes `tests/seams-v45-etf-tax.test.ts`: GOLDBEES symbol-only STT 0 AND head; NIFTYBEES STT ₹1 AND 111A/112A; LIQUIDBEES acquired 2023-05-02 SMF-deemed; an unlisted INF ETF: equity-share rate, blank head, DQ; HYBRIDETF STT 0 + UNDET) → **the fix-list wave** (`fix-list-wave.md`: 11 items from this session + 3 from the v4.4.0 list; two DECIDED there) → gate → bump 4.5.0 → the `release` skill → `vyuha-release-steward` → **STOP for "tag"**. The release copy owed is listed in `fix-list-wave.md` §C. README says "9745 tests" in 6 places — ~1,000 stale, guarded only for self-agreement — fix in the release-copy step.
