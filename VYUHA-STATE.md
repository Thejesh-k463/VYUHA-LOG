...
## §0 START HERE — reconciled 2026-09-21 IST by the fourteenth session: **v4.4.0 is BUILT, BUMPED, PUSHED, CI 6/6, installer built and signature-checked, steward READY TO TAG — NOT tagged** (the tag is the owner's one word); published release is still **v4.3.0**; the commit that landed this block is the next one in `git log`
...
- **Next — on the owner's "tag":** annotated tag `v4.4.0` on `main`'s HEAD (CI 6/6 first) → release workflow 3/3 → `npm run release:verify v4.4.0 -- --deep` 3/3 → publish with `--latest` → `releases/latest` → v4.4.0 → `npm run winget:manifest` from the CI asset sha (submission still HELD, row 4) → fetch the Pages site live. **If the installer is REBUILT for any reason, rename the old client ZIP aside first** (`build-client-package.mjs` refuses to overwrite) and re-hash.
...
| 2b | **v4.4.0 release** | **READY TO TAG 2026-09-21** — bumped `80eb164`, CI 6/6, installer built + verified, steward clean | §0 "Next — on the owner's tag" | owner says **"tag"**; release 3/3; deep verify 3/3; `releases/latest` → v4.4.0; WDSI handed over |
