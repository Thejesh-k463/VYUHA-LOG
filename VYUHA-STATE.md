...
**The one-line state (verified 2026-09-11):** published release = **v4.2.0** (tag `9da7bc8` → `b488dde`). v4.3.0 wave 1 (`265d684`) audited twice, fix waves `beaea48` / `786d288` / `6198c8b` CI 6/6. **v4.3.0 wave 2 = `a00ddd0`** — CI run 34472737988 SUCCESS 6/6, seam-tested, **audited (round 3, 2026-09-11: 142 → 14 → skeptic 14) → fix wave 3 = `f167e6c` (24 repo paths + README, pushed; **CI 34518836233 SUCCESS 6/6**, Pages 34518835928 3/3); round 4 = that fix wave's own audit DONE (117 → 8 → skeptic 5, all five fixed) → **fix wave 4 = the commit on top of `f167e6c`** (9 code/test paths + README + this file + DECISIONS; its sha and CI id are written into §2 by the docs-only commit that follows its green CI); **round 5 = a SCOPED audit of fix wave 4 (money + ui + test-integrity on Fable + skeptic) NOT yet run; NOT bumped, NOT tagged**. Gate on the fix-wave-4 tree: EXIT 0, 378 files / 7,632 passed / 35 skipped (raw vitest line). Estate paths moved under `T:/Thejesh/CLAUDE-CODE/VYUHA/` on 2026-09-10 (sweep record: `docs/owner/HANDOFF-HYGIENE-2026-09-10.md`).

### §0.1 Pickup table — one row per open item

| # | Item | Status | Resume from EXACTLY here | Closed when |
|---|---|---|---|---|
| 1 | Audit ladder on wave 2: round 3 (14 → 14) → fix wave 3 `f167e6c` (CI 6/6) → round 4 (117 → 8 → 5) → **fix wave 4 = the commit on top of `f167e6c`** (pushed; CI id in §2 after its run) → **round 5 = SCOPED audit of fix wave 4, NOT yet run** | fix wave 4 committed; gate EXIT 0, 378 / 7,632 / 35 | `vyuha-audit` skill, scoped: `vyuha-auditor` money + ui-regressions + test-integrity on Fable over `f167e6c..<fix-wave-4 sha>`, then the Fable `skeptic`; known inputs = the "Recorded, not defects (round 4)" bullet and the "Fix wave 4" bullet in DECISIONS "2026-09-11 — v4.3.0 audit round 4" (U-1 pinned by source shape, the `committedAt` guard reasoned not observed; a stale refusal stays silent by decision; the T-1 superset test will not notice a future widening of the help gate — recorded) | round 5 reaches 0 open findings (or every survivor is ruled recorded); CI 6/6 on the last fix commit; §0.3 V1–V5 all PASS → then row 2 |
