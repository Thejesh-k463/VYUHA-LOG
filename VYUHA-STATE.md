...
## §0 START HERE — reconciled 2026-09-25 IST by the twenty-sixth session (v4.6.0 wave W4 BUILT — the Help revamp: task-first topics, search hero, accordion, large-text dialog, "?" deep links, glossary, shortcuts sheet, 14 bundled screenshots; and the getting-started strip): **v4.5.0 is the published release and is CLOSED** (tag `v4.5.0` = `b6b8863`; NO REWORK on any closed item). **v4.6.0 is IN BUILD: W1 (market calendar), W8 (OpenAlgo safety), W2 (stock universe), W9 (Fyers + Nuvama), W3 (analytics hubs) and W4 (Help + strip) are DONE and committed; W5 (Q51 cohorts + Atlas v3) is NEXT and opens by ASKING its twenty 06a AQ questions in two groups of ten, then W6, W7.** Only v4.6.0 and v4.7.0 carry everything left (rulings B1–B6). Fixed in the tree, not yet released: the day's mark waits for the official close (W1); an OpenAlgo pull refuses sandbox, pre-2.0.2.6, `NCO`/`NCDEX` and zero-quantity MCX rows (W8); sectors come from the exchanges' own classification and the cap band is AMFI's (W2); Fyers and Nuvama are brokers with rate cards and native file parsers (W9); seven analytics screens are tabs of three hubs and their old URLs answer a real 307 (W3); every help entry carries 3–5 task-first steps, one watch-out and related topics, `/help` is search-first with an accordion and an 18px dialog, every page header has a "?" to its topic, "?" opens a shortcuts sheet, hub tabs are found as screens in global search, and the dashboard shows a five-step getting-started strip until it is done or dismissed (W4). Two hook proposals still `[PENDING]`.
...
4. `AGENTS.md` before any code change (on conflict AGENTS.md wins over this file; the code wins over both). Owner rulings: the spec's §0 table; `docs/DECISIONS.md` newest entries at the END (grep `^## 2026-09-2`, never read it whole — the newest six are the 2026-09-25 W8 entry, the W2 entry, the W9 entry, the W3 entry and the **W4 entry**, with the W1 and brokers entries before them) — binding, never re-ask one.
5. §3 of this file for where any other answer lives. Every earlier §0 block is in `VYUHA-STATE-ARCHIVE.md`, byte-for-byte (the twenty-fifth session's block archived by this session).

**The one-line state (re-verified 2026-09-25 by the twenty-sixth session at its close: the W4 build commit `6a4b966` sits on `0d9a36e` (W3) and this close-out docs commit sits on it — the CI run on `6a4b966` is 36121354870 SUCCESS 6/6, both shas are in the session log; newest migration **0075** (W4 added none — Help and the strip are data, components and a script; W6's `side` is 0076+); tag v4.5.0 = `b6b8863`; `releases/latest` v4.5.0):** published = **v4.5.0**. **Gate on the W4 tree: `npm run verify` (vyuha-verifier) EXIT 0 — 482 files / 11,447 passed / 35 skipped**, lint 0 errors (6 pre-existing warnings), `next build` "Compiled successfully"; 35 skipped = the owner-file cases with `VYUHA_LICENSE_PEM` set (unset: 40 skipped). Playwright: 135 flows in 37 specs by `--list` (z-help adds 7); the five specs W4 can touch (z-help, command-palette, z-onboarding, z-section-order, zz-console-audit) were run locally — the result line is in the session log; CI runs both jobs on the W4 commit. `package-lock.json` gained exactly two `packages` entries (`@radix-ui/react-accordion` 1.2.15, `@radix-ui/react-collapsible` 1.1.15) and one root line — 62 additions, 0 deletions, spliced by text, `npm ci` clean.

- **What the twenty-sixth session did:** W4 from a written contract (scratchpad `W4-CONTRACT.md`) after a scout mapped the surface (no accordion/hover-card/collapsible in the lock; `components/help/` did not exist; no shortcuts sheet; no last-backup fact anywhere; `help-desk.tsx` read by four tests). TWO Opus agents on disjoint files: a UI builder (the dep splice, `components/help/*` ×9, `page-header.tsx` "?", the palette Help source + "Keyboard shortcuts" action, the search "screens" rows for hub tabs (D-9 closed), `lib/domain/{shortcuts,getting-started,glossary-split}.ts`, `lib/queries/getting-started.ts`, the strip, `scripts/capture-help-screens.mjs`, four test files + `e2e/z-help.spec.ts`; 13 red-on-revert proofs) and a content writer (`lib/domain/help-topics.ts`: 50 records = 40 screens + 3 hubs + 7 tabs, longest card 107 words; `lib/domain/glossary.ts`: 35 terms). The session ran the capture (14 webp, 941 KB, dark, demo DB), the skeptic (15 claims: 13 CONFIRMED / 2 REFUTED — an /instruments step named a Portfolio Risk control, and the broker-compare watch-out said "excluded from the ranking" where the code lists partial brokers last with an asterisk; both fixed), and fixed the three things the builder left outside its set (`account-isolation` OWNERS row, the THIRD-PARTY-NOTICES row, README counts). LEDGER U-7, U-8, F-18, L-19, L-20, D-10.
- **What WRONG looks like from here:** `/help` showing the old card list (no search hero, no accordion); a topic dialog that does not close on Esc or leaves `#topic-…` in the URL after closing; a `#topic-…` deep link that scrolls instead of opening the dialog; a page header without the "?" when a topic exists for its href (a hub TAB must resolve to the tab's topic, never the hub's); the empty palette listing help rows (they appear only under a typed query); a hub tab missing from global search's "screens"; `tests/help-content.test.ts` red on `public/help/<name>.webp` (run `npm run help:shots`, never delete the pin — FAIL-T); a screenshot with `theme-light`, a real journal path, or (after the bump) a sidebar reading `v4.5`; a `HELP_TASKS` record without an entry or an entry without a record (the join throws at load); the strip counting trades across accounts in a single-account view; a second literal list of hub tab hrefs; `package-lock.json` with a deletion; `OPENALGO_DISCLOSURE_VERSION` bumped to "5" before v4.6.0 ships; a `"15:30"` / `15 * 60` / `+05:30` literal outside the calendar; a wave tag in git.
- **Owed outside the build:** the OWNER reviews the Help copy (ruling H4 — every step and watch-out in `lib/domain/help-topics.ts`, every glossary meaning in `lib/domain/glossary.ts`); **re-run `npm run help:shots` AFTER `npm run bump-version 4.6.0`** and before the release build — the 14 shots carry the sidebar footer (`v4.5` today) and the dashboard shot carries the strip (LEDGER L-19); a LIVE OpenAlgo 2.0.2.6 preview pull (owner, before the v4.6.0 release — record the version in `docs/OPENALGO_SETUP.md` Part 4 and DECISIONS); a Fyers tradebook and a Nuvama report WITH equity rows whenever the owner has them (`docs/BROKER_FORMATS.md`); Q24's citation (NSE/INVG/67858 A.8) for the release claims audit; the 2027 NSE holiday list + the Muhurat 2026 circular when NSE publishes them; two hook proposals in `~/.claude/coord/learnings/HOOK-PROPOSALS.md`, both `[PENDING]`; next `/fleet-tune` ~2026-09-30.

### §0.1 Pickup table — one row per open item

| # | Item | Status | Resume from EXACTLY here | Closed when |
|---|---|---|---|---|
| 18 | **v4.6.0** (W1 calendar ✓ → W8 OpenAlgo safety ✓ → W2 universe + row 10 ✓ → W9 Fyers + Nuvama ✓ → W3 hubs ✓ → W4 Help + strip ✓ → **W5 Q51/Atlas v3** → W6 row 9 → W7 row 17) | **W1 + W8 + W2 + W9 + W3 + W4 DONE 2026-09-25; W5 next** | spec §2 W5 — FIRST ask the 20 unanswered 06a AQ questions (`VYUHA/LIVE-DESK-RESEARCH/19-BACKLOG-RESEARCH-2026-09-16/06a-atlas-panel-v3.md`) in two AskUserQuestion groups of ten, recommended option first, BEFORE any feature code; then industry cohorts from the universe (W2), the 3 honesty gaps + freshness catch-up (`…/Q51-cohort-analytics.md`), Atlas panel v3 in `lib/queries/atlas.ts` + `components/atlas/*`; W6 `side` and W7 need `vyuha-design-reviewer` first — W6 also owns overnight F&O shorts (`pairLegs` files them `opening-sell` + `open`, LEDGER D-8) | owner publishes v4.6.0 |
| 19 | **v4.7.0** (C1–C2 Edge Clinic → the Clinic TAB of `/reports/edge-clinic` becomes the default, C3 journal Kelly, C4 row 6 Positions tab, C5 Q18 Telegram, **C6 native pulls Fyers / Kotak Neo / Nuvama**, **C7 OpenAlgo WebSocket feed**) | SPECIFIED; opens after 4.6.0 is published | spec §3; C1 adds a `clinic` tab to `HUBS` and flips `defaultTab` (the 307s stay temporary for exactly this); a new tab needs a `HELP_ENTRIES` entry AND a `HELP_TASKS` record (the join throws otherwise); C6 needs `vyuha-design-reviewer` first (identity across file, OpenAlgo and native sources — Fyers' `/trade-history` fields are unpinned until a first real pull); retire `sector-map.json` there if its fallback-only count (8 on 2026-09-25) is still about zero | owner publishes v4.7.0 |
| 6 / 8 / 9 / 10 / 17 | Rows folded into 18/19 | row 6 → v4.7.0 C4; row 8 → Q51+Atlas v3 (W5), Kelly (C3), Q18 (C5); row 9 → strip DONE in W4, `side` + speed lever + `--ref` in W6; row 10 → DONE in W2; row 17 → W7 | the spec | with their release |
...
### §0.2 Decisions taken this session under the decision policy (recorded in DECISIONS 2026-09-25 W4 entry; the owner may overrule)
**Twenty-sixth session (2026-09-25):** the task-first layer is ADDED over each entry (steps, watchOut, related, screenshot) and the existing answers/body/refusals render unchanged under "In detail", so the ~60 consent/SEBI copy pins hold; the 120-word budget is the CARD (title + answers + steps + watchOut), test-pinned; topic ids are `topic-<slug of href>` (the `?tab=` value kept), disjoint from `options-…` anchors by pin; the content lives in `lib/domain/help-topics.ts`, joined to `HELP_ENTRIES` at load (throws on a miss or an orphan); the "hovercard" is the existing Radix Tooltip (the ruling allowed ONE new dependency — the accordion — and collapsible rides in as its dependency); `help-desk.tsx` stays the `/help` root (four tests read it) and the Options section stays inside it, unmoved; help rows appear in the palette only under a typed query; global search lists hub tabs as screens "Hub › Tab" (D-9 closed); the strip's "backup taken" is a per-machine localStorage marker written by the backup control (no DB state; the copy says so); the "?" sheet key is swallowed by `/help`'s auto-focused search box (the hero button opens it there — recorded, accepted); screenshots are re-captured after the version bump (L-19); `search-route`'s 5 s timeout under the full suite is FAIL-J — the same test takes 2.3 s on HEAD's `search.ts` and 2.0 s on W4's, measured alone; after it fired in two of three gate runs, that ONE `it` carries a 15 s timeout with the measured times in its comment (AGENTS Testing), not a re-rolled gate. The earlier sessions' decisions (DECISIONS W1, brokers, W8, W2, W9, W3 entries) stand.

### §0.3 Verification — PASS and FAIL signatures (V3–V6 of the v4.4.0 era are in the archive)

**V1 — CI on the newest CODE sha.** `gh run list --limit 5`; judge only the run whose `workflowName` is `CI` with 6 jobs. **PASS:** `success`, 6/6. **FAIL-B** — a job red with a timeout / network / cold-runner signature (ubuntu Playwright dead at the webServer start with `next/font/google` resolve errors; a spec stalled 90 s on a cold macOS runner with no ECONNREFUSED or memory line): re-run that job; never re-tag. **FAIL-G** — macOS Playwright red with ONE spec dead on `ECONNREFUSED :3100` and the dev server's memory-threshold restart line: the heap watchdog. **FAIL-H** — BOTH Playwright jobs red on ONE spec with `strict mode violation`: pin both, never `.first()`. **FAIL-K** — a Playwright spec asserting a market-open/closed label or a mark minute: re-pin to the calendar's answer, never to a literal. **FAIL-R** — `z-hubs.spec.ts` red on `config-level redirect: expected 307` — the `redirects()` block in `next.config.ts` was removed or no longer derives every legacy href; fix the config, never the spec. **FAIL-A / C / D / E / F** as before (archive).

**V2 — the gate.** `rtk proxy npm run verify` (heavy; coord serialises it; a builder NEVER runs it — `vyuha-verifier` does, and NOT while another agent is loading the machine: a concurrent skeptic made `seams-v42-fix5`'s 30 s hook time out once — FAIL-J). **PASS:** exit 0 and the RAW vitest line `Test Files ≥ 482 passed`, `Tests ≥ 11,447 passed | 35 skipped` with `VYUHA_LICENSE_PEM` set (unset: 5 fewer passed, 40 skipped), no `expected fail`. **FAIL-T (new)** — `tests/help-content.test.ts` red on "its webp exists": `public/help/` is missing a file a topic names — run `npm run help:shots` (dark theme is asserted; a light page refuses), never delete the pin or the `screenshot` field; after a version bump, re-run it anyway (L-19). **FAIL-S** — `tests/hubs.test.ts` or `tests/pro-gating.test.ts` red after a screen is added, moved or re-keyed: every surface that keys by href (nav, `PRO_FEATURES`, help entries AND `HELP_TASKS`, palette, `SCREEN_DOMAIN`, the saved nav order, `/help`'s grouping, the search screens) is a READER to re-point — derive from `HUBS` / `navGroupHrefs`, never add a second literal list. **FAIL-Q** — a test pinning the SET of brokers with more than one plan (`broker-plan*.test.ts`), a seed-row count or a broker hand-list red after a broker or plan is added: re-derive the number by RUNNING the seed and fix the pin, never the seed (DECISIONS W9, 27 pins). **FAIL-P** (`tests/stock-universe.test.ts` after a refresh: rebuild both snapshots in the AGENTS order, never loosen a floor), **FAIL-M** (the OpenAlgo request-list pins), **FAIL-N** (a dotted version in PRIVACY item 3), **FAIL-O** (a CRLF-blind doc-copy regex), **FAIL-J** (a 5 s / 30 s timeout under load: check free memory and concurrent agents, re-run the file alone; `search-route` "answers no-store" — ~2 s alone, 5.2 s under the suite — carries a 15 s timeout since W4 with the measured times in its comment; a NEW timeout gets the same treatment only after it is measured alone and under the suite), **FAIL-L** (a timing literal outside the calendar), **FAIL-E** (README's counts — fix the prose; a redirect-only `page.tsx` is not a screen), **FAIL-I**, **FAIL-A**, **FAIL-B**, **FAIL-C**, **FAIL-D** as before.

**V7 — the release-gate Playwright pass:** 120 / 120 in 35 specs on the v4.5.0 tagged tree; 135 in 37 by `--list` on the W4 tree; the five W4-touching specs ran locally (session log). **PASS signature:** `135 passed`, `PROBE-END console-audit (0 message(s))`. A red `zz-console-audit` is a product regression, never a flake. A red `z-help.spec.ts` on the "?" sheet test means a focused editable element ate the key — check what has focus on that route before touching the key handler.

**V8 — the release itself.** `npm run release:verify <tag> -- --deep` must print N signatures ALL `4FF85F3BBE1DA21D` and ALL "verifies over"; `gh release list --limit 2` must show the new tag as `Latest` and `gh release view revocations --json isPrerelease` must stay `true`. A background Bash `until` loop on `gh run view --json status` is the reliable wait. `$CLAUDE_SCRATCHPAD` is NOT set inside the Bash tool's shell here — write the absolute scratchpad path. The OWNER publishes (stop after `release:verify --deep`). **Before the release build: `npm run help:shots` after the bump (L-19).**
...
1. **Answered 2026-09-25 (twenty-fourth session, W9):** Nuvama legs as trades; negotiated Nuvama plan → published plans only; Fyers default Standard; no equity export yet. 2. **Before W5 — the next session's FIRST act:** the 20 unanswered 06a AQ questions (Atlas panel v3), in two groups of ten. 3. Before W7: harvest `sttSplit` counting (recommended: one per ladder, fills listed beneath) and what "accepted" means for the ~₹1 Upstox residual. 4. Before C2 (v4.7.0): the setup-grade labels, and whether intra-trade high/low is typed or later auto-filled. 5. Before C6: the first real Fyers `/trade-history` pull pins its fields; Kotak Neo stays "documented, not verified" (B2). 6. Still open from before: whether the A4 / A7 decisions and the sixteenth session's §0.2 stand (the `stated-bill` note marker rather than a column); row 5 / the derivative-marks schema release (archive §0.4 item 3). 7. Noted, not asked: BSE SME companies (486) DO get an AMFI band (small) — U3 names NSE Emerge only; say so if Emerge-style blanking should extend to them. 8. Noted, not asked: a Fyers tradebook or a Nuvama report WITH equity rows, whenever one exists — it pins the two UNVERIFIED equity layouts. 9. **Owner review owed by ruling H4:** the Help copy in `lib/domain/help-topics.ts` (50 cards) and `lib/domain/glossary.ts` (35 terms) — read on `/help`; a wrong step is a one-line edit there and the tests re-check the budget and the join. 10. Noted, not asked: the "backup taken" step counts a backup on THIS machine only (a localStorage marker) — say so if a journal-side record is wanted (that would be new schema, W6+).
---
## 1. What Vyuha is
