...
- **OPEN, the owner's decisions:** (1) the Upstox F&O brokerage rate — the rate card says ₹20 flat, Upstox's own ledger implies ~₹30/order (gap 69.81 on the fixture day); a fix is a `charge_config` migration, NOT built, NOT claimed. (2) rows 1b, 3, 6 as before.
...
- **Owed outside the release:** `/fleet-tune` (due 12 days; skipped again on the owner's credit instruction) and one hook proposal awaiting the owner in `~/.claude/coord/learnings/HOOK-PROPOSALS.md`. After the tag: Session 2 of the plan (row 1b auto-close rebuild + wave-3 tax, ONE design review first).
