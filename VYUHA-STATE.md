...
>
> **VERIFY (2026-09-07, FIX-WAVE-3b gate, the tree after `1ec6de1` + fix wave 3b): `npm run verify` EXIT 0 — typecheck clean, lint 0 errors (1 pre-existing `react-hooks/incompatible-library` warning on the desk virtualiser), vitest 341 files / 6,392 passed / 35 skipped (6,427), `next build` compiled. Playwright on the fix-wave-3 tree: `e2e/z-live-desk.spec.ts` 9 passed. Earlier gates: fix wave 3 (`1ec6de1`, CI SUCCESS 34054833245) 341 / 6,381 / 35; fix wave 2 (`65dd329`, CI SUCCESS 34051766379) 339 / 6,334 / 35; fix wave (`4b55620`, CI SUCCESS) 337 / 6,273 / 35; wave (`66aa319`, CI 6/6) 334 / 6,177 / 35. README counts 6392 / 341 / 93 flows in 29 specs, all re-measured on this tree (`find tests -name "*.test.ts" | wc -l` = 341: fix wave 3 added `tests/typed-mark-day-row.test.ts` and `tests/seams-v41-fix3.test.ts`).**
