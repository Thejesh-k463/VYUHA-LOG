...
Everything in §2 was verified against the repo and the live release on 2026-09-11, not recalled;
§0 below is a LATER reconciliation record (2026-09-21/22) and outranks §2.
...
## 2. History — the v4.3.0 build chain up to fix wave F1 `5f7f098` (NOT the current state — §0 is; this heading claimed "Current state" until 2026-09-11, R23) — as recorded at F1, 2026-09-11 IST (`npm run verify` EXIT 0 — **379 files / 7,661 passed / 35 skipped**, the raw line; CI 34571547482 SUCCESS 6/6; the desktop rate-card refresh now keys on the 5-column `charge_config_uq`, runs in one transaction, keeps the user-edit guard and has its own handler — DECISIONS "2026-09-11 — v4.3.0 STATE §0.3 V3/V4/V5 … and F1"; scoped round 1 = 8 → 8, money 0 → micro fix wave F1-2 owed) — before it: AUDIT LADDER CLOSED AT ROUND 9; MICRO FIX WAVE 8 = `5a15aa1` (378 files / 7,646 passed / 35 skipped; CI 34564133527 SUCCESS 6/6; **fix wave 6 `5bbed43`** — the confirmed ref is the confirmed HISTORY: an accepted reply replays the gesture's action through `shelfReducer` with the route's re-read as `present`, a refusal reverts wholesale (R6-U-1), five pins rewritten, `sameSelection` body pinned; CI 34558028790 6/6; DECISIONS "2026-09-11 — v4.3.0 audit round 6 (scoped)" — **fix wave 7 `5015f65`** — nesting slices + occurrence-count pins over the reply callback, a harness test through the stale-accepted branch, R7-U-1 (an undo/redo replayed on a depth-diverged confirmed stack) RECORDED with its comments corrected, the source-shape caveat WIDENED by ruling; CI 34562338623 6/6 (attempt 2 after an Ubuntu e2e flake); DECISIONS "… round 7 (scoped)" — **micro fix wave 8 `5a15aa1`** — the per-reply slice reaches `run`'s end, `using|enum` pinned, test file only; DECISIONS "… round 8 (scoped, test-integrity only)" — **round 9 over `5015f65..5a15aa1` = 17 → 0, skeptic upheld; NOT bumped, NOT tagged; §0.3 V3–V5 not yet run**; earlier chain: wave 1 `265d684` → rounds 1–2 → **wave 2 `a00ddd0`** (CI 34472737988 6/6) → rounds 3–5 → fix waves `f167e6c` / `34db076` / `5494d9f` (DECISIONS rounds 3, 4, 5 (scoped))). **v4.2.0 remains the PUBLISHED release** (section below, unchanged).
...
> trade groups with a "Remove the copy" button + confirm `lib/analytics/data-quality.ts`, `app/data-quality/actions.ts`,
...
> the Data Quality close of the stale Dhan rows — auto-close is OFF in 4.3.0 by the 2026-09-11 ruling — the spot chip, the strategy screen with his own option book, both feeds). Every deviation a
...
must not fall below. The 4.0.0 figure is 328 files / 6,057 passed / 35 skipped (EXIT 0, after re-audit fix waves 2 and 3, commit `4158f6c`); see `VERIFY:` below.**
...
`mtm_prices` reads "Stored mark" with no date (that table has no reliable as-of column), and a row
with only the trade's own closing price reads a bare "End of day"; the Sizing Lab
...
**VERIFY (re-audit fix wave, 17 items): EXIT 0 — 328 files / 6,057 passed / 35 skipped, `next build` compiled, on the wave-3 tree = commit `4158f6c` (2026-09-06 ~12:45 IST; the aba43f5 tree measured 6,055). No `v4.0.0` tag exists yet; the latest tag is `v3.9.1`.** The 5938aa6 line below is the prior run and stays as history.
...
| Why is this constant/threshold what it is? | **`docs/DECISIONS.md`** (≈ 610 KB on 2026-09-11 — grep `^## 2026-`, never read whole; dated entries, NEWEST AT THE END since the 2026-09-04 v3.8 reconciliation, except the v3.9 W2 entry under the header) |
...
- ~~**Parsers not built**~~ (SHIPPED v3.9.0 — see §0.1 rows 9 and 15; the live gaps are the 2026-09-17 parser wave): Dhan DP charges (`.xls`), demat holdings, Upstox ledger, Angel One ledger
