...
## §0 START HERE — reconciled 2026-09-25 IST by the twenty-eighth session (v4.6.0 wave W6 BUILT — `trades.side` (migration **0077**) + overnight F&O shorts, the `/trades` speed lever measured then built, referral `--ref`): **v4.5.0 is the published release and is CLOSED** (tag `v4.5.0` = `b6b8863`; NO REWORK on any closed item). **v4.6.0 is IN BUILD: W1, W8, W2, W9, W3, W4, W5 and W6 are DONE and committed; W7 (row 17 tax follow-ups) is the LAST wave and needs `vyuha-design-reviewer` FIRST and its two owner questions at ITS start.** Only v4.6.0 and v4.7.0 carry everything left (rulings B1–B6). Fixed in the tree, not yet released: everything W1–W5 listed before (archive), plus: a closed short is no longer read as a long — the side that opened a flat row is STORED and every leg writer writes it (W6); an overnight F&O sell-then-buy in one file is ONE closed short position instead of an opening sell plus a phantom long, and an uncovered sale stays an opening sell (W6); a pre-4.6 file whose short now pairs differently is refused as already imported, never duplicated (W6); `/trades` no longer reads the whole book for its KPI strip (W6); the share card points buyers at the landing page, and a sale records who referred it (W6). Two hook proposals still `[PENDING]`.
...
4. `AGENTS.md` before any code change (on conflict AGENTS.md wins over this file; the code wins over both). Owner rulings: the spec's §0 table; `docs/DECISIONS.md` newest entries at the END (grep `^## 2026-09-2`, never read it whole — the newest is the 2026-09-25 **W6 built** entry, with the W5 built, W5 opened, W4, W3, W9, W2, W8, W1 and brokers entries before it) — binding, never re-ask one.
5. §3 of this file for where any other answer lives. Every earlier §0 block is in `VYUHA-STATE-ARCHIVE.md`, byte-for-byte (the twenty-seventh session's block archived by this session).

**The one-line state (re-verified 2026-09-25 by the twenty-eighth session at its close: the W6 build commit `the W6 build commit (sha named by the close-out commit; see the session log)` sits on `0407a88` (the W5 close-out) and this close-out docs commit sits on it — the CI run on `the W6 build commit (sha named by the close-out commit; see the session log)` is named by the close-out commit; newest migration **0077** (`trades-side`; W7's first migration, if any, is 0078); tag v4.5.0 = `b6b8863`; `releases/latest` v4.5.0):** published = **v4.5.0**. **Gate on the W6 tree: `npm run verify` (vyuha-verifier, alone) EXIT 0 — 495 files / 11749 passed / 35 skipped**, lint 0 errors (6 pre-existing warnings), `next build` "Compiled successfully"; 35 skipped = the owner-file cases with `VYUHA_LICENSE_PEM` set (unset: 5 fewer passed, 5 more skipped). Playwright: **141 flows in 38 specs** by `--list` (unchanged: W6 adds no screen, no tab, no route). `package-lock.json` and `package.json` UNCHANGED (no new dependency).

- **What the twenty-eighth session did:** a scout mapped `trades.side`'s surface (30 raw derivations in 21 files; `dedupHash` position-level; `NormalizedTrade` has no side), a second scout the speed-lever and referral surfaces; a DECIDED contract (scratchpad `W6-CONTRACT.md`) went to `vyuha-design-reviewer` (Opus) which returned **REVISE with 8 confirmed defects** — the sharpest: the first `shortable` rule would have turned the owner's real Fyers COFORGE sale (bought before the file's window) into a phantom open short, and a stored side must never outrank lopsided quantities (an IPO-linked sale would have CLOSED BY BUYING) — all eight folded into revision 2 before ONE Opus builder (four phases, 62 files) built it; two small Opus builders ran beside it on disjoint files (referral, then the speed lever after an Opus agent MEASURED it on the 25k perf book: the whole-book read was 359-412 ms, one SQL aggregate 5-10 ms and bit-identical). Skeptic (Opus, 83 calls): 11 claims, 8 CONFIRMED / 3 REFUTED → an 8-item fix wave (the per-leg legacy dedup missed partial and multi-lot shorts → the whole symbol group is refused when the pre-4.6 pairing is stored; a guessed `'long'` is now NO side; times by side; a pre-existing generic-mapper time defect; the scan's allow marker; parser seams for all eight callers; `tests/side-sequences.test.ts`). LEDGER U-11…U-13, L-28…L-35, F-25…F-28, D-13.
- **What WRONG looks like from here:** a closed short whose Entry/Exit prices, holding period or R come out as a long's; an IPO-linked or basis-written sale that closes by BUYING (buyQty doubles) or whose auto-MTM sign is inverted; the Fyers golden pin moving off `{closed 22, open 1, openingSells 1}` (COFORGE must stay an opening sell); an uncovered derivative sale becoming an `open` short; an equity (`eq_*`) sell-then-buy across two days pairing as a short; a pre-4.6 F&O file re-imported after W6 raising the sold quantity (the sale stated twice — `harness-book-sequences` I1 is the guard); a row written with a quantity and no `side` (`readers-follow-writers` red); a raw `sellQty > buyQty` anywhere outside `lib/domain/side.ts` without the `// side-scan: quantity` marker (`side-column` red); a same-day flat row with no note getting a stated side; `/trades` calling `getJournalTrades()` again; a KPI figure differing from `tradeStatsOf` by even a paisa; a `ref` value inside a signed licence key; the share card naming the source repo; `package-lock.json` with any change; a wave tag; `OPENALGO_DISCLOSURE_VERSION` bumped before v4.6.0 ships.
- **Owed outside the build:** **W7 asks its two questions at its start** (harvest `sttSplit` counting; what "accepted" means for the Upstox residual); the release-day **perf sweep** re-measures `/trades` through `next start` (the lever was proven at the query level only); the OWNER reviews the Help copy (H4); **re-run `npm run help:shots` AFTER `npm run bump-version 4.6.0`** (L-19); **measure `/atlas` on the owner's real DB before the release** (L-25); a LIVE OpenAlgo 2.0.2.6 preview pull (owner); a Fyers tradebook and a Nuvama report WITH equity rows whenever the owner has them; Q24's citation for the release claims audit; the 2027 NSE holiday list + the Muhurat 2026 circular; two hook proposals in `~/.claude/coord/learnings/HOOK-PROPOSALS.md`, both `[PENDING]`; next `/fleet-tune` ~2026-09-30.

### §0.1 Pickup table — one row per open item

| # | Item | Status | Resume from EXACTLY here | Closed when |
|---|---|---|---|---|
| 18 | **v4.6.0** (W1 ✓ → W8 ✓ → W2 ✓ → W9 ✓ → W3 ✓ → W4 ✓ → W5 ✓ → W6 ✓ → **W7 row 17**) | **W1–W6 DONE 2026-09-25; W7 next, then the release** | spec §2 W7 — ask its two questions FIRST (spec §4), then `vyuha-design-reviewer` over the DECIDED design (tax + money: AIS purchase split per entry leg in `lib/analytics/realised-rows.ts`, harvest `sttSplit` counting, FMV editor per symbol+ISIN, the Upstox ₹1 residual sized against the ledger first), ONE Opus builder, gated by `oracle-counted-once`, `readers-follow-writers`, `preview-equals-save-matrix`; any migration is **0078**. After W7: `vyuha-seam-tester` → ONE rigorous `vyuha-audit` → one fix wave max → bump 4.6.0 → `help:shots` → the release skill → `release:verify --deep` → **STOP; the owner publishes** | owner publishes v4.6.0 |
| 19 | **v4.7.0** (C1–C2 Edge Clinic → the Clinic TAB of `/reports/edge-clinic` becomes the default, C3 journal Kelly, C4 row 6 Positions tab, C5 Q18 Telegram, **C6 native pulls Fyers / Kotak Neo / Nuvama**, **C7 OpenAlgo WebSocket feed**) | SPECIFIED; opens after 4.6.0 is published | spec §3; C1 adds a `clinic` tab to `HUBS` and flips `defaultTab`; a new tab needs a `HELP_ENTRIES` entry AND a `HELP_TASKS` record; C6 needs `vyuha-design-reviewer` first; retire `sector-map.json` there if its fallback-only count (8 on 2026-09-25) is still about zero; a Dhan API overnight short across two daily pulls (LEDGER D-8, recorded not built in W6) belongs to C6/C7 if anywhere | owner publishes v4.7.0 |
| 6 / 8 / 9 / 10 / 17 | Rows folded into 18/19 | row 6 → v4.7.0 C4; row 8 → DONE in W5 (Q51/Atlas), Kelly (C3), Q18 (C5); **row 9 → DONE (strip W4; `side`, speed lever, `--ref` W6)**; row 10 → DONE in W2; row 17 → W7 | the spec | with their release |
...
### §0.2 Decisions taken this session under the decision policy (recorded in DECISIONS 2026-09-25 W6 built entry; the owner may overrule)
**Twenty-eighth session (2026-09-25):** `side` nullable with a CHECK, never `DEFAULT 'long'`; the migration ADDs the column only and the data fix `trades-side-v1` backfills in JS through `normalizeDate` (4.2.x rows hold `DD-MM-YYYY`); `sideOf` reads quantities first and the column only on a flat row; a flat same-day/undated row with no short note gets NO side (`statedSideOf` null; the stop-migration map keeps excluding it); every leg writer writes `side` (AST rule); a basis write writes `'long'`; editor: lopsided → quantities, flat with two dates → the date rule, else the stored side; a short exists only when a LATER buy in the same file covers it; `shortable` = not `eq_*` by `classify()`; the legacy pair is joined through the existing confirm dialog, never automatically; the dedup hash is unchanged and a re-import refuses the whole symbol group when the pre-4.6 pairing is stored; no UI toggle for side; the Dhan API overnight short is recorded, not built; the speed lever is aggregate + narrow query (a cached stats row rejected; a zero book prints "₹0" not "-₹0"); referral codes are per CREATOR, `--ref` on issue AND sell, a revoked key is not "active", an upgrade carries the ref, the card carries the buyer link and no code; W7 was NOT run beside W6 (item 11 below stays the owner's call, now moot: W7 is the last wave). The earlier sessions' decisions stand.
...
**V2 — the gate.** `rtk proxy npm run verify` (heavy; coord serialises it; a builder NEVER runs it — `vyuha-verifier` does, and NOT while another agent is loading the machine — FAIL-J). **PASS:** exit 0 and the RAW vitest line `Test Files ≥ 495 passed`, `Tests ≥ 11749 passed | 35 skipped` with `VYUHA_LICENSE_PEM` set, no `expected fail`. **FAIL-Y (new)** — `tests/side-column.test.ts` red on the scan: a raw quantity or date-order comparison was written outside `lib/domain/side.ts` — route it through `sideOf` / `entryDateOf`, or, ONLY for a genuine quantity test, mark the line `// side-scan: quantity`; never widen the allow list. **FAIL-Z (new)** — `tests/readers-follow-writers.test.ts` red on `trade-side-writer`: an insert/update set a quantity without `side` — add the side to THAT statement, never loosen the rule. **FAIL-AA (new)** — `tests/golden-books.test.ts` red on the Fyers shape (`openingSells 1` → 0) or `tests/harness-book-sequences.test.ts` red on "the sale is counted twice": the pairing or the legacy refusal changed — a product defect, never a pin move. **FAIL-AB (new)** — `tests/trades-kpi-sql.test.ts` red on a KPI field or on panel-row order: the aggregate's scope or the panel predicate drifted from the page's JS rule — fix the SQL, never `toBeCloseTo`. **FAIL-W / X / T / S / Q / P / M / N / O / J / L / E / I / A / B / C / D** as before (archive and the W5 block).

**V7 — the release-gate Playwright pass:** 120 / 120 in 35 specs on the v4.5.0 tagged tree; 141 in 38 by `--list` on the W6 tree (unchanged since W5; nothing ran locally this session — W6 touched no screen, tab or route). **PASS signature:** `141 passed`, `PROBE-END console-audit (0 message(s))`.
...
1. **Before W7 (asked at W7's start, the next session):** harvest `sttSplit` counting (recommended: one per ladder, fills listed beneath) and what "accepted" means for the ~₹1 Upstox residual. 2. Before C2 (v4.7.0): the setup-grade labels, and whether intra-trade high/low is typed or later auto-filled. 3. Before C6: the first real Fyers `/trade-history` pull pins its fields; Kotak Neo stays "documented, not verified" (B2). 4. Still open from before: whether the A4 / A7 decisions and the sixteenth session's §0.2 stand; row 5 / the derivative-marks schema release (archive §0.4 item 3). 5. Noted, not asked: BSE SME companies (486) DO get an AMFI band (small) — U3 names NSE Emerge only. 6. Noted, not asked: a Fyers tradebook or a Nuvama report WITH equity rows, whenever one exists. 7. **Owner review owed by ruling H4:** the Help copy in `lib/domain/help-topics.ts` (50 cards) and `lib/domain/glossary.ts` (35 terms). 8. Noted, not asked: the "backup taken" strip step counts a backup on THIS machine only. 9. **Noted, not asked (W5):** `COVERAGE_FLOOR_PPM` 30%, the cohort floor 5 priced / 60% coverage and the group floors 3 / 8 are PROPOSALS. 10. **Noted, not asked (W6):** the legacy F&O pair is joined through the confirm dialog (the contract said "never one-click"; read as "never automatic") — say so if the join should not be offered at all; and an uncovered derivative sale ALWAYS stays an opening sell (the COFORGE reading) — say so if a broker exists whose file should read it as a short. 11. Item 11 of the W5 block (W7 beside W6) is moot: W6 is done and W7 is the last wave.
---
