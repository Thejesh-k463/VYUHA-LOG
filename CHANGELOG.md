...
- **Live Desk (`/live`) — your open positions, priced.** Free, and free for
  ...
  into `mtm_prices` — that table has no reliable as-of column — a bare **End of day** when
  only the trade's own closing price is on file, or **No mark**. Only a dated mark can earn
  the **Stale** badge, shown when
...
- **No broker feed prices the desk.** A row priced from the end-of-day bhavcopy
  ...
  source or reliable as-of column; a row with only the trade's own closing price reads a
  bare "End of day"; only dated marks can show Stale. The OpenAlgo
